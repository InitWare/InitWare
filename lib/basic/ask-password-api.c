...
static void
backspace_chars(int ttyfd, size_t p)
{
	if (ttyfd < 0)
		return;
...
int
ask_password_tty(const char *message, usec_t until, bool echo,
	const char *flag_file, char **_passphrase)
{
	...
	if (ttyfd >= 0) {
		...
		loop_write(ttyfd, ANSI_HIGHLIGHT_ON,
			sizeof(ANSI_HIGHLIGHT_ON) - 1, false);
		loop_write(ttyfd, message, strlen(message), false);
		loop_write(ttyfd, " ", 1, false);
		loop_write(ttyfd, ANSI_HIGHLIGHT_OFF,
			sizeof(ANSI_HIGHLIGHT_OFF) - 1, false);
...
int
ask_password_agent(const char *message, const char *icon, const char *id,
	usec_t until, bool echo, bool accept_cached, char ***_passphrases)
{
	...
	fprintf(f,
		...
		"Echo=%i\n"
		"NotAfter=" USEC_FMT "\n",
		getpid(), socket_name, accept_cached ? 1 : 0, echo ? 1 : 0,
		until);
	...
	for (;;) {
		...
		if (msghdr.msg_controllen < CMSG_LEN(sizeof(struct ucred)) ||
			...
			control.cmsghdr.cmsg_len !=
				CMSG_LEN(sizeof(struct ucred))) {
			log_warning(
				"Received message without credentials. Ignoring.");
			continue;
		}
		...
		if (ucred->uid != 0) {
			log_warning(
				"Got request from unprivileged user. Ignoring.");
			continue;
		}
...
int
ask_password_auto(const char *message, const char *icon, const char *id,
	usec_t until, bool accept_cached, char ***_passphrases)
{
	...
	if (isatty(STDIN_FILENO)) {
		...
		*_passphrases = l;
		...
		return ask_password_agent(message, icon, id, until, false,
			accept_cached, _passphrases);
}
