...
int
utmp_get_runlevel(int *runlevel, int *previous)
{
...
static void
init_timestamp(struct utmpx *store, usec_t t)
{
	assert(store);
...
static void
init_entry(struct utmpx *store, usec_t t)
{
	struct utsname uts = {};
...
static int
write_entry_utmp(const struct utmpx *store)
{
	int r;
...
static int
write_entry_wtmp(const struct utmpx *store)
{
	assert(store);
...
static int
write_utmp_wtmp(const struct utmpx *store_utmp, const struct utmpx *store_wtmp)
{
	int r, s;
...
static int
write_entry_both(const struct utmpx *store)
{
	return write_utmp_wtmp(store, store);
}

int
utmp_put_shutdown(void)
{
	struct utmpx store = {};
...
int
utmp_put_init_process(const char *id, pid_t pid, pid_t sid, const char *line)
{
	struct utmpx store = {
...
int
utmp_put_dead_process(const char *id, pid_t pid, int code, int status)
{
	struct utmpx lookup = {
...
int
utmp_put_runlevel(int runlevel, int previous)
{
	struct utmpx store = {};
	int r;
...
static int
write_to_terminal(const char *tty, const char *message)
{
...
int
utmp_wall(const char *message, const char *username,
	bool (*match_tty)(const char *tty))
{
	...
	if (asprintf(&text,
		    ...
		    "%s\r\n\r\n",
		    un ?: username, hn, tty ? " on " : "", strempty(tty),
	...
	while ((u = getutxent())) {
		...
		if (path_startswith(u->ut_line, "/dev/"))
			...
			if (asprintf(&buf, "/dev/%.*s", (int)sizeof(u->ut_line),
				    u->ut_line) < 0)
				return -ENOMEM;
