...
sigbus_push(void *addr)
{
	...
	for (u = 0; u < SIGBUS_QUEUE_MAX; u++)
		if (__sync_bool_compare_and_swap(&sigbus_queue[u], NULL,
			    addr)) {
...
sigbus_pop(void **ret)
{
	...
	for (;;) {
		...
		for (u = 0; u < SIGBUS_QUEUE_MAX; u++) {
			...
			if (__sync_bool_compare_and_swap(&sigbus_queue[u], addr,
				    NULL)) {
...
static void
sigbus_handler(int sn, siginfo_t *si, void *data)
{
	unsigned long ul;
	void *aligned;
