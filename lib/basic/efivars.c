...
int
is_efi_secure_boot(void)
{
	return read_flag("SecureBoot");
}

int
is_efi_secure_boot_setup_mode(void)
{
	return read_flag("SetupMode");
}

int
efi_get_variable(sd_id128_t vendor, const char *name, uint32_t *attribute,
...
int
efi_get_variable_string(sd_id128_t vendor, const char *name, char **p)
{
...
static void
efi_guid_to_id128(const void *guid, sd_id128_t *id128)
{
	struct uuid {
		uint32_t u1;
		uint16_t u2;
...
int
efi_get_boot_option(uint16_t id, char **title, sd_id128_t *part_uuid,
	char **path)
{
	struct boot_option {
		uint32_t attr;
	...
	err = efi_get_variable(EFI_VENDOR_GLOBAL, boot_id, NULL, (void **)&buf,
		&l);
	if (err < 0)
		return err;
...
int
efi_get_boot_order(uint16_t **order)
{
	void *buf;
	size_t l;
	int r;
...
static int
cmp_uint16(const void *_a, const void *_b)
{
...
int
efi_get_boot_options(uint16_t **options)
{
...
static int
read_usec(sd_id128_t vendor, const char *name, usec_t *u)
{
...
int
efi_loader_get_boot_usec(usec_t *firmware, usec_t *loader)
{
	uint64_t x, y;
	int r;
...
int
efi_loader_get_device_part_uuid(sd_id128_t *u)
{
	_cleanup_free_ char *p = NULL;
	int r, parsed[16];

	r = efi_get_variable_string(EFI_VENDOR_LOADER, "LoaderDevicePartUUID",
		&p);
	if (r < 0)
		return r;
	...
	if (sscanf(p,
		    ...
		    &parsed[10], &parsed[11], &parsed[12], &parsed[13],
		    &parsed[14], &parsed[15]) != 16)
		return -EIO;
