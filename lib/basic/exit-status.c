...
const char *
exit_status_to_string(ExitStatus status, ExitStatusLevel level)
{
...
bool
is_clean_exit(int code, int status, ExitClean clean,
	ExitStatusSet *success_status)
{
	if (code == CLD_EXITED)
		return status == 0 ||
			(success_status &&
				set_contains(success_status->status,
					INT_TO_PTR(status)));
	...
	if (code == CLD_KILLED)
		return (clean == EXIT_CLEAN_DAEMON &&
			       IN_SET(status, SIGHUP, SIGINT, SIGTERM,
				       SIGPIPE)) ||
			(success_status &&
				set_contains(success_status->signal,
					INT_TO_PTR(status)));
...
is_clean_exit_lsb(int code, int status, ExitStatusSet *success_status)
{
	...
	return code == CLD_EXITED &&
		(status == EXIT_NOTINSTALLED || status == EXIT_NOTCONFIGURED);
}

void
exit_status_set_free(ExitStatusSet *x)
{
	assert(x);
...
bool
exit_status_set_is_empty(ExitStatusSet *x)
{
	if (!x)
		return true;
