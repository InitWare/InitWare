...
int
acl_find_uid(acl_t acl, uid_t uid, acl_entry_t *entry)
{
	acl_entry_t i;
	int r;
	...
	for (r = acl_get_entry(acl, ACL_FIRST_ENTRY, &i); r > 0;
...
int
calc_acl_mask_if_needed(acl_t *acl_p)
{
	acl_entry_t i;
	int r;
...
int
add_base_acls_if_needed(acl_t *acl_p, const char *path)
{
	acl_entry_t i;
	int r;
...
int
acl_search_groups(const char *path, char ***ret_groups)
{
...
int
parse_acl(char *text, acl_t *acl_access, acl_t *acl_default, bool want_mask)
{
	_cleanup_free_ char **a = NULL,
			    **d = NULL; /* strings are not be freed */
...
static int
acl_entry_equal(acl_entry_t a, acl_entry_t b)
{
...
static int
find_acl_entry(acl_t acl, acl_entry_t entry, acl_entry_t *out)
{
	acl_entry_t i;
	int r;
...
int
acls_for_file(const char *path, acl_type_t type, acl_t new, acl_t *acl)
{
	...
	for (r = acl_get_entry(new, ACL_FIRST_ENTRY, &i); r > 0;
