...
int
acpi_get_boot_usec(usec_t *loader_start, usec_t *loader_exit)
{
	...
	if (l < sizeof(struct acpi_table_header) +
	...
	for (rec = (struct acpi_fpdt_header *)(buf +
	...
	if (hbrec.length < sizeof(struct acpi_fpdt_boot_header) +
	...
	if (brec.startup_start == 0 ||
		brec.exit_services_exit < brec.startup_start)
		return -EINVAL;
