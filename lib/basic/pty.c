...
int
pty_make_parent(Pty *pty, pid_t child)
{
	assert_return(pty, -EINVAL);
	assert_return(pty_is_unknown(pty), -EALREADY);
...
pty_dispatch_read(Pty *pty)
{
	...
	for (i = 0; i < 8; ++i) {
		...
		pty->in_buf[len] = 0;
		r = pty->event_fn(pty, pty->event_fn_userdata, PTY_DATA,
			pty->in_buf, len);
		if (r < 0)
			return r;
	}
...
static int
pty_fd_fn(sd_event_source *source, int fd, uint32_t revents, void *userdata)
{
	Pty *pty = userdata;
	...
	if (r_hup < 0 || r_write < 0 || r_read < 0) {
		...
		pty_close(pty);
		r = pty->event_fn(pty, pty->event_fn_userdata, PTY_HUP, NULL,
			0);
		if (r < 0)
			return r;
	}
...
static int
pty_fd_prepare_fn(sd_event_source *source, void *userdata)
{
	Pty *pty = userdata;
	int r;
	...
	if (pty->needs_requeue) {
		/*
                 ...
                 * an event is pending.
                 */
		r = sd_event_source_set_io_events(source,
...
static int
pty_child_fn(sd_event_source *source, const siginfo_t *si, void *userdata)
{
	Pty *pty = userdata;
	int r;
	...
	r = pty->event_fn(pty, pty->event_fn_userdata, PTY_CHILD, si,
		sizeof(*si));
	if (r < 0)
		return r;
...
int
pty_attach_event(Pty *pty, sd_event *event, pty_event_t event_fn,
	void *event_fn_userdata)
{
	int r;
	...
	if (pty_is_open(pty)) {
		r = sd_event_add_io(event, &pty->fd_source, pty->fd,
			EPOLLHUP | EPOLLERR | EPOLLIN | EPOLLOUT | EPOLLET,
			pty_fd_fn, pty);
		if (r < 0)
			goto error;

		r = sd_event_source_set_prepare(pty->fd_source,
			pty_fd_prepare_fn);
		if (r < 0)
			goto error;
	}
	...
	if (pty_has_child(pty)) {
		r = sd_event_add_child(event, &pty->child_source, pty->child,
			WEXITED, pty_child_fn, pty);
		if (r < 0)
			goto error;
	}
...
int
pty_write(Pty *pty, const void *buf, size_t size)
{
	bool was_empty;
	int r;
...
int
pty_resize(Pty *pty, unsigned short term_width, unsigned short term_height)
{
	struct winsize ws = {
		.ws_col = term_width,
		.ws_row = term_height,
...
pid_t
pty_fork(Pty **out, sd_event *event, pty_event_t event_fn,
	void *event_fn_userdata, unsigned short initial_term_width,
	unsigned short initial_term_height)
{
	_pty_unref_ Pty *pty = NULL;
	int r;
	pid_t pid;
	...
	if (pid == 0) {
		...
		if (event) {
			r = pty_attach_event(pty, event, event_fn,
				event_fn_userdata);
			if (r < 0)
				goto parent_error;
		}
