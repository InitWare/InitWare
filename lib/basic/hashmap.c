...
#define DIB_RAW_INIT                                                           \
	((char)DIB_RAW_FREE) /* a byte to memset a DIB store with when initializing */

#define DIB_FREE UINT_MAX

#ifdef ENABLE_DEBUG_HASHMAP
struct hashmap_debug_info {
	IWLIST_FIELDS(struct hashmap_debug_info, debug_list);
...
{
	return (struct hashmap_base_entry *)(storage_ptr(h) +
...
{
	return (dib_raw_t *)(storage_ptr(h) +
...
static void
bucket_move_entry(HashmapBase *h, struct swap_entries *swap, unsigned from,
	unsigned to)
{
	...
	if (h->type == HASHMAP_TYPE_ORDERED) {
		...
		if (le->iterate_next != IDX_NIL)
			ordered_bucket_at(lh, le->iterate_next)
				->iterate_previous = le->iterate_previous;
		else
	...
	enum HashmapType type HASHMAP_DEBUG_PARAMS)
{
	...
	h = use_pool ? mempool_alloc0_tile(hi->mempool) :
			     malloc0(hi->head_size);
...
#ifdef ENABLE_DEBUG_HASHMAP
	IWLIST_PREPEND(debug_list, hashmap_debug_list, &h->debug);
	h->debug.func = func;
	h->debug.file = file;
	h->debug.line = line;
#endif
...
static int
hashmap_base_ensure_allocated(HashmapBase **h, const struct hash_ops *hash_ops,
	enum HashmapType type HASHMAP_DEBUG_PARAMS)
{
	HashmapBase *q;
...
#ifdef ENABLE_DEBUG_HASHMAP
	IWLIST_REMOVE(debug_list, hashmap_debug_list, &h->debug);
#endif
 ...
 */
static bool
hashmap_put_robin_hood(HashmapBase *h, unsigned idx, struct swap_entries *swap)
{
	dib_raw_t raw_dib, *dibs;
 ...
 */
static int
hashmap_base_put_boldly(HashmapBase *h, unsigned idx, struct swap_entries *swap,
	bool may_resize)
{
...
resize_buckets(HashmapBase *h, unsigned entries_add)
{
	...
	new_shift = log2u_round_up(
		MAX(new_n_buckets * (hi->entry_size + sizeof(dib_raw_t)),
		...
		do {
			...
			if (rehash_next)
				optimal_idx = bucket_hash(h,
					bucket_at_swap(&swap, IDX_PUT)->p.b.key);
		} while (rehash_next);
	}
...
int
hashmap_remove_and_put(Hashmap *h, const void *old_key, const void *new_key,
	void *value)
{
	struct swap_entries swap;
	struct plain_hashmap_entry *e;
...
int
hashmap_remove_and_replace(Hashmap *h, const void *old_key, const void *new_key,
	void *value)
{
	struct swap_entries swap;
	struct plain_hashmap_entry *e;
...
int
internal_hashmap_move_one(HashmapBase *h, HashmapBase *other, const void *key)
{
	struct swap_entries swap;
