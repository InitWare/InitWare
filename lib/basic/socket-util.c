...
int
socket_address_parse(SocketAddress *a, const char *s)
{
	char *e, *n;
	unsigned u;
	int r;
...
int
socket_address_parse_and_warn(SocketAddress *a, const char *s)
{
	SocketAddress b;
	int r;
	...
	if (!socket_ipv6_is_supported() &&
		b.sockaddr.sa.sa_family == AF_INET6) {
		log_warning(
...
int
socket_address_print(const SocketAddress *a, char **ret)
{
	int r;
...
bool
socket_address_can_accept(const SocketAddress *a)
{
	assert(a);
...
bool
socket_address_is(const SocketAddress *a, const char *s, int type)
{
...
const char *
socket_address_get_path(const SocketAddress *a)
{
	assert(a);
...
socket_ipv6_is_supported(void)
{
	...
	if (read_one_line_file("/proc/sys/net/ipv6/conf/all/disable_ipv6", &l) <
		0)
		return true;
...
bool
socket_address_matches_fd(const SocketAddress *a, int fd)
{
	SocketAddress b;
	socklen_t solen;
...
int
sockaddr_port(const struct sockaddr *_sa)
{
	...
	return ntohs(sa->sa.sa_family == AF_INET6 ? sa->in6.sin6_port :
							  sa->in.sin_port);
}

int
sockaddr_pretty(const struct sockaddr *_sa, socklen_t salen,
	bool translate_ipv6, bool include_port, char **ret)
{
	...
	case AF_INET: {
		...
		if (include_port) {
			if (asprintf(&p, "%u.%u.%u.%u:%u", a >> 24,
				    (a >> 16) & 0xFF, (a >> 8) & 0xFF, a & 0xFF,
				    ntohs(sa->in.sin_port)) < 0)
				return -ENOMEM;
		} else {
			if (asprintf(&p, "%u.%u.%u.%u", a >> 24,
				    (a >> 16) & 0xFF, (a >> 8) & 0xFF,
				    a & 0xFF) < 0)
				return -ENOMEM;
		}
	...
	case AF_INET6: {
		...
		if (translate_ipv6 &&
			memcmp(&sa->in6.sin6_addr, ipv4_prefix,
				sizeof(ipv4_prefix)) == 0) {
			const uint8_t *a = sa->in6.sin6_addr.s6_addr + 12;
			if (include_port) {
				if (asprintf(&p, "%u.%u.%u.%u:%u", a[0], a[1],
					    a[2], a[3],
					    ntohs(sa->in6.sin6_port)) < 0)
					return -ENOMEM;
			} else {
				if (asprintf(&p, "%u.%u.%u.%u", a[0], a[1],
					    a[2], a[3]) < 0)
					return -ENOMEM;
			}
		} else {
...
int
getpeername_pretty(int fd, bool include_port, char **ret)
{
...
int
getsockname_pretty(int fd, char **ret)
{
	union sockaddr_union sa;
	socklen_t salen = sizeof(sa);
...
int
socknameinfo_pretty(union sockaddr_union *sa, socklen_t salen, char **_ret)
{
	int r;
...
int
getnameinfo_pretty(int fd, char **ret)
{
	union sockaddr_union sa;
	socklen_t salen = sizeof(sa);
...
int
socket_address_unlink(SocketAddress *a)
{
	assert(a);
...
bool
sockaddr_equal(const union sockaddr_union *a, const union sockaddr_union *b)
{
	assert(a);
	assert(b);
