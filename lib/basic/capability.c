...
int
capability_update_inherited_set(cap_t caps, uint64_t set)
{
	unsigned long i;
	...
	for (i = 0; i < cap_last_cap(); i++) {
		if (set & (UINT64_C(1) << i)) {
			...
			if (cap_set_flag(caps, CAP_INHERITABLE, 1, &v,
				    CAP_SET) < 0)
				return -errno;
		}
	}
...
int
capability_ambient_set_apply(uint64_t set, bool also_inherit)
{
	unsigned long i;
...
int
capability_bounding_set_drop(uint64_t keep, bool right_now)
{
	...
	for (i = 0; i <= cap_last_cap(); i++) {
		if (!(keep & (UINT64_C(1) << i))) {
			...
			/* Also drop it from the inheritable set, so
                         ...
                         * capability for good. */
			if (cap_set_flag(after_cap, CAP_INHERITABLE, 1, &v,
				    CAP_CLEAR) < 0) {
				r = -errno;
				goto finish;
			}
			...
			if (right_now) {
				if (cap_set_flag(after_cap, CAP_PERMITTED, 1,
					    &v, CAP_CLEAR) < 0 ||
					cap_set_flag(after_cap, CAP_EFFECTIVE,
						1, &v, CAP_CLEAR) < 0) {
					r = -errno;
					goto finish;
				}
...
static int
drop_from_file(const char *fn, uint64_t keep)
{
	int r, k;
	uint32_t hi, lo;
...
int
capability_bounding_set_drop_usermode(uint64_t keep)
{
	int r;
...
int
drop_privileges(uid_t uid, gid_t gid, uint64_t keep_capabilities)
{
...
int
drop_capability(cap_value_t cv)
{
