...
static int
detect_vm_cpuid(const char **_id)
{
...
#define REG_b "rbx"
	...
	if (hypervisor) {
		...
		__asm__ __volatile__(
			...
			: "=a"(eax), "=r"(sig.sig32[0]), "=c"(sig.sig32[1]),
			"=d"(sig.sig32[2])
			: "0"(eax));
...
static int
detect_vm_devicetree(const char **_id)
{
...
detect_vm(const char **id)
{
	...
	for (;;) {
		...
		if (t) {
			if (startswith(t, "User Mode Linux")) {
				log_debug(
					"UML virtualization found in /proc/cpuinfo");
				_id = "uml";
				r = 1;
				goto finish;
			}
...
#if defined(__s390__)
	{
		...
		r = get_status_field("/proc/sysinfo",
			"VM00 Control Program:", &t);
		if (r >= 0) {
			if (streq(t, "z/VM"))
				_id = "zvm";
			else
...
int
detect_container(const char **id)
{
	static thread_local int cached_found = -1;
	static thread_local const char *cached_id = NULL;
