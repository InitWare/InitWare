...
int
mac_selinux_init(const char *prefix)
{
	int r = 0;
...
int
mac_selinux_fix(const char *path, bool ignore_enoent, bool ignore_erofs)
{
#ifdef HAVE_SELINUX
	struct stat st;
	int r;
	...
	if (r < 0) {
		...
		log_enforcing(
			"Unable to fix SELinux security context of %s: %m",
			path);
...
int
mac_selinux_apply(const char *path, const char *label)
{
#ifdef HAVE_SELINUX
	assert(path);
	assert(label);
	...
	if (setfilecon(path, (security_context_t)label) < 0) {
		log_enforcing(
			"Failed to set SELinux security context %s on path %s: %m",
			label, path);
		if (security_getenforce() == 1)
			return -errno;
	}
...
int
mac_selinux_get_create_label_from_exe(const char *exe, char **label)
{
	int r = -EOPNOTSUPP;

#ifdef HAVE_SELINUX
	_cleanup_security_context_free_ security_context_t mycon = NULL,
							   fcon = NULL;
	...
	r = security_compute_create(mycon, fcon, sclass,
...
int
mac_selinux_get_our_label(char **label)
{
	int r = -EOPNOTSUPP;
...
int
mac_selinux_get_child_mls_label(int socket_fd, const char *exe,
...
int
mac_selinux_create_socket_prepare(const char *label)
{
#ifdef HAVE_SELINUX
	if (!mac_selinux_use())
		return 0;
	...
	if (setsockcreatecon((security_context_t)label) < 0) {
		log_enforcing(
			"Failed to set SELinux security context %s for sockets: %m",
			label);
...
void
mac_selinux_create_socket_clear(void)
{
#ifdef HAVE_SELINUX
	PROTECT_ERRNO;
...
int
mac_selinux_bind(int fd, const struct sockaddr *addr, socklen_t addrlen)
{
