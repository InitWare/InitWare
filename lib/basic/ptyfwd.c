...
static bool
look_for_escape(PTYForward *f, const char *buffer, size_t n)
{
	const char *p;
	...
	while ((f->stdin_readable && f->in_buffer_full <= 0) ||
		...
		if (f->stdin_readable && f->in_buffer_full < LINE_MAX) {
			...
			if (k < 0) {
				...
				f->stdin_event_source = sd_event_source_unref(
					f->stdin_event_source);
			} else {
				/* Check if ^] has been
				...
				if (look_for_escape(f,
					    f->in_buffer + f->in_buffer_full,
					    k))
					return sd_event_exit(f->event,
						EXIT_FAILURE);
		...
		if (f->master_writable && f->in_buffer_full > 0) {
			...
			if (k < 0) {
				...
				memmove(f->in_buffer, f->in_buffer + k,
					f->in_buffer_full - k);
				f->in_buffer_full -= k;
			}
		}
		...
		if (f->stdout_writable && f->out_buffer_full > 0) {
			k = write(STDOUT_FILENO, f->out_buffer,
				f->out_buffer_full);
			if (k < 0) {
				if (errno == EAGAIN)
...
static int
on_master_event(sd_event_source *e, int fd, uint32_t revents, void *userdata)
{
...
static int
on_stdin_event(sd_event_source *e, int fd, uint32_t revents, void *userdata)
{
	PTYForward *f = userdata;
...
static int
on_stdout_event(sd_event_source *e, int fd, uint32_t revents, void *userdata)
{
	PTYForward *f = userdata;
...
static int
on_sigwinch_event(sd_event_source *e, const struct signalfd_siginfo *si,
	void *userdata)
{
	PTYForward *f = userdata;
	struct winsize ws;
...
int
pty_forward_new(sd_event *event, int master, bool ignore_vhangup,
	bool read_only, PTYForward **ret)
{
	...
	if (!read_only) {
		...
		r = sd_event_add_io(f->event, &f->stdin_event_source,
			STDIN_FILENO, EPOLLIN | EPOLLET, on_stdin_event, f);
		if (r < 0 && r != -EPERM)
			return r;
	}
...
pty_forward_free(PTYForward *f)
{
	if (f) {
		...
		if (f->saved_stdout)
			tcsetattr(STDOUT_FILENO, TCSANOW,
				&f->saved_stdout_attr);
		if (f->saved_stdin)
...
int
pty_forward_get_last_char(PTYForward *f, char *ch)
{
	assert(f);
	assert(ch);
...
int
pty_forward_set_ignore_vhangup(PTYForward *f, bool ignore_vhangup)
{
	int r;
...
int
pty_forward_get_ignore_vhangup(PTYForward *f)
{
	assert(f);
