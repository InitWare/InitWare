...
int
socket_address_listen(const SocketAddress *a, int flags, int backlog,
	...
	mode_t socket_mode, const char *label)
{
	...
	if (socket_address_family(a) == AF_INET6 &&
		only != SOCKET_ADDRESS_DEFAULT) {
		int flag = only == SOCKET_ADDRESS_IPV6_ONLY;

		if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &flag,
			    sizeof(flag)) < 0)
			return -errno;
	}
...
int
make_socket_fd(int log_level, const char *address, int flags)
{
	SocketAddress a;
	int fd, r;
	...
	if (r < 0) {
		log_error("Failed to parse socket address \"%s\": %s", address,
			strerror(-r));
		return r;
	}
