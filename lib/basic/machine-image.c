...
static const char image_search_path[] = "/var/lib/machines\0"
					"/var/lib/container\0"
...
static int
image_new(ImageType t, const char *pretty, const char *path,
	const char *filename, bool read_only, usec_t crtime, usec_t mtime,
	Image **ret)
{
...
static int
image_make(const char *pretty, int dfd, const char *path, const char *filename,
	Image **ret)
{
	struct stat st;
	bool read_only;
	...
	if (S_ISDIR(st.st_mode)) {
		...
		if (st.st_ino == 256) {
			...
			if (r) {
				...
				r = image_new(IMAGE_SUBVOLUME, pretty, path,
					filename, info.read_only || read_only,
					info.otime, 0, ret);
				if (r < 0)
					return r;
...
int
image_find(const char *name, Image **ret)
{
	const char *path;
	int r;
...
image_discover(Hashmap *h)
{
	...
	NULSTR_FOREACH (path, image_search_path) {
		...
		FOREACH_DIRENT_ALL (de, d, return -errno) {
			...
			r = image_make(NULL, dirfd(d), path, de->d_name,
				&image);
			if (r == 0 || r == -ENOENT)
				continue;
			if (r < 0)
...
void
image_hashmap_free(Hashmap *map)
{
	Image *i;
...
int
image_remove(Image *i)
{
	_cleanup_release_lock_file_ LockFile global_lock = LOCK_FILE_INIT,
					     local_lock = LOCK_FILE_INIT;
	int r;

	assert(i);

	if (path_equal(i->path, "/") || path_startswith(i->path, "/usr"))
		return -EROFS;

	/* Make sure we don't interfere with a running nspawn */
	r = image_path_lock(i->path, LOCK_EX | LOCK_NB, &global_lock,
		&local_lock);
	if (r < 0)
		return r;
...
int
image_rename(Image *i, const char *new_name)
{
	_cleanup_release_lock_file_ LockFile global_lock = LOCK_FILE_INIT,
					     local_lock = LOCK_FILE_INIT,
...
int
image_clone(Image *i, const char *new_name, bool read_only)
{
...
int
image_read_only(Image *i, bool b)
{
	_cleanup_release_lock_file_ LockFile global_lock = LOCK_FILE_INIT,
					     local_lock = LOCK_FILE_INIT;
	int r;
...
int
image_path_lock(const char *path, int operation, LockFile *global,
	LockFile *local)
{
...
int
image_name_lock(const char *name, int operation, LockFile *ret)
{
	const char *p;
...
bool
image_name_is_valid(const char *s)
{
