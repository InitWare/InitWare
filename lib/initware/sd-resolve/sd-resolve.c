...
struct sd_resolve {
	...
	IWLIST_HEAD(sd_resolve_query, queries);
};
...
struct sd_resolve_query {
	...
	IWLIST_FIELDS(sd_resolve_query, queries);
};
...
#define RESOLVE_DONT_DESTROY(resolve)                                          \
	_cleanup_resolve_unref_ _unused_ sd_resolve *_dont_destroy_##resolve = \
		sd_resolve_ref(resolve)

static int
send_died(int out_fd)
{
	RHeader rh = {
		.type = RESPONSE_DIED,
...
static void *
serialize_addrinfo(void *p, const struct addrinfo *ai, size_t *length,
	size_t maxlength)
{
	...
	if (ai->ai_canonname)
		memcpy((char *)p + sizeof(AddrInfoSerialization) +
				ai->ai_addrlen,
			ai->ai_canonname, cnl);
...
static int
send_addrinfo_reply(int out_fd, unsigned id, int ret, struct addrinfo *ai,
	int _errno, int _h_errno)
{
	...
	iov[0] = (struct iovec){ .iov_base = &resp,
		.iov_len = sizeof(AddrInfoResponse) };
	iov[1] = (struct iovec){ .iov_base = &buffer,
		.iov_len = resp.header.length - sizeof(AddrInfoResponse) };
...
static int
send_nameinfo_reply(int out_fd, unsigned id, int ret, const char *host,
	const char *serv, int _errno, int _h_errno)
{
	...
	iov[0] = (struct iovec){ .iov_base = &resp,
		.iov_len = sizeof(NameInfoResponse) };
...
static int
send_res_reply(int out_fd, unsigned id, const unsigned char *answer, int ret,
	int _errno, int _h_errno)
{
	ResResponse resp = {
...
static int
handle_request(int out_fd, const Packet *packet, size_t length)
{
	const RHeader *req;
	...
	case REQUEST_ADDRINFO: {
		...
		assert(length ==
			sizeof(AddrInfoRequest) + ai_req->node_len +
				ai_req->service_len);
		...
		node = ai_req->node_len ?
			      ...
			      NULL;
		service = ai_req->service_len ? (const char *)ai_req +
				sizeof(AddrInfoRequest) + ai_req->node_len :
						      NULL;
	...
	case REQUEST_NAMEINFO: {
		...
		return send_nameinfo_reply(out_fd, req->id, ret,
			...
			ret == 0 && ni_req->getserv ? servbuf : NULL, errno,
			h_errno);
	}
	...
	case REQUEST_RES_SEARCH: {
		...
		return send_res_reply(out_fd, req->id, (unsigned char *)&answer,
			ret, errno, h_errno);
	}
...
thread_worker(void *p)
{
	...
	while (!resolve->dead) {
		...
		length = recv(resolve->fds[REQUEST_RECV_FD], &buf, sizeof(buf),
			0);
		if (length < 0) {
			if (errno == EINTR)
				continue;
...
static int
start_threads(sd_resolve *resolve, unsigned extra)
{
	unsigned n;
	int r;
	...
	while (resolve->n_valid_workers < n) {
		r = pthread_create(&resolve->workers[resolve->n_valid_workers],
			NULL, thread_worker, resolve);
		if (r != 0)
			return -r;
...
static bool
resolve_pid_changed(sd_resolve *r)
{
	assert(r);
...
sd_resolve_new(sd_resolve **ret)
{
	...
	r = socketpair(PF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0,
		resolve->fds + REQUEST_RECV_FD);
	if (r < 0) {
		r = -errno;
		goto fail;
	}
...
_public_ int
sd_resolve_default(sd_resolve **ret)
{
...
_public_ int
sd_resolve_get_tid(sd_resolve *resolve, pid_t *tid)
{
...
static void
resolve_free(sd_resolve *resolve)
{
	PROTECT_ERRNO;
	sd_resolve_query *q;
	unsigned i;
	...
	if (resolve->fds[REQUEST_SEND_FD] >= 0) {
		RHeader req = { .type = REQUEST_TERMINATE,
			.length = sizeof(req) };

		/* Send one termination packet for each worker */
		for (i = 0; i < resolve->n_valid_workers; i++)
			send(resolve->fds[REQUEST_SEND_FD], &req, req.length,
				MSG_NOSIGNAL);
	}
...
_public_ sd_resolve *
sd_resolve_ref(sd_resolve *resolve)
{
...
_public_ sd_resolve *
sd_resolve_unref(sd_resolve *resolve)
{
	if (!resolve)
		return NULL;
...
_public_ int
sd_resolve_get_fd(sd_resolve *resolve)
{
	assert_return(resolve, -EINVAL);
	assert_return(!resolve_pid_changed(resolve), -ECHILD);
...
_public_ int
sd_resolve_get_events(sd_resolve *resolve)
{
	assert_return(resolve, -EINVAL);
	assert_return(!resolve_pid_changed(resolve), -ECHILD);
...
_public_ int
sd_resolve_get_timeout(sd_resolve *resolve, uint64_t *usec)
{
	assert_return(resolve, -EINVAL);
...
static sd_resolve_query *
lookup_query(sd_resolve *resolve, unsigned id)
{
	sd_resolve_query *q;
...
static int
complete_query(sd_resolve *resolve, sd_resolve_query *q)
{
	int r;
...
static int
unserialize_addrinfo(const void **p, size_t *length, struct addrinfo **ret_ai)
{
	AddrInfoSerialization s;
	size_t l;
	struct addrinfo *ai;
	...
	if (s.ai_addrlen > 0) {
		ai->ai_addr = memdup((const uint8_t *)*p +
	...
	if (s.canonname_len > 0) {
		ai->ai_canonname = memdup((const uint8_t *)*p +
				sizeof(AddrInfoSerialization) + s.ai_addrlen,
			s.canonname_len);
		if (!ai->ai_canonname) {
			free(ai->ai_addr);
			free(ai);
...
static int
handle_response(sd_resolve *resolve, const Packet *packet, size_t length)
{
	const RHeader *resp;
	sd_resolve_query *q;
	int r;
	...
	case RESPONSE_NAMEINFO: {
		...
		if (ni_resp->hostlen > 0) {
			q->host = strndup((const char *)ni_resp +
		...
		if (ni_resp->servlen > 0) {
			q->serv = strndup((const char *)ni_resp +
					sizeof(NameInfoResponse) +
					ni_resp->hostlen,
				ni_resp->servlen - 1);
			if (!q->serv) {
	...
	case RESPONSE_RES: {
		...
		assert(q->type == REQUEST_RES_QUERY ||
			q->type == REQUEST_RES_SEARCH);
		...
		if (res_resp->ret >= 0) {
			q->answer =
				memdup((const char *)resp + sizeof(ResResponse),
					res_resp->ret);
			if (!q->answer) {
				q->ret = -1;
				q->_errno = ENOMEM;
...
_public_ int
sd_resolve_process(sd_resolve *resolve)
{
...
_public_ int
sd_resolve_wait(sd_resolve *resolve, uint64_t timeout_usec)
{
	int r;
...
static int
alloc_query(sd_resolve *resolve, bool floating, sd_resolve_query **_q)
{
	sd_resolve_query *q;
	int r;
	...
	IWLIST_PREPEND(queries, resolve->queries, q);
	resolve->n_queries++;
...
_public_ int
sd_resolve_getaddrinfo(sd_resolve *resolve, sd_resolve_query **_q,
	...
	sd_resolve_getaddrinfo_handler_t callback, void *userdata)
{
	...
	req.header.length =
		sizeof(AddrInfoRequest) + req.node_len + req.service_len;
	...
	iov[mh.msg_iovlen++] = (struct iovec){ .iov_base = &req,
		.iov_len = sizeof(AddrInfoRequest) };
	if (node)
		iov[mh.msg_iovlen++] = (struct iovec){ .iov_base = (void *)node,
			.iov_len = req.node_len };
	if (service)
		iov[mh.msg_iovlen++] =
			(struct iovec){ .iov_base = (void *)service,
				.iov_len = req.service_len };
	mh.msg_iov = iov;
...
static int
getaddrinfo_done(sd_resolve_query *q)
{
	assert(q);
	assert(q->done);
...
_public_ int
sd_resolve_getnameinfo(sd_resolve *resolve, sd_resolve_query **_q,
	const struct sockaddr *sa, socklen_t salen, int flags, uint64_t get,
	sd_resolve_getnameinfo_handler_t callback, void *userdata)
{
	NameInfoRequest req = {};
	struct msghdr mh = {};
	struct iovec iov[2];
	...
	iov[0] = (struct iovec){ .iov_base = &req,
		.iov_len = sizeof(NameInfoRequest) };
...
static int
getnameinfo_done(sd_resolve_query *q)
{
	assert(q);
	assert(q->done);
...
static int
resolve_res(sd_resolve *resolve, sd_resolve_query **_q, QueryType qtype,
	...
	sd_resolve_res_handler_t callback, void *userdata)
{
	...
	iov[0] = (struct iovec){ .iov_base = &req,
		.iov_len = sizeof(ResRequest) };
	iov[1] = (struct iovec){ .iov_base = (void *)dname,
		.iov_len = req.dname_len };
...
_public_ int
sd_resolve_res_query(sd_resolve *resolve, sd_resolve_query **q,
	const char *dname, int class, int type,
	sd_resolve_res_handler_t callback, void *userdata)
{
...
_public_ int
sd_resolve_res_search(sd_resolve *resolve, sd_resolve_query **q,
	const char *dname, int class, int type,
	sd_resolve_res_handler_t callback, void *userdata)
{
...
static int
res_query_done(sd_resolve_query *q)
{
	assert(q);
	assert(q->done);
...
_public_ sd_resolve_query *
sd_resolve_query_ref(sd_resolve_query *q)
{
...
static void
resolve_freeaddrinfo(struct addrinfo *ai)
{
	while (ai) {
...
static void
resolve_query_disconnect(sd_resolve_query *q)
{
	sd_resolve *resolve;
	unsigned i;
	...
	IWLIST_REMOVE(queries, resolve->queries, q);
	resolve->n_queries--;
...
static void
resolve_query_free(sd_resolve_query *q)
{
	assert(q);
...
_public_ sd_resolve_query *
sd_resolve_query_unref(sd_resolve_query *q)
{
	if (!q)
		return NULL;
...
_public_ int
sd_resolve_query_is_done(sd_resolve_query *q)
{
...
_public_ void *
sd_resolve_query_set_userdata(sd_resolve_query *q, void *userdata)
{
	void *ret;
...
_public_ void *
sd_resolve_query_get_userdata(sd_resolve_query *q)
{
	assert_return(q, NULL);
	assert_return(!resolve_pid_changed(q->resolve), NULL);
...
_public_ sd_resolve *
sd_resolve_query_get_resolve(sd_resolve_query *q)
{
	assert_return(q, NULL);
	assert_return(!resolve_pid_changed(q->resolve), NULL);
...
_public_ int
sd_resolve_attach_event(sd_resolve *resolve, sd_event *event, int priority)
{
	int r;
	...
	r = sd_event_add_io(resolve->event, &resolve->event_source,
		resolve->fds[RESPONSE_RECV_FD], POLLIN, io_callback, resolve);
	if (r < 0)
		goto fail;
...
_public_ int
sd_resolve_detach_event(sd_resolve *resolve)
{
	assert_return(resolve, -EINVAL);
	...
	if (resolve->event_source) {
		sd_event_source_set_enabled(resolve->event_source,
			SD_EVENT_OFF);
		resolve->event_source =
			sd_event_source_unref(resolve->event_source);
	}
...
_public_ sd_event *
sd_resolve_get_event(sd_resolve *resolve)
{
	assert_return(resolve, NULL);
