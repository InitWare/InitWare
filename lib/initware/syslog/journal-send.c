...
sd_journal_printv(int priority, const char *format, va_list ap)
{
	...
	char buffer[8 + LINE_MAX],
		p[sizeof("PRIORITY=") - 1 + DECIMAL_STR_MAX(int) + 1];
	struct iovec iov[2];
...
sd_journal_sendv(const struct iovec *iov, int n)
{
	...
	struct sockaddr_un sa = {
		.sun_family = AF_UNIX,
		.sun_path = SVC_PKGRUNSTATEDIR "/journal/socket",
	};
	struct msghdr mh = {
		.msg_name = &sa,
...
static int
fill_iovec_perror_and_send(const char *message, int skip, struct iovec iov[])
{
	PROTECT_ERRNO;
	size_t n, k;
...
_public_ int
sd_journal_print_with_location(int priority, const char *file, const char *line,
	const char *func, const char *format, ...)
{
	...
	va_start(ap, format);
	r = sd_journal_printv_with_location(priority, file, line, func, format,
		ap);
	va_end(ap);
...
_public_ int
sd_journal_printv_with_location(int priority, const char *file,
...
_public_ int
sd_journal_perror_with_location(const char *file, const char *line,
