...
log_assert_errno(const char *text, int eno, const char *file, int line,
	const char *func)
{
	log_internal(LOG_CRIT, 0, file, line, func,
		"'%s' failed at %s:%u (%s): %s.", text, file, line, func,
		strerror(eno));
	abort();
}
...
#define assert_ret(expr)                                                       \
	do {                                                                   \
		int _r_ = (expr);                                              \
		if (_unlikely_(_r_ < 0))                                       \
			log_assert_errno(#expr, -_r_, __FILE__, __LINE__,      \
				__PRETTY_FUNCTION__);                          \
	} while (false)
...
static void
test_close(JournalFile *f)
{
	journal_file_close(f);
}

static void
append_number(JournalFile *f, int n, uint64_t *seqnum)
{
	char *p;
	dual_timestamp ts;
	...
	assert_ret(journal_file_append_entry(f, &ts, iovec, 1, seqnum, NULL,
		NULL));
	free(p);
}

static void
test_check_number(sd_journal *j, int n)
{
	const void *d;
...
static void
test_check_numbers_down(sd_journal *j, int count)
{
	int i;
...
static void
test_check_numbers_up(sd_journal *j, int count)
{
...
test_sequence_numbers(void)
{
	...
	assert_se(journal_file_open("one.journal", O_RDWR | O_CREAT, 0644, true,
	...
	assert_se(
		!sd_id128_equal(one->header->file_id, one->header->machine_id));
	...
	assert_se(journal_file_open("two.journal", O_RDWR | O_CREAT, 0644, true,
			  false, NULL, NULL, one, &two) == 0);

	assert_se(two->header->state == STATE_ONLINE);
	assert_se(!sd_id128_equal(two->header->file_id, one->header->file_id));
	assert_se(sd_id128_equal(one->header->machine_id,
		one->header->machine_id));
	assert_se(sd_id128_equal(one->header->boot_id, one->header->boot_id));
	assert_se(
		sd_id128_equal(one->header->seqnum_id, one->header->seqnum_id));
	...
	assert_se(journal_file_open("two.journal", O_RDWR, 0, true, false, NULL,
			  NULL, NULL, &two) == 0);
