...
static void
verify_contents(sd_journal *j, unsigned skip)
{
	unsigned i;
...
main(int argc, char *argv[])
{
	...
	for (i = 0; i < N_ENTRIES; i++) {
		...
		if (i % 10 == 0)
			assert_se(journal_file_append_entry(three, &ts, iovec,
					  2, NULL, NULL, NULL) == 0);
		else {
			if (i % 3 == 0)
				assert_se(journal_file_append_entry(two, &ts,
						  iovec, 2, NULL, NULL,
						  NULL) == 0);

			assert_se(journal_file_append_entry(one, &ts, iovec, 2,
					  NULL, NULL, NULL) == 0);
		}
