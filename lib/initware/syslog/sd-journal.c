...
static bool
journal_pid_changed(sd_journal *j)
{
	assert(j);
...
static int
journal_put_error(sd_journal *j, int r, const char *path)
{
	char *copy;
	int k;
...
static void
init_location(Location *l, LocationType type, JournalFile *f, Object *o)
{
	assert(l);
	...
	l->seqnum_set = l->realtime_set = l->monotonic_set = l->xor_hash_set =
		true;
}

static void
set_location(sd_journal *j, JournalFile *f, Object *o)
{
	assert(j);
	assert(f);
	assert(o);
...
static int
match_is_valid(const void *data, size_t size)
{
	const char *b, *p;
...
static bool
same_field(const void *_a, size_t s, const void *_b, size_t t)
{
...
match_new(Match *p, MatchType t)
{
	...
	if (p) {
		m->parent = p;
		IWLIST_PREPEND(matches, p->matches, m);
	}
...
match_free(Match *m)
{
	...
	if (m->parent)
		IWLIST_REMOVE(matches, m->parent->matches, m);
...
_public_ int
sd_journal_add_match(sd_journal *j, const void *data, size_t size)
{
...
_public_ int
sd_journal_add_conjunction(sd_journal *j)
{
	assert_return(j, -EINVAL);
	assert_return(!journal_pid_changed(j), -ECHILD);
...
_public_ int
sd_journal_add_disjunction(sd_journal *j)
{
	assert_return(j, -EINVAL);
	assert_return(!journal_pid_changed(j), -ECHILD);
...
char *
journal_make_match_string(sd_journal *j)
{
	assert(j);
...
_public_ void
sd_journal_flush_matches(sd_journal *j)
{
	if (!j)
		return;
...
_pure_ static int
compare_with_location(JournalFile *f, Location *l)
{
	assert(f);
	assert(l);
	assert(f->location_type == LOCATION_SEEK);
...
static int
next_for_match(sd_journal *j, Match *m, JournalFile *f, uint64_t after_offset,
	direction_t direction, Object **ret, uint64_t *offset)
{
	...
	if (m->type == MATCH_DISCRETE) {
		...
		r = journal_file_find_data_object_with_hash(f, m->data, m->size,
			le64toh(m->le_hash), NULL, &dp);
		if (r <= 0)
			return r;

		return journal_file_move_to_entry_by_offset_for_data(f, dp,
			after_offset, direction, ret, offset);
		...
		IWLIST_FOREACH (matches, i, m->matches) {
			...
			if (r < 0)
				...
				if (np == 0 ||
					(direction == DIRECTION_DOWN ? cp < np :
									     cp > np))
					np = cp;
			}
		}
		...
		r = next_for_match(j, m->matches, f, after_offset, direction,
			NULL, &np);
		if (r <= 0)
			return r;
		...
		IWLIST_LOOP_BUT_ONE(matches, i, m->matches, last_moved)
		{
			uint64_t cp;
			...
			assert(direction == DIRECTION_DOWN ? cp >= np :
								   cp <= np);
...
static int
find_location_for_match(sd_journal *j, Match *m, JournalFile *f,
	direction_t direction, Object **ret, uint64_t *offset)
{
	int r;
	...
	if (m->type == MATCH_DISCRETE) {
		...
		if (j->current_location.type == LOCATION_HEAD)
			return journal_file_next_entry_for_data(f, NULL, 0, dp,
				DIRECTION_DOWN, ret, offset);
		if (j->current_location.type == LOCATION_TAIL)
			return journal_file_next_entry_for_data(f, NULL, 0, dp,
				DIRECTION_UP, ret, offset);
		if (j->current_location.seqnum_set &&
			sd_id128_equal(j->current_location.seqnum_id,
				f->header->seqnum_id))
			return journal_file_move_to_entry_by_seqnum_for_data(f,
				dp, j->current_location.seqnum, direction, ret,
				offset);
		if (j->current_location.monotonic_set) {
			r = journal_file_move_to_entry_by_monotonic_for_data(f,
				dp, j->current_location.boot_id,
				j->current_location.monotonic, direction, ret,
				offset);
			if (r != -ENOENT)
				return r;
		}
		if (j->current_location.realtime_set)
			return journal_file_move_to_entry_by_realtime_for_data(
				f, dp, j->current_location.realtime, direction,
				ret, offset);
		...
		IWLIST_FOREACH (matches, i, m->matches) {
			...
			if (r < 0)
				...
				if (np == 0 ||
					(direction == DIRECTION_DOWN ? np > cp :
									     np < cp))
					np = cp;
			}
		}
...
static int
find_location_with_matches(sd_journal *j, JournalFile *f, direction_t direction,
	Object **ret, uint64_t *offset)
{
	...
	if (!j->level0) {
		...
		if (j->current_location.type == LOCATION_HEAD)
			return journal_file_next_entry(f, 0, DIRECTION_DOWN,
				ret, offset);
		if (j->current_location.type == LOCATION_TAIL)
			return journal_file_next_entry(f, 0, DIRECTION_UP, ret,
				offset);
		if (j->current_location.seqnum_set &&
			sd_id128_equal(j->current_location.seqnum_id,
				f->header->seqnum_id))
			return journal_file_move_to_entry_by_seqnum(f,
				j->current_location.seqnum, direction, ret,
				offset);
		if (j->current_location.monotonic_set) {
		...
		if (j->current_location.realtime_set)
			return journal_file_move_to_entry_by_realtime(f,
				j->current_location.realtime, direction, ret,
				offset);

		return journal_file_next_entry(f, 0, direction, ret, offset);
	} else
		return find_location_for_match(j, j->level0, f, direction, ret,
			offset);
}

static int
next_with_matches(sd_journal *j, JournalFile *f, direction_t direction,
	Object **ret, uint64_t *offset)
{
	assert(j);
	assert(f);
	...
	/* No matches is easy. We simple advance the file
         * pointer by one. */
	if (!j->level0)
		return journal_file_next_entry(f, f->current_offset, direction,
			ret, offset);
...
static int
next_beyond_location(sd_journal *j, JournalFile *f, direction_t direction)
{
	Object *c;
...
static int
real_journal_next(sd_journal *j, direction_t direction)
{
	...
	ORDERED_HASHMAP_FOREACH (f, j->files, i) {
		...
		if (r < 0) {
			log_debug_errno(r,
				"Can't iterate through %s, ignoring: %m",
				f->path);
...
_public_ int
sd_journal_previous(sd_journal *j)
{
	return real_journal_next(j, DIRECTION_UP);
}

static int
real_journal_next_skip(sd_journal *j, direction_t direction, uint64_t skip)
{
	int c = 0, r;
...
_public_ int
sd_journal_next_skip(sd_journal *j, uint64_t skip)
{
	return real_journal_next_skip(j, DIRECTION_DOWN, skip);
}

_public_ int
sd_journal_previous_skip(sd_journal *j, uint64_t skip)
{
...
sd_journal_get_cursor(sd_journal *j, char **cursor)
{
	...
	if (asprintf(cursor,
		    "s=%s;i=%" PRIx64 ";b=%s;m=%" PRIx64 ";t=%" PRIx64
		    ";x=%" PRIx64,
		    sid, le64toh(o->entry.seqnum), bid,
		    le64toh(o->entry.monotonic), le64toh(o->entry.realtime),
...
sd_journal_seek_cursor(sd_journal *j, const char *cursor)
{
	...
	if ((!seqnum_set || !seqnum_id_set) &&
		(!monotonic_set || !boot_id_set) && !realtime_set)
		return -EINVAL;
...
sd_journal_test_cursor(sd_journal *j, const char *cursor)
{
	...
	FOREACH_WORD_SEPARATOR(word, l, cursor, ";", state)
	{
		...
		case 's':
			...
			if (k < 0)
				return k;
			if (!sd_id128_equal(id,
				    j->current_file->header->seqnum_id))
				return 0;
			break;
...
_public_ int
sd_journal_seek_monotonic_usec(sd_journal *j, sd_id128_t boot_id, uint64_t usec)
{
	assert_return(j, -EINVAL);
	assert_return(!journal_pid_changed(j), -ECHILD);
...
check_network(sd_journal *j, int fd)
{
	...
	j->on_network = F_TYPE_EQUAL(sfs.f_type, CIFS_MAGIC_NUMBER) ||
...
static bool
file_has_type_prefix(const char *prefix, const char *filename)
{
	...
	return streq(filename, full) || streq(filename, tilded) ||
		startswith(filename, atted);
}

static bool
file_type_wanted(int flags, const char *filename)
{
	assert(filename);
...
add_any_file(sd_journal *j, const char *path)
{
	...
	if (stat(path, &st) < 0) {
		r = log_debug_errno(errno, "Failed to stat file '%s': %m",
			path);
		return -errno;
	}
	...
	if (!S_ISREG(st.st_mode)) {
		log_debug("Uh, file '%s' is not a regular file? Refusing.",
			path);
		return -EBADFD;
	}
	...
	r = journal_file_open(path, O_RDONLY, 0, false, false, NULL, j->mmap,
		NULL, &f);
	if (r < 0) {
...
static int
add_file(sd_journal *j, const char *prefix, const char *filename)
{
	const char *path;
...
static void
remove_file(sd_journal *j, const char *prefix, const char *filename)
{
	const char *path;
	JournalFile *f;
...
static void
remove_file_real(sd_journal *j, JournalFile *f)
{
	assert(j);
	assert(f);
...
static int
dirname_is_machine_id(const char *fn)
{
...
static bool
dirent_is_journal_file(const struct dirent *de)
{
	assert(de);
...
static bool
dirent_is_id128_subdir(const struct dirent *de)
{
	assert(de);
...
static int
directory_open(sd_journal *j, const char *path, DIR **ret)
{
	DIR *d;
...
static void
directory_enumerate(sd_journal *j, Directory *m, DIR *d)
{
	struct dirent *de;
...
static void
directory_watch(sd_journal *j, Directory *m, int fd, uint32_t mask)
{
	int r;
...
static int
add_directory(sd_journal *j, const char *prefix, const char *dirname)
{
	...
	if ((j->flags & SD_JOURNAL_LOCAL_ONLY) &&
		!(dirname_is_machine_id(dirname) > 0 ||
			path_startswith(prefix, "/run")))
		return 0;
	...
	if (r < 0) {
		r = log_debug_errno(errno, "Failed to open directory '%s': %m",
			path);
		goto fail;
	}
...
static int
add_root_directory(sd_journal *j, const char *p, bool missing_ok)
{
	_cleanup_closedir_ DIR *d = NULL;
	Directory *m;
	int r, k;
	...
	if ((j->flags & SD_JOURNAL_RUNTIME_ONLY) && !path_startswith(p, "/run"))
		return -EINVAL;
	...
	if (m->wd <= 0 && j->inotify_fd >= 0) {
		...
		if (m->wd > 0 &&
			hashmap_put(j->directories_by_wd, INT_TO_PTR(m->wd),
				m) < 0)
...
static void
remove_directory(sd_journal *j, Directory *d)
{
	assert(j);
...
static int
add_current_paths(sd_journal *j)
{
	Iterator i;
	JournalFile *f;
...
static sd_journal *
journal_new(int flags, const char *path)
{
	sd_journal *j;
...
_public_ int
sd_journal_open_container(sd_journal **ret, const char *machine, int flags)
{
	...
	assert_return(ret, -EINVAL);
	assert_return((flags & ~(SD_JOURNAL_LOCAL_ONLY | SD_JOURNAL_SYSTEM)) ==
			0,
		-EINVAL);
...
_public_ int
sd_journal_open_directory(sd_journal **ret, const char *path, int flags)
{
	sd_journal *j;
	int r;
...
_public_ int
sd_journal_open_files(sd_journal **ret, const char **paths, int flags)
{
	sd_journal *j;
	const char **path;
	int r;
...
_public_ int
sd_journal_get_monotonic_usec(sd_journal *j, uint64_t *ret,
...
static bool
field_is_valid(const char *field)
{
	const char *p;
...
_public_ int
sd_journal_get_data(sd_journal *j, const char *field, const void **data,
	size_t *size)
{
	JournalFile *f;
	uint64_t i, n;
	...
	for (i = 0; i < n; i++) {
		...
		if (compression) {
			...
			if (decompress_startswith(compression, o->data.payload,
				    l, &f->compress_buffer,
				    &f->compress_buffer_size, field,
				    field_length, '=')) {
				size_t rsize;

				r = decompress_blob(compression,
					o->data.payload, l, &f->compress_buffer,
					&f->compress_buffer_size, &rsize,
					j->data_threshold);
				if (r < 0)
					return r;
...
static int
return_data(sd_journal *j, JournalFile *f, Object *o, const void **data,
	size_t *size)
{
	size_t t;
	uint64_t l;
...
_public_ int
sd_journal_enumerate_data(sd_journal *j, const void **data, size_t *size)
{
	JournalFile *f;
	uint64_t p, n;
	le64_t le_hash;
...
static int
reiterate_all_paths(sd_journal *j)
{
	assert(j);
...
_public_ int
sd_journal_get_events(sd_journal *j)
{
	int fd;
...
_public_ int
sd_journal_get_timeout(sd_journal *j, uint64_t *timeout_usec)
{
	int fd;
...
static void
process_q_overflow(sd_journal *j)
{
	JournalFile *f;
	Directory *m;
	Iterator i;
	...
	ORDERED_HASHMAP_FOREACH (f, j->files, i) {
		...
		log_debug(
			"File '%s' hasn't been seen in this enumeration, removing.",
			f->path);
		remove_file_real(j, f);
	}
...
static void
process_inotify_event(sd_journal *j, struct inotify_event *e)
{
	Directory *d;
	...
	if (d) {
		...
		if (!(e->mask & IN_ISDIR) && e->len > 0 &&
			...
			if (e->mask &
				...
				(IN_DELETE | IN_MOVED_FROM | IN_UNMOUNT))
			...
			sd_id128_from_string(e->name, &id) >= 0) {
...
_public_ int
sd_journal_wait(sd_journal *j, uint64_t timeout_usec)
{
	int r;
	uint64_t t;
	...
	if (j->inotify_fd < 0) {
		...
		ORDERED_HASHMAP_FOREACH (f, j->files, i) {
			...
			if (r == -EIDRM)
				...
				log_error(
					"Failed to fstat() journal file '%s' : %m",
					f->path);
				continue;
			}
		}
...
_public_ int
sd_journal_get_cutoff_realtime_usec(sd_journal *j, uint64_t *from, uint64_t *to)
{
	Iterator i;
	JournalFile *f;
	bool first = true;
...
_public_ int
sd_journal_get_cutoff_monotonic_usec(sd_journal *j, sd_id128_t boot_id,
...
void
journal_print_header(sd_journal *j)
{
	Iterator i;
	JournalFile *f;
	bool newline = false;
...
_public_ int
sd_journal_get_usage(sd_journal *j, uint64_t *bytes)
{
	Iterator i;
	JournalFile *f;
	uint64_t sum = 0;
...
_public_ int
sd_journal_query_unique(sd_journal *j, const char *field)
{
	char *f;
...
_public_ int
sd_journal_enumerate_unique(sd_journal *j, const void **data, size_t *l)
{
	size_t k;
	...
	for (;;) {
		...
		if (j->unique_offset == 0) {
			r = journal_file_find_field_object(j->unique_file,
				j->unique_field, k, &o, NULL);
			if (r < 0)
				return r;

			j->unique_offset =
				r > 0 ? le64toh(o->field.head_data_offset) : 0;
		} else {
			r = journal_file_move_to_object(j->unique_file,
				OBJECT_DATA, j->unique_offset, &o);
			if (r < 0)
				return r;
		...
		if (j->unique_offset == 0) {
			j->unique_file = ordered_hashmap_next(j->files,
				j->unique_file->path);
			if (!j->unique_file)
				return 0;
		...
		if (ol <= k) {
			log_debug(
				...
				j->unique_file->path, j->unique_offset, ol,
				k + 1);
			return -EBADMSG;
		}

		if (memcmp(odata, j->unique_field, k) ||
			((const char *)odata)[k] != '=') {
			log_debug("%s:offset " OFSfmt
		...
		ORDERED_HASHMAP_FOREACH (of, j->files, i) {
			...
			r = journal_file_find_data_object_with_hash(of, odata,
				ol, le64toh(o->data.hash), &oo, &op);
			if (r < 0)
				return r;
...
_public_ void
sd_journal_restart_unique(sd_journal *j)
{
	if (!j)
		return;
...
_public_ int
sd_journal_reliable_fd(sd_journal *j)
{
	assert_return(j, -EINVAL);
	assert_return(!journal_pid_changed(j), -ECHILD);
...
static char *
lookup_field(const char *field, void *userdata)
{
...
_public_ int
sd_journal_get_catalog(sd_journal *j, char **ret)
{
	const void *data;
	size_t size;
	sd_id128_t id;
...
_public_ int
sd_journal_get_data_threshold(sd_journal *j, size_t *sz)
{
	assert_return(j, -EINVAL);
	assert_return(!journal_pid_changed(j), -ECHILD);
...
_public_ int
sd_journal_has_runtime_files(sd_journal *j)
{
	assert_return(j, -EINVAL);
...
_public_ int
sd_journal_has_persistent_files(sd_journal *j)
{
	assert_return(j, -EINVAL);
