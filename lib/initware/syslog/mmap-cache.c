...
_pure_ static bool window_matches(Window *w, int fd, int prot, uint64_t offset, size_t size)
{
	...
	return w->fd && fd == w->fd->fd && prot == w->prot && offset >= w->offset &&
...
static int try_context(MMapCache *m, int fd, int prot, unsigned context, bool keep_always,
    uint64_t offset, size_t size, void **ret, void **release_cookie)
{
...
static int find_mmap(MMapCache *m, int fd, int prot, unsigned context, bool keep_always,
    uint64_t offset, size_t size, void **ret, void **release_cookie)
{
...
static int add_mmap(MMapCache *m, int fd, int prot, unsigned context, bool keep_always,
