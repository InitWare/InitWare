...
static int
journal_file_set_online(JournalFile *f)
{
	assert(f);
...
int
journal_file_set_offline(JournalFile *f)
{
	assert(f);
...
void
journal_file_close(JournalFile *f)
{
	if (!f)
		return;
...
static int
journal_file_init_header(JournalFile *f, JournalFile *template)
{
	Header h = {};
	ssize_t k;
	int r;
...
static int
journal_file_refresh_header(JournalFile *f)
{
	sd_id128_t boot_id;
	int r;
...
static int
journal_file_verify_header(JournalFile *f)
{
	uint32_t flags;
	...
	if (flags & ~HEADER_INCOMPATIBLE_SUPPORTED) {
		if (flags & ~HEADER_INCOMPATIBLE_ANY)
			log_debug(
				"Journal file %s has unknown incompatible flags %" PRIx32,
				f->path, flags & ~HEADER_INCOMPATIBLE_ANY);
		flags = (flags & HEADER_INCOMPATIBLE_ANY) &
			~HEADER_INCOMPATIBLE_SUPPORTED;
		if (flags)
			log_debug(
				"Journal file %s uses incompatible flags %" PRIx32
	...
	if (f->writable && (flags & ~HEADER_COMPATIBLE_SUPPORTED)) {
		if (flags & ~HEADER_COMPATIBLE_ANY)
			log_debug(
				"Journal file %s has unknown compatible flags %" PRIx32,
				f->path, flags & ~HEADER_COMPATIBLE_ANY);
		flags = (flags & HEADER_COMPATIBLE_ANY) &
			~HEADER_COMPATIBLE_SUPPORTED;
		if (flags)
			log_debug(
				"Journal file %s uses compatible flags %" PRIx32
				" disabled at compilation time.",
				f->path, flags);
		return -EPROTONOSUPPORT;
	}
	...
	if ((le64toh(f->header->header_size) + le64toh(f->header->arena_size)) >
		(uint64_t)f->last_stat.st_size)
		return -ENODATA;

	if (le64toh(f->header->tail_object_offset) >
	...
	if (f->writable) {
		...
		if (state == STATE_ONLINE) {
			...
			log_debug("Journal file %s has unknown state %i.",
				f->path, state);
			return -EBUSY;
		}
	}
...
int
journal_file_fstat(JournalFile *f)
{
	assert(f);
	assert(f->fd >= 0);
...
static int
journal_file_allocate(JournalFile *f, uint64_t offset, uint64_t size)
{
	...
	old_size = le64toh(f->header->header_size) +
		le64toh(f->header->arena_size);
	...
	if (new_size <= old_size) {
		...
		if (f->last_stat_usec + LAST_STAT_REFRESH_USEC >
			now(CLOCK_MONOTONIC))
			return 0;
	...
	new_size = ((new_size + FILE_SIZE_INCREASE - 1) / FILE_SIZE_INCREASE) *
		FILE_SIZE_INCREASE;
	if (f->metrics.max_size > 0 && new_size > f->metrics.max_size)
	...
	/* Note that the glibc fallocate() fallback is very
           ...
           as we can. */
#if defined(SVC_PLATFORM_NetBSD) || defined(SVC_PLATFORM_OpenBSD)
	...
	f->header->arena_size =
		htole64(new_size - le64toh(f->header->header_size));

	return journal_file_fstat(f);
}

static unsigned
type_to_context(ObjectType type)
{
...
static int
journal_file_move_to(JournalFile *f, ObjectType type, bool keep_always,
	uint64_t offset, uint64_t size, void **ret)
{
	int r;
	...
	return mmap_cache_get(f->mmap, f->fd, f->prot, type_to_context(type),
		keep_always, offset, size, &f->last_stat, ret);
}

static uint64_t
minimum_header_size(Object *o)
{
...
int
journal_file_move_to_object(JournalFile *f, ObjectType type, uint64_t offset,
	Object **ret)
{
	int r;
	void *t;
	...
	r = journal_file_move_to(f, type, false, offset, sizeof(ObjectHeader),
		&t);
	if (r < 0)
		return r;
...
static uint64_t
journal_file_entry_seqnum(JournalFile *f, uint64_t *seqnum)
{
	uint64_t r;
...
int
journal_file_append_object(JournalFile *f, ObjectType type, uint64_t size,
...
static int
journal_file_setup_data_hash_table(JournalFile *f)
{
	uint64_t s, p;
	Object *o;
	int r;
	...
	r = journal_file_append_object(f, OBJECT_DATA_HASH_TABLE,
		offsetof(Object, hash_table.items) + s, &o, &p);
	if (r < 0)
		return r;
...
int
journal_file_map_data_hash_table(JournalFile *f)
{
	uint64_t s, p;
	void *t;
	int r;
	...
	r = journal_file_move_to(f, OBJECT_DATA_HASH_TABLE, true, p, s, &t);
	if (r < 0)
		return r;
...
int
journal_file_map_field_hash_table(JournalFile *f)
{
	uint64_t s, p;
	void *t;
	int r;
	...
	r = journal_file_move_to(f, OBJECT_FIELD_HASH_TABLE, true, p, s, &t);
	if (r < 0)
		return r;
...
static int
journal_file_link_field(JournalFile *f, Object *o, uint64_t offset,
	uint64_t hash)
{
	uint64_t p, h, m;
	int r;
...
static int
journal_file_link_data(JournalFile *f, Object *o, uint64_t offset,
	uint64_t hash)
{
	uint64_t p, h, m;
	int r;
...
int
journal_file_find_field_object_with_hash(JournalFile *f, const void *field,
	uint64_t size, uint64_t hash, Object **ret, uint64_t *offset)
{
...
int
journal_file_find_field_object(JournalFile *f, const void *field, uint64_t size,
	Object **ret, uint64_t *offset)
{
	uint64_t hash;
	...
	return journal_file_find_field_object_with_hash(f, field, size, hash,
		ret, offset);
}

int
journal_file_find_data_object_with_hash(JournalFile *f, const void *data,
	uint64_t size, uint64_t hash, Object **ret, uint64_t *offset)
{
	uint64_t p, osize, h, m;
	int r;
...
int
journal_file_find_data_object(JournalFile *f, const void *data, uint64_t size,
	Object **ret, uint64_t *offset)
{
	uint64_t hash;
...
static int
journal_file_append_field(JournalFile *f, const void *field, uint64_t size,
	Object **ret, uint64_t *offset)
{
	uint64_t hash, p;
	uint64_t osize;
	...
	r = journal_file_find_field_object_with_hash(f, field, size, hash, &o,
		&p);
	if (r < 0)
		return r;
	else if (r > 0) {
...
static int
journal_file_append_data(JournalFile *f, const void *data, uint64_t size,
	Object **ret, uint64_t *offset)
{
	uint64_t hash, p;
	uint64_t osize;
	...
	r = journal_file_find_data_object_with_hash(f, data, size, hash, &o,
		&p);
	if (r < 0)
		return r;
	if (r > 0) {
	...
	if (JOURNAL_FILE_COMPRESS(f) && size >= COMPRESSION_SIZE_THRESHOLD) {
		...
		compression =
			compress_blob(data, size, o->data.payload, &rsize);

		if (compression >= 0) {
			o->object.size =
				htole64(offsetof(Object, data.payload) + rsize);
			o->object.flags |= compression;

			log_debug("Compressed data object %" PRIu64
				  " -> %zu using %s",
				size, rsize,
	...
	if (eq && eq > data) {
		...
		/* Create field object ... */
		r = journal_file_append_field(f, data,
			(uint8_t *)eq - (uint8_t *)data, &fo, &fp);
		if (r < 0)
			return r;
...
uint64_t
journal_file_entry_n_items(Object *o)
{
	assert(o);
...
uint64_t
journal_file_entry_array_n_items(Object *o)
{
	assert(o);
...
uint64_t
journal_file_hash_table_n_items(Object *o)
{
	assert(o);
...
static int
link_entry_into_array(JournalFile *f, le64_t *first, le64_t *idx, uint64_t p)
{
	int r;
	...
	r = journal_file_append_object(f, OBJECT_ENTRY_ARRAY,
		offsetof(Object, entry_array.items) + n * sizeof(uint64_t), &o,
		&q);
	if (r < 0)
		return r;
...
static int
link_entry_into_array_plus_one(JournalFile *f, le64_t *extra, le64_t *first,
	le64_t *idx, uint64_t p)
{
	int r;
...
static int
journal_file_link_entry_item(JournalFile *f, Object *o, uint64_t offset,
	uint64_t i)
{
	uint64_t p;
	int r;
	...
	return link_entry_into_array_plus_one(f, &o->data.entry_offset,
		&o->data.entry_array_offset, &o->data.n_entries, offset);
}

static int
journal_file_link_entry(JournalFile *f, Object *o, uint64_t offset)
{
	uint64_t n, i;
	int r;
...
static int
journal_file_append_entry_internal(JournalFile *f, const dual_timestamp *ts,
...
void
journal_file_post_change(JournalFile *f)
{
	assert(f);
...
static int
entry_item_cmp(const void *_a, const void *_b)
{
...
int
journal_file_append_entry(JournalFile *f, const dual_timestamp *ts,
	const struct iovec iovec[], unsigned n_iovec, uint64_t *seqnum,
	Object **ret, uint64_t *offset)
{
	unsigned i;
	EntryItem *items;
	...
	for (i = 0; i < n_iovec; i++) {
		...
		r = journal_file_append_data(f, iovec[i].iov_base,
			iovec[i].iov_len, &o, &p);
		if (r < 0)
			return r;
	...
	r = journal_file_append_entry_internal(f, ts, xor_hash, items, n_iovec,
		seqnum, ret, offset);
...
static void
chain_cache_put(OrderedHashmap *h, ChainCacheItem *ci, uint64_t first,
	uint64_t array, uint64_t begin, uint64_t total, uint64_t last_index)
{
	if (!ci) {
...
static int
generic_array_get(JournalFile *f, uint64_t first, uint64_t i, Object **ret,
	uint64_t *offset)
{
	Object *o;
	uint64_t p = 0, a, t = 0;
...
static int
generic_array_get_plus_one(JournalFile *f, uint64_t extra, uint64_t first,
	uint64_t i, Object **ret, uint64_t *offset)
{
	Object *o;
...
enum { TEST_FOUND, TEST_LEFT, TEST_RIGHT };

static int
generic_array_bisect(JournalFile *f, uint64_t first, uint64_t n,
	uint64_t needle,
	...
	direction_t direction, Object **ret, uint64_t *offset, uint64_t *idx)
{
	...
	while (a > 0) {
		...
		r = journal_file_move_to_object(f, OBJECT_ENTRY_ARRAY, a,
			&array);
		if (r < 0)
			return r;
...
static int
generic_array_bisect_plus_one(JournalFile *f, uint64_t extra, uint64_t first,
	uint64_t n, uint64_t needle,
	int (*test_object)(JournalFile *f, uint64_t p, uint64_t needle),
	direction_t direction, Object **ret, uint64_t *offset, uint64_t *idx)
{
	int r;
	...
	r = generic_array_bisect(f, first, n - 1, needle, test_object,
		direction, ret, offset, idx);
...
_pure_ static int
test_object_offset(JournalFile *f, uint64_t p, uint64_t needle)
{
	assert(f);
	assert(p > 0);
...
int
journal_file_move_to_entry_by_seqnum(JournalFile *f, uint64_t seqnum,
	direction_t direction, Object **ret, uint64_t *offset)
{
	return generic_array_bisect(f, le64toh(f->header->entry_array_offset),
		le64toh(f->header->n_entries), seqnum, test_object_seqnum,
		direction, ret, offset, NULL);
}

static int
test_object_realtime(JournalFile *f, uint64_t p, uint64_t needle)
{
	Object *o;
	int r;
...
int
journal_file_move_to_entry_by_realtime(JournalFile *f, uint64_t realtime,
	direction_t direction, Object **ret, uint64_t *offset)
{
	return generic_array_bisect(f, le64toh(f->header->entry_array_offset),
		le64toh(f->header->n_entries), realtime, test_object_realtime,
		direction, ret, offset, NULL);
}

static int
test_object_monotonic(JournalFile *f, uint64_t p, uint64_t needle)
{
	Object *o;
	int r;
...
static int
find_data_object_by_boot_id(JournalFile *f, sd_id128_t boot_id, Object **o,
	uint64_t *b)
{
...
int
journal_file_move_to_entry_by_monotonic(JournalFile *f, sd_id128_t boot_id,
	uint64_t monotonic, direction_t direction, Object **ret,
	uint64_t *offset)
{
	Object *o;
	int r;
	...
	return generic_array_bisect_plus_one(f, le64toh(o->data.entry_offset),
		le64toh(o->data.entry_array_offset), le64toh(o->data.n_entries),
		monotonic, test_object_monotonic, direction, ret, offset, NULL);
}

void
journal_file_reset_location(JournalFile *f)
{
...
void
journal_file_save_location(JournalFile *f, Object *o, uint64_t offset)
{
...
int
journal_file_compare_locations(JournalFile *af, JournalFile *bf)
{
	assert(af);
	assert(bf);
...
int
journal_file_next_entry(JournalFile *f, uint64_t p, direction_t direction,
	Object **ret, uint64_t *offset)
{
	uint64_t i, n, ofs;
	int r;
	...
	if (p > 0 && (direction == DIRECTION_DOWN ? ofs <= p : ofs >= p)) {
...
int
journal_file_next_entry_for_data(JournalFile *f, Object *o, uint64_t p,
	uint64_t data_offset, direction_t direction, Object **ret,
	uint64_t *offset)
{
	uint64_t n, i;
	int r;
	...
	return generic_array_get_plus_one(f, le64toh(d->data.entry_offset),
		le64toh(d->data.entry_array_offset), i, ret, offset);
}

int
journal_file_move_to_entry_by_offset_for_data(JournalFile *f,
	uint64_t data_offset, uint64_t p, direction_t direction, Object **ret,
	uint64_t *offset)
{
	int r;
	Object *d;
	...
	return generic_array_bisect_plus_one(f, le64toh(d->data.entry_offset),
		le64toh(d->data.entry_array_offset), le64toh(d->data.n_entries),
		p, test_object_offset, direction, ret, offset, NULL);
}

int
journal_file_move_to_entry_by_monotonic_for_data(JournalFile *f,
	uint64_t data_offset, sd_id128_t boot_id, uint64_t monotonic,
	direction_t direction, Object **ret, uint64_t *offset)
{
	Object *o, *d;
	int r;
	...
	r = generic_array_bisect_plus_one(f, le64toh(o->data.entry_offset),
		le64toh(o->data.entry_array_offset), le64toh(o->data.n_entries),
		monotonic, test_object_monotonic, direction, NULL, &z, NULL);
	if (r <= 0)
		return r;
	...
	for (;;) {
		...
		r = journal_file_move_to_object(f, OBJECT_DATA, data_offset,
			&d);
		if (r < 0)
			return r;
		...
		r = generic_array_bisect_plus_one(f,
			le64toh(d->data.entry_offset),
			le64toh(d->data.entry_array_offset),
			le64toh(d->data.n_entries), z, test_object_offset,
			direction, NULL, &p, NULL);
		if (r <= 0)
			return r;
...
int
journal_file_move_to_entry_by_seqnum_for_data(JournalFile *f,
	uint64_t data_offset, uint64_t seqnum, direction_t direction,
	Object **ret, uint64_t *offset)
{
	Object *d;
	int r;
	...
	return generic_array_bisect_plus_one(f, le64toh(d->data.entry_offset),
		le64toh(d->data.entry_array_offset), le64toh(d->data.n_entries),
		seqnum, test_object_seqnum, direction, ret, offset, NULL);
}

int
journal_file_move_to_entry_by_realtime_for_data(JournalFile *f,
	uint64_t data_offset, uint64_t realtime, direction_t direction,
	Object **ret, uint64_t *offset)
{
	Object *d;
	int r;
	...
	return generic_array_bisect_plus_one(f, le64toh(d->data.entry_offset),
		le64toh(d->data.entry_array_offset), le64toh(d->data.n_entries),
		realtime, test_object_realtime, direction, ret, offset, NULL);
}

void
journal_file_dump(JournalFile *f)
{
	Object *o;
	int r;
	uint64_t p;
	...
	while (p != 0) {
		...
		case OBJECT_TAG:
			printf("Type: OBJECT_TAG seqnum=%" PRIu64
			       " epoch=%" PRIu64 "\n",
...
static const char *
format_timestamp_safe(char *buf, size_t l, usec_t t)
{
	const char *x;
...
void
journal_file_print_header(JournalFile *f)
{
	...
	printf("File Path: %s\n"
		...
		f->header->state == STATE_OFFLINE	   ? "OFFLINE" :
			...
			f->header->state == STATE_ARCHIVED ? "ARCHIVED" :
								   "UNKNOWN",
		JOURNAL_HEADER_SEALED(f->header) ? " SEALED" : "",
		(le32toh(f->header->compatible_flags) &
		...
		JOURNAL_HEADER_COMPRESSED_LZ4(f->header) ? " COMPRESSED-LZ4" :
								 "",
		(le32toh(f->header->incompatible_flags) &
		...
		format_timespan(z, sizeof(z),
			le64toh(f->header->tail_entry_monotonic),
			USEC_PER_MSEC),
		le64toh(f->header->n_objects), le64toh(f->header->n_entries));
...
journal_file_warn_btrfs(JournalFile *f)
{
	...
	if (attrs & FS_NOCOW_FL) {
		log_debug(
			"Detected btrfs file system with copy-on-write disabled, all is good.");
		return 0;
	}
	...
	JournalFile *template, JournalFile **ret)
{
	...
	if (!endswith(fname, ".journal") && !endswith(fname, ".journal~"))
		return -EINVAL;
	...
	r = mmap_cache_get(f->mmap, f->fd, f->prot, CONTEXT_HEADER, true, 0,
		PAGE_ALIGN(sizeof(Header)), &f->last_stat, &h);
	if (r < 0)
		goto fail;
...
int
journal_file_get_cutoff_realtime_usec(JournalFile *f, usec_t *from, usec_t *to)
{
	assert(f);
	assert(from || to);
...
int
journal_file_get_cutoff_monotonic_usec(JournalFile *f, sd_id128_t boot_id,
	usec_t *from, usec_t *to)
{
	...
	if (to) {
		...
		r = generic_array_get_plus_one(f, le64toh(o->data.entry_offset),
			le64toh(o->data.entry_array_offset),
			le64toh(o->data.n_entries) - 1, &o, NULL);
		if (r <= 0)
			return r;
...
bool
journal_file_rotate_suggested(JournalFile *f, usec_t max_file_usec)
{
	assert(f);
	...
	if (le64toh(f->header->header_size) < sizeof(Header)) {
		log_debug("%s uses an outdated header, suggesting rotation.",
			f->path);
		return true;
	}
	...
	if (JOURNAL_HEADER_CONTAINS(f->header, n_data))
		if (le64toh(f->header->n_data) * 4ULL >
			(le64toh(f->header->data_hash_table_size) /
				...
				3ULL) {
			log_debug(
				"Data hash table of %s has a fill level at %.1f (%" PRIu64
				" of %" PRIu64
