...
static int journal_file_object_verify(JournalFile *f, uint64_t offset, Object *o)
{
	...
	if ((o->object.flags & OBJECT_COMPRESSED) && o->object.type != OBJECT_DATA)
		return -EBADMSG;
	...
	case OBJECT_DATA: {
		...
		if (le64toh(o->object.size) - offsetof(DataObject, payload) <= 0) {
			log_error(OFSfmt ": bad object size (<= %zu): %" PRIu64, offset,
		...
		if (o->object.flags & OBJECT_COMPRESSED) {
			...
			if (!uncompress_blob(o->data.payload,
				le64toh(o->object.size) - offsetof(Object, data.payload), &b,
				&alloc, &b_size, 0)) {
			...
			h2 = hash64(o->data.payload,
			    le64toh(o->object.size) - offsetof(Object, data.payload));

		if (h1 != h2) {
			log_error(OFSfmt ": invalid hash (%08" PRIx64 " vs. %08" PRIx64, offset,
			    h1, h2);
			return -EBADMSG;
		}

		if (!VALID64(o->data.next_hash_offset) || !VALID64(o->data.next_field_offset) ||
		    !VALID64(o->data.entry_offset) || !VALID64(o->data.entry_array_offset)) {
			log_error(OFSfmt ": invalid offset (next_hash_offset=" OFSfmt
			    ...
			    offset, o->data.next_hash_offset, o->data.next_field_offset,
			    o->data.entry_offset, o->data.entry_array_offset);
			return -EBADMSG;
		}
	...
	case OBJECT_FIELD:
		if (le64toh(o->object.size) - offsetof(FieldObject, payload) <= 0) {
			log_error(OFSfmt ": bad field size (<= %zu): %" PRIu64, offset,
			    offsetof(FieldObject, payload), le64toh(o->object.size));
			return -EBADMSG;
		}

		if (!VALID64(o->field.next_hash_offset) || !VALID64(o->field.head_data_offset)) {
			log_error(OFSfmt ": invalid offset (next_hash_offset=" OFSfmt
					 ", head_data_offset=" OFSfmt,
			    offset, o->field.next_hash_offset, o->field.head_data_offset);
			return -EBADMSG;
		}
		break;

	case OBJECT_ENTRY:
		if ((le64toh(o->object.size) - offsetof(EntryObject, items)) % sizeof(EntryItem) !=
		    0) {
			log_error(OFSfmt ": bad entry size (<= %zu): %" PRIu64, offset,
		...
		if ((le64toh(o->object.size) - offsetof(EntryObject, items)) / sizeof(EntryItem) <=
		    0) {
			log_error(OFSfmt ": invalid number items in entry: %" PRIu64, offset,
		...
		if (le64toh(o->entry.seqnum) <= 0) {
			log_error(OFSfmt ": invalid entry seqnum: %" PRIx64, offset,
			    le64toh(o->entry.seqnum));
			return -EBADMSG;
		}

		if (!VALID_REALTIME(le64toh(o->entry.realtime))) {
			log_error(OFSfmt ": invalid entry realtime timestamp: %" PRIu64, offset,
			    le64toh(o->entry.realtime));
			return -EBADMSG;
		}

		if (!VALID_MONOTONIC(le64toh(o->entry.monotonic))) {
			log_error(OFSfmt ": invalid entry monotonic timestamp: %" PRIu64, offset,
		...
		for (i = 0; i < journal_file_entry_n_items(o); i++) {
			if (o->entry.items[i].object_offset == 0 ||
				...
				log_error(OFSfmt ": invalid entry item (%" PRIu64 "/%" PRIu64
						 " offset: " OFSfmt,
				    offset, i, journal_file_entry_n_items(o),
	...
	case OBJECT_FIELD_HASH_TABLE:
		if ((le64toh(o->object.size) - offsetof(HashTableObject, items)) % sizeof(HashItem) !=
			0 ||
		    (le64toh(o->object.size) - offsetof(HashTableObject, items)) / sizeof(HashItem) <=
			0) {
			log_error(OFSfmt ": invalid %s hash table size: %" PRIu64, offset,
		...
		for (i = 0; i < journal_file_hash_table_n_items(o); i++) {
			if (o->hash_table.items[i].head_hash_offset != 0 &&
				...
				log_error(OFSfmt ": invalid %s hash table item (%" PRIu64
						 "/%" PRIu64 ") head_hash_offset: " OFSfmt,
				    offset,
				    o->object.type == OBJECT_DATA_HASH_TABLE ? "data" : "field", i,
			...
			if (o->hash_table.items[i].tail_hash_offset != 0 &&
				...
				log_error(OFSfmt ": invalid %s hash table item (%" PRIu64
						 "/%" PRIu64 ") tail_hash_offset: " OFSfmt,
				    offset,
				    o->object.type == OBJECT_DATA_HASH_TABLE ? "data" : "field", i,
				    journal_file_hash_table_n_items(o),
	...
	case OBJECT_ENTRY_ARRAY:
		if ((le64toh(o->object.size) - offsetof(EntryArrayObject, items)) % sizeof(le64_t) !=
			0 ||
		    (le64toh(o->object.size) - offsetof(EntryArrayObject, items)) / sizeof(le64_t) <=
			0) {
			log_error(OFSfmt ": invalid object entry array size: %" PRIu64, offset,
			    le64toh(o->object.size));
			return -EBADMSG;
		}
		...
		if (!VALID64(o->entry_array.next_entry_array_offset)) {
			log_error(OFSfmt
			    ...
			    offset, o->entry_array.next_entry_array_offset);
			return -EBADMSG;
		}
		...
		for (i = 0; i < journal_file_entry_array_n_items(o); i++)
			if (le64toh(o->entry_array.items[i]) != 0 &&
				...
				log_error(OFSfmt ": invalid object entry array item (%" PRIu64
						 "/%" PRIu64 "): " OFSfmt,
				    offset, i, journal_file_entry_array_n_items(o),
	...
	case OBJECT_TAG:
		if (le64toh(o->object.size) != sizeof(TagObject)) {
			log_error(OFSfmt ": invalid object tag size: %" PRIu64, offset,
			    le64toh(o->object.size));
			return -EBADMSG;
		}

		if (!VALID_EPOCH(o->tag.epoch)) {
			log_error(OFSfmt ": invalid object tag epoch: %" PRIu64, offset,
			    o->tag.epoch);
			return -EBADMSG;
		}
...
static int contains_uint64(MMapCache *m, int fd, uint64_t n, uint64_t p)
{
	...
	while (a < b) {
		...
		r = mmap_cache_get(m, fd, PROT_READ | PROT_WRITE, 0, false, c * sizeof(uint64_t),
		    sizeof(uint64_t), NULL, (void **) &z, NULL);
		if (r < 0)
			return r;
...
static int entry_points_to_data(JournalFile *f, int entry_fd, uint64_t n_entries, uint64_t entry_p,
    uint64_t data_p)
{
	...
	while (i < n) {
		...
		if (entry_p <= le64toh(o->entry_array.items[u - 1])) {
			...
			log_error("Entry object doesn't exist in main entry array at %" PRIu64,
			    entry_p);
			return -EBADMSG;
		}
...
static int verify_data(JournalFile *f, Object *o, uint64_t p, int entry_fd, uint64_t n_entries,
    int entry_array_fd, uint64_t n_entry_arrays)
{
	...
	if (a && n < 2) {
		log_error("Entry array present (entry_array_offset=%" PRIu64
...
static int verify_hash_table(JournalFile *f, int data_fd, uint64_t n_data, int entry_fd,
    uint64_t n_entries, int entry_array_fd, uint64_t n_entry_arrays, usec_t *last_usec,
    bool show_progress)
{
	...
	for (i = 0; i < n; i++) {
		...
		while (p != 0) {
			...
			if (!contains_uint64(f->mmap, data_fd, n_data, p)) {
				log_error("Invalid data object at hash entry %" PRIu64
					  " of %" PRIu64,
				    i, n);
				return -EBADMSG;
			}
			...
			if (next != 0 && next <= p) {
				log_error("Hash chain has a cycle in hash entry %" PRIu64
					  " of %" PRIu64,
				    i, n);
				return -EBADMSG;
			}
			...
			if (le64toh(o->data.hash) % n != i) {
				log_error("Hash value mismatch in hash entry %" PRIu64
					  " of %" PRIu64,
				    i, n);
				return -EBADMSG;
			}

			r = verify_data(f, o, p, entry_fd, n_entries, entry_array_fd,
			    n_entry_arrays);
			if (r < 0)
				return r;
...
static int verify_entry(JournalFile *f, Object *o, uint64_t p, int data_fd, uint64_t n_data)
{
...
static int verify_entry_array(JournalFile *f, int data_fd, uint64_t n_data, int entry_fd,
    uint64_t n_entries, int entry_array_fd, uint64_t n_entry_arrays, usec_t *last_usec,
    bool show_progress)
{
	...
	while (i < n) {
		...
		for (j = 0; i < n && j < m; i++, j++) {
			...
			if (p <= last) {
				log_error("Entry array not sorted at %" PRIu64 " of %" PRIu64, i, n);
				return -EBADMSG;
			}
			last = p;

			if (!contains_uint64(f->mmap, entry_fd, n_entries, p)) {
				log_error("Invalid array entry at %" PRIu64 " of %" PRIu64, i, n);
				return -EBADMSG;
			}
...
int journal_file_verify(JournalFile *f, const char *key, usec_t *first_contained,
    usec_t *last_validated, usec_t *last_contained, bool show_progress)
{
	int r;
	Object *o;
	...
	uint64_t n_weird = 0, n_objects = 0, n_entries = 0, n_data = 0, n_fields = 0,
		 n_data_hash_tables = 0, n_field_hash_tables = 0, n_entry_arrays = 0, n_tags = 0;
	usec_t last_usec = 0;
	...
	while (p != 0) {
		if (show_progress)
			draw_progress(0x7FFF * p / le64toh(f->header->tail_object_offset),
			    &last_usec);
		...
		case OBJECT_ENTRY:
			...
			if (entry_seqnum_set && entry_seqnum >= le64toh(o->entry.seqnum)) {
				log_error("Entry sequence number out of synchronization at " OFSfmt,
				    p);
				r = -EBADMSG;
				goto fail;
			}
		...
		case OBJECT_TAG:
			...
			if (le64toh(o->tag.seqnum) != n_tags + 1) {
				log_error("Tag sequence number out of synchronization at " OFSfmt,
				    p);
				r = -EBADMSG;
				goto fail;
			}
			...
			if (f->seal) {
				...
				if (entry_realtime_set &&
					...
					log_error("Tag/entry realtime timestamp out of synchronization at " OFSfmt,
					    p);
					r = -EBADMSG;
					goto fail;
				}
	...
	r = verify_entry_array(f, data_fd, n_data, entry_fd, n_entries, entry_array_fd,
	    n_entry_arrays, &last_usec, show_progress);
	if (r < 0)
		goto fail;

	r = verify_hash_table(f, data_fd, n_data, entry_fd, n_entries, entry_array_fd,
	    n_entry_arrays, &last_usec, show_progress);
	if (r < 0)
		goto fail;
...
fail:
	...
	log_error("File corruption detected at %s:" OFSfmt " (of %llu bytes, %" PRIu64 "%%).",
	    f->path, p, (unsigned long long) f->last_stat.st_size, 100 * p / f->last_stat.st_size);
