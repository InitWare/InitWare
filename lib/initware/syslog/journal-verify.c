...
static void
draw_progress(uint64_t p, usec_t *last_usec)
{
	unsigned n, i, j, k;
	usec_t z, x;
...
static uint64_t
scale_progress(uint64_t scale, uint64_t p, uint64_t m)
{
...
static int
journal_file_object_verify(JournalFile *f, uint64_t offset, Object *o)
{
	uint64_t i;
	...
	case OBJECT_DATA: {
		...
		if (h1 != h2) {
			error(offset,
				"Invalid hash (%08" PRIx64 " vs. %08" PRIx64,
				h1, h2);
			return -EBADMSG;
		}
		...
		if (!VALID64(o->data.next_hash_offset) ||
			...
			error(offset,
				...
				o->data.next_field_offset, o->data.entry_offset,
	...
	case OBJECT_ENTRY:
		...
		for (i = 0; i < journal_file_entry_n_items(o); i++) {
			if (o->entry.items[i].object_offset == 0 ||
				...
				error(offset,
					"Invalid entry item (%" PRIu64
					"/%" PRIu64 " offset: " OFSfmt,
	...
	case OBJECT_FIELD_HASH_TABLE:
		if ((le64toh(o->object.size) -
			...
			error(offset, "Invalid %s hash table size: %" PRIu64,
				o->object.type == OBJECT_DATA_HASH_TABLE ?
					      "data" :
					      "field",
				le64toh(o->object.size));
			return -EBADMSG;
		...
		for (i = 0; i < journal_file_hash_table_n_items(o); i++) {
			if (o->hash_table.items[i].head_hash_offset != 0 &&
				!VALID64(le64toh(o->hash_table.items[i]
							 .head_hash_offset))) {
				error(offset,
					"Invalid %s hash table item (%" PRIu64
					"/%" PRIu64
			...
			if ((o->hash_table.items[i].head_hash_offset != 0) !=
				(o->hash_table.items[i].tail_hash_offset !=
					0)) {
				error(offset,
					"Invalid %s hash table item (%" PRIu64
					"/%" PRIu64
	...
	case OBJECT_ENTRY_ARRAY:
		if ((le64toh(o->object.size) -
			    offsetof(EntryArrayObject, items)) %
					sizeof(le64_t) !=
				0 ||
			(le64toh(o->object.size) -
				offsetof(EntryArrayObject, items)) /
...
static int
write_uint64(int fd, uint64_t p)
{
	ssize_t k;
...
static int
contains_uint64(MMapCache *m, int fd, uint64_t n, uint64_t p)
{
	uint64_t a, b;
	int r;
...
static int
entry_points_to_data(JournalFile *f, int entry_fd, uint64_t n_entries,
	uint64_t entry_p, uint64_t data_p)
{
	int r;
	uint64_t i, n, a;
	Object *o;
	...
	if (!found) {
		error(entry_p,
			...
			" not referenced by linked entry",
			data_p);
		return -EBADMSG;
	}
...
static int
verify_data(JournalFile *f, Object *o, uint64_t p, int entry_fd,
	uint64_t n_entries, int entry_array_fd, uint64_t n_entry_arrays)
{
	...
	while (i < n) {
		...
		if (!contains_uint64(f->mmap, entry_array_fd, n_entry_arrays,
			    a)) {
		...
		for (j = 0; i < n && j < m; i++, j++) {
			...
			r = journal_file_move_to_object(f, OBJECT_ENTRY_ARRAY,
				a, &o);
			if (r < 0)
				return r;
		}
...
static int
verify_hash_table(JournalFile *f, int data_fd, uint64_t n_data, int entry_fd,
	uint64_t n_entries, int entry_array_fd, uint64_t n_entry_arrays,
	usec_t *last_usec, bool show_progress)
{
	uint64_t i, n;
	int r;
	...
	for (i = 0; i < n; i++) {
		...
		if (show_progress)
			draw_progress(0xC000 + scale_progress(0x3FFF, i, n),
				last_usec);
		...
		while (p != 0) {
			...
			r = verify_data(f, o, p, entry_fd, n_entries,
...
static int
data_object_in_hash_table(JournalFile *f, uint64_t hash, uint64_t p)
{
	uint64_t n, h, q;
	int r;
	assert(f);
...
static int
verify_entry(JournalFile *f, Object *o, uint64_t p, int data_fd,
	uint64_t n_data)
{
	uint64_t i, n;
	int r;
...
static int
verify_entry_array(JournalFile *f, int data_fd, uint64_t n_data, int entry_fd,
	uint64_t n_entries, int entry_array_fd, uint64_t n_entry_arrays,
	usec_t *last_usec, bool show_progress)
{
	...
	while (i < n) {
		...
		if (show_progress)
			draw_progress(0x8000 + scale_progress(0x3FFF, i, n),
				last_usec);
		...
		if (next != 0 && next <= a) {
			error(a,
				...
				" of %" PRIu64 " (jumps back from to " OFSfmt
				")",
				i, n, next);
			return -EBADMSG;
		}
...
int
journal_file_verify(JournalFile *f, const char *key, usec_t *first_contained,
...
#define ENOKEY ENOTSUP
	...
	for (i = 0; i < sizeof(f->header->reserved); i++)
		if (f->header->reserved[i] != 0) {
			error(offsetof(Header, reserved[i]),
	...
	for (;;) {
		...
		case OBJECT_TAG:
			...
			if (f->seal) {
				...
				rt = f->fss_start_usec +
					o->tag.epoch * f->fss_interval_usec;
				if (entry_realtime_set &&
					entry_realtime >=
				...
				while (q <= p) {
					...
					r = journal_file_hmac_put_object(f,
						OBJECT_UNUSED, o, q);
					if (r < 0)
						goto fail;
	...
	if (!found_main_entry_array &&
		le64toh(f->header->entry_array_offset) != 0) {
	...
	r = verify_entry_array(f, data_fd, n_data, entry_fd, n_entries,
		entry_array_fd, n_entry_arrays, &last_usec, show_progress);
	if (r < 0)
		goto fail;

	r = verify_hash_table(f, data_fd, n_data, entry_fd, n_entries,
		entry_array_fd, n_entry_arrays, &last_usec, show_progress);
	if (r < 0)
		goto fail;
