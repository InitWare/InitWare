...
static void patch_realtime(const char *dir, const char *fn, const struct stat *st,
...
int journal_directory_vacuum(const char *directory, uint64_t max_use, usec_t max_retention_usec,
    usec_t *oldest_usec)
{
	...
	for (;;) {
		...
		if (endswith(de->d_name, ".journal")) {
			...
			if (sscanf(de->d_name + q - 8 - 16 - 1 - 16, "%16llx-%16llx.journal",
				&seqnum, &realtime) != 2) {
				free(p);
				continue;
			}
			...
			if (sscanf(de->d_name + q - 1 - 8 - 16 - 1 - 16, "%16llx-%16llx.journal~",
				&realtime, &tmp) != 2) {
				free(p);
				continue;
			}
	...
	for (i = 0; i < n_list; i++) {
		...
		if (unlinkat(dirfd(d), list[i].filename, 0) >= 0) {
			log_debug("Deleted archived journal %s/%s (%" PRIu64 " bytes).", directory,
			    list[i].filename, list[i].usage);
			freed += list[i].usage;
