...
int journal_file_append_tag(JournalFile *f)
{
	...
	log_debug("Writing tag %" PRIu64 " for epoch %" PRIu64 "", le64toh(o->tag.seqnum),
...
int journal_file_hmac_put_object(JournalFile *f, int type, Object *o, uint64_t p)
{
	...
	case OBJECT_FIELD:
		...
		gcry_md_write(f->hmac, o->field.payload,
		    le64toh(o->object.size) - offsetof(FieldObject, payload));
		break;

	case OBJECT_ENTRY:
		/* All */
		gcry_md_write(f->hmac, &o->entry.seqnum,
		    le64toh(o->object.size) - offsetof(EntryObject, seqnum));
		break;
...
int journal_file_fss_load(JournalFile *f)
{
	...
	f->fss_file = mmap(NULL, PAGE_ALIGN(f->fss_file_size), PROT_READ | PROT_WRITE, MAP_SHARED,
	    fd, 0);
