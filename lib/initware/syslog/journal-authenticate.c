...
static uint64_t
journal_file_tag_seqnum(JournalFile *f)
{
	uint64_t r;
...
int
journal_file_append_tag(JournalFile *f)
{
	Object *o;
	uint64_t p;
	int r;
	...
	r = journal_file_append_object(f, OBJECT_TAG, sizeof(struct TagObject),
		&o, &p);
	if (r < 0)
		return r;
	...
	log_debug("Writing tag %" PRIu64 " for epoch %" PRIu64 "",
		le64toh(o->tag.seqnum), FSPRG_GetEpoch(f->fsprg_state));
...
int
journal_file_hmac_start(JournalFile *f)
{
...
static int
journal_file_get_epoch(JournalFile *f, uint64_t realtime, uint64_t *epoch)
{
	uint64_t t;
...
static int
journal_file_fsprg_need_evolve(JournalFile *f, uint64_t realtime)
{
	uint64_t goal, epoch;
	int r;
	assert(f);
...
int
journal_file_fsprg_evolve(JournalFile *f, uint64_t realtime)
{
	uint64_t goal, epoch;
	int r;
	...
	epoch = FSPRG_GetEpoch(f->fsprg_state);
	if (epoch < goal)
		log_debug("Evolving FSPRG key from epoch %" PRIu64
			  " to %" PRIu64 ".",
			epoch, goal);
...
int
journal_file_fsprg_seek(JournalFile *f, uint64_t goal)
{
	void *msk;
	uint64_t epoch;
	...
	FSPRG_GenMK(msk, NULL, f->fsprg_seed, f->fsprg_seed_size,
...
int
journal_file_maybe_append_tag(JournalFile *f, uint64_t realtime)
{
	int r;
...
int
journal_file_hmac_put_object(JournalFile *f, ObjectType type, Object *o,
	uint64_t p)
{
	int r;
	...
	case OBJECT_DATA:
		...
		gcry_md_write(f->hmac, o->data.payload,
	...
	case OBJECT_FIELD:
		...
		gcry_md_write(f->hmac, o->field.payload,
			le64toh(o->object.size) -
	...
	case OBJECT_ENTRY:
		/* All */
		gcry_md_write(f->hmac, &o->entry.seqnum,
			le64toh(o->object.size) -
				offsetof(EntryObject, seqnum));
		break;
...
int
journal_file_hmac_put_header(JournalFile *f)
{
	int r;
	...
	gcry_md_write(f->hmac, f->header->signature,
		offsetof(Header, state) - offsetof(Header, signature));
	gcry_md_write(f->hmac, &f->header->file_id,
		offsetof(Header, boot_id) - offsetof(Header, file_id));
	gcry_md_write(f->hmac, &f->header->seqnum_id,
		offsetof(Header, arena_size) - offsetof(Header, seqnum_id));
	gcry_md_write(f->hmac, &f->header->data_hash_table_offset,
...
int
journal_file_fss_load(JournalFile *f)
{
	int r, fd = -1;
	char *p = NULL;
	struct stat st;
	...
	m = mmap(NULL, PAGE_ALIGN(sizeof(FSSHeader)), PROT_READ, MAP_SHARED, fd,
		0);
	if (m == MAP_FAILED) {
		m = NULL;
		r = -errno;
	...
	if (le64toh(m->fsprg_state_size) !=
		FSPRG_stateinbytes(le16toh(m->fsprg_secpar))) {
		r = -EBADMSG;
		goto finish;
	}

	f->fss_file_size =
		le64toh(m->header_size) + le64toh(m->fsprg_state_size);
	...
	if (le64toh(m->start_usec) <= 0 || le64toh(m->interval_usec) <= 0) {
		r = -EBADMSG;
		goto finish;
	}
...
int
journal_file_hmac_setup(JournalFile *f)
{
	gcry_error_t e;
...
int
journal_file_append_first_tag(JournalFile *f)
{
	int r;
	uint64_t p;
...
bool
journal_file_next_evolve_usec(JournalFile *f, usec_t *u)
{
	uint64_t epoch;
	...
	*u = (usec_t)(f->fss_start_usec + f->fss_interval_usec * epoch +
		f->fss_interval_usec);
