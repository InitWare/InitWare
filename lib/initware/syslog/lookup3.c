...
*/
uint32_t
jenkins_hashword(const uint32_t *k, /* the key, an array of uint32_t values */
...
*/
void
jenkins_hashword2(const uint32_t *k, /* the key, an array of uint32_t values */
	size_t length, /* the length of the key, in uint32_ts */
...
jenkins_hashlittle(const void *key, size_t length, uint32_t initval)
{
	...
	if (HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) {
		const uint32_t *k =
			(const uint32_t *)key; /* read 32-bit chunks */
     ...
     */
		...
		const uint16_t *k =
			(const uint16_t *)key; /* read 16-bit chunks */
		const uint8_t *k8;
 ...
 */
void
jenkins_hashlittle2(const void *key, /* the key to hash */
 ...
 */
uint32_t
jenkins_hashbig(const void *key, size_t length, uint32_t initval)
{
	uint32_t a, b, c;
	union {
		const void *ptr;
