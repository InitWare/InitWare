...
node_lookup_f(sd_hwdb *hwdb, const struct trie_node_f *node, uint8_t c)
{
	...
	search.c = c;
	child = bsearch(&search, trie_node_children(hwdb, node),
...
static int
trie_fnmatch_f(sd_hwdb *hwdb, const struct trie_node_f *node, size_t p,
	struct linebuf *buf, const char *search)
{
	...
	for (i = 0; i < node->children_count; i++) {
		...
		err = trie_fnmatch_f(hwdb,
			trie_node_from_off(hwdb, child->child_off), 0, buf,
			search);
		if (err < 0)
			return err;
	...
	if (le64toh(node->values_count) &&
		...
		for (i = 0; i < le64toh(node->values_count); i++) {
			err = hwdb_add_property(hwdb,
				trie_string(hwdb,
					trie_node_values(hwdb, node)[i].key_off),
				trie_string(hwdb,
					trie_node_values(hwdb, node)[i]
						.value_off));
			if (err < 0)
				return err;
		}
...
sd_hwdb_new(sd_hwdb **ret)
{
	...
	NULSTR_FOREACH (hwdb_bin_path, hwdb_bin_paths) {
		...
		if (hwdb->f)
			...
			return log_debug_errno(errno, "error reading %s: %m",
				hwdb_bin_path);
	}
	...
	if (fstat(fileno(hwdb->f), &hwdb->st) < 0 ||
		(size_t)hwdb->st.st_size <
			offsetof(struct trie_header_f, strings_len) + 8)
		return log_debug_errno(errno, "error reading %s: %m",
			hwdb_bin_path);

	hwdb->map = mmap(0, hwdb->st.st_size, PROT_READ, MAP_SHARED,
		fileno(hwdb->f), 0);
	if (hwdb->map == MAP_FAILED)
		return log_debug_errno(errno, "error mapping %s: %m",
			hwdb_bin_path);
