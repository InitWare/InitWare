...
sd_listen_fds(int unset_environment)
{
	...
	for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + (int)n;
		fd++) {
...
_public_ int
sd_is_fifo(int fd, const char *path)
{
	struct stat st_fd;
...
_public_ int
sd_is_special(int fd, const char *path)
{
	struct stat st_fd;
...
static int
sd_is_socket_internal(int fd, int type, int listening)
{
	struct stat st_fd;
...
_public_ int
sd_is_socket_unix(int fd, int type, int listening, const char *path,
	size_t length)
{
	union sockaddr_union sockaddr = {};
	socklen_t l = sizeof(sockaddr);
	...
	if (path) {
		...
		if (path[0])
			...
			return (l >= offsetof(struct sockaddr_un, sun_path) +
					       length + 1) &&
				memcmp(path, sockaddr.un.sun_path,
					length + 1) == 0;
		else
	...
	const int *fds, unsigned n_fds)
{
	...
	send_ucred = (pid != 0 && pid != getpid()) || getuid() != geteuid() ||
		getgid() != getegid();
	...
	if (n_fds > 0 || send_ucred) {
		...
		msghdr.msg_controllen =
			...
			+ (send_ucred ? CMSG_SPACE(CMSG_CREDS_STRUCT_SIZE) : 0)
#endif
			;
...
_public_ int
sd_pid_notifyf(pid_t pid, int unset_environment, const char *format, ...)
{
