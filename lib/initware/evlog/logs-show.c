...
static bool print_multiline(FILE *f, unsigned prefix, unsigned n_columns, OutputFlags flags,
    int priority, const char *message, size_t message_len)
{
	...
	for (pos = message; pos < message + message_len; pos = end + 1, line++) {
		...
		if (flags & (OUTPUT_FULL_WIDTH | OUTPUT_SHOW_ALL) ||
			...
			fprintf(f, "%*s%s%.*s%s\n", continuation * prefix, "", color_on, len, pos,
			    color_off);
			continue;
		}
...
static int
    output_short(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags)
{
	...
	size_t length;
	size_t n = 0;
	_cleanup_free_ char *hostname = NULL, *identifier = NULL, *comm = NULL, *pid = NULL,
			    *fake_pid = NULL, *message = NULL, *realtime = NULL, *monotonic = NULL,
			    *priority = NULL;
	size_t hostname_len = 0, identifier_len = 0, comm_len = 0, pid_len = 0, fake_pid_len = 0,
	       message_len = 0, realtime_len = 0, monotonic_len = 0, priority_len = 0;
	int p = LOG_INFO;
	bool ellipsized = false;
	...
	sd_journal_set_data_threshold(j,
	    flags & (OUTPUT_SHOW_ALL | OUTPUT_FULL_WIDTH) ? 0 : PRINT_CHAR_THRESHOLD + 1);
	...
	JOURNAL_FOREACH_DATA_RETVAL (j, data, length, r) {
		...
		r = parse_field(data, length, "_SOURCE_REALTIME_TIMESTAMP=", &realtime,
		    &realtime_len);
		if (r < 0)
			return r;
		else if (r > 0)
		...
		r = parse_field(data, length, "_SOURCE_MONOTONIC_TIMESTAMP=", &monotonic,
		    &monotonic_len);
		if (r < 0)
			return r;
		else if (r > 0)
	...
	if (mode == OUTPUT_SHORT_MONOTONIC) {
		...
		fprintf(f, "[%5llu.%06llu]", (unsigned long long) (t / USEC_PER_SEC),
		...
		case OUTPUT_SHORT_PRECISE:
			...
			if (r > 0) {
				snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), ".%06llu",
				    x % USEC_PER_SEC);
			}
			break;
		default:
...
static int
    output_verbose(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags)
{
	...
	if (r == -ENOENT)
		...
		if (r < 0)
			...
			if (r < 0)
				log_debug("Failed to parse realtime timestamp: %s", strerror(-r));
		}
	}
	...
	fprintf(f, "%s [%s]\n", format_timestamp_us(ts, sizeof(ts), realtime), cursor);
...
static int
    output_export(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags)
{
	...
	fprintf(f,
	    ...
	    "_BOOT_ID=%s\n",
	    cursor, (unsigned long long) realtime, (unsigned long long) monotonic,
	...
	JOURNAL_FOREACH_DATA_RETVAL (j, data, length, r) {
		...
		if (length >= 9 && startswith(data, "_BOOT_ID="))
			continue;
...
static int output_json(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags)
{
		...
		SD_JOURNAL_FOREACH_DATA(j, data, length)
		{
			...
			if (u == 0) {
				...
				while (sd_journal_enumerate_data(j, &data, &length) > 0) {
					...
					fputs(", ", f);
					json_escape(f, (const char *) data + m + 1, length - m - 1,
					    flags);
				}
...
static int output_cat(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags)
{
...
static int (*output_funcs[_OUTPUT_MODE_MAX])(FILE *f, sd_journal *j, OutputMode mode,
...
int output_journal(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns, OutputFlags flags,
    bool *ellipsized)
{
...
static int show_journal(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
    usec_t not_before, unsigned how_many, OutputFlags flags, bool *ellipsized)
{
...
int add_matches_for_unit(sd_journal *j, const char *unit)
{
	...
	(void) (
	    ...
	    (r = sd_journal_add_match(j, "_UID=0", 0)) || (r = sd_journal_add_match(j, m2, 0)) ||

	    /* Look for messages from PID 1 about this service */
	    (r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, "_PID=1", 0)) ||
	    (r = sd_journal_add_match(j, m3, 0)) ||

	    /* Look for messages from authorized daemons about this service */
	    (r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, "_UID=0", 0)) ||
	...
	if (r == 0 && endswith(unit, ".slice")) {
		...
		(void) ((r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, m5, 0)));
	}
...
int add_matches_for_user_unit(sd_journal *j, const char *unit, uid_t uid)
{
	...
	(void) (
	    ...
	    (r = sd_journal_add_match(j, m1, 0)) || (r = sd_journal_add_match(j, muid, 0)) ||

	    /* Look for messages from systemd about this service */
	    (r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, m2, 0)) ||
	    (r = sd_journal_add_match(j, muid, 0)) ||

	    /* Look for coredumps of the service */
	    (r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, m3, 0)) ||
	    (r = sd_journal_add_match(j, muid, 0)) || (r = sd_journal_add_match(j, "_UID=0", 0)) ||

	    /* Look for messages from authorized daemons about this service */
	    (r = sd_journal_add_disjunction(j)) || (r = sd_journal_add_match(j, m4, 0)) ||
	    (r = sd_journal_add_match(j, muid, 0)) || (r = sd_journal_add_match(j, "_UID=0", 0)));
	...
	if (r == 0 && endswith(unit, ".slice")) {
		...
		(void) ((r = sd_journal_add_disjunction(j)) ||
		    (r = sd_journal_add_match(j, m5, 0)) || (r = sd_journal_add_match(j, muid, 0)));
	}
...
int show_journal_by_unit(FILE *f, const char *unit, OutputMode mode, unsigned n_columns,
    usec_t not_before, unsigned how_many, uid_t uid, OutputFlags flags, bool system,
    bool *ellipsized)
{
...
static const char *const output_mode_table[_OUTPUT_MODE_MAX] = { [OUTPUT_SHORT] = "short",
