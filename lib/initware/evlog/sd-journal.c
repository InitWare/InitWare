...
static int compare_entry_order(JournalFile *af, Object *_ao, JournalFile *bf, uint64_t bp)
{
	...
	if (sd_id128_equal(ao->entry.boot_id, bo->entry.boot_id) &&
	    ...
	    ao->entry.realtime == bo->entry.realtime && ao->entry.xor_hash == bo->entry.xor_hash)
		return 0;
...
_pure_ static int compare_with_location(JournalFile *af, Object *ao, Location *l)
{
	...
	if (l->monotonic_set && sd_id128_equal(ao->entry.boot_id, l->boot_id) && l->realtime_set &&
	    le64toh(ao->entry.realtime) == l->realtime && l->xor_hash_set &&
	    le64toh(ao->entry.xor_hash) == l->xor_hash)
		return 0;

	if (l->seqnum_set && sd_id128_equal(af->header->seqnum_id, l->seqnum_id)) {
	...
	if (l->monotonic_set && sd_id128_equal(ao->entry.boot_id, l->boot_id)) {
...
static int next_for_match(sd_journal *j, Match *m, JournalFile *f, uint64_t after_offset,
    direction_t direction, Object **ret, uint64_t *offset)
{
	...
	if (m->type == MATCH_DISCRETE) {
		...
		return journal_file_move_to_entry_by_offset_for_data(f, dp, after_offset,
...
static int find_location_for_match(sd_journal *j, Match *m, JournalFile *f, direction_t direction,
    Object **ret, uint64_t *offset)
{
	...
	if (m->type == MATCH_DISCRETE) {
		...
		if (j->current_location.type == LOCATION_HEAD)
			return journal_file_next_entry_for_data(f, NULL, 0, dp, DIRECTION_DOWN,
			    ret, offset);
		if (j->current_location.type == LOCATION_TAIL)
			return journal_file_next_entry_for_data(f, NULL, 0, dp, DIRECTION_UP, ret,
			    offset);
		if (j->current_location.seqnum_set &&
		    sd_id128_equal(j->current_location.seqnum_id, f->header->seqnum_id))
			return journal_file_move_to_entry_by_seqnum_for_data(f, dp,
			    j->current_location.seqnum, direction, ret, offset);
		if (j->current_location.monotonic_set) {
			r = journal_file_move_to_entry_by_monotonic_for_data(f, dp,
			    j->current_location.boot_id, j->current_location.monotonic, direction,
			    ret, offset);
			if (r != -ENOENT)
				return r;
		}
		if (j->current_location.realtime_set)
			return journal_file_move_to_entry_by_realtime_for_data(f, dp,
			    j->current_location.realtime, direction, ret, offset);
...
static int find_location_with_matches(sd_journal *j, JournalFile *f, direction_t direction,
    Object **ret, uint64_t *offset)
{
	...
	if (!j->level0) {
		...
		if (j->current_location.seqnum_set &&
		    sd_id128_equal(j->current_location.seqnum_id, f->header->seqnum_id))
			return journal_file_move_to_entry_by_seqnum(f, j->current_location.seqnum,
			    direction, ret, offset);
		if (j->current_location.monotonic_set) {
...
static int next_with_matches(sd_journal *j, JournalFile *f, direction_t direction, Object **ret,
    uint64_t *offset)
{
	...
	return next_for_match(j, j->level0, f, direction == DIRECTION_DOWN ? cp + 1 : cp - 1,
	    direction, ret, offset);
}

static int next_beyond_location(sd_journal *j, JournalFile *f, direction_t direction, Object **ret,
    uint64_t *offset)
{
	Object *c;
	uint64_t cp;
...
_public_ int sd_journal_get_cursor(sd_journal *j, char **cursor)
{
	...
	if (asprintf(cursor, "s=%s;i=%" PRIx64 ";b=%s;m=%" PRIx64 ";t=%" PRIx64 ";x=%" PRIx64, sid,
		le64toh(o->entry.seqnum), bid, le64toh(o->entry.monotonic),
...
static void check_network(sd_journal *j, int fd)
{
	...
	j->on_network = F_TYPE_EQUAL(sfs.f_type, CIFS_MAGIC_NUMBER) ||
	    ...
	    F_TYPE_EQUAL(sfs.f_type, NFS_SUPER_MAGIC) || F_TYPE_EQUAL(sfs.f_type, SMB_SUPER_MAGIC);
#else
	unimplemented();
	j->on_network = false;
#endif
...
static bool file_has_type_prefix(const char *prefix, const char *filename)
{
	...
	return streq(filename, full) || streq(filename, tilded) || startswith(filename, atted);
}
...
static int add_directory(sd_journal *j, const char *prefix, const char *dirname)
{
	...
	for (;;) {
		...
		if (dirent_is_file_with_suffix(de, ".journal") ||
			...
			if (r < 0) {
				log_debug("Failed to add file %s/%s: %s", m->path, de->d_name,
				    strerror(-r));
				r = set_put_error(j, r);
				if (r < 0)
					return r;
...
static int add_root_directory(sd_journal *j, const char *p)
{
	...
	if (m->wd <= 0 && j->inotify_fd >= 0) {
		...
		m->wd = inotify_add_watch(j->inotify_fd, m->path,
		    IN_CREATE | IN_MOVED_TO | IN_MODIFY | IN_ATTRIB | IN_DELETE | IN_ONLYDIR);
	...
	for (;;) {
		...
		if (dirent_is_file_with_suffix(de, ".journal") ||
			...
			if (r < 0)
				log_debug("Failed to add directory %s/%s: %s", m->path, de->d_name,
				    strerror(-r));
		}
	}
...
_public_ void sd_journal_close(sd_journal *j)
{
	...
	if (j->mmap) {
		log_debug("mmap cache statistics: %u hit, %u miss", mmap_cache_get_hit(j->mmap),
...
#ifdef HAVE_XZ
			if (uncompress_startswith(o->data.payload, l, &f->compress_buffer,
	...
	if (d) {
		...
		if (!(e->mask & IN_ISDIR) && e->len > 0 &&
			...
			if (e->mask & (IN_CREATE | IN_MOVED_TO | IN_MODIFY | IN_ATTRIB)) {
				...
				if (r < 0) {
					log_debug("Failed to add file %s/%s: %s", d->path, e->name,
					    strerror(-r));
					set_put_error(j, r);
				}
				...
				if (r < 0)
					log_debug("Failed to remove file %s/%s: %s", d->path,
					    e->name, strerror(-r));
			}
			...
			if (e->mask & (IN_DELETE_SELF | IN_MOVE_SELF | IN_UNMOUNT)) {
				...
				if (r < 0)
					log_debug("Failed to remove directory %s: %s", d->path,
					    strerror(-r));
			}
			...
			if (e->mask & (IN_CREATE | IN_MOVED_TO | IN_MODIFY | IN_ATTRIB)) {
				...
				if (r < 0)
					log_debug("Failed to add directory %s/%s: %s", d->path,
					    e->name, strerror(-r));
			}
		}
...
_public_ int sd_journal_get_cutoff_monotonic_usec(sd_journal *j, sd_id128_t boot_id,
...
_public_ int sd_journal_enumerate_unique(sd_journal *j, const void **data, size_t *l)
{
	...
	for (;;) {
		...
		if (j->unique_offset == 0) {
			r = journal_file_find_field_object(j->unique_file, j->unique_field, k, &o,
			    NULL);
			if (r < 0)
				return r;
		...
		if (o->object.type != OBJECT_DATA) {
			log_error("%s:offset " OFSfmt ": object has type %d, expected %d",
			    j->unique_file->path, j->unique_offset, o->object.type, OBJECT_DATA);
			return -EBADMSG;
		}
		...
		HASHMAP_FOREACH (of, j->files, i) {
			...
			r = journal_file_find_data_object_with_hash(of, odata, ol,
			    le64toh(o->data.hash), &oo, &op);
			if (r < 0)
				return r;
