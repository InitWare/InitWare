...
_public_ int
sd_bus_creds_get_pid_starttime(sd_bus_creds *c, uint64_t *usec)
{
	assert_return(c, -EINVAL);
...
sd_bus_creds_get_cmdline(sd_bus_creds *c, char ***cmdline)
{
	...
	if (!c->cmdline_array) {
		c->cmdline_array = strv_parse_nulstr(c->cmdline,
		    c->cmdline_size);
		if (!c->cmdline_array)
			return -ENOMEM;
	}
...
_public_ int
sd_bus_creds_get_connection_name(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
has_cap(sd_bus_creds *c, unsigned offset, int capability)
{
	...
	return !!(c->capability[offset * sz + (capability / 8)] &
	    (1 << (capability % 8)));
}
...
parse_caps(sd_bus_creds *c, unsigned offset, const char *p)
{
	...
	for (i = 0; i < sz; i++) {
		...
		c->capability[offset * sz + (sz - i - 1)] = (uint8_t) x << 4 |
		    (uint8_t) y;
	}
...
bus_creds_add_more(sd_bus_creds *c, uint64_t mask, pid_t pid, pid_t tid)
{
	...
	if (missing &
	    (SD_BUS_CREDS_UID | SD_BUS_CREDS_GID | SD_BUS_CREDS_EFFECTIVE_CAPS |
		...
		FOREACH_LINE(line, f, return -errno)
		{
			...
			if (missing & SD_BUS_CREDS_EFFECTIVE_CAPS) {
				...
				if (p) {
					r = parse_caps(c, CAP_OFFSET_EFFECTIVE,
					    p);
					if (r < 0)
						return r;
			...
			if (missing & SD_BUS_CREDS_PERMITTED_CAPS) {
				...
				if (p) {
					r = parse_caps(c, CAP_OFFSET_PERMITTED,
					    p);
					if (r < 0)
						return r;
			...
			if (missing & SD_BUS_CREDS_BOUNDING_CAPS) {
				...
				if (p) {
					r = parse_caps(c, CAP_OFFSET_BOUNDING,
					    p);
					if (r < 0)
						return r;
	...
	if (tid > 0 && (missing & SD_BUS_CREDS_TID_COMM)) {
		...
		if (asprintf(&p, "/proc/%lu/task/%lu/comm", (unsigned long) pid,
			(unsigned long) tid) < 0)
			return -ENOMEM;
