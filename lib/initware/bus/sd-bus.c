...
#ifndef Use_CGroups
#define cg_pid_get_owner_uid(pid, no) getuid()
#endif
...
static void
bus_node_destroy(sd_bus *b, struct node *n)
{
	...
	while ((v = n->vtables)) {
		IWLIST_REMOVE(struct node_vtable, vtables, n->vtables, v);
		free(v->interface);
		free(v);
	}

	while ((e = n->enumerators)) {
		IWLIST_REMOVE(struct node_enumerator, enumerators,
		    n->enumerators, e);
		free(e);
	}
...
bus_free(sd_bus *b)
{
	...
	while ((f = b->filter_callbacks)) {
		IWLIST_REMOVE(struct filter_callback, callbacks,
		    b->filter_callbacks, f);
		free(f);
	}
...
sd_bus_new(sd_bus **ret)
{
	...
	r->message_version = 1;
	r->creds_mask |= SD_BUS_CREDS_WELL_KNOWN_NAMES |
...
_public_ int
sd_bus_set_bus_client(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_negotiate_fds(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_negotiate_timestamp(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_negotiate_creds(sd_bus *bus, uint64_t mask)
{
	assert_return(bus, -EINVAL);
	...
	bus->creds_mask = mask | SD_BUS_CREDS_WELL_KNOWN_NAMES |
	    SD_BUS_CREDS_UNIQUE_NAME;
...
_public_ int
sd_bus_set_server(sd_bus *bus, int b, sd_id128_t server_id)
{
	assert_return(bus, -EINVAL);
...
_public_ int
sd_bus_set_anonymous(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_set_trusted(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_set_name(sd_bus *bus, const char *name)
{
	char *n;
...
static int
hello_callback(sd_bus *bus, sd_bus_message *reply, void *userdata,
    sd_bus_error *error)
{
	const char *s;
	int r;
...
bus_send_hello(sd_bus *bus)
{
	...
	return sd_bus_call_async(bus, m, hello_callback, NULL, 0,
	    &bus->hello_cookie);
}

int
bus_start_running(sd_bus *bus)
{
	assert(bus);
...
parse_unix_address(sd_bus *b, const char **p, char **guid)
{
	...
	if (path) {
		...
		b->sockaddr_size = offsetof(struct sockaddr_un, sun_path) + 1 +
		    l;
	}
...
static void
bus_reset_parsed_address(sd_bus *b)
{
	assert(b);
...
static int
bus_parse_next_address(sd_bus *b)
{
...
#ifdef ENABLE_KDBUS
			asprintf(&b->address,
			    KERNEL_USER_BUS_FMT ";" UNIX_USER_BUS_FMT,
...
_public_ int
sd_bus_open_system_remote(sd_bus **ret, const char *host)
{
	...
	p = strjoin("unixexec:path=ssh,argv1=-xT,argv2=", e,
...
_public_ int
sd_bus_open_system_container(sd_bus **ret, const char *machine)
{
	_cleanup_free_ char *e = NULL;
	sd_bus *bus;
	char *p;
...
#ifdef ENABLE_KDBUS
	p = strjoin("x-container-kernel:machine=", e,
	    ";x-container-unix:machine=", e, NULL);
#else
...
_public_ int
sd_bus_can_send(sd_bus *bus, char type)
{
	int r;
...
_public_ int
sd_bus_get_server_id(sd_bus *bus, sd_id128_t *server_id)
{
	int r;
...
int
bus_seal_synthetic_message(sd_bus *b, sd_bus_message *m)
{
	assert(b);
	assert(m);
...
static int
bus_write_message(sd_bus *bus, sd_bus_message *m, bool hint_sync_call,
    size_t *idx)
{
	int r;
	...
	if (*idx >= BUS_MESSAGE_SIZE(m))
		log_debug("Sent message type=%s sender=%s destination=%s object=%s interface=%s member=%s cookie=%" PRIu64
...
static int
bus_read_message(sd_bus *bus, bool hint_priority, int64_t priority)
{
	assert(bus);
...
static int
dispatch_rqueue(sd_bus *bus, bool hint_priority, int64_t priority,
    sd_bus_message **m)
{
	int r, ret = 0;
	...
	for (;;) {
		if (bus->rqueue_size > 0) {
			...
			*m = bus->rqueue[0];
			bus->rqueue_size--;
			memmove(bus->rqueue, bus->rqueue + 1,
...
static int
bus_send_internal(sd_bus *bus, sd_bus_message *_m, uint64_t *cookie,
    bool hint_sync_call)
{
...
_public_ int
sd_bus_send(sd_bus *bus, sd_bus_message *m, uint64_t *cookie)
{
	return bus_send_internal(bus, m, cookie, false);
}

_public_ int
sd_bus_send_to(sd_bus *bus, sd_bus_message *m, const char *destination,
    uint64_t *cookie)
{
	int r;
...
static int
timeout_compare(const void *a, const void *b)
{
...
_public_ int
sd_bus_call_async(sd_bus *bus, sd_bus_message *_m,
    ...
    uint64_t *cookie)
{
	...
	assert_return(!(m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED),
	    -EINVAL);
	...
	r = hashmap_ensure_allocated(&bus->reply_callbacks, uint64_hash_func,
	    uint64_compare_func);
	if (r < 0)
		return r;

	r = prioq_ensure_allocated(&bus->reply_callbacks_prioq,
	    timeout_compare);
	if (r < 0)
		return r;
...
_public_ int
sd_bus_call_async_cancel(sd_bus *bus, uint64_t cookie)
{
	struct reply_callback *c;
...
_public_ int
sd_bus_call(sd_bus *bus, sd_bus_message *_m, uint64_t usec, sd_bus_error *error,
...
_public_ int
sd_bus_get_timeout(sd_bus *bus, uint64_t *timeout_usec)
{
	struct reply_callback *c;
...
process_timeout(sd_bus *bus)
{
	...
	r = bus_message_new_synthetic_error(bus, c->cookie,
...
static int
process_hello(sd_bus *bus, sd_bus_message *m)
{
	assert(bus);
	assert(m);
...
static int
process_reply(sd_bus *bus, sd_bus_message *m)
{
	_cleanup_bus_error_free_ sd_bus_error error_buffer = SD_BUS_ERROR_NULL;
	struct reply_callback *c;
	int r;
...
static int
process_filter(sd_bus *bus, sd_bus_message *m)
{
	_cleanup_bus_error_free_ sd_bus_error error_buffer = SD_BUS_ERROR_NULL;
...
static int
process_match(sd_bus *bus, sd_bus_message *m)
{
	int r;
...
static int
process_builtin(sd_bus *bus, sd_bus_message *m)
{
	...
	if (streq_ptr(m->member, "Ping"))
		...
		r = sd_bus_message_append(reply, "s",
		    sd_id128_to_string(id, sid));
	} else {
		r = sd_bus_message_new_method_errorf(m, &reply,
		    SD_BUS_ERROR_UNKNOWN_METHOD,
		    "Unknown method '%s' on interface '%s'.", m->member,
		    m->interface);
	}
...
process_message(sd_bus *bus, sd_bus_message *m)
{
	...
	log_debug("Got message type=%s sender=%s destination=%s object=%s interface=%s member=%s cookie=%" PRIu64
		  " reply_cookie=%" PRIu64 " error=%s",
	    bus_message_type_to_string(m->header->type),
	    strna(sd_bus_message_get_sender(m)),
	    strna(sd_bus_message_get_destination(m)),
...
static int
process_running(sd_bus *bus, bool hint_priority, int64_t priority,
    sd_bus_message **ret)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	int r;
...
static int
process_closing(sd_bus *bus, sd_bus_message **ret)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	struct reply_callback *c;
	int r;
	...
	if (c) {
		...
		if (c->timeout != 0)
			prioq_remove(bus->reply_callbacks_prioq, c,
			    &c->prioq_idx);
...
static int
bus_process_internal(sd_bus *bus, bool hint_priority, int64_t priority,
    sd_bus_message **ret)
{
...
_public_ int
sd_bus_process(sd_bus *bus, sd_bus_message **ret)
{
	return bus_process_internal(bus, false, 0, ret);
}

_public_ int
sd_bus_process_priority(sd_bus *bus, int64_t priority, sd_bus_message **ret)
{
...
_public_ int
sd_bus_wait(sd_bus *bus, uint64_t timeout_usec)
{
...
_public_ int
sd_bus_add_filter(sd_bus *bus, sd_bus_message_handler_t callback,
    void *userdata)
{
	...
	IWLIST_PREPEND(struct filter_callback, callbacks, bus->filter_callbacks,
	    f);
	return 0;
}

_public_ int
sd_bus_remove_filter(sd_bus *bus, sd_bus_message_handler_t callback,
    void *userdata)
{
...
_public_ int
sd_bus_add_match(sd_bus *bus, const char *match,
    sd_bus_message_handler_t callback, void *userdata)
{
	...
	if (bus->bus_client) {
		...
		r = bus_add_match_internal(bus, match, components, n_components,
		    cookie);
		if (r < 0)
			goto finish;
	}
...
_public_ int
sd_bus_remove_match(sd_bus *bus, const char *match,
    sd_bus_message_handler_t callback, void *userdata)
{
	...
	bus->match_callbacks_modified = true;
	r = bus_match_remove(&bus->match_callbacks, components, n_components,
...
_public_ int
sd_bus_get_tid(sd_bus *b, pid_t *tid)
{
...
_public_ char *
sd_bus_label_escape(const char *s)
{
	return bus_label_escape(s);
}

_public_ char *
sd_bus_label_unescape(const char *f)
{
	return bus_label_unescape(f);
}

_public_ int
sd_bus_get_peer_creds(sd_bus *bus, uint64_t mask, sd_bus_creds **ret)
{
	sd_bus_creds *c;
	pid_t pid = 0;
	int r;
	...
	if (bus->ucred_valid) {
		...
		c->mask |= (SD_BUS_CREDS_UID | SD_BUS_CREDS_PID |
			       SD_BUS_CREDS_GID) &
		    mask;
	}
...
_public_ int
sd_bus_get_name(sd_bus *bus, const char **name)
{
	assert_return(bus, -EINVAL);
	assert_return(name, -EINVAL);
	assert_return(!bus_pid_changed(bus), -ECHILD);
