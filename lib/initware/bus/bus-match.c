...
value_node_test(struct bus_match_node *node,
    ...
    const char *value_str, sd_bus_message *m)
{
...
bus_match_run(sd_bus *bus, struct bus_match_node *node, sd_bus_message *m)
{
	...
	case BUS_MATCH_ARG_PATH ... BUS_MATCH_ARG_PATH_LAST:
		test_str = bus_message_get_arg(m,
		    node->type - BUS_MATCH_ARG_PATH);
		break;

	case BUS_MATCH_ARG_NAMESPACE ... BUS_MATCH_ARG_NAMESPACE_LAST:
		test_str = bus_message_get_arg(m,
		    node->type - BUS_MATCH_ARG_NAMESPACE);
		break;
    ...
    struct bus_match_node **ret)
{
	...
	if (c) {
		...
		if (t == BUS_MATCH_MESSAGE_TYPE) {
			c->compare.children = hashmap_new(trivial_hash_func,
			...
			c->compare.children = hashmap_new(string_hash_func,
			    string_compare_func);
			if (!c->compare.children) {
				r = -ENOMEM;
				goto fail;
    ...
    struct bus_match_node **ret)
{
	...
	if (t == BUS_MATCH_MESSAGE_TYPE)
		...
		for (n = c->child; !value_node_same(n, t, value_u8, value_str);
		     n = n->next)
			;
	}
...
bus_match_add_leaf(struct bus_match_node *where,
    sd_bus_message_handler_t callback, void *userdata, uint64_t cookie,
    struct bus_match_node **ret)
{
    ...
    struct bus_match_node **ret)
{
	...
	for (c = where->child; c; c = c->next) {
		if (c->type == BUS_MATCH_LEAF && c->leaf.callback == callback &&
...
int
bus_match_remove(struct bus_match_node *root,
    struct bus_match_component *components, unsigned n_components,
    sd_bus_message_handler_t callback, void *userdata, uint64_t *cookie)
{
