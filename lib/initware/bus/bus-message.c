    ...
    bool add_offset)
{
	...
	m->destination = adjust_pointer(m->destination, op, old_size,
	    m->header);
	...
	if (add_offset) {
		...
		m->header_offsets[m->n_header_offsets++] = new_size -
...
static int
message_append_field_string(sd_bus_message *m, uint8_t h, char type,
...
static int
message_append_field_signature(sd_bus_message *m, uint8_t h, const char *s,
    const char **ret)
{
...
static int
message_append_field_uint32(sd_bus_message *m, uint8_t h, uint32_t x)
{
	uint8_t *p;
...
int
bus_message_from_header(sd_bus *bus, void *buffer, size_t length, int *fds,
    ...
    size_t extra, sd_bus_message **ret)
{
	...
	if (ucred) {
		...
		m->creds.mask |= SD_BUS_CREDS_UID | SD_BUS_CREDS_PID |
		    SD_BUS_CREDS_GID;
	}
    ...
    sd_bus_message **ret)
{
	...
	r = bus_message_from_header(bus, buffer, length, fds, n_fds, ucred,
	    label, 0, &m);
	if (r < 0)
		return r;
	...
	sz = length - sizeof(struct bus_header) -
...
message_new_reply(sd_bus_message *call, uint8_t type, sd_bus_message **m)
{
	...
	r = message_append_field_uint32(t, BUS_MESSAGE_HEADER_REPLY_SERIAL,
	    (uint32_t) t->reply_cookie);
	if (r < 0)
		goto fail;
...
message_extend_body(sd_bus_message *m, size_t align, size_t sz, bool add_offset)
{
	...
	if (added > 0) {
		...
		add_new_part = m->n_body_parts <= 0 || m->body_end->sealed ||
		    padding !=
			ALIGN_TO(m->body_end->size, align) - m->body_end->size;
		...
		if (add_new_part) {
			...
			m->error.message =
			    (const char *) adjust_pointer(m->error.message, op,
				os, part->data);
		}
	} else
...
static size_t
determine_word_size(size_t sz, size_t extra)
{
	if (sz + extra <= 0xFF)
		return 1;
...
static void
write_word_le(void *p, size_t sz, size_t value)
{
	union {
		uint16_t u16;
		uint32_t u32;
...
bus_message_close_array(sd_bus_message *m, struct bus_container *c)
{
	...
	if (c->need_offsets) {
		...
		a = message_extend_body(m, 1, 0,
		    true); /* let's add offset to parent */
		if (!a)
			return -ENOMEM;
	}
    ...
    bool add_offset)
{
	...
	if (n_variable <= 0) {
		...
		sz = determine_word_size(m->header->body_size - c->begin,
		    n_variable);
...
bus_message_seal(sd_bus_message *m, uint64_t cookie, usec_t timeout)
{
	...
	if (!isempty(m->root_container.signature)) {
		r = message_append_field_signature(m,
		    BUS_MESSAGE_HEADER_SIGNATURE, m->root_container.signature,
		    NULL);
		if (r < 0)
			return r;
	}
	...
	m->timeout = m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED ? 0 :
									      timeout;
	...
	if (c->enclosing == SD_BUS_TYPE_ARRAY) {
		...
		if (sz < 0) {
			...
			*rindex = ALIGN_TO(c->offsets[c->offset_index],
			    alignment);
			c->item_size = c->offsets[c->offset_index + 1] -
			    *rindex;
		} else {
	...
	if (c->signature[c->index] == SD_BUS_TYPE_VARIANT) {
		if (contents) {
			...
			if (BUS_MESSAGE_IS_GVARIANT(m)) {
				...
				free(c->peeked_signature);
				c->peeked_signature = strndup((char *) q + 1,
				    k - 1);
...
message_read_ap(sd_bus_message *m, const char *types, va_list ap)
{
	...
	n_struct = strlen(
	    types); /* length of current struct contents signature */
    ...
    size_t nbytes, void **ret)
{
	...
	return buffer_peek(BUS_MESSAGE_FIELDS(m), BUS_MESSAGE_FIELDS_SIZE(m),
	    rindex, align, nbytes, ret);
}
...
bus_message_parse_fields(sd_bus_message *m)
{
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		if (sz > 0) {
			...
			r = message_peek_fields(m, &ri, 1,
			    BUS_MESSAGE_FIELDS_SIZE(m) - framing, &offsets);
			if (r < 0)
				return r;
	...
	while (ri < BUS_MESSAGE_FIELDS_SIZE(m)) {
		...
		if (BUS_MESSAGE_IS_GVARIANT(m)) {
			...
			if (i == 0)
				...
				ri = ALIGN_TO(read_word_le((uint8_t *) offsets +
						      (i - 1) * sz,
						  sz),
				    8);
		}
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		r = build_struct_offsets(m, m->root_container.signature,
		    BUS_MESSAGE_BODY_SIZE(m), &m->root_container.item_size,
		    &m->root_container.offsets, &m->root_container.n_offsets);
		if (r < 0)
			return r;
	}
...
const char *
bus_message_get_arg(sd_bus_message *m, unsigned i)
{
	int r;
	const char *t = NULL;
	unsigned j;
...
bool
bus_header_is_complete(struct bus_header *h, size_t size)
{
	size_t full;
	...
	full = sizeof(struct bus_header) +
	    (h->endian == BUS_NATIVE_ENDIAN ? h->fields_size :
						    bswap32(h->fields_size));

	return size >= full;
}

int
bus_header_message_size(struct bus_header *h, size_t *sum)
{
	size_t fs, bs;
	...
	if (h->endian == BUS_NATIVE_ENDIAN) {
		...
		fs = bswap32(h->fields_size);
		bs = bswap32(h->body_size);
	} else
		return -EBADMSG;
...
sd_bus_message_copy(sd_bus_message *m, sd_bus_message *source, int all)
{
	...
	do {
		...
		if (type == SD_BUS_TYPE_OBJECT_PATH ||
		    type == SD_BUS_TYPE_SIGNATURE || type == SD_BUS_TYPE_STRING)
...
bus_message_remarshal(sd_bus *bus, sd_bus_message **m)
{
	...
	case SD_BUS_MESSAGE_METHOD_ERROR:
		...
		r = message_append_field_uint32(n,
		    BUS_MESSAGE_HEADER_REPLY_SERIAL,
		    (uint32_t) n->reply_cookie);
		if (r < 0)
			return r;
		...
		if ((*m)->header->type == SD_BUS_MESSAGE_METHOD_ERROR &&
			...
			r = message_append_field_string(n,
			    BUS_MESSAGE_HEADER_ERROR_NAME, SD_BUS_TYPE_STRING,
