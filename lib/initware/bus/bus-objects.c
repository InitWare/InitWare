...
static int add_object_vtable_internal(
        ...
        r = hashmap_ensure_allocated(&bus->vtable_methods, string_hash_func, string_compare_func);
        if (r < 0)
                return r;

        r = hashmap_ensure_allocated(&bus->vtable_properties, string_hash_func, string_compare_func);
        if (r < 0)
                return r;
        ...
        IWLIST_INSERT_AFTER(struct node_vtable, vtables, n->vtables, existing, c);
        bus->nodes_modified = true;
