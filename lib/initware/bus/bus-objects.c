...
static int
node_vtable_get_userdata(sd_bus *bus, const char *path, struct node_vtable *c,
...
static void *
vtable_property_convert_userdata(const sd_bus_vtable *p, void *u)
{
	assert(p);
...
static int
vtable_property_get_userdata(sd_bus *bus, const char *path,
    struct vtable_member *p, void **userdata, sd_bus_error *error)
{
...
static int
node_callbacks_run(sd_bus *bus, sd_bus_message *m, struct node_callback *first,
...
static int
check_access(sd_bus *bus, sd_bus_message *m, struct vtable_member *c,
    sd_bus_error *error)
{
	...
	return sd_bus_error_setf(error, SD_BUS_ERROR_ACCESS_DENIED,
	    "Access to %s.%s() not permitted.", c->interface, c->member);
}

static int
method_callbacks_run(sd_bus *bus, sd_bus_message *m, struct vtable_member *c,
    bool require_fallback, bool *found_object)
{
...
static int
invoke_property_get(sd_bus *bus, const sd_bus_vtable *v, const char *path,
    ...
    void *userdata, sd_bus_error *error)
{
	...
	if (v->x.property.get) {
		r = v->x.property.get(bus, path, interface, property, reply,
		    userdata, error);
		if (r < 0)
			return r;
		if (sd_bus_error_is_set(error))
	...
	return sd_bus_message_append_basic(reply, v->x.property.signature[0],
	    p);
}

static int
invoke_property_set(sd_bus *bus, const sd_bus_vtable *v, const char *path,
    ...
    void *userdata, sd_bus_error *error)
{
	...
	if (v->x.property.set) {
		r = v->x.property.set(bus, path, interface, property, value,
		    userdata, error);
		if (r < 0)
			return r;
		if (sd_bus_error_is_set(error))
...
static int
property_get_set_callbacks_run(sd_bus *bus, sd_bus_message *m,
    struct vtable_member *c, bool require_fallback, bool is_get,
    bool *found_object)
{
	...
	*found_object = true;
	...
	if (is_get) {
		...
		r = invoke_property_get(bus, c->vtable, m->path, c->interface,
		...
		r = invoke_property_set(bus, c->vtable, m->path, c->interface,
		    c->member, m, u, &error);
		if (r < 0)
			return bus_maybe_reply_error(m, r, &error);
...
static int
vtable_append_one_property(sd_bus *bus, sd_bus_message *reply, const char *path,
    struct node_vtable *c, const sd_bus_vtable *v, void *userdata,
    sd_bus_error *error)
{
	...
	r = invoke_property_get(bus, v, path, c->interface,
	    v->x.property.member, reply,
...
static int
vtable_append_all_properties(sd_bus *bus, sd_bus_message *reply,
    const char *path, struct node_vtable *c, void *userdata,
    sd_bus_error *error)
{
	...
	for (v = c->vtable + 1; v->type != _SD_BUS_VTABLE_END; v++) {
		...
		r = vtable_append_one_property(bus, reply, path, c, v, userdata,
		    error);
		if (r < 0)
			return r;
		if (bus->nodes_modified)
...
static int
property_get_all_callbacks_run(sd_bus *bus, sd_bus_message *m,
    struct node_vtable *first, bool require_fallback, const char *iface,
    bool *found_object)
{
	...
	IWLIST_FOREACH (vtables, c, first) {
		...
		*found_object = true;
		...
		r = vtable_append_all_properties(bus, reply, m->path, c, u,
		    &error);
		if (r < 0)
			return bus_maybe_reply_error(m, r, &error);
		if (bus->nodes_modified)
	...
	if (!found_interface) {
		r = sd_bus_reply_method_errorf(m,
		    SD_BUS_ERROR_UNKNOWN_INTERFACE, "Unknown interface '%s'.",
		    iface);
		if (r < 0)
			return r;
...
static bool
bus_node_with_object_manager(sd_bus *bus, struct node *n)
{
	assert(bus);
	assert(n);
...
static bool
bus_node_exists(sd_bus *bus, struct node *n, const char *path,
    ...
    bool require_fallback, bool *found_object)
{
	...
	r = introspect_write_default_interfaces(&intro,
	    bus_node_with_object_manager(bus, n));
	if (r < 0)
		return r;
...
static int
object_manager_serialize_path(sd_bus *bus, sd_bus_message *reply,
    const char *prefix, const char *path, bool require_fallback,
    sd_bus_error *error)
{
	...
	IWLIST_FOREACH (vtables, i, n->vtables) {
		...
		if (!found_something) {
			...
			r = sd_bus_message_open_container(reply, 'e',
			    "oa{sa{sv}}");
			if (r < 0)
				return r;
...
static int
object_manager_serialize_path_and_fallbacks(sd_bus *bus, sd_bus_message *reply,
    const char *path, sd_bus_error *error)
{
    ...
    bool require_fallback, bool *found_object)
{
	...
	if (empty) {
		...
		SET_FOREACH (path, s, i) {
			r = object_manager_serialize_path_and_fallbacks(bus,
			    reply, path, &error);
			if (r < 0)
				return r;
...
static int
object_find_and_run(sd_bus *bus, sd_bus_message *m, const char *p,
    bool require_fallback, bool *found_object)
{
	...
	if (v) {
		r = method_callbacks_run(bus, m, v, require_fallback,
		    found_object);
		if (r != 0)
			return r;
		if (bus->nodes_modified)
	...
	if (streq(m->interface, "org.freedesktop.DBus.Properties")) {
		...
		if (get || streq(m->member, "Set")) {
			...
			if (v) {
				r = property_get_set_callbacks_run(bus, m, v,
				    require_fallback, get, found_object);
				if (r != 0)
					return r;
			}
			...
			r = property_get_all_callbacks_run(bus, m, n->vtables,
			    require_fallback, iface, found_object);
			if (r != 0)
				return r;
		}
		...
		r = process_introspect(bus, m, n, require_fallback,
		    found_object);
		if (r != 0)
			return r;
...
int
bus_process_object(sd_bus *bus, sd_bus_message *m)
{
	int r;
	size_t pl;
	...
	do {
		...
		OBJECT_PATH_FOREACH_PREFIX (prefix, m->path) {
			...
			r = object_find_and_run(bus, m, prefix, true,
			    &found_object);
			if (r != 0)
				return r;
		}
	...
	if (sd_bus_message_is_method_call(m, "org.freedesktop.DBus.Properties",
		...
		r = sd_bus_reply_method_errorf(m, SD_BUS_ERROR_UNKNOWN_METHOD,
		    "Unknown method '%s' or interface '%s'.", m->member,
		    m->interface);
...
static struct node *
bus_node_allocate(sd_bus *bus, const char *path)
{
	...
	r = hashmap_ensure_allocated(&bus->nodes, string_hash_func,
	    string_compare_func);
	if (r < 0)
		return NULL;
	...
	if (parent)
		IWLIST_PREPEND(struct node, siblings, parent->child, n);

	return n;
}

static void
bus_node_gc(sd_bus *b, struct node *n)
{
	assert(b);
	...
	if (n->parent)
		IWLIST_REMOVE(struct node, siblings, n->parent->child, n);
...
static int
bus_add_object(sd_bus *bus, bool fallback, const char *path,
    sd_bus_message_handler_t callback, void *userdata)
{
	...
	IWLIST_PREPEND(struct node_callback, callbacks, n->callbacks, c);
...
static int
bus_remove_object(sd_bus *bus, bool fallback, const char *path,
    sd_bus_message_handler_t callback, void *userdata)
{
	...
	IWLIST_FOREACH (callbacks, c, n->callbacks)
	...
	IWLIST_REMOVE(struct node_callback, callbacks, n->callbacks, c);
	free(c);
...
_public_ int
sd_bus_add_object(sd_bus *bus, const char *path,
    sd_bus_message_handler_t callback, void *userdata)
{
...
_public_ int
sd_bus_remove_object(sd_bus *bus, const char *path,
    sd_bus_message_handler_t callback, void *userdata)
{
...
_public_ int
sd_bus_add_fallback(sd_bus *bus, const char *prefix,
    sd_bus_message_handler_t callback, void *userdata)
{
...
_public_ int
sd_bus_remove_fallback(sd_bus *bus, const char *prefix,
    sd_bus_message_handler_t callback, void *userdata)
{
...
static void
free_node_vtable(sd_bus *bus, struct node_vtable *w)
{
	assert(bus);
	...
	if (w->interface && w->node && w->vtable) {
		...
		for (v = w->vtable; v->type != _SD_BUS_VTABLE_END; v++) {
			...
			case _SD_BUS_VTABLE_WRITABLE_PROPERTY: {
				...
				x = hashmap_remove(bus->vtable_properties,
				    &key);
				break;
			}
			}
...
static int
add_object_vtable_internal(sd_bus *bus, const char *path, const char *interface,
    const sd_bus_vtable *vtable, bool fallback, sd_bus_object_find_t find,
    void *userdata)
{
	...
	r = hashmap_ensure_allocated(&bus->vtable_methods, string_hash_func,
	    string_compare_func);
	if (r < 0)
		return r;

	r = hashmap_ensure_allocated(&bus->vtable_properties, string_hash_func,
	    string_compare_func);
	if (r < 0)
		return r;
	...
	for (v = c->vtable + 1; v->type != _SD_BUS_VTABLE_END; v++) {
		...
		case _SD_BUS_VTABLE_METHOD: {
			...
			if (!member_name_is_valid(v->x.method.member) ||
				...
				false) ||
			    !signature_is_valid(strempty(v->x.method.result),
				false) ||
			    !(v->x.method.handler ||
				(isempty(v->x.method.signature) &&
		...
		case _SD_BUS_VTABLE_PROPERTY: {
			...
			if (!member_name_is_valid(v->x.property.member) ||
			    !signature_is_single(v->x.property.signature,
				false) ||
			    !(v->x.property.get ||
				bus_type_is_basic(v->x.property.signature[0]) ||
	...
	IWLIST_INSERT_AFTER(struct node_vtable, vtables, n->vtables, existing,
	    c);
	bus->nodes_modified = true;
...
static int
remove_object_vtable_internal(sd_bus *bus, const char *path,
    const char *interface, const sd_bus_vtable *vtable, bool fallback,
    sd_bus_object_find_t find, void *userdata)
{
	...
	IWLIST_FOREACH (vtables, c, n->vtables)
		if (streq(c->interface, interface) &&
		    c->is_fallback == fallback && c->vtable == vtable &&
		    c->find == find && c->userdata == userdata)
			break;

	if (!c)
		return 0;

	IWLIST_REMOVE(struct node_vtable, vtables, n->vtables, c);
...
_public_ int
sd_bus_add_object_vtable(sd_bus *bus, const char *path, const char *interface,
    const sd_bus_vtable *vtable, void *userdata)
{

	return add_object_vtable_internal(bus, path, interface, vtable, false,
	    NULL, userdata);
}

_public_ int
sd_bus_remove_object_vtable(sd_bus *bus, const char *path,
    const char *interface, const sd_bus_vtable *vtable, void *userdata)
{

	return remove_object_vtable_internal(bus, path, interface, vtable,
	    false, NULL, userdata);
}

_public_ int
sd_bus_add_fallback_vtable(sd_bus *bus, const char *path, const char *interface,
    const sd_bus_vtable *vtable, sd_bus_object_find_t find, void *userdata)
{

	return add_object_vtable_internal(bus, path, interface, vtable, true,
	    find, userdata);
}

_public_ int
sd_bus_remove_fallback_vtable(sd_bus *bus, const char *path,
    const char *interface, const sd_bus_vtable *vtable,
    sd_bus_object_find_t find, void *userdata)
{

	return remove_object_vtable_internal(bus, path, interface, vtable, true,
	    find, userdata);
}

_public_ int
sd_bus_add_node_enumerator(sd_bus *bus, const char *path,
    sd_bus_node_enumerator_t callback, void *userdata)
{
	...
	IWLIST_PREPEND(struct node_enumerator, enumerators, n->enumerators, c);
...
_public_ int
sd_bus_remove_node_enumerator(sd_bus *bus, const char *path,
    sd_bus_node_enumerator_t callback, void *userdata)
{
	...
	IWLIST_FOREACH (enumerators, c, n->enumerators)
		if (c->callback == callback && c->userdata == userdata)
			break;
	...
	IWLIST_REMOVE(struct node_enumerator, enumerators, n->enumerators, c);
	free(c);
...
static int
emit_properties_changed_on_interface(sd_bus *bus, const char *prefix,
    const char *path, const char *interface, bool require_fallback,
    bool *found_interface, char **names)
{
	...
	r = sd_bus_message_new_signal(bus, &m, path,
		...
		*found_interface = true;
		...
		if (names) {
			...
			STRV_FOREACH (property, names) {
				...
				assert_return(member_name_is_valid(*property),
				    -EINVAL);
				...
				r = vtable_append_one_property(bus, m, m->path,
				    c, v->vtable, u, &error);
				if (r < 0)
					return r;
				if (bus->nodes_modified)
			...
			for (v = c->vtable + 1; v->type != _SD_BUS_VTABLE_END;
			     v++) {
				if (v->type != _SD_BUS_VTABLE_PROPERTY &&
				    v->type != _SD_BUS_VTABLE_WRITABLE_PROPERTY)
					continue;
		...
		IWLIST_FOREACH (vtables, c, n->vtables) {
			...
			if (names) {
				STRV_FOREACH (property, names) {
					...
					key.member = *property;
					assert_se(
					    v = hashmap_get(bus->vtable_properties,
						&key));
					assert(c == v->parent);
...
_public_ int
sd_bus_emit_properties_changed_strv(sd_bus *bus, const char *path,
    const char *interface, char **names)
{
	...
	do {
		...
		r = emit_properties_changed_on_interface(bus, path, path,
		    interface, false, &found_interface, names);
		if (r != 0)
			return r;
		if (bus->nodes_modified)
			continue;
...
_public_ int
sd_bus_emit_properties_changed(sd_bus *bus, const char *path,
...
static int
interfaces_added_append_one_prefix(sd_bus *bus, sd_bus_message *m,
    const char *prefix, const char *path, const char *interface,
    bool require_fallback)
{
...
static int
interfaces_added_append_one(sd_bus *bus, sd_bus_message *m, const char *path,
...
_public_ int
sd_bus_emit_interfaces_added_strv(sd_bus *bus, const char *path,
    char **interfaces)
{
	BUS_DONT_DESTROY(bus);
...
_public_ int
sd_bus_emit_interfaces_added(sd_bus *bus, const char *path,
    const char *interface, ...)
{
	char **interfaces;
...
_public_ int
sd_bus_emit_interfaces_removed_strv(sd_bus *bus, const char *path,
    char **interfaces)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	int r;
...
_public_ int
sd_bus_emit_interfaces_removed(sd_bus *bus, const char *path,
    const char *interface, ...)
{
	char **interfaces;
...
_public_ int
sd_bus_add_object_manager(sd_bus *bus, const char *path)
{
	struct node *n;
...
_public_ int
sd_bus_remove_object_manager(sd_bus *bus, const char *path)
{
	struct node *n;
