...
static int
bus_get_owner_dbus1(sd_bus *bus, const char *name, uint64_t mask,
    sd_bus_creds **creds)
{
	...
	if (mask != 0) {
		...
		if (mask &
			...
			SD_BUS_CREDS_GID | SD_BUS_CREDS_COMM |
		...
		if (mask & SD_BUS_CREDS_SELINUX_CONTEXT) {
			...
			r = sd_bus_call_method(bus, "org.freedesktop.DBus",
			    "/org/freedesktop/DBus", "org.freedesktop.DBus",
			    "GetConnectionSELinuxSecurityContext", NULL, &reply,
			    "s", unique ? unique : name);
			if (r < 0)
				return r;
...
_public_ int
sd_bus_get_owner(sd_bus *bus, const char *name, uint64_t mask,
    sd_bus_creds **creds)
{
...
bus_add_match_internal_dbus1(sd_bus *bus, const char *match)
{
	...
	return sd_bus_call_method(bus, "org.freedesktop.DBus",
	    "/org/freedesktop/DBus", "org.freedesktop.DBus", "AddMatch", NULL,
	    NULL, "s", match);
}
...
_public_ int
sd_bus_get_owner_machine_id(sd_bus *bus, const char *name, sd_id128_t *machine)
{
