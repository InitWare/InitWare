...
_public_ int
sd_memfd_new(sd_memfd **m, const char *name)
{
	...
	sz = ALIGN8(offsetof(struct kdbus_cmd_memfd_make, items)) +
	    ALIGN8(offsetof(struct kdbus_item, str)) + l + 1;
...
_public_ int
sd_memfd_new_from_fd(sd_memfd **m, int fd)
{
	sd_memfd *n;
	uint64_t sz;
...
_public_ int
sd_memfd_get_file(sd_memfd *m, FILE **f)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_memfd_map(sd_memfd *m, uint64_t offset, size_t size, void **p)
{
	void *q;
	int sealed;
	...
	q = mmap(NULL, size, sealed ? PROT_READ : PROT_READ | PROT_WRITE,
	    MAP_SHARED, m->fd, offset);
	if (q == MAP_FAILED)
		return -errno;
...
_public_ int
sd_memfd_set_sealed(sd_memfd *m, int b)
{
	int r;
...
_public_ int
sd_memfd_get_sealed(sd_memfd *m)
{
	int r, b;
...
_public_ int
sd_memfd_get_size(sd_memfd *m, uint64_t *sz)
{
	int r;
...
_public_ int
sd_memfd_set_size(sd_memfd *m, uint64_t sz)
{
	int r;
...
_public_ int
sd_memfd_new_and_map(sd_memfd **m, const char *name, size_t sz, void **p)
{
	sd_memfd *n;
	int r;
...
_public_ int
sd_memfd_get_name(sd_memfd *m, char **name)
{
	char path[sizeof("/proc/self/fd/") + DECIMAL_STR_MAX(int)],
	    buf[FILENAME_MAX + 1], *e;
