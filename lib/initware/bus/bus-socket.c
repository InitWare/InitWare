...
bus_socket_auth_verify_client(sd_bus *b)
{
	...
	if (f)
		b->can_fds = (f - e == sizeof("\r\nAGREE_UNIX_FD") - 1) &&
		    memcmp(e + 2, "AGREE_UNIX_FD",
			sizeof("AGREE_UNIX_FD") - 1) == 0;
...
bus_socket_read_auth(sd_bus *b)
{
	...
	union {
		...
		uint8_t buf[CMSG_SPACE(sizeof(int) * BUS_FDS_MAX) +
		    CMSG_SPACE(sizeof(struct socket_ucred)) +
	...
	if (handle_cmsg) {
		for (cmsg = CMSG_FIRSTHDR(&mh); cmsg;
			...
			} else if (cmsg_readucred(cmsg, &b->ucred) > 0) {
...
bus_socket_make_message(sd_bus *bus, size_t size)
{
	...
	r = bus_message_from_malloc(bus, bus->rbuffer, size, bus->fds,
	    ...
	    !bus->bus_client && bus->label[0] ? bus->label : NULL, &t);
	if (r < 0) {
		free(b);
		return r;
	}
	...
	if (handle_cmsg) {
		for (cmsg = CMSG_FIRSTHDR(&mh); cmsg;
			...
			if (cmsg->cmsg_level == SOL_SOCKET &&
				...
				bus->fds = f;
				bus->n_fds += n;
			} else if (cmsg_readucred(cmsg, &bus->ucred) > 0) {
