...
BUS_MESSAGE_BSWAP16(sd_bus_message *m, uint16_t u)
{
	return BUS_MESSAGE_NEED_BSWAP(m) ? bswap16(u) : u;
}

static inline uint32_t
BUS_MESSAGE_BSWAP32(sd_bus_message *m, uint32_t u)
{
	return BUS_MESSAGE_NEED_BSWAP(m) ? bswap32(u) : u;
}

static inline uint64_t
BUS_MESSAGE_BSWAP64(sd_bus_message *m, uint64_t u)
{
...
static inline uint32_t
BUS_MESSAGE_BODY_SIZE(sd_bus_message *m)
{
	return BUS_MESSAGE_BSWAP32(m, m->header->body_size);
}

static inline uint32_t
BUS_MESSAGE_FIELDS_SIZE(sd_bus_message *m)
{
	return BUS_MESSAGE_BSWAP32(m, m->header->fields_size);
}

static inline uint32_t
BUS_MESSAGE_SIZE(sd_bus_message *m)
{
	return sizeof(struct bus_header) + ALIGN8(BUS_MESSAGE_FIELDS_SIZE(m)) +
	    BUS_MESSAGE_BODY_SIZE(m);
}

static inline uint32_t
BUS_MESSAGE_BODY_BEGIN(sd_bus_message *m)
{
	return sizeof(struct bus_header) + ALIGN8(BUS_MESSAGE_FIELDS_SIZE(m));
}

static inline void *
BUS_MESSAGE_FIELDS(sd_bus_message *m)
{
	return (uint8_t *) m->header + sizeof(struct bus_header);
}

static inline bool
BUS_MESSAGE_IS_GVARIANT(sd_bus_message *m)
{
...
int bus_message_from_header(sd_bus *bus, void *header, size_t length, int *fds,
