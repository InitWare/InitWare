...
int
bus_verify_polkit(sd_bus *bus, sd_bus_message *m, const char *action,
...
static void
async_polkit_query_free(sd_bus *b, AsyncPolkitQuery *q)
{
...
int
bus_verify_polkit_async(sd_bus *bus, Hashmap **registry, sd_bus_message *m,
    const char *action, bool interactive, sd_bus_error *error,
    sd_bus_message_handler_t callback, void *userdata)
{
	...
	r = hashmap_ensure_allocated(registry, trivial_hash_func,
	    trivial_compare_func);
	if (r < 0)
		return r;

	r = sd_bus_message_new_method_call(bus, &pk,
	...
	r = sd_bus_message_append(pk, "(sa{sv})sa{ss}us", "system-bus-name", 1,
	    "name", "s", sender, action, 0, interactive ? 1 : 0, NULL);
	if (r < 0)
		return r;
...
void
bus_verify_polkit_async_registry_free(sd_bus *bus, Hashmap *registry)
{
#ifdef ENABLE_POLKIT
	AsyncPolkitQuery *q;
...
bus_open_user_systemd(sd_bus **_bus)
{
	...
	if (asprintf(&bus->address, KERNEL_USER_BUS_FMT,
...
int
bus_property_get_tristate(sd_bus *bus, const char *path, const char *interface,
...
bus_parse_unit_info(sd_bus_message *message, UnitInfo *u)
{
	...
	return sd_bus_message_read(message, "(ssssssouso)", &u->id,
	    &u->description, &u->load_state, &u->active_state, &u->sub_state,
	    &u->following, &u->unit_path, &u->job_id, &u->job_type,
	    &u->job_path);
}
...
bus_maybe_reply_error(sd_bus_message *m, int r, sd_bus_error *error)
{
	...
	log_debug("Failed to process message [type=%s sender=%s path=%s interface=%s member=%s signature=%s]: %s",
	    ...
	    strna(m->root_container.signature), bus_error_message(error, r));
