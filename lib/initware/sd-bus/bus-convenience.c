...
_public_ int
sd_bus_emit_signal(sd_bus *bus, const char *path, const char *interface,
...
_public_ int
sd_bus_call_method(sd_bus *bus, const char *destination, const char *path,
	...
	sd_bus_message **reply, const char *types, ...)
{
	...
	r = sd_bus_message_new_method_call(bus, &m, destination, path,
		interface, member);
	if (r < 0)
		return r;
...
_public_ int
sd_bus_reply_method_return(sd_bus_message *call, const char *types, ...)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	int r;
...
_public_ int
sd_bus_reply_method_error(sd_bus_message *call, const sd_bus_error *e)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	int r;
...
_public_ int
sd_bus_reply_method_errorf(sd_bus_message *call, const char *name,
	const char *format, ...)
{
...
_public_ int
sd_bus_reply_method_errno(sd_bus_message *call, int error,
...
_public_ int
sd_bus_reply_method_errnof(sd_bus_message *call, int error, const char *format,
	...)
{
	_cleanup_bus_error_free_ sd_bus_error berror = SD_BUS_ERROR_NULL;
	va_list ap;
...
_public_ int
sd_bus_get_property(sd_bus *bus, const char *destination, const char *path,
	const char *interface, const char *member, sd_bus_error *error,
	sd_bus_message **reply, const char *type)
{
	...
	assert_return(bus, -EINVAL);
	assert_return(isempty(interface) || interface_name_is_valid(interface),
		-EINVAL);
	...
	r = sd_bus_call_method(bus, destination, path,
		"org.freedesktop.DBus.Properties", "Get", error, &rep, "ss",
		strempty(interface), member);
	if (r < 0)
		return r;
...
_public_ int
sd_bus_get_property_trivial(sd_bus *bus, const char *destination,
	const char *path, const char *interface, const char *member,
	sd_bus_error *error, char type, void *ptr)
{
...
_public_ int
sd_bus_get_property_string(sd_bus *bus, const char *destination,
	const char *path, const char *interface, const char *member,
...
_public_ int
sd_bus_get_property_strv(sd_bus *bus, const char *destination, const char *path,
	const char *interface, const char *member, sd_bus_error *error,
	char ***ret)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
...
_public_ int
sd_bus_set_property(sd_bus *bus, const char *destination, const char *path,
	const char *interface, const char *member, sd_bus_error *error,
	const char *type, ...)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
...
_public_ int
sd_bus_query_sender_creds(sd_bus_message *call, uint64_t mask,
	sd_bus_creds **creds)
{
	sd_bus_creds *c;
	...
	if (!c || !(c->mask & SD_BUS_CREDS_PID)) {
		...
		if (call->sender)
			return sd_bus_get_name_creds(call->bus, call->sender,
				mask, creds);
		else
...
_public_ int
sd_bus_query_sender_privilege(sd_bus_message *call, int capability)
{
...

	{
		r = sd_bus_query_sender_creds(call,
			SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID, &creds);
		if (r < 0)
			return r;
	}
