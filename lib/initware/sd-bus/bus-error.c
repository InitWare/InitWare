...
BUS_ERROR_MAP_ELF_REGISTER const sd_bus_error_map bus_standard_errors[] = {
	...
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.ServiceUnknown",
		EHOSTUNREACH),
	...
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.BadAddress",
		EADDRNOTAVAIL),
	...
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.UnixProcessIdUnknown",
		ESRCH),
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.InvalidSignature", EINVAL),
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.InconsistentMessage",
		EBADMSG),
	...
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.InvalidFileContent",
		EINVAL),
	SD_BUS_ERROR_MAP("org.freedesktop.DBus.Error.MatchRuleNotFound",
		ENOENT),
	SD_BUS_ERROR_MAP(
...
static int
bus_error_name_to_errno(const char *name)
{
...
errno_to_bus_error_const(int error)
{
	...
	case ESRCH:
		return SD_BUS_ERROR_MAKE_CONST(
...
static int
errno_to_bus_error_name_new(int error, char **ret)
{
	const char *name;
	char *n;
...
bool
bus_error_is_dirty(sd_bus_error *e)
{
	if (!e)
		return false;
...
_public_ int
sd_bus_error_set(sd_bus_error *e, const char *name, const char *message)
{
	if (!name)
		return 0;
	if (!e)
...
int
bus_error_setfv(sd_bus_error *e, const char *name, const char *format,
	va_list ap)
{
	if (!name)
		return 0;
...
_public_ int
sd_bus_error_setf(sd_bus_error *e, const char *name, const char *format, ...)
{
	if (format) {
		int r;
		va_list ap;
...
_public_ int
sd_bus_error_copy(sd_bus_error *dest, const sd_bus_error *e)
{
...
_public_ int
sd_bus_error_set_const(sd_bus_error *e, const char *name, const char *message)
{
	if (!name)
		return 0;
	if (!e)
...
_public_ int
sd_bus_error_has_name(const sd_bus_error *e, const char *name)
{
	if (!e)
		return 0;
...
static void
bus_error_strerror(sd_bus_error *e, int error)
{
	size_t k = 64;
	char *m;
...
_public_ int
sd_bus_error_set_errno(sd_bus_error *e, int error)
{
	if (error < 0)
		error = -error;
...
_public_ int
sd_bus_error_set_errnofv(sd_bus_error *e, int error, const char *format,
	va_list ap)
{
	PROTECT_ERRNO;
	int r;
...
_public_ int
sd_bus_error_set_errnof(sd_bus_error *e, int error, const char *format, ...)
{
	int r;
...
_public_ int
sd_bus_error_add_map(const sd_bus_error_map *map)
{
	...
	maps = realloc_multiply(additional_error_maps,
		sizeof(struct sd_bus_error_map *), n + 2);
	if (!maps)
		return -ENOMEM;
