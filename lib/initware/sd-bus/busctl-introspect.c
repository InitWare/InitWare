...
static void
context_reset_member(Context *c)
{
	free(c->member_name);
...
static void
context_reset_interface(Context *c)
{
...
static int
parse_xml_annotation(Context *context, uint64_t *flags)
{
	enum {
		STATE_ANNOTATION,
		STATE_NAME,
	...
	for (;;) {
		...
		t = xml_tokenize(&context->current, &name, &context->xml_state,
			NULL);
		if (t < 0) {
...
static int
parse_xml_node(Context *context, const char *prefix, unsigned n_depth)
{
	enum {
		STATE_NODE,
		STATE_NODE_NAME,
		...
		case STATE_NODE:
			if (t == XML_ATTRIBUTE_NAME) {
				...
				if (streq_ptr(name, "interface"))
					...
					r = parse_xml_node(context, np,
						n_depth + 1);
					if (r < 0)
						return r;
				} else {
					log_error("Unexpected <node> tag %s.",
						name);
					return -EBADMSG;
				}
				...
				if (context->ops->on_path) {
					r = context->ops->on_path(
						node_path ? node_path : np,
						context->userdata);
					if (r < 0)
						return r;
				}
		...
		case STATE_NODE_NAME:
			...
			if (t == XML_ATTRIBUTE_VALUE) {
				...
				if (name[0] == '/') {
					...
					if (endswith(prefix, "/"))
						node_path =
							strappend(prefix, name);
					else
						node_path = strjoin(prefix, "/",
							name, NULL);
					if (!node_path)
						return log_oom();
				}
		...
		case STATE_METHOD:
			...
			if (t == XML_ATTRIBUTE_NAME) {
				...
				if (streq_ptr(name, "arg"))
					...
					log_error("Unexpected <method> tag %s.",
						name);
					return -EINVAL;
				}
			} else if (t == XML_TAG_CLOSE_EMPTY ||
				(t == XML_TAG_CLOSE &&
					streq_ptr(name, "method"))) {
				if (n_depth == 0) {
		...
		case STATE_METHOD_ARG:
			...
			if (t == XML_ATTRIBUTE_NAME) {
				...
				if (n_depth == 0) {
					if (argument_type) {
						if (!argument_direction ||
							...
							if (!strextend(
								    ...
								    NULL))
								return log_oom();
						} else if (streq(argument_direction,
								   "out")) {
							if (!strextend(
								    &context->member_result,
								    argument_type,
					...
					argument_type = argument_direction =
						NULL;
				}
		...
		case STATE_SIGNAL:
			...
			if (t == XML_ATTRIBUTE_NAME) {
				...
				if (streq_ptr(name, "arg"))
					...
					log_error("Unexpected <signal> tag %s.",
						name);
					return -EINVAL;
				}
			} else if (t == XML_TAG_CLOSE_EMPTY ||
				(t == XML_TAG_CLOSE &&
					streq_ptr(name, "signal"))) {
				if (n_depth == 0) {
...
int
parse_xml_introspect(const char *prefix, const char *xml,
	const XMLIntrospectOps *ops, void *userdata)
{
	...
	for (;;) {
		...
		r = xml_tokenize(&context.current, &name, &context.xml_state,
			NULL);
		if (r < 0) {
			log_error("XML parse error");
			goto finish;
		...
		if (r == XML_TAG_OPEN) {
			if (streq(name, "node")) {
				...
				log_error(
					"Unexpected tag '%s' in introspection data.",
					name);
				r = -EBADMSG;
				goto finish;
			}
