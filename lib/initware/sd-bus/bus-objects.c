	...
	sd_bus_error *error)
{
	...
	r = sd_bus_query_sender_privilege(m, 0);
	if (r < 0)
		return r;
	if (r > 0)
		return 0;
...
static int
invoke_property_get(sd_bus *bus, sd_bus_slot *slot, const sd_bus_vtable *v,
...
static int
invoke_property_set(sd_bus *bus, sd_bus_slot *slot, const sd_bus_vtable *v,
	const char *path, const char *interface, const char *property,
	...
	bool *found_object)
{
	...
	*found_object = true;
	...
	if (is_get) {
		...
		r = invoke_property_get(bus, slot, c->vtable, m->path,
			c->interface, c->member, reply, u, &error);
		if (r < 0)
			return bus_maybe_reply_error(m, r, &error);
	...
	sd_bus_error *error)
{
	...
	r = invoke_property_get(bus, slot, v, path, c->interface,
...
static int
bus_add_object(sd_bus *bus, sd_bus_slot **slot, bool fallback, const char *path,
...
_public_ int
sd_bus_add_object(sd_bus *bus, sd_bus_slot **slot, const char *path,
	sd_bus_message_handler_t callback, void *userdata)
{
...
_public_ int
sd_bus_add_fallback(sd_bus *bus, sd_bus_slot **slot, const char *prefix,
	sd_bus_message_handler_t callback, void *userdata)
{
	...
	sd_bus_object_find_t find, void *userdata)
{
	...
	r = hashmap_ensure_allocated(&bus->vtable_methods,
	...
	for (v = s->node_vtable.vtable + 1; v->type != _SD_BUS_VTABLE_END;
		v++) {
		switch (v->type) {
		case _SD_BUS_VTABLE_METHOD: {
			...
			if (!member_name_is_valid(v->x.method.member) ||
				!signature_is_valid(
					strempty(v->x.method.signature),
					false) ||
				!signature_is_valid(
					strempty(v->x.method.result), false) ||
				!(v->x.method.handler ||
...
_public_ int
sd_bus_add_object_vtable(sd_bus *bus, sd_bus_slot **slot, const char *path,
	const char *interface, const sd_bus_vtable *vtable, void *userdata)
{
	return add_object_vtable_internal(bus, slot, path, interface, vtable,
		false, NULL, userdata);
}

_public_ int
sd_bus_add_fallback_vtable(sd_bus *bus, sd_bus_slot **slot, const char *prefix,
...
_public_ int
sd_bus_add_node_enumerator(sd_bus *bus, sd_bus_slot **slot, const char *path,
...
static int
object_added_append_all_prefix(sd_bus *bus, sd_bus_message *m, Set *s,
	const char *prefix, const char *path, bool require_fallback)
{
	const char *previous_interface = NULL;
	struct node_vtable *c;
...
static int
object_added_append_all(sd_bus *bus, sd_bus_message *m, const char *path)
{
	...
	r = sd_bus_message_append(m, "{sa{sv}}", "org.freedesktop.DBus.Peer",
		0);
	if (r < 0)
		return r;
...
int
sd_bus_emit_object_added(sd_bus *bus, const char *path)
{
	BUS_DONT_DESTROY(bus);
...
static int
object_removed_append_all_prefix(sd_bus *bus, sd_bus_message *m, Set *s,
	const char *prefix, const char *path, bool require_fallback)
{
	const char *previous_interface = NULL;
	struct node_vtable *c;
...
static int
object_removed_append_all(sd_bus *bus, sd_bus_message *m, const char *path)
{
	_cleanup_set_free_ Set *s = NULL;
	_cleanup_free_ char *prefix = NULL;
	size_t pl;
...
int
sd_bus_emit_object_removed(sd_bus *bus, const char *path)
{
	BUS_DONT_DESTROY(bus);
...
_public_ int
sd_bus_add_object_manager(sd_bus *bus, sd_bus_slot **slot, const char *path)
{
	sd_bus_slot *s;
	struct node *n;
	int r;
