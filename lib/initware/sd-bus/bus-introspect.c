...
int
introspect_begin(struct introspect *i, bool trusted)
{
	assert(i);
...
int
introspect_write_default_interfaces(struct introspect *i, bool object_manager)
{
	assert(i);
...
int
introspect_write_child_nodes(struct introspect *i, Set *s, const char *prefix)
{
	char *node;
...
static void
introspect_write_flags(struct introspect *i, int type, uint64_t flags)
{
...
static int
introspect_write_arguments(struct introspect *i, const char *signature,
...
int
introspect_write_interface(struct introspect *i, const sd_bus_vtable *v)
{
	assert(i);
	assert(v);
	...
	for (; v->type != _SD_BUS_VTABLE_END; v++) {
		...
		case _SD_BUS_VTABLE_METHOD:
			fprintf(i->f, "  <method name=\"%s\">\n",
				v->x.method.member);
			introspect_write_arguments(i,
				strempty(v->x.method.signature), "in");
			introspect_write_arguments(i,
				strempty(v->x.method.result), "out");
		...
		case _SD_BUS_VTABLE_SIGNAL:
			fprintf(i->f, "  <signal name=\"%s\">\n",
				v->x.signal.member);
			introspect_write_arguments(i,
				strempty(v->x.signal.signature), NULL);
			introspect_write_flags(i, v->type, v->flags);
			fputs("  </signal>\n", i->f);
			break;
		}
...
int
introspect_finish(struct introspect *i, sd_bus *bus, sd_bus_message *m,
...
void
introspect_free(struct introspect *i)
{
	assert(i);
