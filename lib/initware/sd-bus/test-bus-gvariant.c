...
static void
test_bus_gvariant_is_fixed_size(void)
{
...
static void
test_bus_gvariant_get_size(void)
{
...
static void
test_bus_gvariant_get_alignment(void)
{
...
test_marshal(void)
{
	...
	assert_se(
		sd_bus_message_new_method_call(bus, &m, "a.service.name",
	...
	assert_se(sd_bus_message_append(m, "a(usv)", 3, 4711,
			  "first-string-parameter", "(st)", "X", (uint64_t)1111,
			  4712, "second-string-parameter", "(a(si))", 2, "Y", 5,
			  "Z", 6, 4713, "third-string-parameter", "(uu)", 1,
			  2) >= 0);
...
#ifdef HAVE_GLIB
	{
		...
		v = g_variant_new_from_data(G_VARIANT_TYPE("(a(usv))"),
			m->body.data, m->user_body_size, false, NULL, NULL);
		assert_se(g_variant_is_normal_form(v));
		t = g_variant_print(v, TRUE);
		printf("%s\n", t);
		g_free(t);
...
#ifdef HAVE_GLIB
	{
		...
		v = g_variant_new_from_data(G_VARIANT_TYPE("(yyyyuta{tv}v)"),
			blob, sz, false, NULL, NULL);
		assert_se(g_variant_is_normal_form(v));
		t = g_variant_print(v, TRUE);
		printf("%s\n", t);
		g_free(t);
	...
	assert_se(bus_message_from_malloc(bus, blob, sz, NULL, 0, NULL, NULL,
			  &n) >= 0);
	blob = NULL;
	...
	assert_se(sd_bus_message_new_method_call(bus, &m, "a.x", "/a/x", "a.x",
			  "Ax") >= 0);
