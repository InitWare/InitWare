...
static int
match_callback(sd_bus *bus, sd_bus_message *m, void *userdata,
...
static int
object_callback(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *ret_error)
{
	int r;
...
server_init(sd_bus **_bus)
{
	...
	log_info("Peer ID is " SD_ID128_FORMAT_STR ".",
	...
	r = sd_bus_add_match(bus, NULL,
		"type='signal',interface='foo.bar',member='Notify'",
		match_callback, NULL);
	if (r < 0) {
...
server(sd_bus *bus)
{
	...
	while (!client1_gone || !client2_gone) {
		...
		sd_bus_creds_get_selinux_context(sd_bus_message_get_creds(m),
			&label);
		log_info("Got message! member=%s pid=" PID_FMT " label=%s",
			strna(sd_bus_message_get_member(m)), pid, strna(label));
		/* bus_message_dump(m); */
...
client1(void *p)
{
	...
	r = sd_bus_call_method(bus, "org.freedesktop.systemd.test", "/",
		"org.freedesktop.systemd.test", "LowerCase", &error, &reply,
		"s", "HELLO");
	if (r < 0) {
	...
	r = sd_bus_call_method(bus, "org.freedesktop.systemd.test", "/",
		"org.freedesktop.systemd.test", "FileDescriptor", &error, NULL,
		"h", pp[1]);
	if (r < 0) {
		log_error_errno(r, "Failed to issue method call: %m");
		goto finish;
	...
	errno = 0;
	if (read(pp[0], &x, 1) <= 0) {
		log_error("Failed to read from pipe: %s",
			errno ? strerror(errno) : "early read");
		goto finish;
	}
...
finish:
	if (bus) {
		...
		r = sd_bus_message_new_method_call(bus, &q,
			"org.freedesktop.systemd.test", "/",
...
static int
quit_callback(sd_bus *b, sd_bus_message *m, void *userdata,
	sd_bus_error *ret_error)
{
	bool *x = userdata;
...
client2(void *p)
{
	...
	r = sd_bus_message_new_method_call(bus, &m,
		"org.freedesktop.systemd.test", "/foo/bar/waldo/piep",
	...
	r = sd_bus_send(bus, m, NULL);
	if (r < 0) {
		log_error("Failed to issue signal: %s",
			bus_error_message(&error, -r));
		goto finish;
	}
	...
	if (r < 0)
		log_info("Failed to issue method call: %s",
			bus_error_message(&error, -r));
	else
	...
	r = sd_bus_call_async(bus, NULL, m, quit_callback, &quit,
		200 * USEC_PER_MSEC);
	if (r < 0) {
		log_info("Failed to issue method call: %s",
			bus_error_message(&error, -r));
