...
static void
print_subtree(const char *prefix, const char *path, char **l)
{
	...
	for (;;) {
		...
		printf("%s%s%s\n", prefix,
			draw_special_char(
				has_more ? DRAW_TREE_BRANCH : DRAW_TREE_RIGHT),
			*l);
...
static int
tree_one(sd_bus *bus, const char *service, const char *prefix, bool many)
{
...
static int
format_cmdline(sd_bus_message *m, FILE *f, bool needs_space)
{
	int r;
	...
	for (;;) {
		...
		if (bus_type_is_container(type) > 0) {
			...
			r = format_cmdline(m, f,
				needs_space ||
					IN_SET(type, SD_BUS_TYPE_ARRAY,
						SD_BUS_TYPE_VARIANT));
			if (r < 0)
				return r;
...
member_hash_func(const void *p, const uint8_t hash_key[])
{
	const Member *m = p;
	unsigned long ul;
...
static int
member_compare_func(const void *a, const void *b)
{
...
static int
member_compare_funcp(const void *a, const void *b)
{
	const Member *const *x = (const Member *const *)a,
			    *const *y = (const Member *const *)b;

	return member_compare_func(*x, *y);
}

static void
member_free(Member *m)
{
	if (!m)
		return;
...
static int
on_interface(const char *interface, uint64_t flags, void *userdata)
{
...
static int
on_method(const char *interface, const char *name, const char *signature,
	const char *result, uint64_t flags, void *userdata)
{
	_cleanup_(member_freep) Member *m;
	Set *members = userdata;
	int r;
...
static int
on_signal(const char *interface, const char *name, const char *signature,
...
static int
on_property(const char *interface, const char *name, const char *signature,
	bool writable, uint64_t flags, void *userdata)
{
	_cleanup_(member_freep) Member *m;
	Set *members = userdata;
	int r;
...
static const char *
strdash(const char *x)
{
	return isempty(x) ? "-" : x;
}

static int
introspect(sd_bus *bus, char **argv)
{
	...
	r = sd_bus_call_method(bus, argv[1], argv[2],
		...
		&reply, "");
	if (r < 0) {
		log_error("Failed to introspect object %s of service %s: %s",
	...
	SET_FOREACH (m, members, i) {
		...
		if (m->signature)
			signature_width =
				MAX(signature_width, strlen(m->signature));
		if (m->result)
	...
	if (arg_legend) {
		printf("%-*s %-*s %-*s %-*s %s\n", (int)name_width, "NAME",
			(int)type_width, "TYPE", (int)signature_width,
			"SIGNATURE", (int)result_width, "RESULT/VALUE",
			"FLAGS");
	}
	...
	for (j = 0; j < k; j++) {
		...
		printf("%s%s%-*s%s %-*s %-*s %-*s%s%s%s%s%s%s\n",
			...
			strdash(streq_ptr(m->type, "interface") ? m->interface :
									m->name),
			is_interface ? ansi_highlight_off() : "",
			(int)type_width, strdash(m->type), (int)signature_width,
			strdash(m->signature), (int)result_width, rv,
...
static int
message_pcap(sd_bus_message *m, FILE *f)
{
	return bus_message_pcap_frame(m, arg_snaplen, f);
}

static int
monitor(sd_bus *bus, char *argv[], int (*dump)(sd_bus_message *m, FILE *f))
{
...
status(sd_bus *bus, char *argv[])
{
	...
	if (argv[1]) {
		...
		if (r < 0)
			r = sd_bus_get_name_creds(bus, argv[1],
				(arg_augment_creds ? SD_BUS_CREDS_AUGMENT : 0) |
					_SD_BUS_CREDS_ALL,
				&creds);
		else
			r = sd_bus_creds_new_from_pid(&creds, pid,
				_SD_BUS_CREDS_ALL);
	} else {
...
static int
message_append_cmdline(sd_bus_message *m, const char *signature, char ***x)
{
	char **p;
	int r;
	...
	for (;;) {
		...
		case SD_BUS_TYPE_BYTE: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as byte (unsigned 8bit integer): %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_INT16: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as signed 16bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_UINT16: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as unsigned 16bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_INT32: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as signed 32bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_UINT32: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as unsigned 32bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_INT64: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as signed 64bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_UINT64: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as unsigned 64bit integer: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_DOUBLE: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse as double precision floating point: %s",
					v);
				return r;
			}
		...
		case SD_BUS_TYPE_ARRAY: {
			...
			if (r < 0) {
				log_error(
					"Failed to parse number of array entries: %s",
					v);
				return r;
			}
...
call(sd_bus *bus, char *argv[])
{
	...
	if (r == 0 && !arg_quiet) {
		if (arg_verbose) {
			...
			fputs(sd_bus_message_get_signature(reply, true),
				stdout);
			fputc(' ', stdout);
...
static int
get_property(sd_bus *bus, char *argv[])
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	unsigned n;
	char **i;
	...
	STRV_FOREACH (i, argv + 4) {
		...
		if (arg_verbose) {
			...
			r = bus_message_dump(reply, stdout,
...
static int
set_property(sd_bus *bus, char *argv[])
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	unsigned n;
...
parse_argv(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "hH:M:q", options, NULL)) >= 0)
		...
		case ARG_EXPECT_REPLY:
			...
			if (r < 0) {
				log_error(
					"Failed to parse --expect-reply= parameter.");
				return r;
			}
		...
		case ARG_AUTO_START:
			...
			if (r < 0) {
				log_error(
					"Failed to parse --auto-start= parameter.");
				return r;
			}
		...
		case ARG_ALLOW_INTERACTIVE_AUTHORIZATION:
			...
			if (r < 0) {
				log_error(
					"Failed to parse --allow-interactive-authorization= parameter.");
				return r;
			}
		...
		case ARG_TIMEOUT:
			...
			if (r < 0) {
				log_error(
					"Failed to parse --timeout= parameter.");
				return r;
			}
