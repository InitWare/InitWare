...
static int
signature_element_length_internal(const char *s, bool allow_dict_entry,
	unsigned array_depth, unsigned struct_depth, size_t *l)
{
	int r;
	...
	if (*s == SD_BUS_TYPE_ARRAY) {
		...
		r = signature_element_length_internal(s + 1, true,
			array_depth + 1, struct_depth, &t);
		if (r < 0)
			return r;
	...
	if (*s == SD_BUS_TYPE_STRUCT_BEGIN) {
		...
		while (*p != SD_BUS_TYPE_STRUCT_END) {
			...
			r = signature_element_length_internal(p, false,
				array_depth, struct_depth + 1, &t);
			if (r < 0)
				return r;
...
int
signature_element_length(const char *s, size_t *l)
{
	return signature_element_length_internal(s, true, 0, 0, l);
}

bool
signature_is_single(const char *s, bool allow_dict_entry)
{
	int r;
	size_t t;
...
bool
signature_is_pair(const char *s)
{
	if (!s)
		return false;
...
bool
signature_is_valid(const char *s, bool allow_dict_entry)
{
	const char *p;
	int r;
