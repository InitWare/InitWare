...
static int
filter(sd_bus *b, sd_bus_message *m, void *userdata, sd_bus_error *ret_error)
{
...
static bool
mask_contains(unsigned a[], unsigned n)
{
	unsigned i, j;
...
static int
match_add(sd_bus_slot *slots, struct bus_match_node *root, const char *match,
	int value)
{
...
main(int argc, char *argv[])
{
	...
	assert_se(match_add(slots, &root, "member='waldo',path='/foo/bar'",
			  8) >= 0);
	...
	assert_se(match_add(slots, &root, "member='waldo',arg2path='/prefix/'",
			  13) >= 0);
	...
	assert_se(sd_bus_message_new_signal(bus, &m, "/foo/bar", "bar.x",
			  "waldo") >= 0);
	assert_se(sd_bus_message_append(m, "ssssas", "one", "two",
			  "/prefix/three", "prefix.four", 3, "pi", "pa",
			  "po") >= 0);
	assert_se(bus_message_seal(m, 1, 0) >= 0);

	zero(mask);
	assert_se(bus_match_run(NULL, &root, m) == 0);
	assert_se(mask_contains(
		(unsigned[]){ 9, 8, 7, 5, 10, 12, 13, 14, 15, 16, 17 }, 11));
	...
	zero(mask);
	assert_se(bus_match_run(NULL, &root, m) == 0);
	assert_se(mask_contains((unsigned[]){ 9, 5, 10, 12, 14, 7, 15, 16, 17 },
		9));
	...
	for (i = 0; i < _BUS_MATCH_NODE_TYPE_MAX; i++) {
		...
		assert_se(
			x = bus_match_node_type_to_string(i, buf, sizeof(buf)));

		if (i >= BUS_MATCH_MESSAGE_TYPE)
			assert_se(bus_match_node_type_from_string(x,
					  strlen(x)) == i);
	}
