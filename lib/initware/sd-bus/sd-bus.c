...
_public_ int
sd_bus_set_monitor(sd_bus *bus, int b)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
_public_ int
sd_bus_negotiate_creds(sd_bus *bus, int b, uint64_t mask)
{
	uint64_t new_flags;
	...
	new_flags = (bus->attach_flags & KDBUS_ATTACH_TIMESTAMP) |
		attach_flags_to_kdbus(bus->creds_mask);
	if (bus->attach_flags == new_flags)
		return 0;
...
_public_ int
sd_bus_set_description(sd_bus *bus, const char *description)
{
	assert_return(bus, -EINVAL);
	assert_return(bus->state == BUS_UNSET, -EPERM);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
static int
parse_kernel_address(sd_bus *b, const char **p, char **guid)
{
...
static int
parse_container_kernel_address(sd_bus *b, const char **p, char **guid)
{
	_cleanup_free_ char *machine = NULL, *pid = NULL;
	int r;
...
sd_bus_start(sd_bus *bus)
{
	...
	if (bus->input_fd >= 0)
		...
		bus->exec_path || bus->kernel || bus->machine)
...
sd_bus_open(sd_bus **ret)
{
	...
	b->creds_mask |= SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
...
_public_ int
sd_bus_open_system_machine(sd_bus **ret, const char *machine)
{
	sd_bus *bus;
	int r;
...
_public_ int
sd_bus_get_bus_id(sd_bus *bus, sd_id128_t *id)
{
	int r;
	...
	size_t *idx)
{
	...
	if (bus->is_kernel || *idx >= BUS_MESSAGE_SIZE(m))
		log_debug(
			"Sent message type=%s sender=%s destination=%s object=%s interface=%s member=%s cookie=%" PRIu64
...
dispatch_wqueue(sd_bus *bus)
{
	...
	while (bus->wqueue_size > 0) {
		...
		if (r < 0)
			...
			return ret;
		else if (bus->is_kernel ||
			bus->windex >= BUS_MESSAGE_SIZE(bus->wqueue[0])) {
...
_public_ int
sd_bus_call_async(sd_bus *bus, sd_bus_slot **slot, sd_bus_message *_m,
...
process_reply(sd_bus *bus, sd_bus_message *m)
{
	...
	if (m->n_fds > 0 && !(bus->hello_flags & KDBUS_HELLO_ACCEPT_FD)) {
		...
		r = bus_message_new_synthetic_error(bus, m->reply_cookie,
...
static int
process_fd_check(sd_bus *bus, sd_bus_message *m)
{
	assert(bus);
	assert(m);
	...
	sd_bus_message **ret)
{
	...
	if (m->header->type == SD_BUS_MESSAGE_METHOD_CALL) {
		log_debug(
			"Unprocessed message call sender=%s object=%s interface=%s member=%s",
			strna(sd_bus_message_get_sender(m)),
			strna(sd_bus_message_get_path(m)),
			strna(sd_bus_message_get_interface(m)),
			strna(sd_bus_message_get_member(m)));
...
_public_ int
sd_bus_add_filter(sd_bus *bus, sd_bus_slot **slot,
...
_public_ int
sd_bus_get_description(sd_bus *bus, const char **description)
{
	assert_return(bus, -EINVAL);
...
_public_ int
sd_bus_get_scope(sd_bus *bus, const char **scope)
{
	int r;
...
_public_ int
sd_bus_get_address(sd_bus *bus, const char **address)
{
	assert_return(bus, -EINVAL);
	assert_return(address, -EINVAL);
	assert_return(!bus_pid_changed(bus), -ECHILD);
...
int
sd_bus_get_creds_mask(sd_bus *bus, uint64_t *mask)
{
	assert_return(bus, -EINVAL);
