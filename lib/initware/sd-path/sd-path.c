...
static int
from_home_dir(const char *envname, const char *suffix, char **buffer,
	const char **ret)
{
...
static int
from_user_dir(const char *field, char **buffer, const char **ret)
{
...
static int
get_path(uint64_t type, char **buffer, const char **ret)
{
	int r;
	...
	case SD_PATH_USER_SHARED:
		return from_home_dir("XDG_DATA_HOME", ".local/share", buffer,
			ret);
...
_public_ int
sd_path_home(uint64_t type, const char *suffix, char **path)
{
	char *buffer = NULL, *cc;
	const char *ret;
	int r;
	...
	if (IN_SET(type, SD_PATH_SEARCH_BINARIES,
...
static int
search_from_environment(char ***list, const char *env_home,
	const char *home_suffix, const char *env_search,
	bool env_search_sufficient, const char *first, ...)
{
	const char *e;
	char *h = NULL;
	char **l = NULL;
...
static int
get_search(uint64_t type, char ***list)
{
	assert(list);
	...
	case SD_PATH_SEARCH_BINARIES:
		return search_from_environment(list, NULL, ".local/bin", "PATH",
			true, "/usr/local/sbin", "/usr/local/bin", "/usr/sbin",
			"/usr/bin",
	...
	case SD_PATH_SEARCH_LIBRARY_ARCH:
		return search_from_environment(list, NULL,
			".local/lib/" LIB_ARCH_TUPLE, "LD_LIBRARY_PATH", true,
			LIBDIR,
#ifdef HAVE_SPLIT_USR
			ROOTLIBDIR,
#endif
	...
	case SD_PATH_SEARCH_SHARED:
		return search_from_environment(list, "XDG_DATA_HOME",
			".local/share", "XDG_DATA_DIRS", false,
	...
	case SD_PATH_SEARCH_CONFIGURATION:
		return search_from_environment(list, "XDG_CONFIG_HOME",
			".config", "XDG_CONFIG_DIRS", false, "/etc", NULL);
	}
...
_public_ int
sd_path_search(uint64_t type, const char *suffix, char ***paths)
{
	char **l, **i, **j, **n;
	int r;
