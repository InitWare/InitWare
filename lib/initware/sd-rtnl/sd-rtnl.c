...
int
sd_rtnl_new_from_netlink(sd_rtnl **ret, int fd)
{
	_cleanup_rtnl_unref_ sd_rtnl *rtnl = NULL;
	socklen_t addrlen;
	int r;
...
static int
rtnl_compute_groups_ap(uint32_t *_groups, unsigned n_groups, va_list ap)
{
	uint32_t groups = 0;
	unsigned i;
...
static int
rtnl_open_fd_ap(sd_rtnl **ret, int fd, unsigned n_groups, va_list ap)
{
	_cleanup_rtnl_unref_ sd_rtnl *rtnl = NULL;
	socklen_t addrlen;
	int r, one = 1;
...
int
sd_rtnl_open_fd(sd_rtnl **ret, int fd, unsigned n_groups, ...)
{
	va_list ap;
	int r;
...
int
sd_rtnl_open(sd_rtnl **ret, unsigned n_groups, ...)
{
	va_list ap;
	int fd, r;
...
static void
rtnl_seal_message(sd_rtnl *rtnl, sd_rtnl_message *m)
{
	assert(rtnl);
...
int
sd_rtnl_send(sd_rtnl *nl, sd_rtnl_message *message, uint32_t *serial)
{
	int r;
	...
	if (nl->wqueue_size <= 0) {
		...
		if (nl->wqueue_size >= RTNL_WQUEUE_MAX) {
			log_debug("rtnl: exhausted the write queue size (%d)",
				RTNL_WQUEUE_MAX);
			return -ENOBUFS;
		}
...
int
rtnl_rqueue_make_room(sd_rtnl *rtnl)
{
	assert(rtnl);

	if (rtnl->rqueue_size >= RTNL_RQUEUE_MAX) {
		log_debug("rtnl: exhausted the read queue size (%d)",
			RTNL_RQUEUE_MAX);
		return -ENOBUFS;
	}
...
int
rtnl_rqueue_partial_make_room(sd_rtnl *rtnl)
{
	assert(rtnl);

	if (rtnl->rqueue_partial_size >= RTNL_RQUEUE_MAX) {
		log_debug("rtnl: exhausted the partial read queue size (%d)",
			RTNL_RQUEUE_MAX);
		return -ENOBUFS;
	}
...
static int
dispatch_rqueue(sd_rtnl *rtnl, sd_rtnl_message **message)
{
	int r;
	...
	*message = rtnl->rqueue[0];
	...
	memmove(rtnl->rqueue, rtnl->rqueue + 1,
		sizeof(sd_rtnl_message *) * rtnl->rqueue_size);

	return 1;
}

static int
dispatch_wqueue(sd_rtnl *rtnl)
{
	int r, ret = 0;
	...
	while (rtnl->wqueue_size > 0) {
		...
		if (r < 0)
			...
			rtnl->wqueue_size--;
			memmove(rtnl->wqueue, rtnl->wqueue + 1,
				sizeof(sd_rtnl_message *) * rtnl->wqueue_size);
...
static int
process_reply(sd_rtnl *rtnl, sd_rtnl_message *m)
{
	struct reply_callback *c;
	uint64_t serial;
	int r;
...
static int
process_match(sd_rtnl *rtnl, sd_rtnl_message *m)
{
	...
	IWLIST_FOREACH (match_callbacks, c, rtnl->match_callbacks) {
		if (type == c->type) {
			...
			if (r != 0) {
				if (r < 0)
					log_debug_errno(r,
						"sd-rtnl: match callback failed: %m");
...
static int
process_running(sd_rtnl *rtnl, sd_rtnl_message **ret)
{
	_cleanup_rtnl_message_unref_ sd_rtnl_message *m = NULL;
	int r;
...
int
sd_rtnl_process(sd_rtnl *rtnl, sd_rtnl_message **ret)
{
...
static int
rtnl_poll(sd_rtnl *rtnl, bool need_more, uint64_t timeout_usec)
{
	struct pollfd p[1] = {};
	struct timespec ts;
	...
	r = ppoll(p, 1, m == (uint64_t)-1 ? NULL : timespec_store(&ts, m),
		NULL);
	if (r < 0)
		return -errno;
...
int
sd_rtnl_wait(sd_rtnl *nl, uint64_t timeout_usec)
{
	assert_return(nl, -EINVAL);
	assert_return(!rtnl_pid_changed(nl), -ECHILD);
...
int
sd_rtnl_call_async(sd_rtnl *nl, sd_rtnl_message *m,
	sd_rtnl_message_handler_t callback, void *userdata, uint64_t usec,
	uint32_t *serial)
{
	struct reply_callback *c;
	uint32_t s;
	...
	if (usec != (uint64_t)-1) {
		r = prioq_ensure_allocated(&nl->reply_callbacks_prioq,
			timeout_compare);
		if (r < 0)
			return r;
	}
...
int
sd_rtnl_call_async_cancel(sd_rtnl *nl, uint32_t serial)
{
	struct reply_callback *c;
	uint64_t s = serial;
...
int
sd_rtnl_call(sd_rtnl *rtnl, sd_rtnl_message *message, uint64_t usec,
	sd_rtnl_message **ret)
{
	usec_t timeout;
	uint32_t serial;
...
int
sd_rtnl_get_events(sd_rtnl *rtnl)
{
	int flags = 0;
...
int
sd_rtnl_get_timeout(sd_rtnl *rtnl, uint64_t *timeout_usec)
{
	struct reply_callback *c;
...
static int
exit_callback(sd_event_source *event, void *userdata)
{
	sd_rtnl *rtnl = userdata;
...
int
sd_rtnl_attach_event(sd_rtnl *rtnl, sd_event *event, int priority)
{
	int r;
	...
	r = sd_event_source_set_prepare(rtnl->io_event_source,
		prepare_callback);
	if (r < 0)
		goto fail;
	...
	r = sd_event_source_set_description(rtnl->time_event_source,
		"rtnl-timer");
	if (r < 0)
		goto fail;

	r = sd_event_add_exit(rtnl->event, &rtnl->exit_event_source,
		exit_callback, rtnl);
	if (r < 0)
		goto fail;

	r = sd_event_source_set_description(rtnl->exit_event_source,
		"rtnl-exit");
	if (r < 0)
		goto fail;
...
int
sd_rtnl_add_match(sd_rtnl *rtnl, uint16_t type,
	sd_rtnl_message_handler_t callback, void *userdata)
{
	struct match_callback *c;
	...
	assert_return(rtnl_message_type_is_link(type) ||
			...
			rtnl_message_type_is_route(type),
		-ENOTSUP);
...
int
sd_rtnl_remove_match(sd_rtnl *rtnl, uint16_t type,
	sd_rtnl_message_handler_t callback, void *userdata)
{
	struct match_callback *c;
