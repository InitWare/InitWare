...
static void
test_message_link_bridge(sd_rtnl *rtnl)
{
	_cleanup_rtnl_message_unref_ sd_rtnl_message *message = NULL;
	uint32_t cost;

	assert_se(
		sd_rtnl_message_new_link(rtnl, &message, RTM_NEWLINK, 1) >= 0);
	...
	assert_se(
		sd_rtnl_message_append_u32(message, IFLA_BRPORT_COST, 10) >= 0);
	...
	assert_se(sd_rtnl_message_read_u32(message, IFLA_BRPORT_COST, &cost) >=
		0);
	assert_se(cost == 10);
...
static void
test_link_configure(sd_rtnl *rtnl, int ifindex)
{
	_cleanup_rtnl_message_unref_ sd_rtnl_message *message = NULL;
	...
	assert_se(sd_rtnl_message_new_link(rtnl, &message, RTM_GETLINK,
			  ifindex) >= 0);
	assert_se(
		sd_rtnl_message_append_string(message, IFLA_IFNAME, name) >= 0);
	assert_se(sd_rtnl_message_append_ether_addr(message, IFLA_ADDRESS,
			  ether_aton(mac)) >= 0);
	...
	assert_se(sd_rtnl_message_read_string(message, IFLA_IFNAME,
			  &name_out) >= 0);
	assert_se(streq(name, name_out));

	assert_se(sd_rtnl_message_read_ether_addr(message, IFLA_ADDRESS,
			  &mac_out) >= 0);
...
static void
test_link_get(sd_rtnl *rtnl, int ifindex)
{
	sd_rtnl_message *m;
	sd_rtnl_message *r;
	...
	assert_se(
		sd_rtnl_message_new_link(rtnl, &m, RTM_GETLINK, ifindex) >= 0);
	assert_se(m);
	...
	assert_se(sd_rtnl_message_read_u32(r, IFLA_NUM_TX_QUEUES, &u32_data) ==
		0);
	assert_se(sd_rtnl_message_read_u32(r, IFLA_NUM_RX_QUEUES, &u32_data) ==
		0);

	assert_se(sd_rtnl_message_read_ether_addr(r, IFLA_ADDRESS, &eth_data) ==
		0);
...
static void
test_address_get(sd_rtnl *rtnl, int ifindex)
{
	sd_rtnl_message *m;
	sd_rtnl_message *r;
	...
	assert_se(sd_rtnl_message_new_addr(rtnl, &m, RTM_GETADDR, ifindex,
			  AF_INET) >= 0);
	assert_se(m);
	...
	assert_se(
		sd_rtnl_message_read_cache_info(r, IFA_CACHEINFO, &cache) == 0);
...
test_route(void)
{
	...
	assert_se(sd_rtnl_message_read_in_addr(req, RTA_GATEWAY, &addr_data) >=
		0);
...
static int
link_handler(sd_rtnl *rtnl, sd_rtnl_message *m, void *userdata)
{
...
test_event_loop(int ifindex)
{
	...
	assert_se(sd_rtnl_call_async(rtnl, m, &link_handler, ifname, 0, NULL) >=
		0);
...
static int
pipe_handler(sd_rtnl *rtnl, sd_rtnl_message *m, void *userdata)
{
...
test_async(int ifindex)
{
	...
	assert_se(sd_rtnl_call_async(rtnl, m, &link_handler, ifname, 0,
			  &serial) >= 0);
...
test_pipe(int ifindex)
{
	...
	assert_se(
		sd_rtnl_message_new_link(rtnl, &m1, RTM_GETLINK, ifindex) >= 0);
	assert_se(
		sd_rtnl_message_new_link(rtnl, &m2, RTM_GETLINK, ifindex) >= 0);

	counter++;
	assert_se(sd_rtnl_call_async(rtnl, m1, &pipe_handler, &counter, 0,
			  NULL) >= 0);

	counter++;
	assert_se(sd_rtnl_call_async(rtnl, m2, &pipe_handler, &counter, 0,
			  NULL) >= 0);
...
test_container(void)
{
	...
	assert_se(
		sd_rtnl_message_append_string(m, IFLA_INFO_KIND, "vlan") >= 0);
	assert_se(sd_rtnl_message_close_container(m) >= 0);
	...
	assert_se(sd_rtnl_message_read_string(m, IFLA_INFO_KIND,
			  &string_data) >= 0);
...
test_match(void)
{
	...
	assert_se(
		sd_rtnl_add_match(rtnl, RTM_NEWLINK, &link_handler, NULL) >= 0);
	assert_se(
		sd_rtnl_add_match(rtnl, RTM_NEWLINK, &link_handler, NULL) >= 0);

	assert_se(sd_rtnl_remove_match(rtnl, RTM_NEWLINK, &link_handler,
			  NULL) == 1);
	assert_se(sd_rtnl_remove_match(rtnl, RTM_NEWLINK, &link_handler,
			  NULL) == 1);
	assert_se(sd_rtnl_remove_match(rtnl, RTM_NEWLINK, &link_handler,
...
static void
test_get_addresses(sd_rtnl *rtnl)
{
	_cleanup_rtnl_message_unref_ sd_rtnl_message *req = NULL, *reply = NULL;
	sd_rtnl_message *m;

	assert_se(sd_rtnl_message_new_addr(rtnl, &req, RTM_GETADDR, 0,
			  AF_UNSPEC) >= 0);
	...
	for (m = reply; m; m = sd_rtnl_message_next(m)) {
		...
		log_info("got IPv%u address on ifindex %i",
			family == AF_INET ? 4 : 6, ifindex);
	}
}
...
main(void)
{
	...
	assert_se(sd_rtnl_message_new_link(rtnl, &m, RTM_GETLINK,
			  if_loopback) >= 0);
	assert_se(m);
	...
	assert_se(sd_rtnl_message_read_string(m, IFLA_IFNAME, &string_data) ==
		-EPERM);
