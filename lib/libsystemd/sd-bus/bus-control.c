...
static int
bus_request_name_kernel(sd_bus *bus, const char *name, uint64_t flags)
{
	struct kdbus_cmd *n;
	size_t size, l;
	int r;
...
static int
bus_release_name_kernel(sd_bus *bus, const char *name)
{
	struct kdbus_cmd *n;
	size_t size, l;
	int r;
...
static int
kernel_get_list(sd_bus *bus, uint64_t flags, char ***x)
{
	...
	name_list = (struct kdbus_info *)((uint8_t *)bus->kdbus_buffer +
		cmd.offset);
...
static int
bus_list_names_kernel(sd_bus *bus, char ***acquired, char ***activatable)
{
	_cleanup_strv_free_ char **x = NULL, **y = NULL;
	int r;

	if (acquired) {
		r = kernel_get_list(bus, KDBUS_LIST_UNIQUE | KDBUS_LIST_NAMES,
			&x);
		if (r < 0)
			return r;
	}
...
static int
bus_populate_creds_from_items(sd_bus *bus, struct kdbus_info *info,
	uint64_t mask, sd_bus_creds *c)
{
	struct kdbus_item *item;
	uint64_t m;
	int r;
	...
	KDBUS_ITEM_FOREACH(item, info, items)
	{
		...
		case KDBUS_ITEM_CREDS:
			...
			if (mask & SD_BUS_CREDS_UID &&
				(uid_t)item->creds.uid != UID_INVALID) {
				c->uid = (uid_t)item->creds.uid;
				c->mask |= SD_BUS_CREDS_UID;
			}

			if (mask & SD_BUS_CREDS_EUID &&
				(uid_t)item->creds.euid != UID_INVALID) {
				c->euid = (uid_t)item->creds.euid;
				c->mask |= SD_BUS_CREDS_EUID;
			}

			if (mask & SD_BUS_CREDS_SUID &&
				(uid_t)item->creds.suid != UID_INVALID) {
				c->suid = (uid_t)item->creds.suid;
				c->mask |= SD_BUS_CREDS_SUID;
			}

			if (mask & SD_BUS_CREDS_FSUID &&
				(uid_t)item->creds.fsuid != UID_INVALID) {
				c->fsuid = (uid_t)item->creds.fsuid;
				c->mask |= SD_BUS_CREDS_FSUID;
			}

			if (mask & SD_BUS_CREDS_GID &&
				(gid_t)item->creds.gid != GID_INVALID) {
				c->gid = (gid_t)item->creds.gid;
				c->mask |= SD_BUS_CREDS_GID;
			}

			if (mask & SD_BUS_CREDS_EGID &&
				(gid_t)item->creds.egid != GID_INVALID) {
				c->egid = (gid_t)item->creds.egid;
				c->mask |= SD_BUS_CREDS_EGID;
			}

			if (mask & SD_BUS_CREDS_SGID &&
				(gid_t)item->creds.sgid != GID_INVALID) {
				c->sgid = (gid_t)item->creds.sgid;
				c->mask |= SD_BUS_CREDS_SGID;
			}

			if (mask & SD_BUS_CREDS_FSGID &&
				(gid_t)item->creds.fsgid != GID_INVALID) {
		...
		case KDBUS_ITEM_CMDLINE:
			if (mask & SD_BUS_CREDS_CMDLINE) {
				c->cmdline_size = item->size -
					offsetof(struct kdbus_item, data);
				c->cmdline =
					memdup(item->data, c->cmdline_size);
				if (!c->cmdline)
					return -ENOMEM;
		...
		case KDBUS_ITEM_CGROUP:
			...
			if (m) {
				...
				r = free_and_strdup(&c->cgroup_root,
					bus->cgroup_root);
				if (r < 0)
					return r;
				...
				if (item->caps.last_cap != cap_last_cap() ||
					item->size -
							offsetof(
								struct kdbus_item,
								caps.caps) <
						DIV_ROUND_UP(item->caps.last_cap,
							32U) *
							4 * 4)
					return -EBADMSG;

				c->capability = memdup(item->caps.caps,
					item->size -
		...
		case KDBUS_ITEM_AUDIT:
			...
			if (mask & SD_BUS_CREDS_AUDIT_LOGIN_UID &&
				(uid_t)item->audit.loginuid != UID_INVALID) {
				c->audit_login_uid =
					(uid_t)item->audit.loginuid;
		...
		case KDBUS_ITEM_OWNED_NAME:
			if ((mask & SD_BUS_CREDS_WELL_KNOWN_NAMES) &&
				service_name_is_valid(item->name.name)) {
				r = strv_extend(&c->well_known_names,
					item->name.name);
				if (r < 0)
					return r;
...
int
bus_get_name_creds_kdbus(sd_bus *bus, const char *name, uint64_t mask,
	bool allow_activator, sd_bus_creds **creds)
{
	...
	if ((mask & SD_BUS_CREDS_AUGMENT) &&
		(mask &
			(SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
				...
				SD_BUS_CREDS_COMM | SD_BUS_CREDS_TID_COMM |
				SD_BUS_CREDS_EXE | SD_BUS_CREDS_CMDLINE |
	...
	conn_info = (struct kdbus_info *)((uint8_t *)bus->kdbus_buffer +
		cmd->offset);
	...
	if (mask & SD_BUS_CREDS_UNIQUE_NAME) {
		if (asprintf(&c->unique_name, ":1.%llu",
			    (unsigned long long)conn_info->id) < 0) {
			r = -ENOMEM;
			goto fail;
		}
...
static int
bus_get_owner_creds_kdbus(sd_bus *bus, uint64_t mask, sd_bus_creds **ret)
{
	_cleanup_bus_creds_unref_ sd_bus_creds *c = NULL;
	struct kdbus_cmd_info cmd = {
	...
	creator_info = (struct kdbus_info *)((uint8_t *)bus->kdbus_buffer +
		cmd.offset);
...
static int
add_name_change_match(sd_bus *bus, uint64_t cookie, const char *name,
	const char *old_owner, const char *new_owner)
{
	uint64_t name_id = KDBUS_MATCH_ID_ANY, old_owner_id = 0,
		 new_owner_id = 0;
	int is_name_id = -1, r;
	struct kdbus_item *item;
	...
	if (is_name_id <= 0) {
		...
		sz = ALIGN8(offsetof(struct kdbus_cmd_match, items) +
			...
			offsetof(struct kdbus_notify_name_change, name) + l);
		...
		item = m->items;
		item->size = offsetof(struct kdbus_item, name_change) +
			offsetof(struct kdbus_notify_name_change, name) + l;
...
int
bus_add_match_internal_kernel(sd_bus *bus,
	...
	uint64_t cookie)
{
	...
	for (i = 0; i < n_components; i++) {
		...
		case BUS_MATCH_SENDER:
			...
			if (r < 0)
				...
				sz += ALIGN8(offsetof(struct kdbus_item, id) +
					sizeof(uint64_t));
			else {
				sender = c->value_str;
		...
		case BUS_MATCH_MESSAGE_TYPE:
			...
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, "message-type",
				bus_message_type_to_string(c->value_u8));
			using_bloom = true;
			break;
		...
		case BUS_MATCH_INTERFACE:
			...
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, "interface", c->value_str);
			using_bloom = true;
			break;
		...
		case BUS_MATCH_MEMBER:
			...
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, "member", c->value_str);
			using_bloom = true;
			break;
		...
		case BUS_MATCH_PATH:
			...
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, "path", c->value_str);
			using_bloom = true;
			break;
		...
		case BUS_MATCH_PATH_NAMESPACE:
			if (!streq(c->value_str, "/")) {
				bloom_add_pair(bloom, bus->bloom_size,
					bus->bloom_n_hash, "path-slash-prefix",
					c->value_str);
				using_bloom = true;
			}
			break;
		...
		case BUS_MATCH_ARG ... BUS_MATCH_ARG_LAST: {
			...
			if (c->type - BUS_MATCH_ARG < 3)
				name_change_arg[c->type - BUS_MATCH_ARG] =
					c->value_str;

			xsprintf(buf, "arg%i", c->type - BUS_MATCH_ARG);
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, buf, c->value_str);
			using_bloom = true;
			break;
		}

		case BUS_MATCH_ARG_PATH ... BUS_MATCH_ARG_PATH_LAST: {
			char buf[sizeof("arg") - 1 + 2 +
				sizeof("-slash-prefix")];

			xsprintf(buf, "arg%i-slash-prefix",
				c->type - BUS_MATCH_ARG_PATH);
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, buf, c->value_str);
			using_bloom = true;
			break;
		...
		case BUS_MATCH_ARG_NAMESPACE ... BUS_MATCH_ARG_NAMESPACE_LAST: {
			...
			xsprintf(buf, "arg%i-dot-prefix",
				c->type - BUS_MATCH_ARG_NAMESPACE);
			bloom_add_pair(bloom, bus->bloom_size,
				bus->bloom_n_hash, buf, c->value_str);
			using_bloom = true;
			break;
	...
	if (using_bloom)
		sz += ALIGN8(
			offsetof(struct kdbus_item, data64) + bus->bloom_size);
	...
	if (sender) {
		item->size =
			offsetof(struct kdbus_item, str) + sender_length + 1;
	...
	if (matches_name_change) {
		...
		r = add_name_change_match(bus, cookie, name_change_arg[0],
			name_change_arg[1], name_change_arg[2]);
		if (r < 0)
			return r;
	}
	...
	uint64_t cookie)
{
	...
	if (bus->is_kernel)
		return bus_add_match_internal_kernel(bus, components,
			n_components, cookie);
	else
...
int
bus_remove_match_internal_kernel(sd_bus *bus, uint64_t cookie)
{
	struct kdbus_cmd_match m = {
