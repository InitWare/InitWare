...
static void *
adjust_pointer(const void *p, void *old_base, size_t sz, void *new_base)
{
	if (p == NULL)
		return NULL;
...
static void
message_free_part(sd_bus_message *m, struct bus_body_part *part)
{
	assert(m);
	assert(part);
	...
	if (part->memfd >= 0) {
		...
		if (!part->sealed) {
			...
			bus_kernel_push_memfd(m->bus, part->memfd, part->data,
				part->mapped, part->allocated);
		} else {
			if (part->mapped > 0)
				assert_se(munmap(part->mmap_begin,
						  part->mapped) == 0);
...
static void
message_reset_parts(sd_bus_message *m)
{
...
static struct bus_container *
message_get_container(sd_bus_message *m)
{
	assert(m);
...
static void
message_free_last_container(sd_bus_message *m)
{
...
static void
message_reset_containers(sd_bus_message *m)
{
	assert(m);
...
message_free(sd_bus_message *m)
{
	...
	if (m->release_kdbus)
		bus_kernel_cmd_free(m->bus,
			(uint8_t *)m->kdbus - (uint8_t *)m->bus->kdbus_buffer);
...
static void *
message_extend_fields(sd_bus_message *m, size_t align, size_t sz,
	bool add_offset)
{
	void *op, *np;
	...
	m->destination =
		adjust_pointer(m->destination, op, old_size, m->header);
...
static int
message_append_field_string(sd_bus_message *m, uint64_t h, char type,
...
static int
message_append_field_signature(sd_bus_message *m, uint64_t h, const char *s,
	const char **ret)
{
	size_t l;
	uint8_t *p;
...
static int
message_append_field_uint32(sd_bus_message *m, uint64_t h, uint32_t x)
{
	uint8_t *p;
...
static int
message_append_field_uint64(sd_bus_message *m, uint64_t h, uint64_t x)
{
	uint8_t *p;
...
static int
message_append_reply_cookie(sd_bus_message *m, uint64_t cookie)
{
	assert(m);

	if (BUS_MESSAGE_IS_GVARIANT(m))
		return message_append_field_uint64(m,
			BUS_MESSAGE_HEADER_REPLY_SERIAL, cookie);
	else {
		/* 64bit cookies are not supported on dbus1 */
		if (cookie > 0xffffffffUL)
			return -ENOTSUP;

		return message_append_field_uint32(m,
			BUS_MESSAGE_HEADER_REPLY_SERIAL, (uint32_t)cookie);
	}
}

int
bus_message_from_header(sd_bus *bus, void *header, size_t header_accessible,
	void *footer, size_t footer_accessible, size_t message_size, int *fds,
	unsigned n_fds, const struct ucred *ucred, const char *label,
	size_t extra, sd_bus_message **ret)
{
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		m->body_size = message_size -
			(sizeof(struct bus_header) + ALIGN8(m->fields_size));
	} else {
		if (h->dbus1.serial == 0)
			return -EBADMSG;
		...
		if (sizeof(struct bus_header) + ALIGN8(m->fields_size) +
				m->body_size !=
			message_size)
			return -EBADMSG;
	}
...
int
bus_message_from_malloc(sd_bus *bus, void *buffer, size_t length, int *fds,
	unsigned n_fds, const struct ucred *ucred, const char *label,
	sd_bus_message **ret)
{
	sd_bus_message *m;
	...
	r = bus_message_from_header(bus, buffer,
		length, /* in this case the initial bytes and the final bytes are the same */
		buffer, length, length, fds, n_fds, ucred, label, 0, &m);
	if (r < 0)
		return r;
...
message_new(sd_bus *bus, uint8_t type)
{
	sd_bus_message *m;
	...
	m->n_ref = 1;
	m->header = (struct bus_header *)((uint8_t *)m +
	...
	const char *interface, const char *member)
{
	...
	r = message_append_field_string(t, BUS_MESSAGE_HEADER_INTERFACE,
		SD_BUS_TYPE_STRING, interface, &t->interface);
	if (r < 0)
		goto fail;
...
_public_ int
sd_bus_message_new_method_call(sd_bus *bus, sd_bus_message **m,
	...
	const char *member)
{
	...
	assert_return(bus, -ENOTCONN);
	assert_return(bus->state != BUS_UNSET, -ENOTCONN);
	assert_return(!destination || service_name_is_valid(destination),
		-EINVAL);
	assert_return(object_path_is_valid(path), -EINVAL);
	assert_return(!interface || interface_name_is_valid(interface),
		-EINVAL);
	assert_return(member_name_is_valid(member), -EINVAL);
	assert_return(m, -EINVAL);
	...
	if (destination) {
		r = message_append_field_string(t,
			BUS_MESSAGE_HEADER_DESTINATION, SD_BUS_TYPE_STRING,
...
static int
message_new_reply(sd_bus_message *call, uint8_t type, sd_bus_message **m)
{
	sd_bus_message *t;
	int r;
...
_public_ int
sd_bus_message_new_method_return(sd_bus_message *call, sd_bus_message **m)
{
	return message_new_reply(call, SD_BUS_MESSAGE_METHOD_RETURN, m);
}

_public_ int
sd_bus_message_new_method_error(sd_bus_message *call, sd_bus_message **m,
	const sd_bus_error *e)
{
	...
	r = message_append_field_string(t, BUS_MESSAGE_HEADER_ERROR_NAME,
		SD_BUS_TYPE_STRING, e->name, &t->error.name);
	if (r < 0)
		goto fail;
...
_public_ int
sd_bus_message_new_method_errorf(sd_bus_message *call, sd_bus_message **m,
...
_public_ int
sd_bus_message_new_method_errno(sd_bus_message *call, sd_bus_message **m,
	int error, const sd_bus_error *p)
{
...
_public_ int
sd_bus_message_new_method_errnof(sd_bus_message *call, sd_bus_message **m,
...
void
bus_message_set_sender_local(sd_bus *bus, sd_bus_message *m)
{
	assert(bus);
	assert(m);
...
void
bus_message_set_sender_driver(sd_bus *bus, sd_bus_message *m)
{
	assert(bus);
	assert(m);
...
int
bus_message_new_synthetic_error(sd_bus *bus, uint64_t cookie,
...
sd_bus_message_ref(sd_bus_message *m)
{
...
_public_ sd_bus_message *
sd_bus_message_unref(sd_bus_message *m)
{
	if (!m)
		return NULL;
...
_public_ int
sd_bus_message_get_cookie(sd_bus_message *m, uint64_t *cookie)
{
	uint64_t c;
...
_public_ int
sd_bus_message_get_reply_cookie(sd_bus_message *m, uint64_t *cookie)
{
	assert_return(m, -EINVAL);
	assert_return(cookie, -EINVAL);
...
_public_ int
sd_bus_message_get_expect_reply(sd_bus_message *m)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_get_auto_start(sd_bus_message *m)
{
	assert_return(m, -EINVAL);
...
_public_ const char *
sd_bus_message_get_path(sd_bus_message *m)
{
	assert_return(m, NULL);
...
_public_ const char *
sd_bus_message_get_interface(sd_bus_message *m)
{
	assert_return(m, NULL);
...
_public_ const char *
sd_bus_message_get_member(sd_bus_message *m)
{
	assert_return(m, NULL);
...
_public_ const char *
sd_bus_message_get_destination(sd_bus_message *m)
{
	assert_return(m, NULL);
...
_public_ const char *
sd_bus_message_get_sender(sd_bus_message *m)
{
	assert_return(m, NULL);
...
_public_ int
sd_bus_message_get_monotonic_usec(sd_bus_message *m, uint64_t *usec)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_get_realtime_usec(sd_bus_message *m, uint64_t *usec)
{
	assert_return(m, -EINVAL);
	assert_return(usec, -EINVAL);
...
_public_ int
sd_bus_message_get_seqnum(sd_bus_message *m, uint64_t *seqnum)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_is_signal(sd_bus_message *m, const char *interface,
	const char *member)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_is_method_call(sd_bus_message *m, const char *interface,
	const char *member)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_is_method_error(sd_bus_message *m, const char *name)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_set_expect_reply(sd_bus_message *m, int b)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_set_auto_start(sd_bus_message *m, int b)
{
	assert_return(m, -EINVAL);
	assert_return(!m->sealed, -EPERM);
...
message_append_part(sd_bus_message *m)
{
	struct bus_body_part *part;
...
static void
part_zero(struct bus_body_part *part, size_t sz)
{
	assert(part);
	assert(sz > 0);
	assert(sz < 8);
...
static int
part_make_space(struct sd_bus_message *m, struct bus_body_part *part, size_t sz,
	void **q)
{
	void *n;
	int r;
	...
	if (!part->data && part->memfd < 0) {
		part->memfd = bus_kernel_pop_memfd(m->bus, &part->data,
			&part->mapped, &part->allocated);
	...
	if (part->memfd >= 0) {
		...
		if (!part->data || sz > part->mapped) {
			...
			if (part->mapped <= 0)
				n = mmap(NULL, psz, PROT_READ | PROT_WRITE,
					MAP_SHARED, part->memfd, 0);
			else
				n = mremap(part->mmap_begin, part->mapped, psz,
					MREMAP_MAYMOVE);
...
static int
message_add_offset(sd_bus_message *m, size_t offset)
{
	struct bus_container *c;
...
static void
message_extend_containers(sd_bus_message *m, size_t expand)
{
	struct bus_container *c;
...
static void *
message_extend_body(sd_bus_message *m, size_t align, size_t sz, bool add_offset,
	bool force_inline)
{
	...
	if (added > 0) {
		...
		add_new_part = m->n_body_parts <= 0 || m->body_end->sealed ||
			(padding !=
				ALIGN_TO(m->body_end->size, align) -
					m->body_end->size) ||
			(force_inline &&
				m->body_end->size >
					MEMFD_MIN_SIZE); /* if this must be an inlined extension, let's create a new part if the previous part is large enough to be inlined */
		...
		if (add_new_part) {
			...
			/* Readjust pointers */
			for (c = m->containers;
				c < m->containers + m->n_containers; c++)
				c->array_size = adjust_pointer(c->array_size,
					op, os, part->data);

			m->error.message = (const char *)adjust_pointer(
				m->error.message, op, os, part->data);
		}
	} else
...
static int
message_push_fd(sd_bus_message *m, int fd)
{
	int *f, copy;
...
int
message_append_basic(sd_bus_message *m, char type, const void *p,
	const void **stored)
{
...
_public_ int
sd_bus_message_append_basic(sd_bus_message *m, char type, const void *p)
{
	return message_append_basic(m, type, p, NULL);
}

_public_ int
sd_bus_message_append_string_space(sd_bus_message *m, size_t size, char **s)
{
	struct bus_container *c;
	void *a;
...
_public_ int
sd_bus_message_append_string_iovec(sd_bus_message *m, const struct iovec *iov,
	unsigned n)
{
	size_t size;
	unsigned i;
...
static int
bus_message_open_array(sd_bus_message *m, struct bus_container *c,
	const char *contents, uint32_t **array_size, size_t *begin,
	bool *need_offsets)
{
	unsigned nindex;
	int alignment, r;
	...
	if (c->signature && c->signature[c->index]) {
		...
		e = strextend(&c->signature, CHAR_TO_STR(SD_BUS_TYPE_ARRAY),
			contents, NULL);
		if (!e) {
			m->poisoned = true;
			return -ENOMEM;
...
static int
bus_message_open_variant(sd_bus_message *m, struct bus_container *c,
	const char *contents)
{
	assert(m);
	assert(c);
...
static int
bus_message_open_struct(sd_bus_message *m, struct bus_container *c,
	const char *contents, size_t *begin, bool *need_offsets)
{
	size_t nindex;
	int r;
	...
	if (c->signature && c->signature[c->index]) {
		...
		e = strextend(&c->signature,
			CHAR_TO_STR(SD_BUS_TYPE_STRUCT_BEGIN), contents,
			CHAR_TO_STR(SD_BUS_TYPE_STRUCT_END), NULL);
		if (!e) {
			m->poisoned = true;
			return -ENOMEM;
		}
...
static int
bus_message_open_dict_entry(sd_bus_message *m, struct bus_container *c,
	const char *contents, size_t *begin, bool *need_offsets)
{
	int r;
...
_public_ int
sd_bus_message_open_container(sd_bus_message *m, char type,
	const char *contents)
{
...
static int
bus_message_close_array(sd_bus_message *m, struct bus_container *c)
{
	assert(m);
	assert(c);
	...
	if (c->need_offsets) {
		...
		payload = c->n_offsets > 0 ?
			      c->offsets[c->n_offsets - 1] - c->begin :
			      0;
...
static int
bus_message_close_variant(sd_bus_message *m, struct bus_container *c)
{
	uint8_t *a;
	size_t l;
...
static int
bus_message_close_struct(sd_bus_message *m, struct bus_container *c,
	bool add_offset)
{
	size_t n_variable = 0;
	unsigned i = 0;
...
sd_bus_message_close_container(sd_bus_message *m)
{
	...
	else if (c->enclosing == SD_BUS_TYPE_STRUCT ||
...
static int
type_stack_push(TypeStack *stack, unsigned max, unsigned *i, const char *types,
	unsigned n_struct, unsigned n_array)
{
	assert(stack);
	assert(max > 0);
...
static int
type_stack_pop(TypeStack *stack, unsigned max, unsigned *i, const char **types,
	unsigned *n_struct, unsigned *n_array)
{
	assert(stack);
	assert(max > 0);
...
int
bus_message_append_ap(sd_bus_message *m, const char *types, va_list ap)
{
	...
	for (;;) {
		...
		if (n_array == 0 ||
			(n_array == (unsigned)-1 && n_struct == 0)) {
			r = type_stack_pop(stack, ELEMENTSOF(stack), &stack_ptr,
				&types, &n_struct, &n_array);
			if (r < 0)
				return r;
			if (r == 0)
				break;
		...
		case SD_BUS_TYPE_ARRAY: {
			...
			r = type_stack_push(stack, ELEMENTSOF(stack),
...
_public_ int
sd_bus_message_append(sd_bus_message *m, const char *types, ...)
{
	va_list ap;
	int r;
...
_public_ int
sd_bus_message_append_array_space(sd_bus_message *m, char type, size_t size,
	void **ptr)
{
	ssize_t align, sz;
	void *a;
...
_public_ int
sd_bus_message_append_array(sd_bus_message *m, char type, const void *ptr,
	size_t size)
{
	int r;
	void *p;
...
_public_ int
sd_bus_message_append_array_iovec(sd_bus_message *m, char type,
...
_public_ int
sd_bus_message_append_array_memfd(sd_bus_message *m, char type, int memfd,
...
_public_ int
sd_bus_message_append_string_memfd(sd_bus_message *m, int memfd,
	uint64_t offset, uint64_t size)
{
	_cleanup_close_ int copy_fd = -1;
	struct bus_body_part *part;
...
_public_ int
sd_bus_message_append_strv(sd_bus_message *m, char **l)
{
	char **i;
	int r;
...
static int
bus_message_close_header(sd_bus_message *m)
{
	assert(m);
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		if (m->n_header_offsets >= 1) {
			...
			sz = bus_gvariant_determine_word_size(m->fields_size,
				m->n_header_offsets);
			a = message_extend_fields(m, 1,
				sz * m->n_header_offsets, false);
			if (!a)
				return -ENOMEM;
		...
		*(uint8_t *)d = 0;
		...
		bus_gvariant_write_word_le((uint8_t *)d + 1 + l, sz,
...
int
bus_message_seal(sd_bus_message *m, uint64_t cookie, usec_t timeout)
{
	struct bus_body_part *part;
	size_t a;
	unsigned i;
	...
	if (!isempty(m->root_container.signature) &&
		...
		r = message_append_field_signature(m,
	...
	m->timeout =
		m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED ? 0 : timeout;
	...
	m->root_container.item_size = m->root_container.n_offsets > 0 ?
		      m->root_container.offsets[0] :
		      0;
...
bus_body_part_map(struct bus_body_part *part)
{
	...
	shift = part->memfd_offset -
		((part->memfd_offset / page_size()) * page_size());
	...
	if (part->memfd >= 0)
		...
		p = mmap(NULL, psz, PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1,
			0);
	else
		return -EINVAL;
...
void
bus_body_part_unmap(struct bus_body_part *part)
{
	assert_se(part);
...
static int
buffer_peek(const void *p, uint32_t sz, size_t *rindex, size_t align,
...
static bool
message_end_of_signature(sd_bus_message *m)
{
	struct bus_container *c;
...
static bool
message_end_of_array(sd_bus_message *m, size_t index)
{
	struct bus_container *c;
	...
	if (BUS_MESSAGE_IS_GVARIANT(m))
		...
		assert(c->array_size);
		return index >=
			c->begin + BUS_MESSAGE_BSWAP32(m, *c->array_size);
	}
}

_public_ int
sd_bus_message_at_end(sd_bus_message *m, int complete)
{
	assert_return(m, -EINVAL);
	assert_return(m->sealed, -EPERM);
...
static struct bus_body_part *
find_part(sd_bus_message *m, size_t index, size_t sz, void **p)
{
	struct bus_body_part *part;
	size_t begin;
	int r;
...
static int
container_next_item(sd_bus_message *m, struct bus_container *c, size_t *rindex)
{
	int r;
	...
	if (c->enclosing == SD_BUS_TYPE_ARRAY) {
		...
		if (sz < 0) {
			...
			*rindex = ALIGN_TO(c->offsets[c->offset_index],
				alignment);
			c->item_size =
				c->offsets[c->offset_index + 1] - *rindex;
		} else {
	...
	} else if (c->enclosing == 0 || c->enclosing == SD_BUS_TYPE_STRUCT ||
		c->enclosing == SD_BUS_TYPE_DICT_ENTRY) {
		int alignment;
		size_t n, j;
...
static int
message_peek_body(sd_bus_message *m, size_t *rindex, size_t align,
...
static bool
validate_nul(const char *s, size_t l)
{
...
static bool
validate_string(const char *s, size_t l)
{
	if (!validate_nul(s, l))
		return false;
...
static bool
validate_signature(const char *s, size_t l)
{
	if (!validate_nul(s, l))
		return false;
...
static bool
validate_object_path(const char *s, size_t l)
{
	if (!validate_nul(s, l))
		return false;
...
_public_ int
sd_bus_message_read_basic(sd_bus_message *m, char type, void *p)
{
	struct bus_container *c;
	size_t rindex;
	void *q;
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		if (IN_SET(type, SD_BUS_TYPE_STRING, SD_BUS_TYPE_OBJECT_PATH,
			...
			r = message_peek_body(m, &rindex, align, c->item_size,
				&q);
			if (r < 0)
				return r;
			...
			case SD_BUS_TYPE_UINT16:
				if (p)
					*(uint16_t *)p = BUS_MESSAGE_BSWAP16(m,
						*(uint16_t *)q);
				break;
			...
			case SD_BUS_TYPE_UINT32:
				if (p)
					*(uint32_t *)p = BUS_MESSAGE_BSWAP32(m,
						*(uint32_t *)q);
				break;
			...
			case SD_BUS_TYPE_DOUBLE:
				if (p)
					*(uint64_t *)p = BUS_MESSAGE_BSWAP64(m,
						*(uint64_t *)q);
				break;
...
static int
bus_message_enter_array(sd_bus_message *m, struct bus_container *c,
	const char *contents, uint32_t **array_size, size_t *item_size,
	...
	if (!BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		*offsets = new (size_t, *n_offsets);
		...
		for (i = 0; i < *n_offsets; i++) {
			...
			x = bus_gvariant_read_word_le((uint8_t *)q + i * sz,
				sz);
			if (x > c->item_size - sz)
				return -EBADMSG;
			if (x < p)
...
static int
bus_message_enter_variant(sd_bus_message *m, struct bus_container *c,
...
static int
build_struct_offsets(sd_bus_message *m, const char *signature, size_t size,
	size_t *item_size, size_t **offsets, size_t *n_offsets)
{
...
static int
enter_struct_or_dict_entry(sd_bus_message *m, struct bus_container *c,
	const char *contents, size_t *item_size, size_t **offsets,
	size_t *n_offsets)
{
	int r;
	...
	if (!BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		*n_offsets = 0;
		...
		return build_struct_offsets(m, contents, c->item_size,
			item_size, offsets, n_offsets);

	return 0;
}

static int
bus_message_enter_struct(sd_bus_message *m, struct bus_container *c,
	const char *contents, size_t *item_size, size_t **offsets,
	size_t *n_offsets)
{
	size_t l;
...
static int
bus_message_enter_dict_entry(sd_bus_message *m, struct bus_container *c,
	const char *contents, size_t *item_size, size_t **offsets,
	size_t *n_offsets)
{
	size_t l;
...
_public_ int
sd_bus_message_enter_container(sd_bus_message *m, char type,
	const char *contents)
{
	struct bus_container *c, *w;
	uint32_t *array_size = NULL;
	...
	if (type == SD_BUS_TYPE_ARRAY)
		...
		r = bus_message_enter_struct(m, c, contents, &item_size,
			&offsets, &n_offsets);
	else if (type == SD_BUS_TYPE_DICT_ENTRY)
		r = bus_message_enter_dict_entry(m, c, contents, &item_size,
			&offsets, &n_offsets);
	else
		r = -EINVAL;
...
static void
message_quit_container(sd_bus_message *m)
{
	struct bus_container *c;
...
_public_ int
sd_bus_message_peek_type(sd_bus_message *m, char *type, const char **contents)
{
	struct bus_container *c;
	int r;
	...
	if (c->signature[c->index] == SD_BUS_TYPE_STRUCT_BEGIN ||
		...
		if (contents) {
			...
			r = signature_element_length(c->signature + c->index,
				&l);
			if (r < 0)
				return r;
		...
		if (contents) {
			...
			if (BUS_MESSAGE_IS_GVARIANT(m)) {
				...
				for (k = 2; k <= c->item_size; k++) {
					...
					r = message_peek_body(m, &where, 1, k,
						&q);
					if (r < 0)
						return r;
				...
				free(c->peeked_signature);
				c->peeked_signature =
					strndup((char *)q + 1, k - 1);
				if (!c->peeked_signature)
					return -ENOMEM;

				if (!signature_is_valid(c->peeked_signature,
					    true))
					return -EBADMSG;
...
_public_ int
sd_bus_message_rewind(sd_bus_message *m, int complete)
{
	struct bus_container *c;
...
static int
message_read_ap(sd_bus_message *m, const char *types, va_list ap)
{
	unsigned n_array, n_struct;
	TypeStack stack[BUS_CONTAINER_DEPTH];
	unsigned stack_ptr = 0;
	...
	n_struct =
		strlen(types); /* length of current struct contents signature */
...
_public_ int
sd_bus_message_read(sd_bus_message *m, const char *types, ...)
{
	va_list ap;
	int r;
...
_public_ int
sd_bus_message_skip(sd_bus_message *m, const char *types)
{
	int r;
...
_public_ int
sd_bus_message_read_array(sd_bus_message *m, char type, const void **ptr,
	size_t *size)
{
	struct bus_container *c;
	void *p;
...
static int
message_peek_fields(sd_bus_message *m, size_t *rindex, size_t align,
	size_t nbytes, void **ret)
{
	assert(m);
	assert(rindex);
...
static int
message_peek_field_uint32(sd_bus_message *m, size_t *ri, size_t item_size,
	uint32_t *ret)
{
	int r;
	void *q;
...
static int
message_peek_field_uint64(sd_bus_message *m, size_t *ri, size_t item_size,
	uint64_t *ret)
{
	int r;
	void *q;
...
static int
message_peek_field_string(sd_bus_message *m, bool (*validate)(const char *p),
	size_t *ri, size_t item_size, const char **ret)
{
	uint32_t l;
	int r;
	void *q;
...
static int
message_peek_field_signature(sd_bus_message *m, size_t *ri, size_t item_size,
	const char **ret)
{
	size_t l;
	int r;
...
static int
message_skip_fields(sd_bus_message *m, size_t *ri, uint32_t array_size,
	const char **signature)
{
	...
	for (;;) {
		...
		if (t == SD_BUS_TYPE_STRING) {
			...
			r = message_peek_field_string(m, object_path_is_valid,
				ri, 0, NULL);
			if (r < 0)
				return r;
			...
			assert(l >= 1);
			{
				...
				r = message_peek_fields(m, ri, alignment, 0,
					NULL);
				if (r < 0)
					return r;

				r = message_skip_fields(m, ri, nas,
					(const char **)&s);
				if (r < 0)
					return r;
			}
...
int
bus_message_parse_fields(sd_bus_message *m)
{
	size_t ri;
	int r;
	uint32_t unix_fds = 0;
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		...
		/* Calculate the actual user body size, by removing
                 ...
                 * table */
		m->user_body_size = m->body_size -
			((char *)m->footer + m->footer_accessible - p);
		...
		if (sz > 0) {
			...
			ri = framing;
			r = message_peek_fields(m, &ri, 1,
				m->fields_size - framing, &offsets);
			if (r < 0)
				return r;
	...
	while (ri < m->fields_size) {
		...
		if (BUS_MESSAGE_IS_GVARIANT(m)) {
			...
			end = bus_gvariant_read_word_le(
				(uint8_t *)offsets + i * sz, sz);
		...
		case BUS_MESSAGE_HEADER_INTERFACE:
			...
			r = message_peek_field_string(m,
				interface_name_is_valid, &ri, item_size,
				&m->interface);
			break;
		...
		case BUS_MESSAGE_HEADER_MEMBER:
			...
			r = message_peek_field_string(m, member_name_is_valid,
				&ri, item_size, &m->member);
			break;
		...
		case BUS_MESSAGE_HEADER_ERROR_NAME:
			...
			r = message_peek_field_string(m, error_name_is_valid,
				&ri, item_size, &m->error.name);
			if (r >= 0)
		...
		case BUS_MESSAGE_HEADER_DESTINATION:
			...
			r = message_peek_field_string(m, service_name_is_valid,
				&ri, item_size, &m->destination);
			break;
		...
		case BUS_MESSAGE_HEADER_SENDER:
			...
			r = message_peek_field_string(m, service_name_is_valid,
				&ri, item_size, &m->sender);

			if (r >= 0 && m->sender[0] == ':' &&
				m->bus->bus_client && !m->bus->is_kernel) {
				m->creds.unique_name = (char *)m->sender;
				m->creds.mask |= SD_BUS_CREDS_UNIQUE_NAME &
					m->bus->creds_mask;
			}
		...
		case BUS_MESSAGE_HEADER_SIGNATURE: {
			...
			if (BUS_MESSAGE_IS_GVARIANT(
				    m)) /* only applies to dbus1 */
				return -EBADMSG;
		...
		case BUS_MESSAGE_HEADER_REPLY_SERIAL:
			...
			if (BUS_MESSAGE_IS_GVARIANT(m)) {
				...
				r = message_peek_field_uint32(m, &ri, item_size,
					&serial);
				if (r < 0)
					return r;
	...
	if (BUS_MESSAGE_IS_GVARIANT(m)) {
		r = build_struct_offsets(m, m->root_container.signature,
			m->user_body_size, &m->root_container.item_size,
...
int
bus_message_get_blob(sd_bus_message *m, void **buffer, size_t *sz)
{
	size_t total;
	void *p, *e;
	unsigned i;
...
int
bus_message_read_strv_extend(sd_bus_message *m, char ***l)
{
	int r;
...
_public_ int
sd_bus_message_read_strv(sd_bus_message *m, char ***l)
{
	char **strv = NULL;
	int r;
...
int
bus_message_get_arg(sd_bus_message *m, unsigned i, const char **str,
	char ***strv)
{
	const char *contents;
	unsigned j;
...
_public_ const char *
sd_bus_message_get_signature(sd_bus_message *m, int complete)
{
	struct bus_container *c;
...
_public_ int
sd_bus_message_is_empty(sd_bus_message *m)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_has_signature(sd_bus_message *m, const char *signature)
{
	assert_return(m, -EINVAL);

	return streq(strempty(m->root_container.signature),
		strempty(signature));
}

_public_ int
sd_bus_message_copy(sd_bus_message *m, sd_bus_message *source, int all)
{
	...
	do {
		...
		if (bus_type_is_container(type) > 0) {
			r = sd_bus_message_enter_container(source, type,
				contents);
			if (r < 0)
				return r;
...
_public_ int
sd_bus_message_verify_type(sd_bus_message *m, char type, const char *contents)
{
	const char *c;
	char t;
	int r;
	...
	assert_return(!contents || !type || bus_type_is_container(type),
		-EINVAL);
...
_public_ sd_bus *
sd_bus_message_get_bus(sd_bus_message *m)
{
	assert_return(m, NULL);
...
int
bus_message_remarshal(sd_bus *bus, sd_bus_message **m)
{
	...
	switch ((*m)->header->type) {
	case SD_BUS_MESSAGE_SIGNAL:
		r = sd_bus_message_new_signal(bus, &n, (*m)->path,
			(*m)->interface, (*m)->member);
		if (r < 0)
			return r;
	...
	case SD_BUS_MESSAGE_METHOD_ERROR:
		...
		if ((*m)->header->type == SD_BUS_MESSAGE_METHOD_ERROR &&
			...
			r = message_append_field_string(n,
				...
				SD_BUS_TYPE_STRING, (*m)->error.name,
				&n->error.message);
			if (r < 0)
				return r;
	...
	if ((*m)->sender && !n->sender) {
		r = message_append_field_string(n, BUS_MESSAGE_HEADER_SENDER,
			SD_BUS_TYPE_STRING, (*m)->sender, &n->sender);
		if (r < 0)
			return r;
	}

	n->header->flags |= (*m)->header->flags &
		(BUS_MESSAGE_NO_REPLY_EXPECTED | BUS_MESSAGE_NO_AUTO_START);
...
int
bus_message_append_sender(sd_bus_message *m, const char *sender)
{
	assert(m);
	assert(sender);
...
_public_ int
sd_bus_message_get_priority(sd_bus_message *m, int64_t *priority)
{
	assert_return(m, -EINVAL);
...
_public_ int
sd_bus_message_set_priority(sd_bus_message *m, int64_t priority)
{
	assert_return(m, -EINVAL);
	assert_return(!m->sealed, -EPERM);
