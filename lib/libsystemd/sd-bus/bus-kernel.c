...
int
bus_kernel_parse_unique_name(const char *s, uint64_t *id)
{
	int r;
...
static void
append_payload_vec(struct kdbus_item **d, const void *p, size_t sz)
{
	assert(d);
	assert(sz > 0);
	...
	*d = ALIGN8_PTR(*d);
	...
	(*d)->size =
		offsetof(struct kdbus_item, vec) + sizeof(struct kdbus_vec);
...
static void
append_payload_memfd(struct kdbus_item **d, int memfd, size_t start, size_t sz)
{
	assert(d);
	assert(memfd >= 0);
	assert(sz > 0);
	...
	*d = ALIGN8_PTR(*d);
	(*d)->size =
		offsetof(struct kdbus_item, memfd) + sizeof(struct kdbus_memfd);
...
static void
append_destination(struct kdbus_item **d, const char *s, size_t length)
{
	assert(d);
	assert(s);
...
append_bloom(struct kdbus_item **d, size_t length)
{
	struct kdbus_item *i;
	...
	i->size = offsetof(struct kdbus_item, bloom_filter) +
		offsetof(struct kdbus_bloom_filter, data) + length;
...
static void
append_fds(struct kdbus_item **d, const int fds[], unsigned n_fds)
{
	assert(d);
	assert(fds);
	assert(n_fds > 0);
...
static void
add_bloom_arg(void *data, size_t size, unsigned n_hash, unsigned i,
	const char *t)
{
...
static int
bus_message_setup_bloom(sd_bus_message *m, struct kdbus_bloom_filter *bloom)
{
	void *data;
	unsigned i;
	int r;
	...
	bloom_add_pair(data, m->bus->bloom_size, m->bus->bloom_n_hash,
		"message-type", bus_message_type_to_string(m->header->type));
	...
	if (m->path) {
		bloom_add_pair(data, m->bus->bloom_size, m->bus->bloom_n_hash,
			"path", m->path);
		bloom_add_pair(data, m->bus->bloom_size, m->bus->bloom_n_hash,
			"path-slash-prefix", m->path);
		bloom_add_prefixes(data, m->bus->bloom_size,
			m->bus->bloom_n_hash, "path-slash-prefix", m->path,
			'/');
	}
	...
	for (i = 0; i < 64; i++) {
		...
		if (IN_SET(type, SD_BUS_TYPE_STRING, SD_BUS_TYPE_OBJECT_PATH,
			...
			add_bloom_arg(data, m->bus->bloom_size,
				m->bus->bloom_n_hash, i, t);
		}
		if (type == SD_BUS_TYPE_ARRAY &&
			...
			while ((r = sd_bus_message_read_basic(m, contents[0],
					&t)) > 0)
				add_bloom_arg(data, m->bus->bloom_size,
					m->bus->bloom_n_hash, i, t);
			if (r < 0)
...
static int
bus_message_setup_kmsg(sd_bus *b, sd_bus_message *m)
{
	...
	sz += (1 + m->n_body_parts) *
		ALIGN8(offsetof(struct kdbus_item, vec) +
			MAX(sizeof(struct kdbus_vec),
				sizeof(struct kdbus_memfd)));

	/* Add space for bloom filter */
	sz += ALIGN8(offsetof(struct kdbus_item, bloom_filter) +
		offsetof(struct kdbus_bloom_filter, data) + m->bus->bloom_size);
	...
	if (m->n_fds > 0)
		sz += ALIGN8(offsetof(struct kdbus_item, fds) +
			sizeof(int) * m->n_fds);
	...
	m->kdbus->flags = ((m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED) ?
						0 :
						KDBUS_MSG_EXPECT_REPLY) |
		((m->header->flags & BUS_MESSAGE_NO_AUTO_START) ?
				      KDBUS_MSG_NO_AUTO_START :
				      0) |
		((m->header->type == SD_BUS_MESSAGE_SIGNAL) ? KDBUS_MSG_SIGNAL :
								    0);
	...
	if (well_known)
		...
		m->kdbus->dst_id =
			destination ? unique : KDBUS_DST_ID_BROADCAST;
	...
	MESSAGE_FOREACH_PART (part, i, m) {
		...
		if (part->memfd >= 0 && part->sealed && destination) {
			...
			append_payload_memfd(&d, part->memfd,
				part->memfd_offset, part->size);
			continue;
		}
...
static void
unset_memfds(struct sd_bus_message *m)
{
	struct bus_body_part *part;
	unsigned i;
...
static void
message_set_timestamp(sd_bus *bus, sd_bus_message *m,
...
static int
bus_kernel_make_message(sd_bus *bus, struct kdbus_msg *k)
{
	...
	KDBUS_ITEM_FOREACH(d, k, items)
	{
		...
		case KDBUS_ITEM_PAYLOAD_OFF:
			if (!header) {
				header = (struct bus_header *)((uint8_t *)k +
					d->vec.offset);
	...
	if (header->version != 2 || header->endian != BUS_NATIVE_ENDIAN)
		return -EPROTOTYPE;

	r = bus_message_from_header(bus, header, header_size, footer,
		footer_size, n_bytes, fds, n_fds, NULL, seclabel, 0, &m);
	if (r < 0)
		return r;
	...
	KDBUS_ITEM_FOREACH(d, k, items)
	{
		...
		case KDBUS_ITEM_PIDS:
			...
			if (d->pids.pid > 0) {
				...
				m->creds.mask |=
					SD_BUS_CREDS_PID & bus->creds_mask;
			}

			if (d->pids.tid > 0) {
				m->creds.tid = (pid_t)d->pids.tid;
				m->creds.mask |=
					SD_BUS_CREDS_TID & bus->creds_mask;
			}
		...
		case KDBUS_ITEM_TID_COMM:
			...
			m->creds.mask |=
				SD_BUS_CREDS_TID_COMM & bus->creds_mask;
			break;
		...
		case KDBUS_ITEM_AUDIT:
			if ((uint32_t)d->audit.sessionid != (uint32_t)-1) {
				m->creds.audit_session_id =
					(uint32_t)d->audit.sessionid;
				m->creds.mask |= SD_BUS_CREDS_AUDIT_SESSION_ID &
					bus->creds_mask;
			}

			if ((uid_t)d->audit.loginuid != UID_INVALID) {
				m->creds.audit_login_uid =
					(uid_t)d->audit.loginuid;
				m->creds.mask |= SD_BUS_CREDS_AUDIT_LOGIN_UID &
					bus->creds_mask;
			}
			break;
		...
		case KDBUS_ITEM_CAPS:
			if (d->caps.last_cap != cap_last_cap() ||
				d->size -
						offsetof(struct kdbus_item,
							caps.caps) <
					DIV_ROUND_UP(d->caps.last_cap, 32U) *
						4 * 4) {
				r = -EBADMSG;
				goto fail;
			}
			...
			m->creds.mask |= (SD_BUS_CREDS_EFFECTIVE_CAPS |
		...
		case KDBUS_ITEM_OWNED_NAME:
			...
			if (bus->creds_mask & SD_BUS_CREDS_WELL_KNOWN_NAMES) {
				...
				wkn = realloc(m->creds.well_known_names,
					(n + 2) * sizeof(char *));
				if (!wkn) {
					r = -ENOMEM;
					goto fail;
		...
		case KDBUS_ITEM_CONN_DESCRIPTION:
			...
			m->creds.mask |=
				SD_BUS_CREDS_DESCRIPTION & bus->creds_mask;
			break;
		...
		case KDBUS_ITEM_AUXGROUPS:
			...
			if (bus->creds_mask & SD_BUS_CREDS_SUPPLEMENTARY_GIDS) {
				...
				m->creds.n_supplementary_gids =
					(d->size -
						offsetof(struct kdbus_item,
							data32)) /
					sizeof(uint32_t);
				m->creds.supplementary_gids =
					(gid_t *)d->data32;
				m->creds.mask |=
	...
	if (!(m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED) !=
		!!(k->flags & KDBUS_MSG_EXPECT_REPLY)) {
		r = -EBADMSG;
		goto fail;
	}
	...
	if ((m->header->flags & BUS_MESSAGE_NO_REPLY_EXPECTED) &&
		m->reply_cookie != k->cookie_reply) {
		r = -EBADMSG;
		goto fail;
	}

	/* Refuse messages where the autostart flag doesn't match up */
	if (!(m->header->flags & BUS_MESSAGE_NO_AUTO_START) !=
		!(k->flags & KDBUS_MSG_NO_AUTO_START)) {
		r = -EBADMSG;
		goto fail;
	}
	...
	if (k->src_id == KDBUS_SRC_ID_KERNEL)
		...
		snprintf(m->sender_buffer, sizeof(m->sender_buffer), ":1.%llu",
	...
	if (destination)
		...
		m->destination =
			bus->unique_name; /* fill in unique name if the well-known name is missing */
	else {
		snprintf(m->destination_buffer, sizeof(m->destination_buffer),
			":1.%llu", (unsigned long long)k->dst_id);
...
bus_kernel_take_fd(sd_bus *b)
{
	...
	if (b->fake_pids_valid)
		sz += ALIGN8(offsetof(struct kdbus_item, pids) +
	...
	if (b->fake_creds_valid) {
		item->size = offsetof(struct kdbus_item, creds) +
			sizeof(struct kdbus_creds);
	...
	if (b->fake_pids_valid) {
		item->size = offsetof(struct kdbus_item, pids) +
			sizeof(struct kdbus_pids);
	...
	if (!b->kdbus_buffer) {
		b->kdbus_buffer = mmap(NULL, KDBUS_POOL_SIZE, PROT_READ,
			MAP_SHARED, b->input_fd, 0);
	...
	if (!bloom ||
		!bloom_validate_parameters((size_t)bloom->size,
			(unsigned)bloom->n_hash)) {
		r = -ENOTSUP;
		goto fail;
	}
	...
	if (asprintf(&b->unique_name, ":1.%llu",
		    (unsigned long long)hello->id) < 0) {
		r = -ENOMEM;
		goto fail;
	}
...
int
bus_kernel_cmd_free(sd_bus *bus, uint64_t offset)
{
...
static void
close_kdbus_msg(sd_bus *bus, struct kdbus_msg *k)
{
	struct kdbus_item *d;
	...
	KDBUS_ITEM_FOREACH(d, k, items)
	{
		if (d->type == KDBUS_ITEM_FDS)
			close_many(d->fds,
				(d->size - offsetof(struct kdbus_item, fds)) /
					sizeof(int));
...
int
bus_kernel_write_message(sd_bus *bus, sd_bus_message *m, bool hint_sync_call)
{
	...
	if (r < 0) {
		...
		if (errno == EAGAIN || errno == EINTR)
			...
			if (m->header->type == SD_BUS_MESSAGE_METHOD_CALL)
				sd_bus_error_setf(&error,
					...
					"Destination %s not known",
					m->destination);
			else {
				log_debug(
					"Could not deliver message to %s as destination is not known. Ignoring.",
					m->destination);
				return 0;
			}
			...
			if (m->header->type == SD_BUS_MESSAGE_METHOD_CALL)
				sd_bus_error_setf(&error,
					SD_BUS_ERROR_SERVICE_UNKNOWN,
					"Activation of %s not requested",
					m->destination);
			else {
				log_debug(
					"Could not deliver message to %s as destination is not activated. Ignoring.",
					m->destination);
				return 0;
			}
		} else
		...
		r = bus_message_new_synthetic_error(bus, BUS_MESSAGE_COOKIE(m),
			&error, &reply);
		...
		k = (struct kdbus_msg *)((uint8_t *)bus->kdbus_buffer +
			cmd.reply.offset);
		assert(k);
		...
		if (k->payload_type == KDBUS_PAYLOAD_DBUS) {
			...
			log_debug(
				"Ignoring message with unknown payload type %llu.",
				(unsigned long long)k->payload_type);
			close_kdbus_msg(bus, k);
		}
	}
...
static int
push_name_owner_changed(sd_bus *bus, const char *name, const char *old_owner,
	const char *new_owner, const struct kdbus_timestamp *ts)
{
	...
	r = sd_bus_message_new_signal(bus, &m, "/org/freedesktop/DBus",
...
static int
translate_name_change(sd_bus *bus, const struct kdbus_msg *k,
	const struct kdbus_item *d, const struct kdbus_timestamp *ts)
{
	...
	if (d->type == KDBUS_ITEM_NAME_ADD ||
		(d->name_change.old_id.flags &
			(KDBUS_NAME_IN_QUEUE | KDBUS_NAME_ACTIVATOR)))
		old_owner[0] = 0;
	else
		sprintf(old_owner, ":1.%llu",
			(unsigned long long)d->name_change.old_id.id);

	if (d->type == KDBUS_ITEM_NAME_REMOVE ||
		(d->name_change.new_id.flags &
			(KDBUS_NAME_IN_QUEUE | KDBUS_NAME_ACTIVATOR))) {
		...
		sprintf(new_owner, ":1.%llu",
			(unsigned long long)d->name_change.new_id.id);

	return push_name_owner_changed(bus, d->name_change.name, old_owner,
		new_owner, ts);
}

static int
translate_id_change(sd_bus *bus, const struct kdbus_msg *k,
	const struct kdbus_item *d, const struct kdbus_timestamp *ts)
{
	...
	return push_name_owner_changed(bus, owner,
		d->type == KDBUS_ITEM_ID_ADD ? NULL : owner,
		d->type == KDBUS_ITEM_ID_ADD ? owner : NULL, ts);
}

static int
translate_reply(sd_bus *bus, const struct kdbus_msg *k,
	const struct kdbus_item *d, const struct kdbus_timestamp *ts)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL;
	int r;
	...
	r = bus_message_new_synthetic_error(bus, k->cookie_reply,
...
static int
bus_kernel_translate_message(sd_bus *bus, struct kdbus_msg *k)
{
	static int (*const translate[])(sd_bus * bus, const struct kdbus_msg *k,
		const struct kdbus_item *d,
		const struct kdbus_timestamp *ts) = {
		[KDBUS_ITEM_NAME_ADD - _KDBUS_ITEM_KERNEL_BASE] =
			translate_name_change,
		[KDBUS_ITEM_NAME_REMOVE - _KDBUS_ITEM_KERNEL_BASE] =
			translate_name_change,
		[KDBUS_ITEM_NAME_CHANGE - _KDBUS_ITEM_KERNEL_BASE] =
			translate_name_change,

		[KDBUS_ITEM_ID_ADD - _KDBUS_ITEM_KERNEL_BASE] =
			translate_id_change,
		[KDBUS_ITEM_ID_REMOVE - _KDBUS_ITEM_KERNEL_BASE] =
			translate_id_change,

		[KDBUS_ITEM_REPLY_TIMEOUT - _KDBUS_ITEM_KERNEL_BASE] =
			translate_reply,
		[KDBUS_ITEM_REPLY_DEAD - _KDBUS_ITEM_KERNEL_BASE] =
			translate_reply,
	};
	...
	KDBUS_ITEM_FOREACH(d, k, items)
	{
		...
		if (d->type >= _KDBUS_ITEM_KERNEL_BASE &&
			d->type < _KDBUS_ITEM_KERNEL_BASE +
...
int
bus_kernel_read_message(sd_bus *bus, bool hint_priority, int64_t priority)
{
	...
	if (r < 0) {
		...
		if (errno == EOVERFLOW) {
			log_debug("%s: kdbus reports %" PRIu64
				  " dropped broadcast messages, ignoring.",
				strna(bus->description),
				(uint64_t)recv.dropped_msgs);
			return 0;
		}
	...
	if (k->payload_type == KDBUS_PAYLOAD_DBUS) {
		...
		log_debug("Ignoring message with unknown payload type %llu.",
			(unsigned long long)k->payload_type);
		r = 0;
	}
...
int
bus_kernel_pop_memfd(sd_bus *bus, void **address, size_t *mapped,
	size_t *allocated)
{
...
static void
close_and_munmap(int fd, void *address, size_t size)
{
	if (size > 0)
...
void
bus_kernel_push_memfd(sd_bus *bus, int fd, void *address, size_t mapped,
	size_t allocated)
{
	struct memfd_cache *c;
	...
	if (mapped > max_mapped) {
		...
		assert_se(munmap((uint8_t *)address + max_mapped,
				  PAGE_ALIGN(mapped - max_mapped)) >= 0);
...
void
bus_kernel_flush_memfd(sd_bus *b)
{
	unsigned i;
	...
	for (i = 0; i < b->n_memfd_cache; i++)
		close_and_munmap(b->memfd_cache[i].fd,
			b->memfd_cache[i].address, b->memfd_cache[i].mapped);
}
...
int
bus_kernel_create_bus(const char *name, bool world, char **s)
{
	...
	make = alloca0_align(offsetof(struct kdbus_cmd, items) +
			ALIGN8(offsetof(struct kdbus_item, bloom_parameter) +
				sizeof(struct kdbus_bloom_parameter)) +
			ALIGN8(offsetof(struct kdbus_item, data64) +
				sizeof(uint64_t)) +
			ALIGN8(offsetof(struct kdbus_item, data64) +
				sizeof(uint64_t)) +
			ALIGN8(offsetof(struct kdbus_item, str) +
				DECIMAL_STR_MAX(uid_t) + 1 + l + 1),
		8);
...
int
bus_kernel_open_bus_fd(const char *bus, char **path)
{
	char *p;
	int fd;
	size_t len;
	...
	len = strlen("/sys/fs/kdbus/") + DECIMAL_STR_MAX(uid_t) + 1 +
...
int
bus_kernel_create_endpoint(const char *bus_name, const char *ep_name,
	char **ep_path)
{
	...
	make = alloca0_align(ALIGN8(offsetof(struct kdbus_cmd, items)) +
			ALIGN8(offsetof(struct kdbus_item, str) +
				DECIMAL_STR_MAX(uid_t) + 1 + strlen(ep_name) +
				1),
		8);
...
int
bus_kernel_try_close(sd_bus *bus)
{
...
int
bus_kernel_realize_attach_flags(sd_bus *bus)
{
	...
	update->size = offsetof(struct kdbus_cmd, items) + ALIGN8(n->size);
...
int
bus_kernel_fix_attach_mask(void)
{
	...
	if (r == 0) {
		...
		r = write_string_file(
			"/sys/module/kdbus/parameters/attach_flags_mask", buf);
		if (r < 0)
			return log_full_errno(IN_SET(r, -ENOENT, -EROFS) ?
					      LOG_DEBUG :
					      LOG_WARNING,
				r, "Failed to write kdbus attach mask: %m");
	}
...
int
bus_kernel_get_bus_name(sd_bus *bus, char **name)
{
