...
sd_bus_creds_ref(sd_bus_creds *c)
{
...
_public_ sd_bus_creds *
sd_bus_creds_unref(sd_bus_creds *c)
{
	if (!c)
		return NULL;
...
_public_ uint64_t
sd_bus_creds_get_mask(const sd_bus_creds *c)
{
	assert_return(c, 0);
...
_public_ int
sd_bus_creds_new_from_pid(sd_bus_creds **ret, pid_t pid, uint64_t mask)
{
	sd_bus_creds *c;
	int r;
...
_public_ int
sd_bus_creds_get_uid(sd_bus_creds *c, uid_t *uid)
{
...
_public_ int
sd_bus_creds_get_euid(sd_bus_creds *c, uid_t *euid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_suid(sd_bus_creds *c, uid_t *suid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_fsuid(sd_bus_creds *c, uid_t *fsuid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_gid(sd_bus_creds *c, gid_t *gid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_egid(sd_bus_creds *c, gid_t *egid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_sgid(sd_bus_creds *c, gid_t *sgid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_fsgid(sd_bus_creds *c, gid_t *fsgid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_supplementary_gids(sd_bus_creds *c, const gid_t **gids)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_pid(sd_bus_creds *c, pid_t *pid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_tid(sd_bus_creds *c, pid_t *tid)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_selinux_context(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_get_comm(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
_public_ int
sd_bus_creds_get_tid_comm(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
_public_ int
sd_bus_creds_get_exe(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
_public_ int
sd_bus_creds_get_cgroup(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
_public_ int
sd_bus_creds_get_unit(sd_bus_creds *c, const char **ret)
{
	int r;
...
_public_ int
sd_bus_creds_get_user_unit(sd_bus_creds *c, const char **ret)
{
	int r;
...
_public_ int
sd_bus_creds_get_slice(sd_bus_creds *c, const char **ret)
{
	int r;
...
_public_ int
sd_bus_creds_get_owner_uid(sd_bus_creds *c, uid_t *uid)
{
	const char *shifted;
	int r;
...
_public_ int
sd_bus_creds_get_cmdline(sd_bus_creds *c, char ***cmdline)
{
	assert_return(c, -EINVAL);
	...
	if (!c->cmdline_array) {
		c->cmdline_array =
			strv_parse_nulstr(c->cmdline, c->cmdline_size);
		if (!c->cmdline_array)
			return -ENOMEM;
	}
...
_public_ int
sd_bus_creds_get_well_known_names(sd_bus_creds *c, char ***well_known_names)
{
	assert_return(c, -EINVAL);
	...
	if (c->well_known_names_driver) {
		static const char *const wkn[] = { "org.freedesktop.DBus",
			NULL };
	...
	if (c->well_known_names_local) {
		static const char *const wkn[] = { "org.freedesktop.DBus.Local",
			NULL };
...
_public_ int
sd_bus_creds_get_description(sd_bus_creds *c, const char **ret)
{
	assert_return(c, -EINVAL);
	assert_return(ret, -EINVAL);
...
static int
has_cap(sd_bus_creds *c, unsigned offset, int capability)
{
	size_t sz;
	...
	return !!(c->capability[offset * sz + CAP_TO_INDEX(capability)] &
		CAP_TO_MASK(capability));
}

_public_ int
sd_bus_creds_has_effective_cap(sd_bus_creds *c, int capability)
{
	assert_return(c, -EINVAL);
...
_public_ int
sd_bus_creds_has_permitted_cap(sd_bus_creds *c, int capability)
{
	assert_return(c, -EINVAL);
	assert_return(capability >= 0, -EINVAL);
...
_public_ int
sd_bus_creds_has_inheritable_cap(sd_bus_creds *c, int capability)
{
	assert_return(c, -EINVAL);
	assert_return(capability >= 0, -EINVAL);
...
_public_ int
sd_bus_creds_has_bounding_cap(sd_bus_creds *c, int capability)
{
	assert_return(c, -EINVAL);
	assert_return(capability >= 0, -EINVAL);
...
static int
parse_caps(sd_bus_creds *c, unsigned offset, const char *p)
{
	size_t sz, max;
	unsigned i, j;
...
int
bus_creds_add_more(sd_bus_creds *c, uint64_t mask, pid_t pid, pid_t tid)
{
	uint64_t missing;
	int r;
	...
	if (tid > 0 && (missing & SD_BUS_CREDS_TID_COMM)) {
		...
		if (asprintf(&p, "/proc/" PID_FMT "/task/" PID_FMT "/comm", pid,
			    tid) < 0)
			return -ENOMEM;
...
int
bus_creds_extend_by_pid(sd_bus_creds *c, uint64_t mask, sd_bus_creds **ret)
{
	...
	if (c->mask & mask & SD_BUS_CREDS_SUPPLEMENTARY_GIDS) {
		n->supplementary_gids = newdup(gid_t, c->supplementary_gids,
			c->n_supplementary_gids);
	...
	if (c->mask & mask &
		(SD_BUS_CREDS_CGROUP | SD_BUS_CREDS_SESSION |
			SD_BUS_CREDS_UNIT | SD_BUS_CREDS_USER_UNIT |
			SD_BUS_CREDS_SLICE | SD_BUS_CREDS_OWNER_UID)) {
	...
	if (c->mask & mask &
		(SD_BUS_CREDS_EFFECTIVE_CAPS | SD_BUS_CREDS_PERMITTED_CAPS |
			SD_BUS_CREDS_INHERITABLE_CAPS |
			SD_BUS_CREDS_BOUNDING_CAPS)) {
		n->capability = memdup(c->capability,
	...
	r = bus_creds_add_more(n, mask, c->mask & SD_BUS_CREDS_PID ? c->pid : 0,
