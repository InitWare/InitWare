...
struct sd_rtnl_message {
	...
	struct nlmsghdr *hdr;
	const struct NLTypeSystem *(container_type_system
			[RTNL_CONTAINER_DEPTH]); /* the type of the container and all its parents */
	size_t container_offsets
		[RTNL_CONTAINER_DEPTH]; /* offset from hdr to each container's start */
...
int rtnl_message_read_internal(sd_rtnl_message *m, unsigned short type,
	void **data);
