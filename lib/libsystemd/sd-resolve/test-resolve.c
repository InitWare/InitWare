...
static int
getaddrinfo_handler(sd_resolve_query *q, int ret, const struct addrinfo *ai,
	void *userdata)
{
...
static int
getnameinfo_handler(sd_resolve_query *q, int ret, const char *host,
	const char *serv, void *userdata)
{
	assert_se(q);
...
static int
res_handler(sd_resolve_query *q, int ret, unsigned char *answer, void *userdata)
{
...
main(int argc, char *argv[])
{
	...
	sd_resolve_getaddrinfo(resolve, NULL, "redhat.com", "http", NULL,
		getaddrinfo_handler, NULL);

	/* Make a name -> address query */
	r = sd_resolve_getaddrinfo(resolve, &q1,
		argc >= 2 ? argv[1] : "www.heise.de", NULL, &hints,
		getaddrinfo_handler, NULL);
	if (r < 0)
	...
	r = sd_resolve_getnameinfo(resolve, &q2, (struct sockaddr *)&sa,
		sizeof(sa), 0, SD_RESOLVE_GET_BOTH, getnameinfo_handler, NULL);
	if (r < 0)
		log_error_errno(r, "sd_resolve_getnameinfo(): %m");

	/* Make a res_query() call */
	r = sd_resolve_res_query(resolve, &q3, "_xmpp-client._tcp.gmail.com",
		C_IN, T_SRV, res_handler, NULL);
	if (r < 0)
