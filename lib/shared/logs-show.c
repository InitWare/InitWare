...
static int
print_catalog(FILE *f, sd_journal *j)
{
	int r;
...
static int
parse_field(const void *data, size_t length, const char *field, char **target,
	size_t *target_size)
{
	size_t fl, nl;
	void *buf;
...
static bool
shall_print(const char *p, size_t l, OutputFlags flags)
{
	assert(p);
...
static bool
print_multiline(FILE *f, unsigned prefix, unsigned n_columns, OutputFlags flags,
	int priority, const char *message, size_t message_len)
{
	...
	for (pos = message; pos < message + message_len;
		pos = end + 1, line++) {
...
static int
output_short(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	OutputFlags flags)
{
	int r;
	const void *data;
	...
	size_t hostname_len = 0, identifier_len = 0, comm_len = 0, pid_len = 0,
	       ...
	       monotonic_len = 0, priority_len = 0;
	int p = LOG_INFO;
	bool ellipsized = false;
	...
	JOURNAL_FOREACH_DATA_RETVAL (j, data, length, r) {
		...
		r = parse_field(data, length, "SYSLOG_PID=", &fake_pid,
			&fake_pid_len);
		if (r < 0)
			return r;
		else if (r > 0)
		...
		case OUTPUT_SHORT_PRECISE:
			...
			if (r > 0) {
				snprintf(buf + strlen(buf),
					sizeof(buf) - strlen(buf), ".%06llu",
					(unsigned long long)(x % USEC_PER_SEC));
			}
			break;
		default:
			r = strftime(buf, sizeof(buf), "%b %d %H:%M:%S",
	...
	if (!(flags & OUTPUT_SHOW_ALL) &&
		!utf8_is_printable(message, message_len)) {
		char bytes[FORMAT_BYTES_MAX];
		fprintf(f, ": [%s blob data]\n",
			format_bytes(bytes, sizeof(bytes), message_len));
	} else {
		fputs(": ", f);
		ellipsized |= print_multiline(f, n + 2, n_columns, flags, p,
			message, message_len);
	}
...
static int
output_verbose(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	OutputFlags flags)
{
	const void *data;
	size_t length;
	...
	r = sd_journal_get_data(j, "_SOURCE_REALTIME_TIMESTAMP", &data,
		&length);
	if (r == -ENOENT)
		log_debug("Source realtime timestamp not found");
	else if (r < 0) {
		log_full(r == -EADDRNOTAVAIL ? LOG_DEBUG : LOG_ERR,
			"Failed to get source realtime timestamp: %s",
			strerror(-r));
		return r;
	} else {
		...
		r = parse_field(data, length,
			"_SOURCE_REALTIME_TIMESTAMP=", &value, &size);
		if (r < 0)
			log_debug_errno(r,
	...
	if (r < 0) {
		...
		if (r < 0) {
			log_full(r == -EADDRNOTAVAIL ? LOG_DEBUG : LOG_ERR,
				"Failed to get realtime timestamp: %s",
				strerror(-r));
			return r;
		}
	}
	...
	JOURNAL_FOREACH_DATA_RETVAL (j, data, length, r) {
		...
		if (flags & OUTPUT_SHOW_ALL ||
			(((length < PRINT_CHAR_THRESHOLD) ||
				 flags & OUTPUT_FULL_WIDTH) &&
				utf8_is_printable(data, length))) {
			fprintf(f, "    %s%.*s=", on, fieldlen,
				(const char *)data);
			print_multiline(f, 4 + fieldlen + 1, 0,
				OUTPUT_FULL_WIDTH, 0, c + 1,
				length - fieldlen - 1);
			fputs(off, f);
		} else {
			char bytes[FORMAT_BYTES_MAX];
...
static int
output_export(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	OutputFlags flags)
{
	sd_id128_t boot_id;
	char sid[33];
	...
	JOURNAL_FOREACH_DATA_RETVAL (j, data, length, r) {
		...
		if (utf8_is_printable_newline(data, length, false))
			...
			fwrite(c + 1, length - (c - (const char *)data) - 1, 1,
				f);
		}
...
void
json_escape(FILE *f, const char *p, size_t l, OutputFlags flags)
{
	assert(f);
	assert(p);
...
static int
output_json(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	OutputFlags flags)
{
	...
	sd_journal_set_data_threshold(j,
		flags & OUTPUT_SHOW_ALL ? 0 : JSON_THRESHOLD);
	...
	do {
		...
		SD_JOURNAL_FOREACH_DATA(j, data, length)
		{
			...
			if (u == 0) {
				...
				while (sd_journal_enumerate_data(j, &data,
					       &length) > 0) {
					if (length < m + 1)
						continue;
...
static int
output_cat(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	OutputFlags flags)
{
	const void *data;
	size_t l;
...
static int (*output_funcs[_OUTPUT_MODE_MAX])(FILE *f, sd_journal *j,
	OutputMode mode, unsigned n_columns, OutputFlags flags) = {
...
int
output_journal(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
...
static int
maybe_print_begin_newline(FILE *f, OutputFlags *flags)
{
	assert(f);
	assert(flags);
...
static int
show_journal(FILE *f, sd_journal *j, OutputMode mode, unsigned n_columns,
	...
	bool *ellipsized)
{
	...
	for (;;) {
		for (;;) {
			...
			if (not_before > 0) {
				r = sd_journal_get_monotonic_usec(j, &usec,
					NULL);
		...
		if (warn_cutoff && line < how_many && not_before > 0) {
			...
			r = sd_journal_get_cutoff_monotonic_usec(j, boot_id,
				&cutoff, NULL);
			if (r < 0)
				goto finish;
...
int
add_matches_for_unit(sd_journal *j, const char *unit)
{
	int r;
	char *m1, *m2, *m3, *m4;
...
static int
get_boot_id_for_machine(const char *machine, sd_id128_t *boot_id)
{
...
int
add_match_this_boot(sd_journal *j, const char *machine)
{
	...
	if (machine) {
		...
		if (r < 0)
			return log_error_errno(r,
				"Failed to get boot id of container %s: %m",
				machine);
	} else {
		r = sd_id128_get_boot(&boot_id);
		if (r < 0)
