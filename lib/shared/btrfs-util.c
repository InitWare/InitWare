...
static int
validate_subvolume_name(const char *name)
{
...
static int
open_parent(const char *path, int flags)
{
...
static int
extract_subvolume_name(const char *path, const char **subvolume)
{
	const char *fn;
	int r;
...
int
btrfs_subvol_snapshot(const char *old_path, const char *new_path,
...
int
btrfs_subvol_make(const char *path)
{
...
int
btrfs_subvol_make_label(const char *path)
{
	int r;
...
int
btrfs_subvol_remove(const char *path)
{
	struct btrfs_ioctl_vol_args args = {};
	_cleanup_close_ int fd = -1;
	const char *subvolume;
...
int
btrfs_subvol_set_read_only_fd(int fd, bool b)
{
...
int
btrfs_subvol_set_read_only(const char *path, bool b)
{
	_cleanup_close_ int fd = -1;
...
int
btrfs_subvol_get_read_only_fd(int fd)
{
	uint64_t flags;
...
int
btrfs_reflink(int infd, int outfd)
{
	int r;
...
int
btrfs_clone_range(int infd, uint64_t in_offset, int outfd, uint64_t out_offset,
	uint64_t sz)
{
...
int
btrfs_get_block_device(const char *path, dev_t *dev)
{
...
int
btrfs_subvol_get_id_fd(int fd, uint64_t *ret)
{
...
static bool
btrfs_ioctl_search_args_inc(struct btrfs_ioctl_search_args *args)
{
	assert(args);
...
btrfs_subvol_get_info_fd(int fd, BtrfsSubvolInfo *ret)
{
	...
	while (btrfs_ioctl_search_args_compare(&args) <= 0) {
		...
		FOREACH_BTRFS_IOCTL_SEARCH_HEADER (i, sh, args) {
			...
			ret->otime =
				(usec_t)le64toh(ri->otime.sec) * USEC_PER_SEC +
				(usec_t)le32toh(ri->otime.nsec) / NSEC_PER_USEC;

			ret->subvol_id = subvol_id;
			ret->read_only = !!(
				le64toh(ri->flags) & BTRFS_ROOT_SUBVOL_RDONLY);
...
int
btrfs_subvol_get_quota_fd(int fd, BtrfsQuotaInfo *ret)
{
	struct btrfs_ioctl_search_args args = {
		/* Tree of quota items */
