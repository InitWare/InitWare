...
static int
ask_password_plymouth(const char *message, usec_t until, const char *flag_file,
...
static int
parse_password(const char *filename, char **wall)
{
		...
		*wall = _wall;
		...
		if (access(socket_name, W_OK) < 0) {
			if (arg_action == ACTION_QUERY)
				log_info(
					"Not querying '%s' (PID %u), lacking privileges.",
					message, pid);
		...
		if (arg_plymouth) {
			...
			r = ask_password_plymouth(message, not_after, filename,
			...
			if (arg_console) {
				tty_fd = acquire_terminal("/dev/console", false,
					false, false, USEC_INFINITY);
				if (tty_fd < 0)
					return tty_fd;
			}

			r = ask_password_tty(message, not_after, echo, filename,
				&password);
		...
		r = sendto(socket_fd, packet, packet_length, MSG_NOSIGNAL,
			&sa.sa,
...
static bool
wall_tty_match(const char *path)
{
	int fd, r;
	struct stat st;
	_cleanup_free_ char *p = NULL;
...
static int
watch_passwords(void)
{
	enum { FD_INOTIFY, FD_SIGNAL, _FD_MAX };
