...
static int
inhibit(sd_bus *bus, sd_bus_error *error)
{
...
static int
print_inhibitors(sd_bus *bus, sd_bus_error *error)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	...
	while ((r = sd_bus_message_read(reply, "(ssssuu)", &what, &who, &why,
		...
		printf("     Who: %s (UID " UID_FMT "/%s, PID " PID_FMT "/%s)\n"
		       ...
		       "     Why: %s\n"
		       "    Mode: %s\n\n",
			who, uid, strna(u), pid, strna(comm), what, why, mode);
...
main(int argc, char *argv[])
{
	...
	if (arg_action == ACTION_LIST) {
		...
		if (pid == 0) {
			...
			log_error_errno(errno, "Failed to execute %s: %m",
				argv[optind]);
			_exit(EXIT_FAILURE);
		}
