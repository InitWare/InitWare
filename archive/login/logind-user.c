...
User *
user_new(Manager *m, uid_t uid, gid_t gid, const char *name)
{
	User *u;
...
user_free(User *u)
{
	...
	if (u->in_gc_queue)
		IWLIST_REMOVE(gc_queue, u->manager->user_gc_queue, u);
...
user_save(User *u)
{
	...
	fprintf(f,
		...
		"NAME=%s\n"
		"STATE=%s\n",
		u->name, user_state_to_string(user_get_state(u)));
	...
	if (dual_timestamp_is_set(&u->timestamp))
		fprintf(f,
			"REALTIME=" USEC_FMT "\n"
			"MONOTONIC=" USEC_FMT "\n",
			u->timestamp.realtime, u->timestamp.monotonic);
...
finish:
	if (r < 0)
		log_error_errno(r, "Failed to save user data %s: %m",
			u->state_file);
...
user_load(User *u)
{
	_cleanup_free_ char *display = NULL, *realtime = NULL,
			    *monotonic = NULL;
	Session *s = NULL;
	int r;
	...
	r = parse_env_file(u->state_file, NEWLINE, "RUNTIME", &u->runtime_path,
		"SERVICE", &u->service, "SERVICE_JOB", &u->service_job, "SLICE",
		&u->slice, "SLICE_JOB", &u->slice_job, "DISPLAY", &display,
		"REALTIME", &realtime, "MONOTONIC", &monotonic, NULL);
	if (r < 0) {
		if (r == -ENOENT)
			return 0;
...
static int
user_mkdir_runtime_path(User *u)
{
	char *p;
	int r;
	...
	if (path_is_mount_point(p, false) <= 0) {
		...
		if (mac_smack_use())
			...
			r = asprintf(&t,
				"mode=0700,uid=" UID_FMT ",gid=" GID_FMT
				",size=%zu",
				u->uid, u->gid, u->manager->runtime_dir_size);
		if (r < 0) {
			r = log_oom();
	...
	if (!u->slice) {
		...
		if (r < 0)
			return log_error_errno(r,
				"Failed to build slice name: %m");

		description = strjoina("User Slice of ", u->name);

		r = manager_start_slice(u->manager, slice, description,
			"systemd-logind.service",
			"systemd-user-sessions.service",
			u->manager->user_tasks_max, &error, &job);
		if (r < 0) {
			if (sd_bus_error_has_name(&error,
				...
				log_error_errno(r,
					...
					slice, bus_error_message(&error, r),
					error.name);
				free(slice);
...
user_stop_slice(User *u)
{
	...
	if (r < 0) {
		log_error("Failed to stop user slice: %s",
...
user_stop_service(User *u)
{
	...
	if (r < 0) {
		log_error("Failed to stop user service: %s",
			bus_error_message(&error, r));
		return r;
	}
...
static int
user_remove_runtime_path(User *u)
{
	int r;
...
int
user_get_idle_hint(User *u, dual_timestamp *t)
{
	Session *s;
	bool idle_hint = true;
...
user_check_linger_file(User *u)
{
	...
	p = strjoina(SVC_PKGLOCALSTATEDIR "/linger/", cc);

	return access(p, F_OK) >= 0;
}

bool
user_check_gc(User *u, bool drop_not_started)
{
	assert(u);
...
user_add_to_gc_queue(User *u)
{
	...
	IWLIST_PREPEND(gc_queue, u->manager->user_gc_queue, u);
	u->in_gc_queue = true;
}
...
user_elect_display(User *u)
{
	...
	if (graphical &&
		(!u->display || u->display->class != SESSION_USER ||
			u->display->stopping ||
...
int
config_parse_tmpfs_size(const char *unit, const char *filename, unsigned line,
	...
	if (e) {
		...
		if (errno != 0 || f != e) {
			log_syntax(unit, LOG_ERR, filename, line,
				errno ? errno : EINVAL,
				"Failed to parse percentage value, ignoring: %s",
				rvalue);
			return 0;
		}
		...
		if (ul <= 0 || ul >= 100) {
			log_syntax(unit, LOG_ERR, filename, line,
				errno ? errno : EINVAL,
				"Percentage value out of range, ignoring: %s",
				rvalue);
			return 0;
		}

		*sz = PAGE_ALIGN((size_t)((physical_memory() * (uint64_t)ul) /
			(uint64_t)100));
	} else {
		off_t o;
		...
		if (r < 0 || (off_t)(size_t)o != o) {
			log_syntax(unit, LOG_ERR, filename, line,
				r < 0 ? -r : ERANGE,
				"Failed to parse size value, ignoring: %s",
				rvalue);
			return 0;
		}
