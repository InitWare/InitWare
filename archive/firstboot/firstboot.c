...
print_welcome(void)
{
	...
	r = parse_env_file(os_release, NEWLINE, "PRETTY_NAME", &pretty_name,
		NULL);
	if (r == -ENOENT) {
		os_release = prefix_roota(arg_root, "/usr/lib/os-release");
		r = parse_env_file(os_release, NEWLINE, "PRETTY_NAME",
			&pretty_name, NULL);
	}
...
static int
show_menu(char **x, unsigned n_columns, unsigned width, unsigned percentage)
{
...
static int
prompt_loop(const char *text, char **l, bool (*is_valid)(const char *name),
	char **ret)
{
	int r;
	...
	for (;;) {
		...
		r = ask_string(&p, "%s %s (empty to skip): ",
			draw_special_char(DRAW_TRIANGULAR_BULLET), text);
		if (r < 0)
		...
		if (r >= 0) {
			...
			if (u <= 0 || u > strv_length(l)) {
				log_error(
					"Specified entry number out of range.");
				continue;
			}
...
prompt_locale(void)
{
	...
	r = prompt_loop("Please enter system locale name or number", locales,
		locale_is_valid, &arg_locale);
	if (r < 0)
		return r;
	...
	r = prompt_loop("Please enter system message locale name or number",
		locales, locale_is_valid, &arg_locale_messages);
	if (r < 0)
		return r;
...
process_locale(void)
{
	...
	if (!isempty(arg_locale_messages) &&
		!streq(arg_locale_messages, arg_locale))
...
prompt_timezone(void)
{
	...
	r = prompt_loop("Please enter timezone name or number", zones,
		timezone_is_valid, &arg_timezone);
	if (r < 0)
		return r;
...
process_timezone(void)
{
	...
	if (symlink(e, etc_localtime) < 0)
		return log_error_errno(errno, "Failed to create %s symlink: %m",
			etc_localtime);
...
static int
write_root_shadow(const char *path, const struct spwd *p)
{
	...
	while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
		...
		case ARG_MACHINE_ID:
			if (sd_id128_from_string(optarg, &arg_machine_id) < 0) {
				log_error("Failed to parse machine id %s.",
					optarg);
				return -EINVAL;
			}
		...
		case ARG_PROMPT:
			arg_prompt_locale = arg_prompt_timezone =
				arg_prompt_hostname = arg_prompt_root_password =
					true;
			break;
