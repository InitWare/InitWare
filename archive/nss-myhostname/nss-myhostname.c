...
static bool
is_gateway(const char *hostname)
{
	assert(hostname);

	return streq(hostname, "gateway") || streq(hostname, "gateway.");
}

enum nss_status
_nss_myhostname_gethostbyname4_r(const char *name, struct gaih_addrtuple **pat,
	char *buffer, size_t buflen, int *errnop, int *h_errnop, int32_t *ttlp)
{
	...
	for (a = addresses + n_addresses - 1, n = 0; (int)n < n_addresses;
		n++, a--) {
		r_tuple = (struct gaih_addrtuple *)(buffer + idx);
		r_tuple->next = r_tuple_prev;
		r_tuple->name = r_name;
...
fill_in_hostent(const char *canonical, const char *additional, int af,
	struct local_address *addresses, unsigned n_addresses,
	uint32_t local_address_ipv4, struct hostent *result, char *buffer,
	size_t buflen, int *errnop, int *h_errnop, int32_t *ttlp, char **canonp)
{
	...
	ms = ALIGN(l_canonical + 1) +
		(additional ? ALIGN(l_additional + 1) : 0) + sizeof(char *) +
...
enum nss_status
_nss_myhostname_gethostbyname3_r(const char *name, int af, struct hostent *host,
	char *buffer, size_t buflen, int *errnop, int *h_errnop, int32_t *ttlp,
	char **canonp)
{
	_cleanup_free_ struct local_address *addresses = NULL;
	...
	if (is_localhost(name)) {
		...
		canonical = hn;
		additional =
			n_addresses <= 0 && af == AF_INET6 ? "localhost" : NULL;
		local_address_ipv4 = LOCALADDRESS_IPV4;
	}

	return fill_in_hostent(canonical, additional, af, addresses,
		n_addresses, local_address_ipv4, host, buffer, buflen, errnop,
		h_errnop, ttlp, canonp);
}

enum nss_status
_nss_myhostname_gethostbyaddr2_r(const void *addr, socklen_t len, int af,
