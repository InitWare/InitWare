...
int
main_analyze(const char *pack_path)
{
	char line[LINE_MAX];
	...
	for (;;) {
		...
		if (stat(path, &st) == 0) {
			...
			printf("  %4jd%% (%2d) %12jd: %s\n",
				(intmax_t)(sections && st.st_size ?
						      size * 100 / st.st_size :
						      100),
				sections ? sections : 1, (intmax_t)size, path);
		} else {
			printf("  %4dp (%2d) %12s: %s (MISSING)\n",
				sections ? pages : -1, sections ? sections : 1,
				"???", path);
			missing++;
		}
	}
