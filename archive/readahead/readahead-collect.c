...
static int
pack_file(FILE *pack, const char *fn, bool on_btrfs)
{
	struct stat st;
...
collect(const char *root)
{
	...
	previous_block_readahead_set = access(pack_fn, F_OK) < 0 &&
	...
	if (fanotify_mark(fanotify_fd, FAN_MARK_ADD | FAN_MARK_MOUNT, FAN_OPEN,
		    AT_FDCWD, root) < 0) {
	...
	for (;;) {
		...
		if ((h = poll(pollfd, _FD_MAX,
			     (int)((not_after - t) / USEC_PER_MSEC))) < 0) {
			if (errno == EINTR)
				continue;
		...
		if (pollfd[FD_INOTIFY].revents) {
			uint8_t inotify_buffer[sizeof(struct inotify_event) +
				FILENAME_MAX];
			struct inotify_event *e;

			if ((n = read(inotify_fd, &inotify_buffer,
				     sizeof(inotify_buffer))) < 0) {
		...
		for (m = &data.metadata; FAN_EVENT_OK(m, n);
			...
			if (k >= 0) {
				...
				log_warning("readlink(%s) failed: %s", fn,
					strerror(-k));
	...
	on_btrfs = statfs(root, &sfs) >= 0 &&
		F_TYPE_EQUAL(sfs.f_type, BTRFS_SUPER_MAGIC);
