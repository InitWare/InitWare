...
int
file_verify(int fd, const char *fn, off_t file_size_max, struct stat *st)
{
	assert(fd >= 0);
	assert(fn);
	assert(st);
	...
	if (st->st_size <= 0 || st->st_size > file_size_max) {
		...
		log_debug(
			"Not preloading file %s with size out of bounds %" PRIu64,
			fn, st->st_size);
		return 0;
	}
...
shared_get(void)
{
	...
	fd = open("/run/systemd/readahead/shared", O_CREAT | O_RDWR | O_CLOEXEC,
		0644);
	if (fd < 0) {
	...
	m = mmap(NULL, sizeof(ReadaheadShared), PROT_WRITE | PROT_READ,
		MAP_SHARED, fd, 0);
	if (m == MAP_FAILED) {
...
int
block_get_readahead(const char *p, uint64_t *bytes)
{
	struct stat st;
	...
	if (asprintf(&ap, "/sys/dev/block/%u:%u/bdi/read_ahead_kb", major(d),
		    minor(d)) < 0) {
		r = -ENOMEM;
		goto finish;
	}
...
int
block_set_readahead(const char *p, uint64_t bytes)
{
	struct stat st;
	char *ap = NULL, *line = NULL;
	int r;
