...
static double
ntp_ts_short_to_d(const struct ntp_ts_short *ts)
{
	return be16toh(ts->sec) + (be16toh(ts->frac) / 65536.0);
}

static double
ntp_ts_to_d(const struct ntp_ts *ts)
{
	return be32toh(ts->sec) + ((double)be32toh(ts->frac) / UINT_MAX);
}

static double
ts_to_d(const struct timespec *ts)
{
	return ts->tv_sec + (1.0e-9 * ts->tv_nsec);
}

static int
manager_timeout(sd_event_source *source, usec_t usec, void *userdata)
{
...
static int
manager_send_request(Manager *m)
{
	_cleanup_free_ char *pretty = NULL;
	...
	/*
         ...
         */
	assert_se(clock_gettime(clock_boottime_or_monotonic(),
			  &m->trans_time_mon) >= 0);
	assert_se(clock_gettime(CLOCK_REALTIME, &m->trans_time) >= 0);
	ntpmsg.trans_time.sec =
		htobe32(m->trans_time.tv_sec + OFFSET_1900_1970);
...
static int
manager_timer(sd_event_source *source, usec_t usec, void *userdata)
{
	Manager *m = userdata;
...
static int
manager_arm_timer(Manager *m, usec_t next)
{
	int r;
	...
	if (m->event_timer) {
		...
		return sd_event_source_set_enabled(m->event_timer,
			SD_EVENT_ONESHOT);
	}

	return sd_event_add_time(m->event, &m->event_timer,
		clock_boottime_or_monotonic(),
		now(clock_boottime_or_monotonic()) + next, 0, manager_timer, m);
}

static int
manager_clock_watch(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	Manager *m = userdata;
...
static int
manager_clock_watch_setup(Manager *m)
{
	...
	if (timerfd_settime(m->clock_watch_fd,
...
static int
manager_adjust_clock(Manager *m, double offset, int leap_sec)
{
	struct timex tmx = {};
	int r;
	...
	log_debug("  status       : %04i %s\n"
		...
		tmx.time.tv_sec,
		(unsigned long long)(tmx.time.tv_usec / NSEC_PER_MSEC),
		tmx.constant, (double)tmx.offset / NSEC_PER_SEC, tmx.freq,
		m->drift_ppm);
...
static bool
manager_sample_spike_detection(Manager *m, double offset, double delay)
{
...
static void
manager_adjust_poll(Manager *m, double offset, bool spike)
{
	assert(m);
...
static int
manager_receive_response(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	Manager *m = userdata;
	struct ntp_msg ntpmsg;
	...
	if (!m->current_server_name || !m->current_server_address ||
		!sockaddr_equal(&server_addr,
			&m->current_server_address->sockaddr)) {
	...
	if (be32toh(ntpmsg.recv_time.sec) < TIME_EPOCH + OFFSET_1900_1970 ||
		be32toh(ntpmsg.trans_time.sec) <
			TIME_EPOCH + OFFSET_1900_1970) {
		log_debug(
			"Invalid reply, returned times before epoch. Ignoring.");
		return manager_connect(m);
	}
	...
	log_debug("NTP response:\n"
		...
		trans - OFFSET_1900_1970, dest - OFFSET_1900_1970, offset,
		delay, m->packet_count, m->samples_jitter,
		spike ? " spike" : "", m->poll_interval_usec / USEC_PER_SEC);
	...
	log_debug("interval/delta/delay/jitter/drift " USEC_FMT
		...
		m->samples_jitter, m->drift_ppm, spike ? " (ignored)" : "");
...
static int
manager_listen_setup(Manager *m)
{
	...
	m->server_socket =
		socket(addr.sa.sa_family, SOCK_DGRAM | SOCK_CLOEXEC, 0);
	if (m->server_socket < 0)
		return -errno;

	r = bind(m->server_socket, &addr.sa,
		m->current_server_address->socklen);
	if (r < 0)
		return -errno;
	...
	(void)setsockopt(m->server_socket, IPPROTO_IP, IP_TOS, &tos,
		sizeof(tos));
...
void
manager_set_server_name(Manager *m, ServerName *n)
{
	assert(m);
...
void
manager_set_server_address(Manager *m, ServerAddress *a)
{
	assert(m);
...
static int
manager_resolve_handler(sd_resolve_query *q, int ret, const struct addrinfo *ai,
	void *userdata)
{
	Manager *m = userdata;
	int r;
	...
	if (ret != 0) {
		log_debug("Failed to resolve %s: %s",
			m->current_server_name->string, gai_strerror(ret));
...
static int
manager_retry_connect(sd_event_source *source, usec_t usec, void *userdata)
{
	Manager *m = userdata;
...
manager_connect(Manager *m)
{
	...
	if (!ratelimit_test(&m->ratelimit)) {
		...
		r = sd_event_add_time(m->event, &m->event_retry,
			clock_boottime_or_monotonic(),
			now(clock_boottime_or_monotonic()) + RETRY_USEC, 0,
	...
	if (m->current_server_address &&
		...
		r = sd_resolve_getaddrinfo(m->resolve, &m->resolve_query,
			m->current_server_name->string, "123", &hints,
			manager_resolve_handler, m);
		if (r < 0)
			return log_error_errno(r,
...
void
manager_flush_server_names(Manager *m, ServerType t)
{
	assert(m);
...
static int
manager_network_read_link_servers(Manager *m)
{
...
static int
manager_network_event_handler(sd_event_source *s, int fd, uint32_t revents,
	void *userdata)
{
	Manager *m = userdata;
...
static int
manager_network_monitor_listen(Manager *m)
{
	int r, fd, events;
