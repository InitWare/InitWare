...
static int
process(const char *controller, const char *path, Hashmap *a, Hashmap *b,
	unsigned iteration)
{
	Group *g;
	int r;
	...
	if (streq(controller, "cpuacct")) {
		...
		if (g->cpu_iteration == iteration - 1) {
			...
			x = ((uint64_t)ts.tv_sec * 1000000000ULL +
				    (uint64_t)ts.tv_nsec) -
				((uint64_t)g->cpu_timestamp.tv_sec *
						1000000000ULL +
					(uint64_t)g->cpu_timestamp.tv_nsec);
		...
		if (g->io_iteration == iteration - 1) {
			...
			x = ((uint64_t)ts.tv_sec * 1000000000ULL +
				    (uint64_t)ts.tv_nsec) -
				((uint64_t)g->io_timestamp.tv_sec *
						1000000000ULL +
					(uint64_t)g->io_timestamp.tv_nsec);
...
static int
refresh_one(const char *controller, const char *path, Hashmap *a, Hashmap *b,
...
static int
refresh(Hashmap *a, Hashmap *b, unsigned iteration)
{
	int r;
...
static int
group_compare(const void *a, const void *b)
{
	...
	if (arg_order == ORDER_IO) {
		if (x->io_valid && y->io_valid) {
			if (x->io_input_bps + x->io_output_bps >
				y->io_input_bps + y->io_output_bps)
				return -1;
			else if (x->io_input_bps + x->io_output_bps <
				y->io_input_bps + y->io_output_bps)
				return 1;
		} else if (x->io_valid)
			return -1;
...
display(Hashmap *a)
{
	...
	HASHMAP_FOREACH (g, a, i)
		if (g->n_tasks_valid || g->cpu_valid || g->memory_valid ||
			g->io_valid)
			array[n++] = g;
	...
	for (j = 0; j < n; j++) {
		...
		format_timespan(buffer, sizeof(buffer),
			(nsec_t)(array[j]->cpu_usage / NSEC_PER_USEC), 0);
	...
	for (j = 0; j < n; j++) {
		...
		if (arg_cpu_type == CPU_PERCENT) {
			...
			printf(" %*s", maxtcpu,
				format_timespan(buffer, sizeof(buffer),
					(nsec_t)(g->cpu_usage / NSEC_PER_USEC),
					0));
...
{
	enum { ARG_VERSION = 0x100, ARG_DEPTH, ARG_CPU_TYPE };
...
main(int argc, char *argv[])
{
	...
	while (!quit) {
		...
		case '%':
			arg_cpu_type = arg_cpu_type == CPU_TIME ? CPU_PERCENT :
									CPU_TIME;
			break;
		...
		case 'h':
			fprintf(stdout,
				...
				"> By tasks; <" ON "c" OFF "> By CPU; <" ON
				"m" OFF "> By memory; <" ON "i" OFF "> By I/O\n"
				"\t<" ON "+" OFF "> Increase delay; <" ON
				"-" OFF "> Decrease delay; <" ON "%%" OFF
				"> Toggle time\n"
				"\t<" ON "q" OFF "> Quit; <" ON "SPACE" OFF
				"> Refresh");
			fflush(stdout);
			sleep(3);
			break;
