...
static int
apply_sysctl(const char *property, const char *value)
{
	...
	if (k < 0) {
		log_full(k == -ENOENT ? LOG_DEBUG : LOG_WARNING,
			"Failed to write '%s' to '%s': %s", value, p,
			strerror(-k));
...
static int
apply_all(OrderedHashmap *sysctl_options)
{
	int r = 0;
	char *property, *value;
	Iterator i;
...
static int
parse_file(OrderedHashmap *sysctl_options, const char *path, bool ignore_enoent)
{
	...
	while (!feof(f)) {
		...
		value = strchr(p, '=');
		if (!value) {
			log_error("Line is not an assignment in file '%s': %s",
				path, value);
	...
	found:
		...
		if (existing) {
			...
			log_debug(
				"Overwriting earlier assignment of %s in file '%s'.",
				p, path);
		...
		if (k < 0) {
			log_error_errno(k,
				"Failed to add sysctl variable %s to hashmap: %m",
				property);
			free(property);
			free(new_value);
			return k;
...
{
	enum { ARG_VERSION = 0x100, ARG_PREFIX };
