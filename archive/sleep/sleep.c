...
static int
write_state(FILE **f, char **states)
{
	char **state;
	int r = 0;
...
static int
execute(char **modes, char **states)
{
	...
	log_struct(LOG_INFO, LOG_MESSAGE_ID(SD_MESSAGE_SLEEP_START),
	...
	log_struct(LOG_INFO, LOG_MESSAGE_ID(SD_MESSAGE_SLEEP_STOP),
		LOG_MESSAGE("System resumed."), "SLEEP=%s", arg_verb, NULL);
...
parse_argv(int argc, char *argv[])
{
	...
	if (argc - optind != 1) {
		log_error("Usage: %s COMMAND", program_invocation_short_name);
		return -EINVAL;
	}

	arg_verb = argv[optind];

	if (!streq(arg_verb, "suspend") && !streq(arg_verb, "hibernate") &&
