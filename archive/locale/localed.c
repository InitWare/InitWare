...
static const char *const names[_LOCALE_MAX] = { [LOCALE_LANG] = "LANG",
...
static void
free_and_replace(char **s, char *v)
{
	free(*s);
	*s = v;
}
...
static bool
startswith_comma(const char *s, const char *prefix)
{
	const char *t;
...
static void
context_free_vconsole(Context *c)
{
...
static void
locale_simplify(Context *c)
{
	int p;

	for (p = LOCALE_LANG + 1; p < _LOCALE_MAX; p++)
		if (isempty(c->locale[p]) ||
			streq_ptr(c->locale[LOCALE_LANG], c->locale[p]))
			free_and_replace(&c->locale[p], NULL);
}

static int
locale_read_data(Context *c)
{
	int r;

	context_free_locale(c);

	r = parse_env_file("/etc/locale.conf", NEWLINE, "LANG",
		&c->locale[LOCALE_LANG], "LANGUAGE",
		&c->locale[LOCALE_LANGUAGE], "LC_CTYPE",
		&c->locale[LOCALE_LC_CTYPE], "LC_NUMERIC",
		&c->locale[LOCALE_LC_NUMERIC], "LC_TIME",
		&c->locale[LOCALE_LC_TIME], "LC_COLLATE",
		&c->locale[LOCALE_LC_COLLATE], "LC_MONETARY",
		&c->locale[LOCALE_LC_MONETARY], "LC_MESSAGES",
		&c->locale[LOCALE_LC_MESSAGES], "LC_PAPER",
		&c->locale[LOCALE_LC_PAPER], "LC_NAME",
		&c->locale[LOCALE_LC_NAME], "LC_ADDRESS",
		&c->locale[LOCALE_LC_ADDRESS], "LC_TELEPHONE",
		&c->locale[LOCALE_LC_TELEPHONE], "LC_MEASUREMENT",
		&c->locale[LOCALE_LC_MEASUREMENT], "LC_IDENTIFICATION",
		&c->locale[LOCALE_LC_IDENTIFICATION], NULL);
...
static int
vconsole_read_data(Context *c)
{
	int r;

	context_free_vconsole(c);

	r = parse_env_file("/etc/vconsole.conf", NEWLINE, "KEYMAP",
		&c->vc_keymap, "KEYMAP_TOGGLE", &c->vc_keymap_toggle, NULL);
...
static int
locale_write_data(Context *c, char ***settings)
{
	int r, p;
...
static int
locale_update_system_manager(Context *c, sd_bus *bus)
{
...
static int
x11_write_data(Context *c)
{
	_cleanup_fclose_ FILE *f = NULL;
	_cleanup_free_ char *temp_path = NULL;
	int r;

	if (isempty(c->x11_layout) && isempty(c->x11_model) &&
		isempty(c->x11_variant) && isempty(c->x11_options)) {
	...
	if (ferror(f) ||
		rename(temp_path, "/etc/X11/xorg.conf.d/00-keyboard.conf") <
			0) {
		r = -errno;
...
static const char *
strnulldash(const char *s)
{
	return isempty(s) || streq(s, "-") ? NULL : s;
}

static int
read_next_mapping(const char *filename, unsigned min_fields,
	unsigned max_fields, FILE *f, unsigned *n, char ***a)
{
	assert(f);
	assert(n);
	assert(a);
...
static int
vconsole_convert_to_x11(Context *c, sd_bus *bus)
{
	bool modified = false;
	...
	if (isempty(c->vc_keymap)) {
		modified = !isempty(c->x11_layout) || !isempty(c->x11_model) ||
			!isempty(c->x11_variant) || !isempty(c->x11_options);
		...
		for (;;) {
			...
			if (!streq_ptr(c->x11_layout, strnulldash(a[1])) ||
				...
				if (free_and_strdup(&c->x11_layout,
					    strnulldash(a[1])) < 0 ||
					free_and_strdup(&c->x11_model,
						strnulldash(a[2])) < 0 ||
					free_and_strdup(&c->x11_variant,
						strnulldash(a[3])) < 0 ||
					free_and_strdup(&c->x11_options,
						strnulldash(a[4])) < 0)
					return -ENOMEM;
	...
	if (modified) {
		...
		log_info(
			...
			strempty(c->x11_layout), strempty(c->x11_model),
			strempty(c->x11_variant), strempty(c->x11_options));

		sd_bus_emit_properties_changed(bus, "/org/freedesktop/locale1",
			"org.freedesktop.locale1", "X11Layout", "X11Model",
			"X11Variant", "X11Options", NULL);
	} else
...
static int
find_converted_keymap(const char *x11_layout, const char *x11_variant,
	char **new_keymap)
{
	const char *dir;
	...
	NULSTR_FOREACH (dir, KBD_KEYMAP_DIRS) {
		...
		if (uncompressed || access(pz, F_OK) == 0) {
			log_debug("Found converted keymap %s at %s", n,
				uncompressed ? p : pz);
...
static int
find_legacy_keymap(Context *c, char **new_keymap)
{
	_cleanup_fclose_ FILE *f;
	unsigned n = 0;
	...
	for (;;) {
		...
		r = read_next_mapping(SYSTEMD_KBD_MODEL_MAP, 5, UINT_MAX, f, &n,
			&a);
		if (r < 0)
			return r;
		if (r == 0)
		...
		if (matching >= MAX(best_matching, 1u)) {
			log_debug("Found legacy keymap %s with score %u", a[0],
				matching);
...
static int
find_language_fallback(const char *lang, char **language)
{
	_cleanup_fclose_ FILE *f = NULL;
	unsigned n = 0;
...
static int
x11_convert_to_vconsole(Context *c, sd_bus *bus)
{
	bool modified = false;
	int r;
	...
	if (isempty(c->x11_layout)) {
		modified =
			!isempty(c->vc_keymap) || !isempty(c->vc_keymap_toggle);

		context_free_x11(c);
	} else {
		char *new_keymap = NULL;

		r = find_converted_keymap(c->x11_layout, c->x11_variant,
			&new_keymap);
		if (r < 0)
			return r;
		else if (r == 0) {
...
static int
property_get_locale(sd_bus *bus, const char *path, const char *interface,
...
static int
method_set_locale(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
	_cleanup_strv_free_ char **l = NULL;
	...
	if (have[LOCALE_LANG] && !have[LOCALE_LANGUAGE]) {
		...
		if (language) {
			log_debug("Converted LANG=%s to LANGUAGE=%s", lang,
				language);
	...
	if (modified) {
		...
		sd_bus_emit_properties_changed(bus, "/org/freedesktop/locale1",
			"org.freedesktop.locale1", "Locale", NULL);
	} else
...
static int
method_set_vc_keyboard(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
	...
	r = sd_bus_message_read(m, "ssbb", &keymap, &keymap_toggle, &convert,
		&interactive);
	if (r < 0)
		return r;
	...
	if (!streq_ptr(keymap, c->vc_keymap) ||
		...
		if (free_and_strdup(&c->vc_keymap, keymap) < 0 ||
			free_and_strdup(&c->vc_keymap_toggle, keymap_toggle) <
				0)
			return -ENOMEM;
		...
		if (r < 0) {
			log_error_errno(r,
				"Failed to set virtual console keymap: %m");
			return sd_bus_error_set_errnof(error, r,
				"Failed to set virtual console keymap: %s",
				strerror(-r));
		}
...
#ifdef HAVE_XKBCOMMON
static void
log_xkb(struct xkb_context *ctx, enum xkb_log_level lvl, const char *format,
	va_list args)
{
	const char *fmt;
...
static int
verify_xkb_rmlvo(const char *model, const char *layout, const char *variant,
	const char *options)
{
...
static int
method_set_x11_keyboard(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
	...
	r = sd_bus_message_read(m, "ssssbb", &layout, &model, &variant,
	...
	if (!streq_ptr(layout, c->x11_layout) ||
		...
		if (r < 0) {
			log_error_errno(r,
				"Failed to set X11 keyboard layout: %m");
			return sd_bus_error_set_errnof(error, r,
				"Failed to set X11 keyboard layout: %s",
				strerror(-r));
		}
...
static const sd_bus_vtable locale_vtable[] = { SD_BUS_VTABLE_START(0),
	SD_BUS_PROPERTY("Locale", "as", property_get_locale, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("X11Layout", "s", NULL, offsetof(Context, x11_layout),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("X11Model", "s", NULL, offsetof(Context, x11_model),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("X11Variant", "s", NULL, offsetof(Context, x11_variant),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("X11Options", "s", NULL, offsetof(Context, x11_options),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("VConsoleKeymap", "s", NULL,
		offsetof(Context, vc_keymap),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("VConsoleKeymapToggle", "s", NULL,
		offsetof(Context, vc_keymap_toggle),
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_METHOD("SetLocale", "asb", NULL, method_set_locale,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("SetVConsoleKeyboard", "ssbb", NULL,
		method_set_vc_keyboard, SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("SetX11Keyboard", "ssssbb", NULL, method_set_x11_keyboard,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_VTABLE_END };
...
connect_bus(Context *c, sd_event *event, sd_bus **_bus)
{
	...
	r = sd_bus_add_object_vtable(bus, NULL, "/org/freedesktop/locale1",
		"org.freedesktop.locale1", locale_vtable, c);
	if (r < 0)
...
main(int argc, char *argv[])
{
	...
	r = bus_event_loop_with_idle(event, bus, "org.freedesktop.locale1",
