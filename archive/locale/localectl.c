...
static void
print_overriden_variables(void)
{
	int r;
	...
	r = parse_env_file("/proc/cmdline", WHITESPACE, "locale.LANG",
		&variables[VARIABLE_LANG], "locale.LANGUAGE",
		&variables[VARIABLE_LANGUAGE], "locale.LC_CTYPE",
		...
		&variables[VARIABLE_LC_IDENTIFICATION], NULL);
...
static void
print_status_info(StatusInfo *i)
{
	assert(i);
...
static int
show_status(sd_bus *bus, char **args, unsigned n)
{
	StatusInfo info = {};
	static const struct bus_properties_map map[] = {
		{ "VConsoleKeymap", "s", NULL,
			offsetof(StatusInfo, vconsole_keymap) },
		{ "VConsoleKeymap", "s", NULL,
			offsetof(StatusInfo, vconsole_keymap) },
		{ "VConsoleKeymapToggle", "s", NULL,
			offsetof(StatusInfo, vconsole_keymap_toggle) },
	...
	r = bus_map_all_properties(bus, "org.freedesktop.locale1",
		"/org/freedesktop/locale1", map, &info);
	if (r < 0) {
...
static int
set_locale(sd_bus *bus, char **args, unsigned n)
{
	...
	r = sd_bus_message_new_method_call(bus, &m, "org.freedesktop.locale1",
...
static int
list_locales(sd_bus *bus, char **args, unsigned n)
{
...
static int
set_vconsole_keymap(sd_bus *bus, char **args, unsigned n)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	...
	r = sd_bus_call_method(bus, "org.freedesktop.locale1",
		"/org/freedesktop/locale1", "org.freedesktop.locale1",
		"SetVConsoleKeyboard", &error, NULL, "ssbb", map, toggle_map,
...
static int
nftw_cb(const char *fpath, const struct stat *sb, int tflag, struct FTW *ftwbuf)
{
	char *p, *e;
	int r;
...
static int
list_vconsole_keymaps(sd_bus *bus, char **args, unsigned n)
{
	_cleanup_strv_free_ char **l = NULL;
	const char *dir;
...
static int
set_x11_keymap(sd_bus *bus, char **args, unsigned n)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	...
	r = sd_bus_call_method(bus, "org.freedesktop.locale1",
		"/org/freedesktop/locale1", "org.freedesktop.locale1",
		"SetX11Keyboard", &error, NULL, "ssssbb", layout, model,
		variant, options, arg_convert, arg_ask_password);
	if (r < 0)
		log_error("Failed to set keymap: %s",
			bus_error_message(&error, -r));
...
static int
list_x11_keymaps(sd_bus *bus, char **args, unsigned n)
{
...
static int
localectl_main(sd_bus *bus, int argc, char *argv[])
{
	static const struct {
		const char *verb;
