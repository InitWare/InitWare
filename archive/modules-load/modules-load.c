...
static void
systemd_kmod_log(void *data, int priority, const char *file, int line,
...
static int
load_module(struct kmod_ctx *ctx, const char *m)
{
	...
	kmod_list_foreach(itr, modlist)
	{
		...
		case KMOD_MODULE_BUILTIN:
			log_info("Module '%s' is builtin",
				kmod_module_get_name(mod));
			break;

		case KMOD_MODULE_LIVE:
			log_debug("Module '%s' is already loaded",
				kmod_module_get_name(mod));
			break;
...
static int
apply_file(struct kmod_ctx *ctx, const char *path, bool ignore_enoent)
{
...
main(int argc, char *argv[])
{
	...
	if (argc > optind) {
		...
		k = conf_files_list_nulstr(&files, ".conf", NULL,
			conf_file_dirs);
		if (k < 0) {
			log_error_errno(k,
