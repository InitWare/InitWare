...
static int
connection_create_pipes(Connection *c, int buffer[2], size_t *sz)
{
	int r;
...
static int
connection_shovel(Connection *c, int *from, int buffer[2], int *to,
	size_t *full, size_t *sz, sd_event_source **from_source,
...
static int
traffic_cb(sd_event_source *s, int fd, uint32_t revents, void *userdata)
{
	...
	r = connection_shovel(c, &c->server_fd, c->server_to_client_buffer,
		...
		&c->server_to_client_buffer_size, &c->server_event_source,
	...
	r = connection_shovel(c, &c->client_fd, c->client_to_server_buffer,
		...
		&c->client_to_server_buffer_size, &c->client_event_source,
...
static int
connection_enable_event_sources(Connection *c)
{
	uint32_t a = 0, b = 0;
	int r;
...
static int
connection_complete(Connection *c)
{
	int r;
...
static int
connect_cb(sd_event_source *s, int fd, uint32_t revents, void *userdata)
{
	Connection *c = userdata;
	socklen_t solen;
	int error, r;
...
static int
connection_start(Connection *c, struct sockaddr *sa, socklen_t salen)
{
	int r;
	...
	c->client_fd = socket(sa->sa_family,
	...
	if (r < 0) {
		if (errno == EINPROGRESS) {
			r = sd_event_add_io(c->context->event,
				&c->client_event_source, c->client_fd, EPOLLOUT,
				connect_cb, c);
			if (r < 0) {
				log_error_errno(r,
...
static int
resolve_cb(sd_resolve_query *q, int ret, const struct addrinfo *ai,
	void *userdata)
{
	Connection *c = userdata;
...
resolve_remote(Connection *c)
{
	...
	if (path_is_absolute(arg_remote_host)) {
		...
		strncpy(sa.un.sun_path, arg_remote_host,
	...
	if (arg_remote_host[0] == '@') {
		sa.un.sun_family = AF_UNIX;
		sa.un.sun_path[0] = 0;
		strncpy(sa.un.sun_path + 1, arg_remote_host + 1,
			sizeof(sa.un.sun_path) - 2);
		sa.un.sun_path[sizeof(sa.un.sun_path) - 1] = 0;
	...
	r = sd_resolve_getaddrinfo(c->context->resolve, &c->resolve_query, node,
		service, &hints, resolve_cb, c);
	if (r < 0) {
...
static int
add_connection_socket(Context *context, int fd)
{
	Connection *c;
	int r;
...
static int
accept_cb(sd_event_source *s, int fd, uint32_t revents, void *userdata)
{
...
static int
add_listen_socket(Context *context, int fd)
{
...
{
	enum { ARG_VERSION = 0x100, ARG_IGNORE_ENV };
