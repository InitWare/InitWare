...
static int
show_one_name(sd_bus *bus, const char *attr)
{
	...
	r = sd_bus_get_property(bus, "org.freedesktop.hostname1",
		"/org/freedesktop/hostname1", "org.freedesktop.hostname1", attr,
		&error, &reply, "s");
	if (r < 0) {
...
static int
show_all_names(sd_bus *bus)
{
	StatusInfo info = {};

	static const struct bus_properties_map hostname_map[] = {
		{ "Hostname", "s", NULL, offsetof(StatusInfo, hostname) },
		{ "StaticHostname", "s", NULL,
			offsetof(StatusInfo, static_hostname) },
		{ "PrettyHostname", "s", NULL,
			offsetof(StatusInfo, pretty_hostname) },
		...
		{ "KernelRelease", "s", NULL,
			offsetof(StatusInfo, kernel_release) },
	...
	static const struct bus_properties_map manager_map[] = {
		{ "Virtualization", "s", NULL,
			offsetof(StatusInfo, virtualization) },
		{ "Architecture", "s", NULL,
			offsetof(StatusInfo, architecture) },
		{}
	};
	...
	r = bus_map_all_properties(bus, "org.freedesktop.hostname1",
		"/org/freedesktop/hostname1", hostname_map, &info);
	if (r < 0)
		goto fail;
...
static int
set_simple_string(sd_bus *bus, const char *method, const char *value)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	int r = 0;
	...
	r = sd_bus_call_method(bus, "org.freedesktop.hostname1",
		...
		method, &error, NULL, "sb", value, arg_ask_password);
	if (r < 0)
...
static int
set_hostname(sd_bus *bus, char **args, unsigned n)
{
...
static int
set_icon_name(sd_bus *bus, char **args, unsigned n)
{
	assert(args);
	assert(n == 2);
...
static int
set_chassis(sd_bus *bus, char **args, unsigned n)
{
	assert(args);
	assert(n == 2);
...
static int
set_deployment(sd_bus *bus, char **args, unsigned n)
{
	assert(args);
	assert(n == 2);
...
static int
set_location(sd_bus *bus, char **args, unsigned n)
{
	assert(args);
	assert(n == 2);
...
static int
hostnamectl_main(sd_bus *bus, int argc, char *argv[])
{
	static const struct {
		const char *verb;
