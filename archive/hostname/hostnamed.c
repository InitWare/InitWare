...
context_read_data(Context *c)
{
	...
	r = parse_env_file("/etc/machine-info", NEWLINE, "PRETTY_HOSTNAME",
		&c->data[PROP_PRETTY_HOSTNAME], "ICON_NAME",
		&c->data[PROP_ICON_NAME], "CHASSIS", &c->data[PROP_CHASSIS],
		"DEPLOYMENT", &c->data[PROP_DEPLOYMENT], "LOCATION",
		&c->data[PROP_LOCATION], NULL);
	if (r < 0 && r != -ENOENT)
		return r;

	r = parse_env_file("/etc/os-release", NEWLINE, "PRETTY_NAME",
		&c->data[PROP_OS_PRETTY_NAME], "CPE_NAME",
		&c->data[PROP_OS_CPE_NAME], NULL);
	if (r == -ENOENT) {
		r = parse_env_file("/usr/lib/os-release", NEWLINE,
			"PRETTY_NAME", &c->data[PROP_OS_PRETTY_NAME],
			"CPE_NAME", &c->data[PROP_OS_CPE_NAME], NULL);
	}
...
static bool
valid_chassis(const char *chassis)
{
	assert(chassis);
...
static bool
valid_deployment(const char *deployment)
{
	assert(deployment);
...
static char *
context_fallback_icon_name(Context *c)
{
	const char *chassis;
...
static bool
hostname_is_useful(const char *hn)
{
	return !isempty(hn) && !is_localhost(hn);
}

static int
context_update_kernel_hostname(Context *c)
{
...
static int
context_write_data_static_hostname(Context *c)
{
	assert(c);
...
static int
context_write_data_machine_info(Context *c)
{
...
static int
property_get_icon_name(sd_bus *bus, const char *path, const char *interface,
...
static int
property_get_chassis(sd_bus *bus, const char *path, const char *interface,
	const char *property, sd_bus_message *reply, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
...
static int
method_set_hostname(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
	const char *name;
...
static int
method_set_static_hostname(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	Context *c = userdata;
	const char *name;
...
static int
set_machine_info(Context *c, sd_bus *bus, sd_bus_message *m, int prop,
	sd_bus_message_handler_t cb, sd_bus_error *error)
{
	...
	r = bus_verify_polkit_async(m, CAP_SYS_ADMIN,
		...
		interactive, &c->polkit_registry, error);
	if (r < 0)
		return r;
	if (r == 0)
		return 1; /* No authorization for now, but the async polkit stuff will call us again when it has it */
...
static int
method_set_pretty_hostname(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	return set_machine_info(userdata, bus, m, PROP_PRETTY_HOSTNAME,
		method_set_pretty_hostname, error);
}

static int
method_set_icon_name(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	return set_machine_info(userdata, bus, m, PROP_ICON_NAME,
		method_set_icon_name, error);
}

static int
method_set_chassis(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
...
static int
method_set_deployment(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	return set_machine_info(userdata, bus, m, PROP_DEPLOYMENT,
		method_set_deployment, error);
}

static int
method_set_location(sd_bus *bus, sd_bus_message *m, void *userdata,
	sd_bus_error *error)
{
	return set_machine_info(userdata, bus, m, PROP_LOCATION,
		method_set_location, error);
}
