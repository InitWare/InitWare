...
static size_t
output_callback(char *buf, size_t size, size_t nmemb, void *userp)
{
	Uploader *u = userp;

	assert(u);

	log_debug("The server answers (%zu bytes): %.*s", size * nmemb,
		(int)(size * nmemb), buf);
...
static int
check_cursor_updating(Uploader *u)
{
...
static int
update_cursor_state(Uploader *u)
{
	_cleanup_free_ char *temp_path = NULL;
	_cleanup_fclose_ FILE *f = NULL;
	int r;
...
load_cursor_state(Uploader *u)
{
	...
	r = parse_env_file(u->state_file, NEWLINE, "LAST_CURSOR",
		&u->last_cursor, NULL);
...
int
start_upload(Uploader *u,
	size_t (*input_callback)(void *ptr, size_t size, size_t nmemb,
		void *userdata),
	void *data)
{
	CURLcode code;
	...
	if (!u->header) {
		...
		h = curl_sIWLIST_APPEND(NULL,
			"Content-Type: application/vnd.fdo.journal");
		if (!h)
			return log_oom();

		h = curl_sIWLIST_APPEND(h, "Transfer-Encoding: chunked");
		if (!h) {
			curl_slist_free_all(h);
			return log_oom();
		}

		h = curl_sIWLIST_APPEND(h, "Accept: text/plain");
		if (!h) {
			curl_slist_free_all(h);
			return log_oom();
		}
	...
	if (!u->easy) {
		...
		easy_setopt(curl, CURLOPT_ERRORBUFFER, u->error, LOG_ERR,
			return -EXFULL);
		...
		easy_setopt(curl, CURLOPT_WRITEDATA, data, LOG_ERR,
			return -EXFULL);

		/* set where to read from */
		easy_setopt(curl, CURLOPT_READFUNCTION, input_callback, LOG_ERR,
			return -EXFULL);

		easy_setopt(curl, CURLOPT_READDATA, data, LOG_ERR,
			return -EXFULL);

		/* use our special own mime type and chunked transfer */
		easy_setopt(curl, CURLOPT_HTTPHEADER, u->header, LOG_ERR,
			return -EXFULL);
		...
		if (arg_key || startswith(u->url, "https://")) {
			easy_setopt(curl, CURLOPT_SSLKEY,
				arg_key ?: PRIV_KEY_FILE, LOG_ERR,
				return -EXFULL);
			easy_setopt(curl, CURLOPT_SSLCERT,
				arg_cert ?: CERT_FILE, LOG_ERR, return -EXFULL);
		}

		if (streq_ptr(arg_trust, "all"))
			easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0, LOG_ERR,
				return -EUCLEAN);
		else if (arg_trust || startswith(u->url, "https://"))
			easy_setopt(curl, CURLOPT_CAINFO,
				arg_trust ?: TRUST_FILE, LOG_ERR,
				return -EXFULL);

		if (arg_key || arg_trust)
			easy_setopt(curl, CURLOPT_SSLVERSION,
				CURL_SSLVERSION_TLSv1, LOG_WARNING, );
...
static size_t
fd_input_callback(void *buf, size_t size, size_t nmemb, void *userp)
{
	Uploader *u = userp;
...
static int
dispatch_fd_input(sd_event_source *event, int fd, uint32_t revents, void *userp)
{
	Uploader *u = userp;
	...
	if (u->uploading) {
		log_warning(
			"dispatch_fd_input called when uploading, ignoring.");
		return 0;
	}
...
static int
open_file_for_upload(Uploader *u, const char *filename)
{
	int fd, r = 0;
	...
	if (arg_follow) {
		r = sd_event_add_io(u->events, &u->input_event, fd, EPOLLIN,
			dispatch_fd_input, u);
		if (r < 0) {
...
static int
dispatch_sigterm(sd_event_source *event, const struct signalfd_siginfo *si,
	void *userdata)
{
	Uploader *u = userdata;
...
setup_signals(Uploader *u)
{
	...
	r = sd_event_add_signal(u->events, &u->sigterm_event, SIGTERM,
		dispatch_sigterm, u);
	if (r < 0)
		return r;

	r = sd_event_add_signal(u->events, &u->sigint_event, SIGINT,
		dispatch_sigterm, u);
	if (r < 0)
		return r;
...
static int
setup_uploader(Uploader *u, const char *url, const char *state_file)
{
	int r;
	...
	if (!(host = startswith(url, "http://")) &&
		!(host = startswith(url, "https://"))) {
		host = url;
		proto = "https://";
	}
	...
	if (strchr(host, ':'))
		...
		u->url = strjoin(proto, t, ":" STRINGIFY(DEFAULT_PORT),
			"/upload", NULL);
	}
	if (!u->url)
		return log_oom();
...
perform_upload(Uploader *u)
{
	...
	if (code) {
		if (u->error[0])
			log_error("Upload to %s failed: %.*s", u->url,
				(int)sizeof(u->error), u->error);
		else
			log_error("Upload to %s failed: %s", u->url,
				curl_easy_strerror(code));
		return -EIO;
	...
	if (status >= 300) {
		log_error("Upload to %s failed with code %ld: %s", u->url,
...
parse_config(void)
{
	const ConfigTableItem items[] = {
		...
		{ "Upload", "ServerCertificateFile", config_parse_path, 0,
			&arg_cert },
...
help(void)
{
	printf("%s -u URL {FILE|-}...\n\n"
	       ...
	       "  -u --url=URL              Upload to this address (default port " STRINGIFY(
		       DEFAULT_PORT) ")\n"
...
parse_argv(int argc, char *argv[])
{
	...
	if (optind < argc &&
		(arg_directory || arg_file || arg_machine ||
			arg_journal_type)) {
...
main(int argc, char **argv)
{
	...
	if (use_journal) {
		...
		if (r < 0)
			goto finish;
		r = open_journal_for_upload(&u, j, arg_cursor ?: u.last_cursor,
			arg_cursor ? arg_after_cursor : true, !!arg_follow);
		if (r < 0)
			goto finish;
	}
