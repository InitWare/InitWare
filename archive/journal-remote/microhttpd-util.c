...
void
microhttpd_logger(void *arg, const char *fmt, va_list ap)
{
	char *f;
...
static int
mhd_respond_internal(struct MHD_Connection *connection,
	enum MHD_RequestTerminationCode code, char *buffer, size_t size,
...
int
mhd_respond(struct MHD_Connection *connection,
	enum MHD_RequestTerminationCode code, const char *message)
{
	return mhd_respond_internal(connection, code, (char *)message,
		strlen(message), MHD_RESPMEM_PERSISTENT);
}

int
mhd_respond_oom(struct MHD_Connection *connection)
{
	return mhd_respond(connection, MHD_HTTP_SERVICE_UNAVAILABLE,
		"Out of memory.\n");
}

int
mhd_respondf(struct MHD_Connection *connection,
	enum MHD_RequestTerminationCode code, const char *format, ...)
{
	char *m;
	int r;
	va_list ap;
	...
	return mhd_respond_internal(connection, code, m, r,
...
void
log_func_gnutls(int level, const char *message)
{
	assert_se(message);

	if (0 <= level && level < (int)ELEMENTSOF(gnutls_log_map)) {
		if (gnutls_log_map[level].enabled)
			log_internal(gnutls_log_map[level].level, 0, NULL, 0,
				NULL, "gnutls %d/%s: %s", level,
				gnutls_log_map[level].names[1], message);
	} else {
		log_debug("Received GNUTLS message with unknown level %d.",
			level);
		log_internal(LOG_DEBUG, 0, NULL, 0, NULL, "gnutls: %s",
			message);
	}
}

int
log_enable_gnutls_category(const char *cat)
{
	unsigned i;
	...
	if (streq(cat, "all")) {
		...
		for (i = 0; i < ELEMENTSOF(gnutls_log_map); i++)
			if (strv_contains((char **)gnutls_log_map[i].names,
				    cat)) {
				gnutls_log_map[i].enabled = true;
				log_reset_gnutls_level();
				return 0;
...
static int
verify_cert_authorized(gnutls_session_t session)
{
	unsigned status;
...
static int
get_client_cert(gnutls_session_t session, gnutls_x509_crt_t *client_cert)
{
...
static int
get_auth_dn(gnutls_x509_crt_t client_cert, char **buf)
{
	size_t len = 0;
	int r;
...
static inline void
gnutls_x509_crt_deinitp(gnutls_x509_crt_t *p)
{
	gnutls_x509_crt_deinit(*p);
}

int
check_permissions(struct MHD_Connection *connection, int *code, char **hostname)
{
	...
	*code = 0;
	...
	if (!ci) {
		log_error(
			"MHD_get_connection_info failed: session is unencrypted");
