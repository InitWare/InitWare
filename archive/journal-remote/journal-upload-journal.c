 ...
 */
static ssize_t
write_entry(char *buf, size_t size, Uploader *u)
{
	int r;
	size_t pos = 0;
	...
	while (true) {
		...
		case ENTRY_CURSOR: {
			...
			r = sd_journal_get_cursor(u->journal,
				&u->current_cursor);
			if (r < 0)
		...
		case ENTRY_MONOTONIC: {
			...
			r = sd_journal_get_monotonic_usec(u->journal,
				&monotonic, &boot_id);
			if (r < 0)
				return log_error_errno(r,
		...
		case ENTRY_BOOT_ID: {
			...
			r = sd_journal_get_monotonic_usec(u->journal, NULL,
				&boot_id);
			if (r < 0)
				return log_error_errno(r,
					"Failed to get monotonic timestamp: %m");
		...
		case ENTRY_BINARY_FIELD: {
			...
			memcpy(buf + pos, (char *)u->field_data + u->field_pos,
				tocopy);
...
static size_t
journal_input_callback(void *buf, size_t size, size_t nmemb, void *userp)
{
	Uploader *u = userp;
	int r;
	sd_journal *j;
	...
	while (j && filled < size * nmemb) {
		if (u->entry_state == ENTRY_DONE) {
			...
			if (r < 0) {
				...
				if (u->input_event)
					...
					log_info(
						"No more entries, closing journal.");
		...
		log_debug("Entry %zu (%s) has been uploaded.", u->entries_sent,
			u->current_cursor);
	}
...
void
close_journal_input(Uploader *u)
{
	assert(u);
...
static int
process_journal_input(Uploader *u, int skip)
{
	int r;
...
int
check_journal_input(Uploader *u)
{
	if (u->input_event) {
		int r;
...
static int
dispatch_journal_input(sd_event_source *event, int fd, uint32_t revents,
	void *userp)
{
	Uploader *u = userp;
	...
	if (u->uploading) {
		log_warning(
			"dispatch_journal_input called when uploading, ignoring.");
		return 0;
	}
...
int
open_journal_for_upload(Uploader *u, sd_journal *j, const char *cursor,
	bool after_cursor, bool follow)
{
	int fd, r, events;
	...
	if (follow) {
		...
		r = sd_event_add_io(u->events, &u->input_event, fd, events,
			dispatch_journal_input, u);
		if (r < 0)
			return log_error_errno(r,
	...
	if (cursor) {
		...
		if (r < 0) {
			return log_error_errno(r,
				"Failed to seek to cursor %s: %m", cursor);
		}
	}
