...
open_journal(RequestMeta *m)
{
	...
	return sd_journal_open(&m->journal,
		SD_JOURNAL_LOCAL_ONLY | SD_JOURNAL_SYSTEM);
}

static ssize_t
request_reader_entries(void *cls, uint64_t pos, char *buf, size_t max)
{
	RequestMeta *m = cls;
	int r;
	size_t n, k;
	...
	while (pos >= m->size) {
		...
		if (m->n_skip < 0)
			r = sd_journal_previous_skip(m->journal,
				(uint64_t)-m->n_skip + 1);
		else if (m->n_skip > 0)
			r = sd_journal_next_skip(m->journal,
				(uint64_t)m->n_skip + 1);
		else
	...
	if (k != n) {
		log_error("Failed to read from file: %s",
			errno ? strerror(errno) : "Premature EOF");
		return MHD_CONTENT_READER_END_WITH_ERROR;
	}
...
static int
request_parse_accept(RequestMeta *m, struct MHD_Connection *connection)
{
	const char *header;
...
static int
request_parse_range(RequestMeta *m, struct MHD_Connection *connection)
{
	...
	range = MHD_lookup_connection_value(connection, MHD_HEADER_KIND,
		"Range");
	if (!range)
		return 0;
...
static int
request_parse_arguments_iterator(void *cls, enum MHD_ValueKind kind,
	const char *key, const char *value)
{
	...
	if (streq(key, "boot")) {
		...
		if (r) {
			...
			r = sd_journal_add_match(m->journal, match,
				sizeof(match) - 1);
			if (r < 0) {
				m->argument_parse_error = r;
				return MHD_NO;
...
static int
request_parse_arguments(RequestMeta *m, struct MHD_Connection *connection)
{
	assert(m);
	assert(connection);
	...
	MHD_get_connection_values(connection, MHD_GET_ARGUMENT_KIND,
		request_parse_arguments_iterator, m);

	return m->argument_parse_error;
}

static int
request_handler_entries(struct MHD_Connection *connection, void *connection_cls)
{
	...
	r = open_journal(m);
	if (r < 0)
		return mhd_respondf(connection, MHD_HTTP_INTERNAL_SERVER_ERROR,
	...
	response = MHD_create_response_from_callback(MHD_SIZE_UNKNOWN, 4 * 1024,
		request_reader_entries, m, NULL);
	if (!response)
...
static int
output_field(FILE *f, OutputMode m, const char *d, size_t l)
{
	const char *eq;
	size_t j;
...
static ssize_t
request_reader_fields(void *cls, uint64_t pos, char *buf, size_t max)
{
	RequestMeta *m = cls;
	int r;
	size_t n, k;
...
static int
request_handler_fields(struct MHD_Connection *connection, const char *field,
	void *connection_cls)
{
	struct MHD_Response *response;
	RequestMeta *m = connection_cls;
	...
	response = MHD_create_response_from_callback(MHD_SIZE_UNKNOWN, 4 * 1024,
		request_reader_fields, m, NULL);
	if (!response)
		return respond_oom(connection);

	MHD_add_response_header(response, "Content-Type",
		mime_types[m->mode == OUTPUT_JSON ? OUTPUT_JSON : OUTPUT_SHORT]);
...
static int
request_handler_redirect(struct MHD_Connection *connection, const char *target)
{
	char *page;
	struct MHD_Response *response;
	int ret;
	...
	ret = MHD_queue_response(connection, MHD_HTTP_MOVED_PERMANENTLY,
		response);
	MHD_destroy_response(response);
...
static int
request_handler_file(struct MHD_Connection *connection, const char *path,
	const char *mime_type)
{
	...
	if (fd < 0)
		return mhd_respondf(connection, MHD_HTTP_NOT_FOUND,
...
static int
request_handler_machine(struct MHD_Connection *connection, void *connection_cls)
{
	struct MHD_Response *response;
	RequestMeta *m = connection_cls;
	int r;
	...
	r = sd_journal_get_cutoff_realtime_usec(m->journal, &cutoff_from,
		&cutoff_to);
	if (r < 0)
		return mhd_respondf(connection, MHD_HTTP_INTERNAL_SERVER_ERROR,
			"Failed to determine disk usage: %s\n", strerror(-r));

	if (parse_env_file("/etc/os-release", NEWLINE, "PRETTY_NAME", &os_name,
		    NULL) == -ENOENT)
		parse_env_file("/usr/lib/os-release", NEWLINE, "PRETTY_NAME",
			&os_name, NULL);
	...
	response = MHD_create_response_from_buffer(strlen(json), json,
		MHD_RESPMEM_MUST_FREE);
	if (!response) {
		free(json);
		return respond_oom(connection);
	...
	size_t *upload_data_size, void **connection_cls)
{
	...
	if (streq(url, "/browse"))
		return request_handler_file(connection,
			DOCUMENT_ROOT "/browse.html", "text/html");
...
main(int argc, char *argv[])
{
	...
	if (n < 0) {
		...
		struct MHD_OptionItem opts[] = {
			...
			{ MHD_OPTION_END, 0, NULL },
			{ MHD_OPTION_END, 0, NULL },
			{ MHD_OPTION_END, 0, NULL }, { MHD_OPTION_END, 0, NULL }
		};
		int opts_pos = 2;
		int flags = MHD_USE_THREAD_PER_CONNECTION | MHD_USE_POLL |
			MHD_USE_DEBUG;
		...
		d = MHD_start_daemon(flags, 19531, NULL, NULL, request_handler,
