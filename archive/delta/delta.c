...
static int
notify_override_masked(const char *top, const char *bottom)
{
	if (!(arg_flags & SHOW_MASKED))
		return 0;

	printf("%s%s%s     %s %s %s\n", ansi_highlight_red(), "[MASKED]",
		ansi_highlight_off(), top, draw_special_char(DRAW_ARROW),
		bottom);
	return 1;
}

static int
notify_override_equivalent(const char *top, const char *bottom)
{
...
static int
notify_override_redirected(const char *top, const char *bottom)
{
	if (!(arg_flags & SHOW_REDIRECTED))
		return 0;

	printf("%s%s%s %s %s %s\n", ansi_highlight(), "[REDIRECTED]",
		ansi_highlight_off(), top, draw_special_char(DRAW_ARROW),
		bottom);
	return 1;
}
...
static int
notify_override_overridden(const char *top, const char *bottom)
{
	if (!(arg_flags & SHOW_OVERRIDDEN))
		return 0;

	printf("%s%s%s %s %s %s\n", ansi_highlight(), "[OVERRIDDEN]",
		ansi_highlight_off(), top, draw_special_char(DRAW_ARROW),
		bottom);
	return 1;
}
...
static int
notify_override_extended(const char *top, const char *bottom)
{
	if (!(arg_flags & SHOW_EXTENDED))
		return 0;

	printf("%s%s%s   %s %s %s\n", ansi_highlight(), "[EXTENDED]",
		ansi_highlight_off(), top, draw_special_char(DRAW_ARROW),
		bottom);
	return 1;
}
...
static int
notify_override_unchanged(const char *f)
{
...
static int
found_override(const char *top, const char *bottom)
{
...
static int
enumerate_dir_d(Hashmap *top, Hashmap *bottom, Hashmap *drops,
...
static int
enumerate_dir(Hashmap *top, Hashmap *bottom, Hashmap *drops, const char *path,
	bool dropins)
{
	...
	for (;;) {
		...
		if (dropins && de->d_type == DT_DIR &&
			endswith(de->d_name, ".d"))
...
static int
process_suffix(const char *suffix, const char *onlyprefix)
{
	const char *p;
	char *f;
...
static int
process_suffixes(const char *onlyprefix)
{
	const char *n;
	int n_found = 0, r;
...
static int
process_suffix_chop(const char *arg)
{
	const char *p;
...
static int
parse_flags(const char *flag_str, int flags)
{
...
{
	enum { ARG_NO_PAGER = 0x100, ARG_DIFF, ARG_VERSION };
