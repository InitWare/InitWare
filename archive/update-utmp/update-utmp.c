...
static int
get_current_runlevel(Context *c)
{
	static const struct {
		const int runlevel;
		const char *special;
...
#ifdef HAVE_AUDIT
	if (c->audit_fd >= 0)
		if (audit_log_user_comm_message(c->audit_fd,
			    AUDIT_SYSTEM_SHUTDOWN, "", "systemd-update-utmp",
			    NULL, NULL, NULL, 1) < 0 &&
			errno != EPERM) {
			log_error_errno(errno,
				"Failed to send audit message: %m");
...
on_runlevel(Context *c)
{
	...
	if (c->audit_fd >= 0) {
		...
		if (audit_log_user_comm_message(c->audit_fd,
			    AUDIT_SYSTEM_RUNLEVEL, s, "systemd-update-utmp",
			    NULL, NULL, NULL, 1) < 0 &&
			errno != EPERM) {
			log_error_errno(errno,
				"Failed to send audit message: %m");
