...
main(int argc, char *argv[])
{
	...
	_cleanup_free_ char *saved = NULL, *escaped_type = NULL,
			    *escaped_path_id = NULL;
	...
	device =
		udev_device_new_from_subsystem_sysname(udev, "rfkill", argv[2]);
	if (!device) {
		log_debug_errno(errno,
			"Failed to get rfkill device '%s', ignoring: %m",
			argv[2]);
		return EXIT_SUCCESS;
	}
	...
	if (streq(argv[1], "load")) {
		...
		if (r < 0) {
			log_debug_errno(r,
				"Failed to write 'soft' attribute on rfkill device, ignoring: %m");
			return EXIT_SUCCESS;
		}
		...
		if (!value) {
			log_debug_errno(r,
				"Failed to read system attribute, ignoring device: %m");
			return EXIT_SUCCESS;
		}
