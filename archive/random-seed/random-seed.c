...
main(int argc, char *argv[])
{
	...
	if (r < 0) {
		log_error_errno(r,
			"Failed to create directory " RANDOM_SEED_DIR ": %m");
		goto finish;
	}
	...
	if (streq(argv[1], "load")) {
		...
		if (random_fd < 0) {
			random_fd = open("/dev/urandom",
				O_WRONLY | O_CLOEXEC | O_NOCTTY, 0600);
			if (random_fd < 0) {
				log_error_errno(errno,
		...
		random_fd =
			open("/dev/urandom", O_RDONLY | O_CLOEXEC | O_NOCTTY);
		if (random_fd < 0) {
			log_error_errno(errno,
				"Failed to open /dev/urandom: %m");
			r = -errno;
	...
	if (cleanup_seed_file) {
		...
		if (k <= 0) {
			log_error(
				"Failed to read new seed from /dev/urandom: %s",
				r < 0 ? strerror(-r) : "EOF");
			r = k == 0 ? -EIO : (int)k;
		} else {
