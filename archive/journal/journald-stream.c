...
void stdout_stream_free(StdoutStream *s) {
        ...
        if (s->server) {
                ...
                if (s->in_notify_queue)
                        IWLIST_REMOVE(stdout_stream_notify_queue, s->server->stdout_streams_notify_queue, s);
        }
...
static int stdout_stream_save(StdoutStream *s) {
        ...
        if (!s->fdstore && !s->in_notify_queue) {
                IWLIST_PREPEND(stdout_stream_notify_queue, s->server->stdout_streams_notify_queue, s);
