...
int samples;
int arg_samples_len =
	DEFAULT_SAMPLES_LEN; /* we record len+1 (1 start sample) */
...
parse_conf(void)
{
	...
	const ConfigTableItem items[] = { { "Bootchart", "Samples",
						  config_parse_int, 0,
						  &arg_samples_len },
...
help(void)
{
	fprintf(stdout,
		...
		program_invocation_short_name, DEFAULT_HZ, DEFAULT_SAMPLES_LEN,
		DEFAULT_SCALE_X, DEFAULT_SCALE_Y, DEFAULT_OUTPUT, DEFAULT_INIT);
}
...
parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "rel", no_argument, NULL,
							 'r' },
	...
	while ((c = getopt_long(argc, argv, "erpf:n:o:i:FCchx:y:", options,
		...
		case 'o':
			...
			strscpy(arg_output_path, sizeof(arg_output_path),
				optarg);
			break;
		case 'i':
			path_kill_slashes(optarg);
...
main(int argc, char *argv[])
{
	...
	for (samples = 0; !exiting && samples < arg_samples_len; samples++) {
		...
		if (!build) {
			if (parse_env_file("/etc/os-release", NEWLINE,
				    "PRETTY_NAME", &build, NULL) == -ENOENT)
		...
		IWLIST_PREPEND(link, head, sampledata);
	}
