...
svg_header(void)
{
	...
	h = 400.0 + (arg_scale_y * 30.0) /* base graphs and title */
		+ (arg_pss ? (100.0 * arg_scale_y) + (arg_scale_y * 7.0) :
				   0.0) /* pss estimate */
		+ psize + ksize + esize;
	...
	svg("<svg width=\"%.0fpx\" height=\"%.0fpx\" version=\"1.1\" ", w, h);
...
svg_graph_box(int height)
{
	...
	svg("<rect class=\"box\" x=\"%.03f\" y=\"0\" width=\"%.03f\" height=\"%.03f\" />\n",
		time_to_graph(0.0), time_to_graph(finalsample - graph_start),
		ps_to_graph(height));

	for (d = graph_start; d <= finalsample;
		d += (arg_scale_x < 2.0		   ? 60.0 :
				arg_scale_x < 10.0 ? 1.0 :
							   0.1)) {
		...
		if (i % 10 == 0)
			svg("  <text class=\"sec\" x=\"%.03f\" y=\"%.03f\" >%.01fs</text>\n",
				time_to_graph(d - graph_start), -5.0,
				d - graph_start);
...
static char *
xml_comment_encode(const char *name)
{
	char *enc_name, *p;
...
svg_pss_graph(void)
{
	...
	for (i = 100000; i < 1000000; i += 100000) {
		svg("  <line class=\"sec01\" x1=\"%.03f\" y1=\"%.0f\" x2=\"%.03f\" y2=\"%.0f\"/>\n",
			time_to_graph(.0), kb_to_graph(i),
			time_to_graph(
				sampledata_last->sampletime - graph_start),
			kb_to_graph(i));
	...
	IWLIST_FOREACH_BEFORE (link, sampledata, head) {
		...
		svg("    <rect class=\"clrw\" style=\"fill: %s\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" height=\"%.03f\" />\n",
			"rgb(64,64,64)",
			time_to_graph(
				prev_sampledata->sampletime - graph_start),
			kb_to_graph(1000000.0 - top),
			time_to_graph(sampledata->sampletime -
				prev_sampledata->sampletime),
...
svg_io_bi_bar(void)
{
	...
	IWLIST_FOREACH_BEFORE (link, sampledata, head) {
		...
		for (k = 0; (k < ((range / 2) - 1)) &&
			(start_sampledata->link_next);
			k++)
			start_sampledata = start_sampledata->link_next;
		for (k = 0; (k < (range / 2)) && (stop_sampledata->link_prev);
			k++)
			stop_sampledata = stop_sampledata->link_prev;

		tot = (double)(stop_sampledata->blockstat.bi -
			      start_sampledata->blockstat.bi) /
			diff;
		...
		tot = (double)(stop_sampledata->blockstat.bo -
			      start_sampledata->blockstat.bo) /
			diff;
	...
	IWLIST_FOREACH_BEFORE (link, sampledata, head) {
		...
		if (pbi > 0.001)
			svg("<rect class=\"bi\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" height=\"%.03f\" />\n",
				time_to_graph(prev_sampledata->sampletime -
					graph_start),
		...
		if (i == max_here) {
			svg("  <text class=\"sec\" x=\"%.03f\" y=\"%.03f\">%0.2fmb/sec</text>\n",
				time_to_graph(
					sampledata->sampletime - graph_start) +
					5,
				((arg_scale_y * 5) -
					(pbi * (arg_scale_y * 5))) +
					15,
...
svg_cpu_bar(int cpu_num)
{
	...
	IWLIST_FOREACH_BEFORE (link, sampledata, head) {
		...
		if (cpu_num < 0)
			for (c = 0; c < cpus; c++)
				trt += sampledata->runtime[c] -
					prev_sampledata->runtime[c];
		else
			trt = sampledata->runtime[cpu_num] -
				prev_sampledata->runtime[cpu_num];
...
svg_wait_bar(int cpu_num)
{
	...
	IWLIST_FOREACH_BEFORE (link, sampledata, head) {
		...
		if (cpu_num < 0)
			for (c = 0; c < cpus; c++)
				twt += sampledata->waittime[c] -
					prev_sampledata->waittime[c];
		else
			twt = sampledata->waittime[cpu_num] -
				prev_sampledata->waittime[cpu_num];
...
static struct ps_struct *
get_next_ps(struct ps_struct *ps)
{
	/*
...
static void
svg_do_initcall(int count_only)
{
	...
	while (!feof(f)) {
		...
		c = sscanf(l, "[%lf] initcall %s %*s %d %*s %d %*s", &t, func,
			&ret, &usecs);
		if (c != 4) {
		...
		svg("  <rect class=\"krnl\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" height=\"%.03f\" />\n",
			...
			ps_to_graph(kcount), time_to_graph(usecs / 1000000.0),
			ps_to_graph(1));

		/* label */
		svg("  <text x=\"%.03f\" y=\"%.03f\">%s <tspan class=\"run\">%.03fs</tspan></text>\n",
			time_to_graph(t - (usecs / 1000000.0)) + 5,
			ps_to_graph(kcount) + 15, func, usecs / 1000000.0);
...
svg_ps_bars(void)
{
	...
	while ((ps = get_next_ps(ps))) {
		...
		if (!ps_filter(ps)) {
			...
			if ((!ps->next) && (ps->parent))
				svg("  <line class=\"dot\" x1=\"%.03f\" y1=\"%.03f\" x2=\"%.03f\" y2=\"%.03f\" />\n",
					ps->parent->pos_x,
					ps_to_graph(j - 1) +
						10.0, /* whee, use the last value here */
					ps->parent->pos_x, ps->parent->pos_y);
			continue;
		}
		...
		svg("  <rect class=\"ps\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" height=\"%.03f\" />\n",
			...
			time_to_graph(
				ps->last->sampledata->sampletime - starttime),
			ps_to_graph(1));
		...
		while (ps->sample->next) {
			...
			prt = (rt / 1000000000) /
				(ps->sample->sampledata->sampletime -
			...
			if ((prt < 0.1) &&
				(wrt < 0.1)) /* =~ 26 (color threshold) */
				continue;

			svg("    <rect class=\"wait\" x=\"%.03f\" y=\"%.03f\" width=\"%.03f\" height=\"%.03f\" />\n",
				time_to_graph(prev->sampledata->sampletime -
					graph_start),
				ps_to_graph(j),
				time_to_graph(
					ps->sample->sampledata->sampletime -
		...
		svg("  <text x=\"%.03f\" y=\"%.03f\"><![CDATA[%s]]> [%i]<tspan class=\"run\">%.03fs</tspan> %s</text>\n",
			...
			ps_to_graph(j) + 14.0, escaped ? escaped : ps->name,
			ps->pid,
		...
		if (ps->parent) {
			...
			svg("  <line class=\"dot\" x1=\"%.03f\" y1=\"%.03f\" x2=\"%.03f\" y2=\"%.03f\" />\n",
				...
				ps_to_graph(j) + 10.0, ps->parent->pos_x,
				ps_to_graph(j) + 10.0);
	...
	while (ps->sample->next && i < (samples - (arg_hz / 2))) {
		...
		ps->sample = ps->sample->next;
		sample_hz = ps->sample;
		for (ii = 0; ((ii < (int)arg_hz / 2) && (sample_hz->next));
			ii++)
		...
		crt = 0.0;
		for (c = 0; c < cpus; c++)
			crt += sample_hz->sampledata->runtime[c] -
				ps->sample->sampledata->runtime[c];
...
svg_top_ten_cpu(void)
{
	...
	for (n = 0; n < 10; n++)
		svg("<text class=\"t3\" x=\"20\" y=\"%d\">%3.03fs - <![CDATA[%s]]> [%d]</text>\n",
			20 + (n * 13), top[n]->total, top[n]->name,
			top[n]->pid);
}

static void
svg_top_ten_pss(void)
{
	struct ps_struct *top[10];
	struct ps_struct emptyps = {};
	struct ps_struct *ps;
