...
int address_pool_new(
        ...
        IWLIST_PREPEND(address_pools, m->address_pools, p);
...
void address_pool_free(AddressPool *p) {
        ...
        if (p->manager)
                IWLIST_REMOVE(address_pools, p->manager->address_pools, p);
...
static bool address_pool_prefix_is_taken(
        ...
        HASHMAP_FOREACH(l, p->manager->links, i) {
                ...
                IWLIST_FOREACH(addresses, a, l->addresses) {
                ...
                IWLIST_FOREACH(addresses, a, l->pool_addresses) {
                        if (a->family != p->family)
                                continue;
        ...
        IWLIST_FOREACH(networks, n, p->manager->networks) {
                Address *a;

                IWLIST_FOREACH(addresses, a, n->static_addresses) {
                        if (a->family != p->family)
                                continue;
