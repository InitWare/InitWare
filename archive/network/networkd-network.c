...
static int network_load_one(Manager *manager, const char *filename) {
        ...
        IWLIST_HEAD_INIT(network->static_addresses);
        IWLIST_HEAD_INIT(network->static_routes);
        IWLIST_HEAD_INIT(network->static_fdb_entries);
        ...
        *d = '\0';
        ...
        IWLIST_PREPEND(networks, manager->networks, network);
        ...
        IWLIST_FOREACH(routes, route, network->static_routes) {
                if (!route->family) {
        ...
        IWLIST_FOREACH(addresses, address, network->static_addresses) {
                if (!address->family) {
...
void network_free(Network *network) {
        ...
        if (network->manager) {
                if (network->manager->networks)
                        IWLIST_REMOVE(networks, network->manager->networks, network);
...
int network_get(Manager *manager, struct udev_device *device,
        ...
        IWLIST_FOREACH(networks, network, manager->networks) {
