...
static void link_free(Link *link) {
        ...
        while ((address = link->addresses)) {
                IWLIST_REMOVE(addresses, link->addresses, address);
                address_free(address);
        }

        while ((address = link->pool_addresses)) {
                IWLIST_REMOVE(addresses, link->pool_addresses, address);
                address_free(address);
        }
...
static Address* link_find_dhcp_server_address(Link *link) {
        ...
        IWLIST_FOREACH(addresses, address, link->network->static_addresses) {
        ...
        IWLIST_FOREACH(addresses, address, link->pool_addresses) {
                if (address->family != AF_INET)
                        continue;
...
static int link_enter_set_routes(Link *link) {
        ...
        IWLIST_FOREACH(routes, rt, link->network->static_routes) {
...
static int link_enter_set_addresses(Link *link) {
        ...
        IWLIST_FOREACH(addresses, ad, link->network->static_addresses) {
...
static int link_set_bridge_fdb(Link *const link) {
        ...
        IWLIST_FOREACH(static_fdb_entries, fdb_entry, link->network->static_fdb_entries) {
...
static Address* link_get_equal_address(Link *link, Address *needle) {
        ...
        IWLIST_FOREACH(addresses, i, link->addresses)
...
int link_rtnl_process_address(sd_rtnl *rtnl, sd_rtnl_message *message, void *userdata) {
        ...
        case RTM_NEWADDR:
                if (existing) {
                        ...
                        IWLIST_PREPEND(addresses, link->addresses, address);
        ...
        case RTM_DELADDR:
                ...
                if (existing) {
                        ...
                        IWLIST_REMOVE(addresses, link->addresses, existing);
...
static void link_update_operstate(Link *link) {
        ...
        if (link->kernel_operstate == IF_OPER_DORMANT)
                ...
                IWLIST_FOREACH(addresses, address, link->addresses) {
