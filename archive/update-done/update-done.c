...
static int
apply_timestamp(const char *path, struct timespec *ts)
{
	...
	if (stat(path, &st) >= 0) {
		...
		if (utimensat(AT_FDCWD, path, twice, AT_SYMLINK_NOFOLLOW) < 0) {
			if (errno == EROFS)
				return log_debug(
					"Can't update timestamp file %s, file system is read-only.",
					path);
		...
		if (r < 0)
			return log_error_errno(r,
				"Failed to set SELinux context for %s: %m",
				path);
		...
		if (fd < 0) {
			if (errno == EROFS)
				return log_debug(
					"Can't create timestamp file %s, file system is read-only.",
					path);
		...
		log_error_errno(errno, "Failed to stat() timestamp file %s: %m",
			path);
