...
int manager_read_resolv_conf(Manager *m) {
        ...
        IWLIST_FOREACH(servers, s, m->dns_servers)
                s->marked = true;
        ...
        IWLIST_FOREACH_SAFE(servers, s, nx, m->dns_servers)
                if (s->marked)
                        dns_server_free(s);
...
int manager_write_resolv_conf(Manager *m) {
        ...
        HASHMAP_FOREACH(l, m->links, i) {
                ...
                IWLIST_FOREACH(servers, s, l->dns_servers) {
                        r = set_put(dns, s);
                        if (r == -EEXIST)
                                continue;
                        if (r < 0)
        ...
        if (set_isempty(dns)) {
                IWLIST_FOREACH(servers, s, m->fallback_dns_servers) {
                        r = set_put(dns, s);
                        if (r == -EEXIST)
                                continue;
                        if (r < 0)
...
void manager_verify_all(Manager *m) {
        ...
        IWLIST_FOREACH(scopes, s, m->dns_scopes)
