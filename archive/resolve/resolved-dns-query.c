...
DnsQuery *dns_query_free(DnsQuery *q) {
        ...
        if (q->manager) {
                IWLIST_REMOVE(queries, q->manager->dns_queries, q);
...
int dns_query_new(Manager *m, DnsQuery **ret, DnsQuestion *question, int ifindex, uint64_t flags) {
        ...
        IWLIST_PREPEND(queries, m->dns_queries, q);
        m->n_dns_queries++;
        q->manager = m;
...
int dns_query_go(DnsQuery *q) {
        ...
        IWLIST_FOREACH(scopes, s, q->manager->dns_scopes) {
                DnsScopeMatch match;
        ...
        IWLIST_FOREACH(scopes, s, first->scopes_next) {
                DnsScopeMatch match;
