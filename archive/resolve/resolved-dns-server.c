...
int dns_server_new(
        ...
        if (type == DNS_SERVER_LINK) {
                IWLIST_FIND_TAIL(servers, l->dns_servers, tail);
                IWLIST_INSERT_AFTER(servers, l->dns_servers, tail, s);
                s->link = l;
        } else if (type == DNS_SERVER_SYSTEM) {
                IWLIST_FIND_TAIL(servers, m->dns_servers, tail);
                IWLIST_INSERT_AFTER(servers, m->dns_servers, tail, s);
        } else if (type == DNS_SERVER_FALLBACK) {
                IWLIST_FIND_TAIL(servers, m->fallback_dns_servers, tail);
                IWLIST_INSERT_AFTER(servers, m->fallback_dns_servers, tail, s);
        } else
...
DnsServer* dns_server_free(DnsServer *s)  {
        ...
        if (s->link) {
                if (s->type == DNS_SERVER_LINK)
                        IWLIST_REMOVE(servers, s->link->dns_servers, s);
        ...
        if (s->manager) {
                if (s->type == DNS_SERVER_SYSTEM)
                        IWLIST_REMOVE(servers, s->manager->dns_servers, s);
                else if (s->type == DNS_SERVER_FALLBACK)
                        IWLIST_REMOVE(servers, s->manager->fallback_dns_servers, s);
