...
static uint32_t
HEADER_HEADER_LENGTH(const Header *h)
{
...
static int
copy_cluster(int sfd, uint64_t soffset, int dfd, uint64_t doffset,
	uint64_t cluster_size, void *buffer)
{
	ssize_t l;
	int r;
...
static int
decompress_cluster(int sfd, uint64_t soffset, int dfd, uint64_t doffset,
	uint64_t compressed_size, uint64_t cluster_size, void *buffer1,
	void *buffer2)
{
...
static int
normalize_offset(const Header *header, uint64_t p, uint64_t *ret,
	bool *compressed, uint64_t *compressed_size)
{
	uint64_t q;
...
static int
verify_header(const Header *header)
{
	assert(header);
	...
	if (HEADER_VERSION(header) != 2 && HEADER_VERSION(header) != 3)
		return -ENOTSUP;
...
int
qcow2_convert(int qcow2_fd, int raw_fd)
{
	...
	for (i = 0; i < HEADER_L1_SIZE(&header); i++) {
		...
		r = normalize_offset(&header, l1_table[i], &l2_begin, NULL,
			NULL);
		if (r < 0)
			return r;
		if (r == 0)
		...
		l = pread(qcow2_fd, l2_table, HEADER_CLUSTER_SIZE(&header),
			l2_begin);
		if (l < 0)
			return -errno;
		...
		for (j = 0; j < HEADER_L2_SIZE(&header); j++) {
			...
			p = ((i << HEADER_L2_BITS(&header)) + j)
				<< HEADER_CLUSTER_BITS(&header);

			r = normalize_offset(&header, l2_table[j], &data_begin,
			...
			if (compressed)
				r = decompress_cluster(qcow2_fd, data_begin,
					raw_fd, p, compressed_size,
					HEADER_CLUSTER_SIZE(&header), buffer1,
					buffer2);
			else
				r = copy_cluster(qcow2_fd, data_begin, raw_fd,
					p, HEADER_CLUSTER_SIZE(&header),
					buffer1);
			if (r < 0)
				return r;
		}
