...
int
tar_import_new(TarImport **ret, sd_event *event, const char *image_root,
...
static void
tar_import_report_progress(TarImport *i, TarProgress p)
{
	unsigned percent;
...
static int
tar_import_make_local_copy(TarImport *i)
{
	int r;
	...
	if (!i->final_path) {
		r = import_make_path(i->tar_job->url, i->tar_job->etag,
			i->image_root, ".tar-", NULL, &i->final_path);
		if (r < 0)
			return log_oom();
	}

	r = import_make_local_copy(i->final_path, i->image_root, i->local,
		i->force_local);
	if (r < 0)
		return r;
...
static bool
tar_import_is_done(TarImport *i)
{
	assert(i);
	assert(i->tar_job);
...
static void
tar_import_job_on_finished(ImportJob *j)
{
	TarImport *i;
	int r;
	...
	if (!i->tar_job->etag_exists) {
		...
		r = import_verify(i->tar_job, i->checksum_job,
			i->signature_job);
		if (r < 0)
			goto finish;
...
static int
tar_import_job_on_open_disk(ImportJob *j)
{
	TarImport *i;
	int r;
	...
	r = import_make_path(j->url, j->etag, i->image_root, ".tar-", NULL,
		&i->final_path);
	if (r < 0)
		return log_oom();
	...
	if (r == -ENOTTY) {
		...
		return log_error_errno(errno,
			"Failed to create subvolume %s: %m", i->temp_path);
...
static void
tar_import_job_on_progress(ImportJob *j)
{
	TarImport *i;
...
int
tar_import_pull(TarImport *i, const char *url, const char *local,
	bool force_local, ImportVerify verify)
{
	...
	r = import_find_old_etags(url, i->image_root, DT_DIR, ".tar-", NULL,
		&i->tar_job->old_etags);
	if (r < 0)
		return r;

	r = import_make_verification_jobs(&i->checksum_job, &i->signature_job,
		verify, url, i->glue, tar_import_job_on_finished, i);
	if (r < 0)
		return r;
