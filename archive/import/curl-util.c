...
static void
curl_glue_check_finished(CurlGlue *g)
{
	CURLMsg *msg;
	int k = 0;
...
static int
curl_glue_on_io(sd_event_source *s, int fd, uint32_t revents, void *userdata)
{
	CurlGlue *g = userdata;
	...
	translated_fd =
		PTR_TO_INT(hashmap_get(g->translate_fds, INT_TO_PTR(fd + 1)));
...
static int
curl_glue_socket_callback(CURLM *curl, curl_socket_t s, int action,
	void *userdata, void *socketp)
{
	...
	if (io) {
		...
		if (sd_event_add_io(g->event, &io, fd, events, curl_glue_on_io,
			    g) < 0)
			return -1;
		...
		r = hashmap_put(g->translate_fds, INT_TO_PTR(fd + 1),
			INT_TO_PTR(s + 1));
		if (r < 0) {
			log_oom();
			hashmap_remove(g->ios, INT_TO_PTR(s + 1));
...
static int
curl_glue_on_timer(sd_event_source *s, uint64_t usec, void *userdata)
{
	CurlGlue *g = userdata;
	int k = 0;
	...
	if (curl_multi_socket_action(g->curl, CURL_SOCKET_TIMEOUT, 0, &k) !=
		CURLM_OK) {
...
static int
curl_glue_timer_callback(CURLM *curl, long timeout_ms, void *userdata)
{
	CurlGlue *g = userdata;
	usec_t usec;
	...
	if (timeout_ms < 0) {
		if (g->timer) {
			if (sd_event_source_set_enabled(g->timer,
				    SD_EVENT_OFF) < 0)
				return -1;
		}
	...
	usec = now(clock_boottime_or_monotonic()) +
		(usec_t)timeout_ms * USEC_PER_MSEC + USEC_PER_MSEC - 1;
	...
	if (g->timer) {
		...
		if (sd_event_add_time(g->event, &g->timer,
			    clock_boottime_or_monotonic(), usec, 0,
			    curl_glue_on_timer, g) < 0)
			return -1;
...
int
curl_glue_new(CurlGlue **glue, sd_event *event)
{
	...
	if (curl_multi_setopt(g->curl, CURLMOPT_SOCKETFUNCTION,
		    curl_glue_socket_callback) != CURLM_OK)
		return -EINVAL;

	if (curl_multi_setopt(g->curl, CURLMOPT_TIMERDATA, g) != CURLM_OK)
		return -EINVAL;

	if (curl_multi_setopt(g->curl, CURLMOPT_TIMERFUNCTION,
		    curl_glue_timer_callback) != CURLM_OK)
		return -EINVAL;
...
int
curl_glue_make(CURL **ret, const char *url, void *userdata)
{
	...
	useragent =
		strjoina(program_invocation_short_name, "/" PACKAGE_VERSION);
...
int
curl_glue_add(CurlGlue *g, CURL *c)
{
	assert(g);
	assert(c);
...
void
curl_glue_remove_and_free(CurlGlue *g, CURL *c)
{
	assert(g);
...
curl_slist_new(const char *first, ...)
{
	...
	l = curl_sIWLIST_APPEND(NULL, first);
	if (!l)
		return NULL;
...
int
curl_header_strdup(const void *contents, size_t sz, const char *field,
	char **value)
{
...
int
curl_parse_http_time(const char *t, usec_t *ret)
{
	const char *e;
	locale_t loc;
	struct tm tm;
