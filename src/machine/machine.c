...
Machine* machine_new(Manager *manager, const char *name) {
        ...
        m->state_file = strappend(AbsDir_PkgRunState "/machines/", m->name);
        if (!m->state_file)
                goto fail;
...
void machine_free(Machine *m) {
        ...
        if (m->in_gc_queue)
                IWLIST_REMOVE(Machine, gc_queue, m->manager->machine_gc_queue, m);
...
int machine_save(Machine *m) {
        ...
        r = mkdir_safe_label(AbsDir_PkgRunState "/machines", 0755, 0, 0);
        if (r < 0)
                goto finish;
...
void machine_add_to_gc_queue(Machine *m) {
        ...
        IWLIST_PREPEND(Machine, gc_queue, m->manager->machine_gc_queue, m);
        m->in_gc_queue = true;
}
