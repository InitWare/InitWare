...
static void
socket_unwatch_control_pid(Socket *s)
{
	assert(s);
...
socket_arm_timer(Socket *s)
{
	...
	if (s->timer_event_source) {
		r = sd_event_source_set_time(s->timer_event_source,
			now(CLOCK_MONOTONIC) + s->timeout_usec);
		if (r < 0)
			return r;
...
int
socket_instantiate_service(Socket *s)
{
...
static bool
have_non_accept_socket(Socket *s)
{
	SocketPort *p;
...
static int
socket_add_mount_links(Socket *s)
{
	SocketPort *p;
	int r;
...
static int
socket_add_device_link(Socket *s)
{
	char *t;
...
static int
socket_add_default_dependencies(Socket *s)
{
	int r;
	assert(s);
...
static const char *
socket_find_symlink_target(Socket *s)
{
...
socket_verify(Socket *s)
{
	...
	if (!strv_isempty(s->symlinks) && !socket_find_symlink_target(s)) {
		log_unit_error(UNIT(s)->id,
			"%s has symlinks set but none or more than one node in the file system. Refusing.",
			UNIT(s)->id);
		return -EINVAL;
	}
...
static void
socket_dump(Unit *u, FILE *f, const char *prefix)
{
	...
	fprintf(f,
		...
		socket_address_bind_ipv6_only_to_string(s->bind_ipv6_only),
		prefix, s->backlog, prefix, s->socket_mode, prefix,
		s->directory_mode, prefix, yes_no(s->keep_alive), prefix,
		yes_no(s->no_delay), prefix, yes_no(s->free_bind), prefix,
		yes_no(s->transparent), prefix, yes_no(s->broadcast), prefix,
		yes_no(s->pass_cred), prefix, yes_no(s->pass_sec), prefix,
		strna(s->tcp_congestion), prefix, yes_no(s->remove_on_stop),
	...
	IWLIST_FOREACH(port, p, s->ports)
	{
		if (p->type == SOCKET_SOCKET) {
			...
			fprintf(f, "%s%s: %s\n", prefix,
				listen_lookup(socket_address_family(
						      &p->address),
					p->address.type),
				t);
			free(k);
...
static int
instance_from_socket(int fd, unsigned nr, char **instance)
{
	socklen_t l;
	char *r;
	...
	case AF_INET: {
		uint32_t a = ntohl(local.in.sin_addr.s_addr),
			 b = ntohl(remote.in.sin_addr.s_addr);

		if (asprintf(&r, "%u-%u.%u.%u.%u:%u-%u.%u.%u.%u:%u", nr,
			    a >> 24, (a >> 16) & 0xFF, (a >> 8) & 0xFF,
			    a & 0xFF, ntohs(local.in.sin_port), b >> 24,
	...
	case AF_INET6: {
		...
		if (memcmp(&local.in6.sin6_addr, ipv4_prefix,
			...
			if (asprintf(&r, "%u-%u.%u.%u.%u:%u-%u.%u.%u.%u:%u", nr,
				    a[0], a[1], a[2], a[3],
				    ntohs(local.in6.sin6_port), b[0], b[1],
				    b[2], b[3],
			...
			if (asprintf(&r, "%u-%s:%u-%s:%u", nr,
				    inet_ntop(AF_INET6, &local.in6.sin6_addr, a,
					    sizeof(a)),
				    ntohs(local.in6.sin6_port),
				    inet_ntop(AF_INET6, &remote.in6.sin6_addr,
					    b, sizeof(b)),
				    ntohs(remote.in6.sin6_port)) < 0)
				return -ENOMEM;
		}
	...
	case AF_UNIX: {
		...
		if (k >= 0) {
			if (asprintf(&r, "%u-" PID_FMT "-" UID_FMT, nr,
				    ucred.pid, ucred.uid) < 0)
				return -ENOMEM;
		} else if (k == -ENODATA) {
...
static void
socket_apply_socket_options(Socket *s, int fd)
{
	int r;
	...
	if (s->keep_alive_interval) {
		...
		if (setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, &value,
			    sizeof(value)) < 0)
			log_unit_warning(UNIT(s)->id,
	...
	if (s->keep_alive_cnt) {
		...
		if (setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &value,
			    sizeof(value)) < 0)
	...
	if (s->ip_ttl >= 0) {
		...
		r = setsockopt(fd, IPPROTO_IP, IP_TTL, &s->ip_ttl,
			sizeof(s->ip_ttl));

		if (socket_ipv6_is_supported())
			x = setsockopt(fd, IPPROTO_IPV6, IPV6_UNICAST_HOPS,
				&s->ip_ttl, sizeof(s->ip_ttl));
		else {
			x = -1;
			errno = EAFNOSUPPORT;
		}
...
static void
socket_apply_fifo_options(Socket *s, int fd)
{
	int r;
...
static int
fifo_address_create(const char *path, mode_t directory_mode, mode_t socket_mode,
	int *_fd)
{
	int fd = -1, r = 0;
	struct stat st;
...
static int
special_address_create(const char *path, int *_fd)
{
	int fd = -1, r = 0;
	struct stat st;
	...
	fd = open(path,
		O_RDONLY | O_CLOEXEC | O_NOCTTY | O_NONBLOCK | O_NOFOLLOW);
	if (fd < 0) {
		r = -errno;
		goto fail;
	}
...
socket_open_fds(Socket *s)
{
	...
	IWLIST_FOREACH(port, p, s->ports)
	{
		...
		if (p->type == SOCKET_SOCKET) {
			if (!know_label) {
				...
				if (s->selinux_context_from_net) {
					...
					if (UNIT_ISSET(s->service) &&
						SERVICE(UNIT_DEREF(s->service))
							->exec_command
								[SERVICE_EXEC_START]) {
						r = mac_selinux_get_create_label_from_exe(
							SERVICE(UNIT_DEREF(
									s->service))
								->exec_command
									[SERVICE_EXEC_START]
			...
			r = socket_address_listen(&p->address,
				SOCK_CLOEXEC | SOCK_NONBLOCK, s->backlog,
				s->bind_ipv6_only, s->bind_to_device,
				s->free_bind, s->transparent, s->directory_mode,
				s->socket_mode, label);
			if (r < 0)
				goto rollback;
			...
			r = special_address_create(p->path, &p->fd);
			if (r < 0)
				goto rollback;

		} else if (p->type == SOCKET_FIFO) {
			r = fifo_address_create(p->path, s->directory_mode,
				s->socket_mode, &p->fd);
			if (r < 0)
				goto rollback;
			...
			r = mq_address_create(p->path, s->socket_mode,
				s->mq_maxmsg, s->mq_msgsize, &p->fd);
			if (r < 0)
				goto rollback;
		} else
...
static void
socket_set_state(Socket *s, SocketState state)
{
	...
	if (!IN_SET(state, SOCKET_START_PRE, SOCKET_START_CHOWN,
		    SOCKET_START_POST, SOCKET_STOP_PRE, SOCKET_STOP_PRE_SIGTERM,
		    SOCKET_STOP_PRE_SIGKILL, SOCKET_STOP_POST,
		    SOCKET_FINAL_SIGTERM, SOCKET_FINAL_SIGKILL)) {
		s->timer_event_source =
			sd_event_source_unref(s->timer_event_source);
		socket_unwatch_control_pid(s);
		s->control_command = NULL;
	...
	if (!IN_SET(state, SOCKET_START_CHOWN, SOCKET_START_POST,
		    SOCKET_LISTENING, SOCKET_RUNNING, SOCKET_STOP_PRE,
		    SOCKET_STOP_PRE_SIGTERM, SOCKET_STOP_PRE_SIGKILL))
		socket_close_fds(s);
...
static int
socket_coldplug(Unit *u, Hashmap *deferred_work)
{
	Socket *s = SOCKET(u);
	int r;
	...
	if (IN_SET(s->deserialized_state, SOCKET_START_PRE, SOCKET_START_CHOWN,
		    SOCKET_START_POST, SOCKET_STOP_PRE, SOCKET_STOP_PRE_SIGTERM,
		    SOCKET_STOP_PRE_SIGKILL, SOCKET_STOP_POST,
		    SOCKET_FINAL_SIGTERM, SOCKET_FINAL_SIGKILL)) {
		if (s->control_pid <= 0)
...
static int
socket_spawn(Socket *s, ExecCommand *c, pid_t *_pid)
{
...
static int
socket_chown(Socket *s, pid_t *_pid)
{
	pid_t pid;
	int r;
	...
	fail_child:
		log_open();
		log_error_errno(r, "Failed to chown socket at step %s: %m",
			exit_status_to_string(ret, EXIT_STATUS_SYSTEMD));
...
static void
socket_enter_dead(Socket *s, SocketResult f)
{
	assert(s);
...
static void
socket_enter_stop_post(Socket *s, SocketResult f)
{
	int r;
	assert(s);
...
fail:
	log_unit_warning(UNIT(s)->id, "%s failed to run 'stop-post' task: %s",
		UNIT(s)->id, strerror(-r));
	socket_enter_signal(s, SOCKET_FINAL_SIGTERM, SOCKET_FAILURE_RESOURCES);
}

static void
socket_enter_signal(Socket *s, SocketState state, SocketResult f)
{
	int r;
...
static void
socket_enter_stop_pre(Socket *s, SocketResult f)
{
	int r;
	assert(s);
...
fail:
	log_unit_warning_errno(UNIT(s)->id, r,
		"%s failed to run 'stop-pre' task: %m", UNIT(s)->id);
	socket_enter_stop_post(s, SOCKET_FAILURE_RESOURCES);
}

static void
socket_enter_listening(Socket *s)
{
	int r;
	assert(s);
	...
	if (r < 0) {
		log_unit_warning_errno(UNIT(s)->id, r,
			"%s failed to watch sockets: %m", UNIT(s)->id);
		goto fail;
	}
...
static void
socket_enter_start_post(Socket *s)
{
	int r;
	assert(s);
	...
	if (s->control_command) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(s)->id, r,
				"%s failed to run 'start-post' task: %m",
				UNIT(s)->id);
			goto fail;
		}
...
static void
socket_enter_start_chown(Socket *s)
{
	int r;
	...
	if (r < 0) {
		log_unit_warning_errno(UNIT(s)->id, r,
			"%s failed to listen on sockets: %m", UNIT(s)->id);
		goto fail;
	}
	...
	if (!isempty(s->user) || !isempty(s->group)) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(s)->id, r,
				"%s failed to fork 'start-chown' task: %m",
				UNIT(s)->id);
			goto fail;
		}
...
static void
socket_enter_start_pre(Socket *s)
{
	int r;
	assert(s);
	...
	if (s->control_command) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(s)->id, r,
				"%s failed to run 'start-pre' task: %m",
				UNIT(s)->id);
			goto fail;
		}
...
static void
socket_enter_running(Socket *s, int cfd)
{
	...
	if (unit_stop_pending(UNIT(s))) {
		...
		if (cfd >= 0)
			...
			if (r < 0) {
				log_unit_warning_errno(UNIT(s)->id, r,
					"%s failed to listen on sockets: %m",
					UNIT(s)->id);
				socket_enter_stop_pre(s,
	...
	if (cfd < 0) {
		...
		if (!pending) {
			if (!UNIT_ISSET(s->service)) {
				log_unit_error(UNIT(s)->id,
					"%s: service to activate vanished, refusing activation.",
					UNIT(s)->id);
				r = -ENOENT;
				goto fail;
			}
			...
			r = manager_add_job(UNIT(s)->manager, JOB_START,
				UNIT_DEREF(s->service), JOB_REPLACE, true,
				&error, NULL);
			if (r < 0)
				goto fail;
		}
...
fail:
	log_unit_warning_errno(UNIT(s)->id, r, "%s failed to run next task: %m",
		UNIT(s)->id);
...
socket_start(Unit *u)
{
	...
	if (IN_SET(s->state, SOCKET_STOP_PRE, SOCKET_STOP_PRE_SIGKILL,
		    SOCKET_STOP_PRE_SIGTERM, SOCKET_STOP_POST,
		    SOCKET_FINAL_SIGTERM, SOCKET_FINAL_SIGKILL))
		return -EAGAIN;
...
static int
socket_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Socket *s = SOCKET(u);
	SocketPort *p;
	int r;
	...
	IWLIST_FOREACH(port, p, s->ports)
	{
		...
		if (p->type == SOCKET_SOCKET) {
			...
			unit_serialize_item_format(u, f, "special", "%i %s",
				copy, p->path);
		else if (p->type == SOCKET_MQUEUE)
			unit_serialize_item_format(u, f, "mqueue", "%i %s",
				copy, p->path);
		else {
			assert(p->type == SOCKET_FIFO);
			unit_serialize_item_format(u, f, "fifo", "%i %s", copy,
				p->path);
		}
	}
...
static int
socket_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Socket *s = SOCKET(u);
	...
	if (streq(key, "state")) {
		...
		if (sscanf(value, "%i %i %n", &fd, &type, &skip) < 2 ||
			fd < 0 || type < 0 || !fdset_contains(fds, fd))
		...
		log_unit_debug(UNIT(s)->id, "Unknown serialization key '%s'",
			key);
...
static int
socket_distribute_fds(Unit *u, FDSet *fds)
{
	Socket *s = SOCKET(u);
	SocketPort *p;
...
socket_sub_state_to_string(Unit *u)
{
	assert(u);
...
const char *
socket_port_type_to_string(SocketPort *p)
{
	assert(p);
...
static int
socket_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	...
	log_unit_debug(UNIT(p->socket)->id, "Incoming traffic on %s",
		UNIT(p->socket)->id);
...
static void
socket_sigchld_event(Unit *u, pid_t pid, int code, int status)
{
	Socket *s = SOCKET(u);
	SocketResult f;
	...
	log_unit_full(u->id, f == SOCKET_SUCCESS ? LOG_DEBUG : LOG_NOTICE,
	...
	if (s->control_command && s->control_command->command_next &&
		f == SOCKET_SUCCESS) {
		log_unit_debug(u->id, "%s running next command for state %s",
		...
		log_unit_debug(u->id, "%s got final SIGCHLD for state %s",
			u->id, socket_state_to_string(s->state));
...
static int
socket_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata)
{
...
int
socket_collect_fds(Socket *s, int **fds, unsigned *n_fds)
{
	int *rfds;
	unsigned rn_fds, k;
	SocketPort *p;
...
static void
socket_notify_service_dead(Socket *s, bool failed_permanent)
{
	assert(s);
...
void
socket_connection_unref(Socket *s)
{
	assert(s);
	...
	log_unit_debug(UNIT(s)->id, "%s: One connection closed, %u left.",
		UNIT(s)->id, s->n_connections);
}

static void
socket_trigger_notify(Unit *u, Unit *other)
{
	Socket *s = SOCKET(u);
	Service *se;
	...
	if (se->state == SERVICE_FAILED)
		socket_notify_service_dead(s,
			se->result == SERVICE_FAILURE_START_LIMIT);

	if (se->state == SERVICE_DEAD || se->state == SERVICE_FINAL_SIGTERM ||
...
static int
socket_kill(Unit *u, KillWho who, int signo, sd_bus_error *error)
{
...
static int
socket_get_timeout(Unit *u, uint64_t *timeout)
{
	Socket *s = SOCKET(u);
	int r;
