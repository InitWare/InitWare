...
device_unset_sysfs(Device *d)
{
	...
	IWLIST_REMOVE(same_sysfs, first, d);

	if (first)
		hashmap_remove_and_replace(devices, d->sysfs, first->sysfs,
			first);
	else
...
static int
device_set_sysfs(Device *d, const char *sysfs)
{
	Device *first;
	char *copy;
	int r;
	...
	IWLIST_PREPEND(same_sysfs, first, d);
...
static void
device_set_state(Device *d, DeviceState state)
{
	...
	if (state != old_state)
		log_unit_debug(UNIT(d)->id, "%s changed %s -> %s", UNIT(d)->id,
...
static int
device_coldplug(Unit *u, Hashmap *deferred_work)
{
	Device *d = DEVICE(u);
	...
	if (d->found & DEVICE_FOUND_UDEV)
		...
		d->deserialized_state != DEVICE_PLUGGED)
...
static int
device_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Device *d = DEVICE(u);
...
static int
device_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Device *d = DEVICE(u);
...
static void
device_dump(Unit *u, FILE *f, const char *prefix)
{
	Device *d = DEVICE(u);
	...
	fprintf(f,
		"%sDevice State: %s\n"
		"%sSysfs Path: %s\n",
		prefix, device_state_to_string(d->state), prefix,
		strna(d->sysfs));
}

_pure_ static UnitActiveState
device_active_state(Unit *u)
{
	assert(u);
...
device_sub_state_to_string(Unit *u)
{
	assert(u);
...
static int
device_update_description(Unit *u, struct udev_device *dev, const char *path)
{
	const char *model;
	int r;
	...
	if (model) {
		...
		if (!label)
			label = udev_device_get_property_value(dev,
				"ID_PART_ENTRY_NAME");
		if (!label)
			label = udev_device_get_property_value(dev,
...
static int
device_add_udev_wants(Unit *u, struct udev_device *dev)
{
	const char *wants;
	...
	property = u->manager->running_as == SYSTEMD_USER ?
		      "SYSTEMD_USER_WANTS" :
		      "SYSTEMD_WANTS";
	...
	FOREACH_WORD_QUOTED(word, l, wants, state)
	{
		...
		if (r < 0)
			return log_unit_error_errno(u->id, r,
				"Failed to add wants dependency: %m");
	}
	if (!isempty(state))
		log_unit_warning(u->id,
			"Property %s on %s has trailing garbage, ignoring.",
...
static int
device_setup_unit(Manager *m, struct udev_device *dev, const char *path,
	bool main)
{
...
static int
device_process_new(Manager *m, struct udev_device *dev)
{
	...
	if (alias) {
		...
		FOREACH_WORD_QUOTED(word, l, alias, state)
		{
			...
			if (path_is_absolute(e))
				...
				log_warning(
					"SYSTEMD_ALIAS for %s is not an absolute path, ignoring: %s",
					sysfs, e);
		}
		if (!isempty(state))
			log_warning(
				"SYSTEMD_ALIAS for %s has trailing garbage, ignoring.",
				sysfs);
	}
...
static void
device_update_found_one(Device *d, bool add, DeviceFound found, bool now)
{
...
static int
device_update_found_by_sysfs(Manager *m, const char *sysfs, bool add,
	DeviceFound found, bool now)
{
	Device *d, *l, *n;
...
static int
device_update_found_by_name(Manager *m, const char *path, bool add,
	DeviceFound found, bool now)
{
	_cleanup_free_ char *e = NULL;
	Unit *u;
...
static bool
device_is_ready(struct udev_device *dev)
{
	const char *ready;
...
device_following(Unit *u)
{
	...
	for (other = d->same_sysfs_prev; other;
		other = other->same_sysfs_prev) {
		if (startswith(UNIT(other)->id, "sys-"))
			return UNIT(other);
...
static int
device_following_set(Unit *u, Set **_set)
{
	...
	if (IWLIST_JUST_US(same_sysfs, d)) {
		*_set = NULL;
		return 0;
	}
...
device_enumerate(Manager *m)
{
	...
	if (!m->udev_monitor) {
		...
		/* This will fail if we are unprivileged, but that
                 ...
                 * during boot. */
		udev_monitor_set_receive_buffer_size(m->udev_monitor,
			128 * 1024 * 1024);

		r = udev_monitor_filter_add_match_tag(m->udev_monitor,
			"systemd");
		if (r < 0)
			goto fail;
		...
		r = sd_event_add_io(m->event, &m->udev_event_source,
			udev_monitor_get_fd(m->udev_monitor), EPOLLIN,
			device_dispatch_io, m);
		if (r < 0)
			goto fail;
	}
	...
	udev_list_entry_foreach(item, first)
	{
		...
		device_update_found_by_sysfs(m, sysfs, true, DEVICE_FOUND_UDEV,
			false);
	}
...
static int
device_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	_cleanup_udev_device_unref_ struct udev_device *dev = NULL;
	Manager *m = userdata;
	...
	if (streq(action, "remove")) {
		...
		/* If we get notified that a device was removed by
                 ...
                 * found bits */
		device_update_found_by_sysfs(m, sysfs, false,
		...
		device_update_found_by_sysfs(m, sysfs, false, DEVICE_FOUND_UDEV,
			true);
	}
...
int
device_found_node(Manager *m, const char *node, bool add, DeviceFound found,
	bool now)
{
	_cleanup_udev_device_unref_ struct udev_device *dev = NULL;
	struct stat st;
