...
*/
static int
audit_callback(void *auditdata, security_class_t cls, char *msgbuf,
	size_t msgbufsize)
{
	...
	snprintf(msgbuf, msgbufsize, "auid=%s uid=%s gid=%s%s%s%s%s%s%s",
		login_uid_buf, uid_buf, gid_buf, audit->path ? " path=\"" : "",
		strempty(audit->path), audit->path ? "\"" : "",
		audit->cmdline ? " cmdline=\"" : "", strempty(audit->cmdline),
		audit->cmdline ? "\"" : "");
		...
		if (r >= 0) {
			if (type == SELINUX_AVC)
				audit_log_user_avc_message(get_audit_fd(),
					AUDIT_USER_AVC, buf, NULL, NULL, NULL,
					0);
...
access_init(void)
{
	...
	selinux_set_callback(SELINUX_CB_AUDIT,
		(union selinux_callback)audit_callback);
...
static int
mac_selinux_access_init(sd_bus_error *error)
{
	int r;
...
*/
int
mac_selinux_generic_access_check(sd_bus_message *message, bool system,
	const char *path, const char *permission, sd_bus_error *error)
{
	...
	log_debug(
		"SELinux access check scon=%s tcon=%s tclass=%s perm=%s path=%s cmdline=%s: %i",
		scon, fcon, tclass, permission, path, cl, r);
...
int
mac_selinux_unit_access_check_strv(char **units, sd_bus_message *message,
	Manager *m, const char *permission, sd_bus_error *error)
{
	...
	STRV_FOREACH (i, units) {
		...
		if (r < 0)
			return r;
		r = mac_selinux_unit_access_check(u, message, permission,
			error);
		if (r < 0)
			return r;
	}
