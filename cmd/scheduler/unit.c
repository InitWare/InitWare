...
void unit_add_to_stop_when_unneeded_queue(Unit *u) {
        ...
        IWLIST_PREPEND(stop_when_unneeded_queue, u->manager->stop_when_unneeded_queue, u);
...
void unit_free(Unit *u) {
        ...
        if (u->in_target_deps_queue)
                IWLIST_REMOVE(target_deps_queue, u->manager->target_deps_queue, u);
        ...
        if (u->type != _UNIT_TYPE_INVALID)
                IWLIST_REMOVE(units_by_type, u->manager->units_by_type[u->type], u);
        ...
        if (u->in_stop_when_unneeded_queue)
                IWLIST_REMOVE(stop_when_unneeded_queue, u->manager->stop_when_unneeded_queue, u);
