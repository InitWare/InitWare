...
static void
transaction_delete_job(Transaction *tr, Job *j, bool delete_dependencies)
{
	assert(tr);
	assert(j);
...
static void
transaction_delete_unit(Transaction *tr, Unit *u)
{
	Job *j;
...
void
transaction_abort(Transaction *tr)
{
	Job *j;
...
static void
transaction_find_jobs_that_matter_to_anchor(Job *j, unsigned generation)
{
	JobDependency *l;
	...
	IWLIST_FOREACH (subject, l, j->subject_list) {
		...
		transaction_find_jobs_that_matter_to_anchor(l->object,
			generation);
	}
}

static void
transaction_merge_and_delete_job(Transaction *tr, Job *j, Job *other, JobType t)
{
...
static int
delete_one_unmergeable_job(Transaction *tr, Job *j)
{
	Job *k;
...
static int
transaction_merge_jobs(Transaction *tr, sd_bus_error *e)
{
	Job *j;
	Iterator i;
	int r;
	...
	HASHMAP_FOREACH (j, tr->jobs, i) {
		...
		t = j->type;
		IWLIST_FOREACH (transaction, k, j->transaction_next) {
			if (job_type_merge_and_collapse(&t, k->type, j->unit) >=
				0)
				continue;
			...
			return sd_bus_error_setf(e,
				...
				job_type_to_string(k->type), k->unit->id);
		}
	}
	...
	HASHMAP_FOREACH (j, tr->jobs, i) {
		...
		IWLIST_FOREACH (transaction, k, j->transaction_next)
			assert_se(job_type_merge_and_collapse(&t, k->type,
					  j->unit) == 0);
...
static void
transaction_drop_redundant(Transaction *tr)
{
	Job *j;
	Iterator i;
...
rescan:
	HASHMAP_FOREACH (j, tr->jobs, i) {
		...
		IWLIST_FOREACH (transaction, k, j) {
			if (tr->anchor_job == k ||
				!job_type_is_redundant(k->type,
					unit_active_state(k->unit)) ||
				(k->unit->job &&
					job_type_is_conflicting(k->type,
						k->unit->job->type)))
				goto next_unit;
		}
...
_pure_ static bool
unit_matters_to_anchor(Unit *u, Job *j)
{
	assert(u);
	assert(!j->transaction_prev);
...
static int
transaction_verify_order_one(Transaction *tr, Job *j, Job *from,
	unsigned generation, sd_bus_error *e)
{
	...
	if (j->generation == generation) {
		...
		/* So, the marker is not NULL and we already have been
                 ...
                 * in there. */
		log_unit_warning(j->unit->id, "Found ordering cycle on %s/%s",
			j->unit->id, job_type_to_string(j->type));

		delete = NULL;
		for (k = from; k;
			k = ((k->generation == generation && k->marker != k) ?
					      k->marker :
					      NULL)) {
			/* logging for j not k here here to provide consistent narrative */
			log_unit_warning(j->unit->id,
				"Found dependency on %s/%s", k->unit->id,
				job_type_to_string(k->type));
...
static int
transaction_verify_order(Transaction *tr, unsigned *generation, sd_bus_error *e)
{
	Job *j;
	int r;
	Iterator i;
...
static void
transaction_collect_garbage(Transaction *tr)
{
	Iterator i;
	Job *j;
...
static int
transaction_is_destructive(Transaction *tr, JobMode mode, sd_bus_error *e)
{
	Iterator i;
	Job *j;
...
static void
transaction_minimize_impact(Transaction *tr)
{
	Job *j;
	Iterator i;
		...
		IWLIST_FOREACH (transaction, j, j) {
			...
			stops_running_service = j->type == JOB_STOP &&
				UNIT_IS_ACTIVE_OR_ACTIVATING(
					unit_active_state(j->unit));

			changes_existing_job = j->unit->job &&
				job_type_is_conflicting(j->type,
					j->unit->job->type);
...
static int
transaction_apply(Transaction *tr, Manager *m, JobMode mode)
{
	Iterator i;
	Job *j;
	int r;
...
int
transaction_activate(Transaction *tr, Manager *m, JobMode mode, sd_bus_error *e)
{
	Iterator i;
	Job *j;
	int r;
	...
	transaction_find_jobs_that_matter_to_anchor(tr->anchor_job,
		generation++);
	...
	if (r < 0) {
		log_notice(
			"Requested transaction contradicts existing jobs: %s",
			bus_error_message(e, r));
		return r;
	}
...
static void
transaction_unlink_job(Transaction *tr, Job *j, bool delete_dependencies)
{
	assert(tr);
	assert(j);
	...
	while (j->object_list) {
		Job *other = j->object_list->matters ? j->object_list->subject :
							     NULL;
		...
		if (other && delete_dependencies) {
			log_unit_debug(other->unit->id,
				...
				other->unit->id,
				job_type_to_string(other->type), j->unit->id,
				job_type_to_string(j->type));
...
int
transaction_add_isolate_jobs(Transaction *tr, Manager *m)
{
	Iterator i;
	Unit *u;
	char *k;
