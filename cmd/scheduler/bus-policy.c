...
int
bus_kernel_translate_policy(const BusNamePolicy *policy,
	struct kdbus_item *item)
{
	...
	item->policy_access.access =
		bus_kernel_translate_access(policy->access);
	...
	bool accept_fd, BusNamePolicy *policy, BusPolicyAccess world_policy)
{
	...
	IWLIST_FOREACH(policy, po, policy)
	policy_cnt++;
	...
	size = offsetof(struct kdbus_cmd_hello, items) +
		...
		policy_cnt *
			ALIGN8(offsetof(struct kdbus_item, policy_access) +
	...
	hello->size = size;
	hello->flags = (activating ? KDBUS_HELLO_ACTIVATOR :
