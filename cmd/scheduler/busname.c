...
static int busname_dispatch_io(sd_event_source *source, int fd,
	uint32_t revents, void *userdata);
static int busname_dispatch_timer(sd_event_source *source, usec_t usec,
	void *userdata);
...
static void
busname_unwatch_control_pid(BusName *n)
{
	assert(n);
...
busname_free_policy(BusName *n)
{
	...
	while ((p = n->policy)) {
		IWLIST_REMOVE(policy, n->policy, p);
...
static void
busname_close_fd(BusName *n)
{
	assert(n);

	n->starter_event_source =
...
busname_arm_timer(BusName *n)
{
	...
	if (n->timeout_usec <= 0) {
		n->timer_event_source =
			sd_event_source_unref(n->timer_event_source);
		return 0;
	}

	if (n->timer_event_source) {
		r = sd_event_source_set_time(n->timer_event_source,
			now(CLOCK_MONOTONIC) + n->timeout_usec);
		if (r < 0)
			return r;

		return sd_event_source_set_enabled(n->timer_event_source,
			SD_EVENT_ONESHOT);
	}

	return sd_event_add_time(UNIT(n)->manager->event,
		&n->timer_event_source, CLOCK_MONOTONIC,
		now(CLOCK_MONOTONIC) + n->timeout_usec, 0,
		busname_dispatch_timer, n);
}

static int
busname_add_default_default_dependencies(BusName *n)
{
	int r;

	assert(n);

	r = unit_add_dependency_by_name(UNIT(n), UNIT_BEFORE,
		SPECIAL_BUSNAMES_TARGET, NULL, true);
	if (r < 0)
		return r;

	if (UNIT(n)->manager->running_as == SYSTEMD_SYSTEM) {
		r = unit_add_two_dependencies_by_name(UNIT(n), UNIT_AFTER,
			UNIT_REQUIRES, SPECIAL_SYSINIT_TARGET, NULL, true);
		if (r < 0)
			return r;
	}

	return unit_add_two_dependencies_by_name(UNIT(n), UNIT_BEFORE,
		UNIT_CONFLICTS, SPECIAL_SHUTDOWN_TARGET, NULL, true);
}

static int
busname_add_extras(BusName *n)
{
	Unit *u = UNIT(n);
	int r;
...
busname_verify(BusName *n)
{
	...
	if (!service_name_is_valid(n->name)) {
		log_unit_error(UNIT(n)->id,
			"%s's Name= setting is not a valid service name Refusing.",
			UNIT(n)->id);
		return -EINVAL;
	}
	...
	if (!unit_has_name(UNIT(n), e)) {
		log_unit_error(UNIT(n)->id,
			"%s's Name= setting doesn't match unit name. Refusing.",
			UNIT(n)->id);
		return -EINVAL;
	}
...
static void
busname_dump(Unit *u, FILE *f, const char *prefix)
{
	BusName *n = BUSNAME(u);
	...
	fprintf(f,
		...
		"%sActivating: %s\n"
		"%sAccept FD: %s\n",
		prefix, busname_state_to_string(n->state), prefix,
		busname_result_to_string(n->result), prefix, n->name, prefix,
		yes_no(n->activating), prefix, yes_no(n->accept_fd));
...
busname_watch_fd(BusName *n)
{
	...
	if (n->starter_event_source)
		...
		r = sd_event_add_io(UNIT(n)->manager->event,
			&n->starter_event_source, n->starter_fd, EPOLLIN,
			busname_dispatch_io, n);
	if (r < 0) {
		log_unit_warning_errno(UNIT(n)->id, r,
...
busname_open_fd(BusName *n)
{
	...
	mode = UNIT(n)->manager->running_as == SYSTEMD_SYSTEM ? "system" :
								      "user";
	n->starter_fd = bus_kernel_open_bus_fd(mode, &path);
	if (n->starter_fd < 0)
		return log_unit_warning_errno(UNIT(n)->id, n->starter_fd,
			"Failed to open %s: %m", path ?: "kdbus");

	return 0;
}

static void
busname_set_state(BusName *n, BusNameState state)
{
	...
	if (!IN_SET(state, BUSNAME_LISTENING, BUSNAME_MAKING,
		    BUSNAME_REGISTERED, BUSNAME_RUNNING))
		busname_close_fd(n);

	if (state != old_state)
		log_unit_debug(UNIT(n)->id, "%s changed %s -> %s", UNIT(n)->id,
			busname_state_to_string(old_state),
...
static int
busname_coldplug(Unit *u, Hashmap *deferred_work)
{
	BusName *n = BUSNAME(u);
	int r;
	...
	if (IN_SET(n->deserialized_state, BUSNAME_MAKING, BUSNAME_SIGTERM,
		    BUSNAME_SIGKILL)) {
		if (n->control_pid <= 0)
			return -EBADMSG;
	...
	if (IN_SET(n->deserialized_state, BUSNAME_MAKING, BUSNAME_LISTENING,
		    BUSNAME_REGISTERED, BUSNAME_RUNNING)) {
		r = busname_open_fd(n);
		if (r < 0)
			return r;
...
static int
busname_make_starter(BusName *n, pid_t *_pid)
{
	pid_t pid;
	int r;
	...
	if (pid == 0) {
		...
		r = bus_kernel_make_starter(n->starter_fd, n->name,
			n->activating, n->accept_fd, n->policy,
			n->policy_world);
		if (r < 0) {
	...
	fail_child:
		log_open();
		log_error_errno(r,
			"Failed to create starter connection at step %s: %m",
...
static void
busname_enter_dead(BusName *n, BusNameResult f)
{
	assert(n);
	...
	busname_set_state(n,
		n->result != BUSNAME_SUCCESS ? BUSNAME_FAILED : BUSNAME_DEAD);
}

static void
busname_enter_signal(BusName *n, BusNameState state, BusNameResult f)
{
	...
	r = unit_kill_context(UNIT(n), &kill_context,
		state != BUSNAME_SIGTERM ? KILL_KILL : KILL_TERMINATE, -1,
		n->control_pid, false);
	if (r < 0) {
		log_unit_warning_errno(UNIT(n)->id, r,
			"%s failed to kill control process: %m", UNIT(n)->id);
		goto fail;
	}
	...
	if (r > 0) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(n)->id, r,
				"%s failed to arm timer: %m", UNIT(n)->id);
			goto fail;
		}
...
static void
busname_enter_listening(BusName *n)
{
	int r;
	...
	if (n->activating) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(n)->id, r,
				"%s failed to watch names: %m", UNIT(n)->id);
			goto fail;
		}
...
static void
busname_enter_making(BusName *n)
{
	int r;
	...
	if (n->policy) {
		...
		if (r < 0) {
			log_unit_warning_errno(UNIT(n)->id, r,
				"%s failed to fork 'making' task: %m",
				UNIT(n)->id);
			goto fail;
		}
		...
		r = bus_kernel_make_starter(n->starter_fd, n->name,
			n->activating, n->accept_fd, NULL, n->policy_world);
		if (r < 0) {
			log_unit_warning_errno(UNIT(n)->id, r,
				"%s failed to make starter: %m", UNIT(n)->id);
			goto fail;
		}
...
static void
busname_enter_running(BusName *n)
{
	...
	if (unit_stop_pending(UNIT(n))) {
		log_unit_debug(UNIT(n)->id,
			"Suppressing activation request on %s since unit stop is scheduled.",
			UNIT(n)->id);
	...
	if (!pending) {
		r = manager_add_job(UNIT(n)->manager, JOB_START,
			UNIT_DEREF(n->service), JOB_REPLACE, true, &error,
			NULL);
		if (r < 0)
			goto fail;
	}
...
fail:
	log_unit_warning(UNIT(n)->id,
		"%s failed to queue service startup job: %s", UNIT(n)->id,
		bus_error_message(&error, r));
	busname_enter_dead(n, BUSNAME_FAILURE_RESOURCES);
}

static int
busname_start(Unit *u)
{
	BusName *n = BUSNAME(u);
	...
	if (n->activating && UNIT_ISSET(n->service)) {
		...
		if (UNIT(service)->load_state != UNIT_LOADED) {
			log_unit_error(u->id,
				"Bus service %s not loaded, refusing.",
				UNIT(service)->id);
			return -ENOENT;
		}
	}
...
busname_stop(Unit *u)
{
	...
	assert(IN_SET(n->state, BUSNAME_REGISTERED, BUSNAME_LISTENING,
		BUSNAME_RUNNING));
...
static int
busname_serialize(Unit *u, FILE *f, FDSet *fds)
{
	BusName *n = BUSNAME(u);
	...
	unit_serialize_item(u, f, "result",
		busname_result_to_string(n->result));
...
static int
busname_deserialize_item(Unit *u, const char *key, const char *value,
	FDSet *fds)
{
	BusName *n = BUSNAME(u);
...
busname_sub_state_to_string(Unit *u)
{
	assert(u);
...
static int
busname_peek_message(BusName *n)
{
	...
	if (r < 0) {
		...
		log_unit_error(UNIT(n)->id,
			"%s: Failed to query activation message: %m",
			UNIT(n)->id);
		return -errno;
	}
	...
	if (p == MAP_FAILED) {
		log_unit_error(UNIT(n)->id,
			"%s: Failed to map activation message: %m",
			UNIT(n)->id);
		r = -errno;
		goto finish;
	}
	...
	if (pid > 0)
		log_unit_debug(UNIT(n)->id,
			"%s: Activation triggered by process " PID_FMT " (%s)",
			UNIT(n)->id, pid, strna(comm));
...
static int
busname_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	BusName *n = userdata;
	...
	if (revents != EPOLLIN) {
		log_unit_error(UNIT(n)->id,
			"%s: Got unexpected poll event (0x%x) on starter fd.",
			UNIT(n)->id, revents);
		goto fail;
	}
...
static void
busname_sigchld_event(Unit *u, pid_t pid, int code, int status)
{
	BusName *n = BUSNAME(u);
	BusNameResult f;
	...
	log_unit_full(u->id, f == BUSNAME_SUCCESS ? LOG_DEBUG : LOG_NOTICE,
...
static int
busname_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata)
{
	...
	case BUSNAME_MAKING:
		log_unit_warning(UNIT(n)->id,
			"%s making timed out. Terminating.", UNIT(n)->id);
		busname_enter_signal(n, BUSNAME_SIGTERM,
			BUSNAME_FAILURE_TIMEOUT);
		break;

	case BUSNAME_SIGTERM:
		log_unit_warning(UNIT(n)->id, "%s stopping timed out. Killing.",
			UNIT(n)->id);
		busname_enter_signal(n, BUSNAME_SIGKILL,
			BUSNAME_FAILURE_TIMEOUT);
		break;
...
static void
busname_trigger_notify(Unit *u, Unit *other)
{
	BusName *n = BUSNAME(u);
	Service *s;
	...
	if (s->state == SERVICE_FAILED &&
		s->result == SERVICE_FAILURE_START_LIMIT)
		busname_enter_dead(n, BUSNAME_FAILURE_SERVICE_FAILED_PERMANENT);
	else if (IN_SET(s->state, SERVICE_DEAD, SERVICE_FAILED, SERVICE_STOP,
...
static int
busname_kill(Unit *u, KillWho who, int signo, sd_bus_error *error)
{
	return unit_kill_common(u, who, signo, -1, BUSNAME(u)->control_pid,
		error);
}

static int
busname_get_timeout(Unit *u, uint64_t *timeout)
{
	BusName *n = BUSNAME(u);
	int r;
