...
main(int argc, char *argv[])
{
	...
	output_flags =
		arg_all * OUTPUT_SHOW_ALL | (arg_full > 0) * OUTPUT_FULL_WIDTH;
	...
	if (optind < argc) {
		...
		for (i = optind; i < argc; i++) {
			...
			if (arg_machine)
				root = strjoin("machine/", arg_machine, "/",
					argv[i], NULL);
			else
				root = strdup(argv[i]);
			if (!root)
		...
		if (path_startswith(p, CGROUP_ROOT_DIR) && !arg_machine) {
			...
			if (arg_machine) {
				...
				if (r < 0) {
					log_error(
						"Failed to query ControlGroup: %s",
			...
			r = show_cgroup(SYSTEMD_CGROUP_CONTROLLER, root, NULL,
				0, arg_kernel_threads, output_flags);
		}
	}
