...
parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = {
		...
		{ "on-unit-active", required_argument, NULL,
			ARG_ON_UNIT_ACTIVE },
		{ "on-unit-inactive", required_argument, NULL,
			ARG_ON_UNIT_INACTIVE },
		{ "on-calendar", required_argument, NULL, ARG_ON_CALENDAR },
		{ "timer-property", required_argument, NULL,
			ARG_TIMER_PROPERTY },
	...
	while ((c = getopt_long(argc, argv, "+hrH:M:p:tqG", options, NULL)) >=
		0)
...
static int
transient_unit_set_properties(sd_bus_message *m, char **properties)
{
	char **i;
	int r;
...
static int
transient_cgroup_set_properties(sd_bus_message *m)
{
	int r;
	assert(m);
	...
	if (!isempty(arg_slice)) {
		...
		slice = unit_name_mangle_with_suffix(arg_slice, MANGLE_NOGLOB,
			".slice");
		if (!slice)
			return -ENOMEM;
...
static int
transient_kill_set_properties(sd_bus_message *m)
{
	assert(m);

	if (arg_send_sighup)
		return sd_bus_message_append(m, "(sv)", "SendSIGHUP", "b",
			arg_send_sighup);
	else
		return 0;
}
...
static int
transient_service_set_properties(sd_bus_message *m, char **argv,
	const char *pty_path)
{
	int r;
	...
	if (arg_service_type) {
		r = sd_bus_message_append(m, "(sv)", "Type", "s",
			arg_service_type);
		if (r < 0)
			return r;
	}
	...
	if (arg_exec_user) {
		r = sd_bus_message_append(m, "(sv)", "User", "s",
			arg_exec_user);
		if (r < 0)
			return r;
	}
	...
	if (arg_exec_group) {
		r = sd_bus_message_append(m, "(sv)", "Group", "s",
			arg_exec_group);
		if (r < 0)
			return r;
	}
	...
	if (pty_path) {
		...
		r = sd_bus_message_append(m, "(sv)(sv)(sv)(sv)",
			"StandardInput", "s", "tty", "StandardOutput", "s",
			"tty", "StandardError", "s", "tty", "TTYPath", "s",
			pty_path);
		if (r < 0)
			return r;
		...
		if (e) {
			...
			n = strjoina("TERM=", e);
			r = sd_bus_message_append(m, "(sv)", "Environment",
				"as", 1, n);
			if (r < 0)
				return r;
		}
...
static int
transient_scope_set_properties(sd_bus_message *m)
{
	int r;
...
static int
transient_timer_set_properties(sd_bus_message *m)
{
	int r;
	...
	if (arg_on_active) {
		r = sd_bus_message_append(m, "(sv)", "OnActiveSec", "t",
			arg_on_active);
		if (r < 0)
			return r;
	}
	...
	if (arg_on_boot) {
		r = sd_bus_message_append(m, "(sv)", "OnBootSec", "t",
			arg_on_boot);
		if (r < 0)
			return r;
	}
	...
	if (arg_on_startup) {
		r = sd_bus_message_append(m, "(sv)", "OnStartupSec", "t",
			arg_on_startup);
		if (r < 0)
			return r;
	}
	...
	if (arg_on_unit_active) {
		r = sd_bus_message_append(m, "(sv)", "OnUnitActiveSec", "t",
			arg_on_unit_active);
		if (r < 0)
			return r;
	}
	...
	if (arg_on_unit_inactive) {
		r = sd_bus_message_append(m, "(sv)", "OnUnitInactiveSec", "t",
			arg_on_unit_inactive);
		if (r < 0)
			return r;
	}
	...
	if (arg_on_calendar) {
		r = sd_bus_message_append(m, "(sv)", "OnCalendar", "s",
			arg_on_calendar);
		if (r < 0)
			return r;
	}
...
static int
start_transient_service(sd_bus *bus, char **argv)
{
	...
	if (arg_pty) {
		if (arg_transport == BUS_TRANSPORT_LOCAL) {
			master = posix_openpt(
				O_RDWR | O_NOCTTY | O_CLOEXEC | O_NDELAY);
			if (master < 0)
	...
	if (arg_unit) {
		service = unit_name_mangle_with_suffix(arg_unit, MANGLE_NOGLOB,
			".service");
		if (!service)
			return log_oom();
	...
	if (r < 0) {
		log_error("Failed to start transient service unit: %s",
			bus_error_message(&error, -r));
		return r;
	}
...
static int
start_transient_scope(sd_bus *bus, char **argv)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	...
	if (arg_unit) {
		scope = unit_name_mangle_with_suffix(arg_unit, MANGLE_NOGLOB,
			".scope");
		if (!scope)
			return log_oom();
	...
	if (r < 0) {
		log_error("Failed to start transient scope unit: %s",
			bus_error_message(&error, -r));
		return r;
	}
	...
	if (arg_exec_group) {
		...
		if (setresgid(gid, gid, gid) < 0)
			return log_error_errno(errno,
				"Failed to change GID to " GID_FMT ": %m", gid);
	}
	...
	if (arg_exec_user) {
		...
		if (setresuid(uid, uid, uid) < 0)
			return log_error_errno(errno,
				"Failed to change UID to " UID_FMT ": %m", uid);
	}
...
static int
start_transient_timer(sd_bus *bus, char **argv)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL, *reply = NULL;
	_cleanup_(bus_wait_for_jobs_freep) BusWaitForJobs *w = NULL;
	...
	if (arg_unit) {
		...
		default:
			...
			timer = unit_name_mangle_with_suffix(arg_unit,
				MANGLE_NOGLOB, ".timer");
			if (!timer)
				return log_oom();
	...
	} else if ((asprintf(&service, "run-" PID_FMT ".service", getpid()) <
			   0) ||
	...
	r = sd_bus_call(bus, m, 0, &error, &reply);
	if (r < 0) {
		log_error("Failed to start transient timer unit: %s",
			bus_error_message(&error, -r));
		return r;
	}
...
main(int argc, char *argv[])
{
	...
	if (argc > optind) {
		r = find_binary(argv[optind],
			arg_transport == BUS_TRANSPORT_LOCAL, &command);
		if (r < 0) {
