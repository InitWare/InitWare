...
static void
print_border(FILE *output, unsigned width)
{
	unsigned x, y;
...
int
print_qr_code(FILE *output, const void *seed, size_t seed_size, uint64_t start,
	uint64_t interval, const char *hn, sd_id128_t machine)
{
	FILE *f;
	char *url = NULL;
	size_t url_size = 0, i;
	...
	fprintf(f, "/%" PRIx64 "-%" PRIx64 "?machine=" SD_ID128_FORMAT_STR,
		start, interval, SD_ID128_FORMAT_VAL(machine));
	...
	for (y = 0; y < (unsigned)qr->width; y += 2) {
		...
		for (x = 0; x < (unsigned)qr->width; x++) {
			...
			a = row1[x] & 1;
			b = (y + 1) < (unsigned)qr->width ? (row2[x] & 1) :
								  false;
