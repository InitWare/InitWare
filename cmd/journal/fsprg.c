...
static void
mpi_export(void *buf, size_t buflen, const gcry_mpi_t x)
{
	unsigned len;
	size_t nwritten;
	...
	assert(len <= buflen);
	memzero(buf, buflen);
	gcry_mpi_print(GCRYMPI_FMT_USG, buf + (buflen - len), len, &nwritten,
		x);
	assert(nwritten == len);
}

static gcry_mpi_t
mpi_import(const void *buf, size_t buflen)
{
	gcry_mpi_t h;
	unsigned len;
...
static void
uint64_export(void *buf, size_t buflen, uint64_t x)
{
	assert(buflen == 8);
...
_pure_ static uint64_t
uint64_import(const void *buf, size_t buflen)
{
	assert(buflen == 8);
...
static void
det_randomize(void *buf, size_t buflen, const void *seed, size_t seedlen,
	uint32_t idx)
{
	gcry_md_hd_t hd, hd2;
	size_t olen, cpylen;
...
static gcry_mpi_t
genprime3mod4(int bits, const void *seed, size_t seedlen, uint32_t idx)
{
	...
	buf[buflen - 1] |=
		0x03; /* set lower two bits, to have result 3 (mod 4) */
...
static gcry_mpi_t
gensquare(const gcry_mpi_t n, const void *seed, size_t seedlen, uint32_t idx,
	unsigned secpar)
{
...
static gcry_mpi_t
twopowmodphi(uint64_t m, const gcry_mpi_t p)
{
	gcry_mpi_t phi, r;
	int n;
...
static void
CRT_decompose(gcry_mpi_t *xp, gcry_mpi_t *xq, const gcry_mpi_t x,
...
static void
CRT_compose(gcry_mpi_t *x, const gcry_mpi_t xp, const gcry_mpi_t xq,
	const gcry_mpi_t p, const gcry_mpi_t q)
{
	gcry_mpi_t a, u;
...
size_t
FSPRG_mskinbytes(unsigned _secpar)
{
...
size_t
FSPRG_mpkinbytes(unsigned _secpar)
{
	VALIDATE_SECPAR(_secpar);
...
size_t
FSPRG_stateinbytes(unsigned _secpar)
{
	VALIDATE_SECPAR(_secpar);
...
static void
store_secpar(void *buf, uint16_t secpar)
{
	secpar = secpar / 16 - 1;
...
static uint16_t
read_secpar(const void *buf)
{
	uint16_t secpar;
	secpar = (uint16_t)(((uint8_t *)buf)[0]) << 8 |
		(uint16_t)(((uint8_t *)buf)[1]) << 0;
	return 16 * (secpar + 1);
}

void
FSPRG_GenMK(void *msk, void *mpk, const void *seed, size_t seedlen,
	unsigned _secpar)
{
	...
	if (!seed) {
		gcry_randomize(iseed, FSPRG_RECOMMENDED_SEEDLEN,
			GCRY_STRONG_RANDOM);
		seed = iseed;
...
void
FSPRG_GenState0(void *state, const void *mpk, const void *seed, size_t seedlen)
{
	gcry_mpi_t n, x;
	uint16_t secpar;
...
uint64_t
FSPRG_GetEpoch(const void *state)
{
	uint16_t secpar;
	secpar = read_secpar(state + 0);
...
void
FSPRG_Seek(void *state, uint64_t epoch, const void *msk, const void *seed,
	size_t seedlen)
{
	...
	x = gensquare(n, seed, seedlen, RND_GEN_X, secpar);
	CRT_decompose(&xp, &xq, x, p,
		q); /* split (mod n) into (mod p) and (mod q) using CRT */
	...
	CRT_compose(&xm, xp, xq, p,
		q); /* combine (mod p) and (mod q) to (mod n) using CRT */
...
void
FSPRG_GetKey(const void *state, void *key, size_t keylen, uint32_t idx)
{
	uint16_t secpar;
