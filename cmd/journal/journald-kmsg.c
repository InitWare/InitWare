...
static void
dev_kmsg_record(Server *s, const char *p, size_t l)
{
	struct iovec iovec[N_IOVEC_META_FIELDS + 7 + N_IOVEC_KERNEL_FIELDS + 2 +
		N_IOVEC_UDEV_FIELDS];
	char *message = NULL, *syslog_priority = NULL, *syslog_pid = NULL,
	     *syslog_facility = NULL, *syslog_identifier = NULL,
	     *source_time = NULL;
	int priority, r;
	unsigned n = 0, z = 0, j;
	...
	*e = 0;
	...
	if ((priority & LOG_FACMASK) == LOG_KERN)
		...
		pl -= syslog_parse_identifier((const char **)&p, &identifier,
			&pid);
...
static int
server_read_dev_kmsg(Server *s)
{
	char buffer[8192 +
		1]; /* the kernel-side limit per record is 8K currently */
	ssize_t l;
...
int
server_flush_dev_kmsg(Server *s)
{
	int r;
...
static int
dispatch_dev_kmsg(sd_event_source *es, int fd, uint32_t revents, void *userdata)
{
	Server *s = userdata;
	...
	if (!(revents & EPOLLIN))
		log_error(
			"Got invalid event from epoll for /dev/kmsg: %" PRIx32,
			revents);
...
server_open_dev_kmsg(Server *s)
{
	...
	s->dev_kmsg_fd =
		open("/dev/kmsg", O_RDWR | O_CLOEXEC | O_NONBLOCK | O_NOCTTY);
	if (s->dev_kmsg_fd < 0) {
...
int
server_open_kernel_seqnum(Server *s)
{
	...
	p = mmap(NULL, sizeof(uint64_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd,
		0);
	if (p == MAP_FAILED) {
		log_error_errno(errno,
