...
int ima_setup(void)
{
	...
	if (stat(IMA_SECFS_POLICY, &st) < 0) {
		log_error(
		    "Another IMA custom policy has already been loaded, "
		    "ignoring.");
		return 0;
	}
	...
	if (policyfd < 0) {
		log_error(
		    "Failed to open the IMA custom policy file %s (%m), "
		    "ignoring.",
		    IMA_POLICY_PATH);
		return 0;
	}
	...
	if (imafd < 0) {
		log_error(
		    "Failed to open the IMA kernel interface %s (%m), "
		    "ignoring.",
		    IMA_SECFS_POLICY);
		goto out;
	}
	...
	if (written != policy_size) {
		log_error(
		    "Failed to load the IMA custom policy file %s (%m), "
		    "ignoring.",
		    IMA_POLICY_PATH);
		goto out_mmap;
	}

	log_info("Successfully loaded the IMA custom policy %s.", IMA_POLICY_PATH);
out_mmap:
