...
int mount_setup(bool loaded_policy) {
	...
	/* Create a few directories we always want around, Note that
         * sd_booted() checks for @INSTALL_PKGRUNSTATE_DIR@/system, so this mkdir
         * really needs to stay for good, otherwise software that
         * copied sd-daemon.c into their sources will misdetect
         * systemd. */
	mkdir_label(INSTALL_PKGRUNSTATE_DIR, 0755);
	mkdir_label(INSTALL_PKGRUNSTATE_DIR "/system", 0755);
	mkdir_label(INSTALL_PKGRUNSTATE_DIR "/inaccessible", 0000);
