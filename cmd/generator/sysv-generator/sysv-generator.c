...
typedef enum RunlevelType { RUNLEVEL_UP, RUNLEVEL_DOWN } RunlevelType;
...
static int
add_alias(const char *service, const char *alias)
{
...
generate_unit_file(SysvStub *s)
{
	...
	if (is_symlink(unit) > 0) {
		log_warning("Overwriting existing symlink %s with real service",
			unit);
		(void)unlink(unit);
	}
	...
	STRV_FOREACH (p, s->wanted_by) {
		...
		if (r < 0)
			log_unit_error_errno(s->name, r,
				"Failed to create 'Wants' symlink to %s: %m",
				*p);
	}
...
static bool
usage_contains_reload(const char *line)
{
...
static char *
sysv_translate_name(const char *name)
{
	char *r;
...
static int
sysv_translate_facility(const char *name, const char *filename, char **_r)
{
...
load_sysv(SysvStub *s)
{
	...
	while (!feof(f)) {
		...
		if ((state == LSB_DESCRIPTION || state == LSB) &&
		...
		if (state == NORMAL) {
			...
			if (j && *j) {
				...
				if (chkconfig_description)
					d = strjoin(chkconfig_description, " ",
						j, NULL);
				else
					d = strdup(j);
			...
			if (startswith_no_case(t, "Provides:")) {
				...
				FOREACH_WORD_QUOTED(word, z, strchr(t, ':') + 1,
					state_)
				{
					_cleanup_free_ char *n = NULL,
							    *m = NULL;
					...
					is_before = startswith_no_case(t,
						"X-Start-Before:");
...
static int
fix_order(SysvStub *s, Hashmap *all_services)
{
	SysvStub *other;
	Iterator j;
	int r;
...
static int
enumerate_sysv(const LookupPaths *lp, Hashmap *all_services)
{
	char **path;
	int r;
	...
	STRV_FOREACH (path, lp->sysvinit_path) {
		...
		while ((de = readdir(d))) {
			...
			r = unit_file_lookup_state(UNIT_FILE_SYSTEM, NULL, lp,
				name, NULL);
			if (r < 0 && r != -ENOENT) {
				log_debug_errno(r,
					"Failed to detect whether %s exists, skipping: %m",
					name);
				continue;
			} else if (r >= 0) {
				log_debug(
					"Native unit for %s already exists, skipping.",
					name);
				continue;
			}
...
static int
set_dependencies_from_rcnd(const LookupPaths *lp, Hashmap *all_services)
{
	char **p;
	unsigned i;
	_cleanup_closedir_ DIR *d = NULL;
	...
	STRV_FOREACH (p, lp->sysvrcnd_path)
		for (i = 0; i < ELEMENTSOF(rcnd_table); i++) {
			...
			while ((de = readdir(d))) {
				...
				if (!service) {
					log_debug(
						"Ignoring %s symlink in %s, not generating %s.",
						de->d_name, rcnd_table[i].path,
						name);
					continue;
				}
				...
				if (de->d_name[0] == 'S') {
					if (rcnd_table[i].type == RUNLEVEL_UP) {
						service->sysv_start_priority = MAX(
							a * 10 + b,
							service->sysv_start_priority);
					}

					r = set_ensure_allocated(
						&runlevel_services[i], NULL);
					if (r < 0)
						goto finish;

					r = set_put(runlevel_services[i],
						service);
					if (r < 0)
						goto finish;
	...
	for (i = 0; i < ELEMENTSOF(rcnd_table); i++)
		SET_FOREACH (service, runlevel_services[i], j) {
			...
			if (r < 0)
				return log_oom();
			r = strv_extend(&service->wanted_by,
				rcnd_table[i].target);
			if (r < 0)
				return log_oom();
		}
...
main(int argc, char *argv[])
{
	...
	r = lookup_paths_init(&lp, SYSTEMD_SYSTEM, true, NULL, NULL, NULL,
		NULL);
	if (r < 0) {
