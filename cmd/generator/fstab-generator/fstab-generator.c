...
static int
add_swap(const char *what, struct mntent *me, bool noauto, bool nofail)
{
...
static bool
mount_is_network(struct mntent *me)
{
	assert(me);
...
static bool
mount_in_initrd(struct mntent *me)
{
	assert(me);
...
static int
write_requires_after(FILE *f, const char *opts)
{
...
static int
write_requires_mounts_for(FILE *f, const char *opts)
{
...
static int
write_idle_timeout(FILE *f, const char *where, const char *opts)
{
	...
	r = fstab_filter_options(opts, "x-systemd.idle-timeout\0", NULL,
		&timeout, NULL);
	if (r < 0)
		return log_warning_errno(r, "Failed to parse options: %m");
	if (r == 0)
...
static int
add_mount(const char *what, const char *where, const char *fstype,
	const char *opts, int passno, bool noauto, bool nofail, bool automount,
	const char *post, const char *source)
{
	...
	if (!is_path(where)) {
		log_warning("Mount point %s is not a valid path, ignoring.",
			where);
		return 0;
	}
	...
	if (!noauto && post) {
		lnk = strjoin(arg_dest, "/", post,
			nofail || automount ? ".wants/" : ".requires/", name,
			NULL);
		if (!lnk)
			return log_oom();
...
parse_fstab(bool initrd)
{
	...
	while ((me = getmntent(f))) {
		...
		where = initrd ? strappend("/sysroot/", me->mnt_dir) :
				       strdup(me->mnt_dir);
		if (!where)
			return log_oom();
		...
		log_debug(
			...
			what, where, me->mnt_type, yes_no(noauto),
			yes_no(nofail));
		...
		if (streq(me->mnt_type, "swap"))
			...
			k = add_mount(what, where, me->mnt_type, me->mnt_opts,
				me->mnt_passno, noauto, nofail, automount, post,
				fstab_path);
		}
...
add_sysroot_mount(void)
{
	...
	if (!path_is_absolute(what)) {
		log_debug("Skipping entry what=%s where=/sysroot type=%s", what,
	...
	if (!arg_root_options)
		...
		!fstab_test_option(arg_root_options,
			"ro\0"
			"rw\0"))
		opts = strjoina(arg_root_options, ",",
			arg_root_rw > 0 ? "rw" : "ro");
	else
	...
	log_debug("Found entry what=%s where=/sysroot type=%s", what,
		strna(arg_root_fstype));
	return add_mount(what, "/sysroot", arg_root_fstype, opts, 1, false,
		false, false, SPECIAL_INITRD_ROOT_FS_TARGET, "/proc/cmdline");
}

static int
add_sysroot_usr_mount(void)
{
	_cleanup_free_ char *what = NULL;
	const char *opts;
	...
	if (!path_is_absolute(what)) {
		log_debug("Skipping entry what=%s where=/sysroot/usr type=%s",
			what, strna(arg_usr_fstype));
		return -1;
	}
	...
	return add_mount(what, "/sysroot/usr", arg_usr_fstype, opts, 1, false,
		false, false, SPECIAL_INITRD_ROOT_FS_TARGET, "/proc/cmdline");
}
