...
static int
bus_get_uint64_property(sd_bus *bus, const char *path, const char *interface,
...
static int
bus_get_unit_property_strv(sd_bus *bus, const char *path, const char *property,
	char ***strv)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	int r;
...
static int
compare_unit_time(const void *a, const void *b)
{
...
static int
compare_unit_start(const void *a, const void *b)
{
...
static void
free_unit_times(struct unit_times *t, unsigned n)
{
	struct unit_times *p;
...
static int
acquire_time_data(sd_bus *bus, struct unit_times **out)
{
...
static int
acquire_boot_times(sd_bus *bus, struct boot_times **bt)
{
...
static void
free_host_info(struct host_info *hi)
{
	free(hi->hostname);
	free(hi->kernel_name);
...
static int
acquire_host_info(sd_bus *bus, struct host_info **hi)
{
	int r;
	...
	static const struct bus_properties_map hostname_map[] = {
		...
		{ "KernelName", "s", NULL,
			offsetof(struct host_info, kernel_name) },
	...
	static const struct bus_properties_map manager_map[] = {
		...
		{ "Architecture", "s", NULL,
			offsetof(struct host_info, architecture) },
		{}
	};
	...
	r = bus_map_all_properties(bus, "org.freedesktop.hostname1",
		"/org/freedesktop/hostname1", hostname_map, host);
	if (r < 0)
		goto fail;
...
static int
pretty_boot_time(sd_bus *bus, char **_buf)
{
	...
	if (t->firmware_time)
		size = strpcpyf(&ptr, size, "%s (firmware) + ",
			format_timespan(ts, sizeof(ts),
				...
				USEC_PER_MSEC));
	if (t->loader_time)
		size = strpcpyf(&ptr, size, "%s (loader) + ",
			format_timespan(ts, sizeof(ts), t->loader_time,
				USEC_PER_MSEC));
	if (t->kernel_time)
	...
	size = strpcpyf(&ptr, size, "%s (userspace) ",
		format_timespan(ts, sizeof(ts),
			t->finish_time - t->userspace_time, USEC_PER_MSEC));
	if (t->kernel_time > 0)
		strpcpyf(&ptr, size, "= %s",
			format_timespan(ts, sizeof(ts),
...
static void
svg_graph_box(double height, double begin, double end)
{
	long long i;
	...
	for (i = ((long long)(begin / 100000)) * 100000; i <= end;
		i += 100000) {
	...
	for (u = times; u < times + n; u++) {
		...
		if (u->time)
			svg_text(b, u->activating, y, "%s (%s)", u->name,
				format_timespan(ts, sizeof(ts), u->time,
					USEC_PER_MSEC));
		else
...
static int
list_dependencies_print(const char *name, unsigned int level,
	unsigned int branches, bool last, struct unit_times *times,
	struct boot_times *boot)
{
	unsigned int i;
	...
	for (i = level; i != 0; i--)
		printf("%s",
			draw_special_char(branches & (1 << (i - 1)) ?
					      DRAW_TREE_VERTICAL :
					      DRAW_TREE_SPACE));
			...
			printf("%s%s @%s +%s%s", ANSI_HIGHLIGHT_RED_ON, name,
				format_timespan(ts, sizeof(ts),
					times->activating -
						boot->userspace_time,
					USEC_PER_MSEC),
				format_timespan(ts2, sizeof(ts2), times->time,
					USEC_PER_MSEC),
				ANSI_HIGHLIGHT_OFF);
...
static int
list_dependencies_one(sd_bus *bus, const char *name, unsigned int level,
	char ***units, unsigned int branches)
{
	...
	STRV_FOREACH (c, deps) {
		...
		r = list_dependencies_print(*c, level, branches, to_print == 0,
			times, boot);
		if (r < 0)
			return r;
...
static int
list_dependencies(sd_bus *bus, const char *name)
{
	...
	if (times) {
		if (times->time)
			printf("%s%s +%s%s\n", ANSI_HIGHLIGHT_RED_ON, id,
				format_timespan(ts, sizeof(ts), times->time,
					USEC_PER_MSEC),
				ANSI_HIGHLIGHT_OFF);
		else if (times->activated > boot->userspace_time)
			printf("%s @%s\n", id,
...
static int
analyze_critical_chain(sd_bus *bus, char *names[])
{
	struct unit_times *times;
	unsigned int i;
	Hashmap *h;
...
analyze_blame(sd_bus *bus)
{
	...
	for (i = 0; i < (unsigned)n; i++) {
		...
		if (times[i].time > 0)
			printf("%16s %s\n",
				format_timespan(ts, sizeof(ts), times[i].time,
					USEC_PER_MSEC),
				times[i].name);
	}
...
static int
graph_one_property(sd_bus *bus, const UnitInfo *u, const char *prop,
	const char *color, char *patterns[])
{
	...
	if (!strv_isempty(arg_dot_from_patterns) && !match_patterns &&
	...
	STRV_FOREACH (unit, units) {
		...
		if (!strv_isempty(arg_dot_to_patterns) && !match_patterns2 &&
...
static int
graph_one(sd_bus *bus, const UnitInfo *u, char *patterns[])
{
	int r;
	...
	if (arg_dot == DEP_REQUIRE || arg_dot == DEP_ALL) {
		...
		if (r < 0)
			return r;
		r = graph_one_property(bus, u, "RequiresOverridable", "black",
			patterns);
		if (r < 0)
			return r;
		r = graph_one_property(bus, u, "RequisiteOverridable",
			"darkblue", patterns);
		if (r < 0)
			return r;
...
static int
set_log_level(sd_bus *bus, char **args)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	int r;
...
parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "help", no_argument, NULL,
		...
		{ "from-pattern", required_argument, NULL,
			ARG_DOT_FROM_PATTERN },
...
main(int argc, char *argv[])
{
	...
	if (streq_ptr(argv[optind], "verify"))
		r = verify_units(argv + optind + 1,
			arg_user ? SYSTEMD_USER : SYSTEMD_SYSTEM, arg_man);
	else {
