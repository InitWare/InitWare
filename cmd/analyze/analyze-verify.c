...
static int
generate_path(char **var, char **filenames)
{
	char **filename;
...
verify_socket(Unit *u)
{
	...
	if (r < 0) {
		log_unit_error(u->id,
			"Socket %s cannot be started, failed to create instance.",
			u->id);
		return r;
	}
	...
	if (UNIT_ISSET(SOCKET(u)->service)) {
		...
		if (UNIT(service)->load_state != UNIT_LOADED) {
			log_unit_error(u->id,
				"Service %s not loaded, %s cannot be started.",
...
static int
verify_executable(Unit *u, ExecCommand *exec)
{
	if (exec == NULL)
		return 0;
...
verify_executables(Unit *u)
{
	...
	exec = u->type == UNIT_SOCKET ? SOCKET(u)->control_command :
		...
		u->type == UNIT_SWAP  ? SWAP(u)->control_command :
					      NULL;
...
static int
verify_documentation(Unit *u, bool check_man)
{
	char **p;
	int r = 0, k;

	STRV_FOREACH (p, u->documentation) {
		log_unit_debug(u->id, "%s: found documentation item %s.", u->id,
			*p);
		if (check_man && startswith(*p, "man:")) {
			...
			if (k != 0) {
				if (k < 0)
					log_unit_error(u->id,
						"%s: can't show %s: %s", u->id,
						*p, strerror(-r));
				else {
					log_unit_error(u->id,
						"%s: man %s command failed with code %d",
						u->id, *p + 4, k);
					k = -ENOEXEC;
				}
				if (r == 0)
...
static int
verify_unit(Unit *u, bool check_man)
{
	...
	r = manager_add_job(u->manager, JOB_START, u, JOB_REPLACE, false, &err,
		&j);
	if (sd_bus_error_is_set(&err))
		log_unit_error(u->id, "Error: %s: %s", err.name, err.message);
	if (r < 0)
		log_unit_error(u->id, "Failed to create %s/start: %s", u->id,
			strerror(-r));
...
int
verify_units(char **filenames, SystemdRunningAs running_as, bool check_man)
{
	_cleanup_bus_error_free_ sd_bus_error err = SD_BUS_ERROR_NULL;
	Manager *m = NULL;
	FILE *serial = NULL;
	...
	STRV_FOREACH (filename, filenames) {
		...
		if (!strchr(*filename, '/') &&
			strlen(*filename) <= UNIT_NAME_MAX) {
			strncat(fname + 2, *filename, UNIT_NAME_MAX);
			k = manager_load_unit(m, NULL, fname, &err,
				&units[count]);
		} else
			k = manager_load_unit(m, NULL, *filename, &err,
				&units[count]);
		if (k < 0) {
