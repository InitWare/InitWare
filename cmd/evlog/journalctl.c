...
static int parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "help", no_argument, NULL, 'h' },
		...
		{ "pager-end", no_argument, NULL, 'e' }, { "follow", no_argument, NULL, 'f' },
		{ "force", no_argument, NULL, ARG_FORCE },
		{ "output", required_argument, NULL, 'o' }, { "all", no_argument, NULL, 'a' },
		{ "full", no_argument, NULL, 'l' }, { "lines", optional_argument, NULL, 'n' },
		{ "no-tail", no_argument, NULL, ARG_NO_TAIL },
		{ "new-id128", no_argument, NULL, ARG_NEW_ID128 },
		{ "quiet", no_argument, NULL, 'q' }, { "merge", no_argument, NULL, 'm' },
		{ "boot", optional_argument, NULL, 'b' },
		{ "list-boots", no_argument, NULL, ARG_LIST_BOOTS },
		{ "this-boot", optional_argument, NULL, 'b' }, /* deprecated */
		{ "dmesg", no_argument, NULL, 'k' }, { "system", no_argument, NULL, ARG_SYSTEM },
		...
		{ "field", required_argument, NULL, 'F' }, { "catalog", no_argument, NULL, 'x' },
		{ "list-catalog", no_argument, NULL, ARG_LIST_CATALOG },
		{ "dump-catalog", no_argument, NULL, ARG_DUMP_CATALOG },
		{ "update-catalog", no_argument, NULL, ARG_UPDATE_CATALOG },
		{ "reverse", no_argument, NULL, 'r' }, { NULL, 0, NULL, 0 } };
	...
	while ((c = getopt_long(argc, argv, "hefo:aln::qmb::kD:p:c:u:F:xr", options, NULL)) >= 0) {
		...
		case 'o':
			...
			if (arg_output == OUTPUT_EXPORT || arg_output == OUTPUT_JSON ||
			    arg_output == OUTPUT_JSON_PRETTY || arg_output == OUTPUT_JSON_SSE ||
		...
		case 'n':
			if (optarg) {
				...
				if (optind < argc && safe_atoi(argv[optind], &n) >= 0 && n >= 0) {
		...
		case 'b':
			...
			if (optarg) {
				...
				if (optind < argc &&
				    parse_boot_descriptor(argv[optind], &arg_boot_id,
					&arg_boot_offset) >= 0)
					optind++;
			}
	...
	STRV_FOREACH (i, args) {
		...
		if (streq(*i, "+")) {
			...
			if (S_ISREG(st.st_mode) && (0111 & st.st_mode)) {
				...
				asprintf(&t, "_KERNEL_DEVICE=c%u:%u", major(st.st_rdev),
				    minor(st.st_rdev));
			else if (S_ISBLK(st.st_mode))
				asprintf(&t, "_KERNEL_DEVICE=b%u:%u", major(st.st_rdev),
				    minor(st.st_rdev));
			else {
				log_error("File is neither a device node, nor regular file, nor executable: %s",
				    *i);
				return -EINVAL;
			}
...
static int list_boots(sd_journal *j)
{
	...
	for (id = all_ids, i = 0; id < all_ids + count; id++, i++) {
		...
		printf("% *i " SD_ID128_FORMAT_STR " %s—%s\n", w, i - count + 1,
		    SD_ID128_FORMAT_VAL(id->id), format_timestamp(a, sizeof(a), id->first),
...
static int setup_keys(void)
{
	...
	r = stat("/var/run/journal", &st);
	if (r < 0 && errno != ENOENT && errno != ENOTDIR) {
		log_error("stat(\"%s\") failed: %m", "/var/run/journal");
		return -errno;
	}
	...
	if (asprintf(&p, "/var/run/journal/" SD_ID128_FORMAT_STR "/fss",
	...
	if (asprintf(&k, "/var/run/journal/" SD_ID128_FORMAT_STR "/fss.tmp.XXXXXX",
		SD_ID128_FORMAT_VAL(machine)) < 0) {
		r = log_oom();
		goto finish;
	}
...
static int access_check_var_log_journal(sd_journal *j)
{
	...
	if (!have_access) {
		...
		r = search_acl_groups(&g, "/var/run/journal/", &have_access);
		if (r < 0)
			return r;
	}
...
static int access_check(sd_journal *j)
{
	...
	if (set_contains(j->errors, INT_TO_PTR(-EACCES))) {
#ifdef HAVE_ACL
		/* If /var/run/journal doesn't even exist,
		 * unprivileged users have no access at all */
		if (access("/var/run/journal", F_OK) < 0 && geteuid() != 0 &&
		...
		/* If /var/run/journal exists, try to pring a nice
		...
		if (geteuid() != 0 && in_group("systemd-journal") <= 0) {
			log_error(
			    "Unprivileged users cannot access messages. Users in the 'systemd-journal' group\n"
...
int main(int argc, char *argv[])
{
	...
	if (arg_action == ACTION_UPDATE_CATALOG || arg_action == ACTION_LIST_CATALOG ||
	    arg_action == ACTION_DUMP_CATALOG) {

		const char *database = "/usr/local/share/InitWare/catalog"; // FIXME: CATALOG_DATABASE;
		_cleanup_free_ char *copy = NULL;
		if (arg_root) {
			copy = strjoin(arg_root, "/", database /* CATALOG_DATABASE */, NULL);
			if (!copy) {
				r = log_oom();
				goto finish;
			}
		...
		while (arg_lines < 0 || n_shown < arg_lines || (arg_follow && !first_line)) {
			...
			if (need_seek) {
				if (!arg_reverse)
					...
					r = sd_journal_previous(j);
				if (r < 0) {
					log_error("Failed to iterate through journal: %s",
					    strerror(-r));
					goto finish;
				}
				if (r == 0)
			...
			flags = arg_all * OUTPUT_SHOW_ALL |
			    ...
			    on_tty() * OUTPUT_COLOR | arg_catalog * OUTPUT_CATALOG;
