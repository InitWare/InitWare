...
static void request_meta_free(void *cls, struct MHD_Connection *connection, void **connection_cls,
...
static int respond_oom_internal(struct MHD_Connection *connection)
{
	...
	response = MHD_create_response_from_buffer(sizeof(m) - 1, (char *) m,
...
static int respond_error(struct MHD_Connection *connection, unsigned code, const char *format, ...)
{
	...
	while (pos >= m->size) {
		...
		if (r < 0) {
			...
			if (m->follow) {
				...
				if (r < 0) {
					log_error("Couldn't wait for journal event: %s",
					    strerror(-r));
					return MHD_CONTENT_READER_END_WITH_ERROR;
				}
...
static int request_parse_arguments_iterator(void *cls, enum MHD_ValueKind kind, const char *key,
    const char *value)
{
...
static int request_handler_fields(struct MHD_Connection *connection, const char *field,
    void *connection_cls)
{
...
static int request_handler_file(struct MHD_Connection *connection, const char *path,
...
static int request_handler_machine(struct MHD_Connection *connection, void *connection_cls)
{
	...
	r = asprintf(&json,
	    ...
	    SD_ID128_FORMAT_VAL(mid), SD_ID128_FORMAT_VAL(bid), hostname_cleanup(hostname, false),
	    os_name ? os_name : "Linux", v ? v : "bare", usage, cutoff_from, cutoff_to);
...
static int parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "help", no_argument, NULL, 'h' },
		...
		{ "cert", required_argument, NULL, ARG_CERT }, { NULL, 0, NULL, 0 } };
...
int main(int argc, char *argv[])
{
	...
	if (n < 0) {
		...
		if (n > 0)
			opts[opts_pos++] = (struct MHD_OptionItem){ MHD_OPTION_LISTEN_SOCKET,
				SD_LISTEN_FDS_START };
		if (key_pem) {
			assert(cert_pem);
			opts[opts_pos++] = (struct MHD_OptionItem){ MHD_OPTION_HTTPS_MEM_KEY, 0,
				key_pem };
			opts[opts_pos++] = (struct MHD_OptionItem){ MHD_OPTION_HTTPS_MEM_CERT, 0,
				cert_pem };
			flags |= MHD_USE_SSL;
		}

		d = MHD_start_daemon(flags, 19531, NULL, NULL, request_handler, NULL,
