...
enum { ARG_PID = 1, ARG_UID, ARG_GID, ARG_SIGNAL, ARG_TIMESTAMP, ARG_COMM, _ARG_MAX };
...
static int divert_coredump(void)
{
	...
	log_info(
	    "Detected coredump of the journal daemon itself, diverting coredump to " AbsDir_PkgVarLib
	    "/coredump/.");
...
int main(int argc, char *argv[])
{
	...
	_cleanup_free_ char *core_pid = NULL, *core_uid = NULL, *core_gid = NULL,
			    *core_signal = NULL, *core_timestamp = NULL, *core_comm = NULL,
			    *core_exe = NULL, *core_unit = NULL, *core_session = NULL,
			    *core_message = NULL, *core_cmdline = NULL, *coredump_data = NULL;
	...
	core_message = strjoin("MESSAGE=Process ", argv[ARG_PID], " (", argv[ARG_COMM],
	    ") dumped core.", NULL);
	if (core_message)
