...
static DEFINE_CONFIG_PARSE_ENUM(config_parse_coredump_storage, coredump_storage,
	CoredumpStorage, "Failed to parse storage setting");
...
parse_config(void)
{
	static const ConfigTableItem items[] = {
		{ "Coredump", "Storage", config_parse_coredump_storage, 0,
			&arg_storage },
		{ "Coredump", "Compress", config_parse_bool, 0, &arg_compress },
		{ "Coredump", "ProcessSizeMax", config_parse_iec_off, 0,
			&arg_process_size_max },
		{ "Coredump", "ExternalSizeMax", config_parse_iec_off, 0,
			&arg_external_size_max },
		{ "Coredump", "JournalSizeMax", config_parse_iec_size, 0,
			&arg_journal_size_max },
		{ "Coredump", "KeepFree", config_parse_iec_off, 0,
			&arg_keep_free },
	...
	return config_parse_many(SVC_PKGSYSCONFDIR "/coredump.conf",
		CONF_DIRS_NULSTR(SVC_PKGDIRNAME "/coredump.conf"), "Coredump\0",
		config_item_table_lookup, items, false, NULL);
}

static int
fix_acl(int fd, uid_t uid)
{
#ifdef HAVE_ACL
...
static int
fix_xattr(int fd, const char *info[_INFO_LEN])
{
	...
	for (i = 0; i < _INFO_LEN; i++) {
		...
		k = fsetxattr(fd, xattrs[i], info[i], strlen(info[i]),
			XATTR_CREATE);
		if (k < 0 && r == 0)
			r = -errno;
	}
...
static int
fix_permissions(int fd, const char *filename, const char *target,
	const char *info[_INFO_LEN], uid_t uid)
{
	...
	if (fsync(fd) < 0)
		return log_error_errno(errno, "Failed to sync coredump %s: %m",
			filename);
...
static int
maybe_remove_external_coredump(const char *filename, off_t size)
{
	...
	if (unlink(filename) < 0 && errno != ENOENT)
		return log_error_errno(errno, "Failed to unlink %s: %m",
			filename);
...
static int
make_filename(const char *info[_INFO_LEN], char **ret)
{
...
static int
save_external_coredump(const char *info[_INFO_LEN], uid_t uid,
	char **ret_filename, int *ret_fd, off_t *ret_size)
{
	...
	if (r == -EFBIG) {
		log_error(
			"Coredump of %s (%s) is larger than configured processing limit, refusing.",
			info[INFO_PID], info[INFO_COMM]);
		goto fail;
	} else if (IN_SET(r, -EDQUOT, -ENOSPC)) {
		log_error(
			"Not enough disk space for coredump of %s (%s), refusing.",
			info[INFO_PID], info[INFO_COMM]);
		goto fail;
	} else if (r < 0) {
	...
	if (maybe_remove_external_coredump(NULL, st.st_size) == 0 &&
		...
		if (r < 0) {
			log_error_errno(r,
				"Failed to determine temporary file name for %s: %m",
				fn_compressed);
			goto uncompressed;
		}

		fd_compressed = open(tmp_compressed,
			O_CREAT | O_EXCL | O_RDWR | O_CLOEXEC | O_NOCTTY |
				O_NOFOLLOW,
			0640);
		if (fd_compressed < 0) {
		...
		if (r < 0) {
			log_error_errno(r, "Failed to compress %s: %m",
				tmp_compressed);
			goto fail_compressed;
		}

		r = fix_permissions(fd_compressed, tmp_compressed,
			fn_compressed, info, uid);
		if (r < 0)
			goto fail_compressed;
...
static int
allocate_journal_field(int fd, size_t size, char **ret, size_t *ret_size)
{
 ...
 */
static int
compose_open_fds(pid_t pid, char **open_fds)
{
	...
	FOREACH_DIRENT (dent, proc_fd_dir, return -errno) {
		...
		r = readlinkat_malloc(dirfd(proc_fd_dir), dent->d_name,
			&fdname);
		if (r < 0)
			return r;
		...
		fd = openat(proc_fdinfo_fd, dent->d_name,
...
main(int argc, char *argv[])
{
	...
	_cleanup_free_ char *core_timestamp = NULL, *core_message = NULL,
			    *coredump_data = NULL, *core_owner_uid = NULL,
			    *core_open_fds = NULL, *core_proc_status = NULL,
			    *core_proc_maps = NULL, *core_proc_limits = NULL,
	...
	if (argc < INFO_COMM + 1) {
		log_error(
			"Not enough arguments passed from kernel (%d, expected %d).",
	...
	if (get_process_comm(pid, &comm) < 0) {
		log_warning(
			"Failed to get COMM, falling back to the command line.");
	...
	if (cg_pid_get_unit(pid, &t) >= 0) {
		if (streq(t, SPECIAL_JOURNALD_SERVICE)) {
			...
			r = save_external_coredump(info, uid, &filename,
				&coredump_fd, &coredump_size);
			if (r < 0)
				goto finish;

			r = maybe_remove_external_coredump(filename,
				coredump_size);
			if (r < 0)
				goto finish;

			log_info(
				"Detected coredump of the journal daemon itself, diverted to %s.",
				filename);
			goto finish;
		}

		if (!set_iovec_field_free(iovec, &n_iovec,
			    "COREDUMP_UNIT=", t)) {
			r = log_oom();
			goto finish;
		}
	...
	if (!set_iovec_string_field(iovec, &n_iovec,
		    "COREDUMP_PID=", info[INFO_PID])) {
		r = log_oom();
		goto finish;
	}

	if (!set_iovec_string_field(iovec, &n_iovec,
		    "COREDUMP_UID=", info[INFO_UID])) {
		r = log_oom();
		goto finish;
	}

	if (!set_iovec_string_field(iovec, &n_iovec,
		    "COREDUMP_GID=", info[INFO_GID])) {
		r = log_oom();
		goto finish;
	}

	if (!set_iovec_string_field(iovec, &n_iovec,
		    "COREDUMP_SIGNAL=", info[INFO_SIGNAL])) {
		r = log_oom();
		goto finish;
	}

	if (comm &&
		!set_iovec_string_field(iovec, &n_iovec,
			"COREDUMP_COMM=", comm)) {
		r = log_oom();
		goto finish;
	}
	...
	if (sd_pid_get_owner_uid(pid, &owner_uid) >= 0) {
		r = asprintf(&core_owner_uid, "COREDUMP_OWNER_UID=" UID_FMT,
			owner_uid);
		if (r > 0)
	...
	if (read_full_file(p, &t, NULL) >= 0)
		set_iovec_field_free(iovec, &n_iovec,
	...
	core_timestamp = strjoin("COREDUMP_TIMESTAMP=", info[INFO_TIMESTAMP],
		"000000", NULL);
	if (core_timestamp)
		IOVEC_SET_STRING(iovec[n_iovec++], core_timestamp);

	IOVEC_SET_STRING(iovec[n_iovec++],
	...
	r = save_external_coredump(info, uid, &filename, &coredump_fd,
		&coredump_size);
	if (r < 0)
	...
	if (coredump_size <= arg_process_size_max) {
		...
		if (r >= 0)
			core_message = strjoin("MESSAGE=Process ",
				info[INFO_PID], " (", comm, ") of user ",
				info[INFO_UID], " dumped core.\n\n", stacktrace,
				NULL);
		else if (r == -EINVAL)
...
#endif
	log:
		core_message = strjoin("MESSAGE=Process ", info[INFO_PID], " (",
			comm, ") of user ", info[INFO_UID], " dumped core.",
			NULL);
	if (core_message)
