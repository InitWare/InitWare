...
static int
add_match(Set *set, const char *match)
{
	int r = -ENOMEM;
	unsigned pid;
	const char *prefix;
...
static int
parse_argv(int argc, char *argv[], Set *matches)
{
	enum {
		ARG_VERSION = 0x100,
		ARG_NO_PAGER,
...
static int
retrieve(const void *data, size_t len, const char *name, char **var)
{
	size_t ident;
	char *v;
...
static void
print_field(FILE *file, sd_journal *j)
{
...
static int
print_list(FILE *file, sd_journal *j, int had_legend)
{
	_cleanup_free_ char *pid = NULL, *uid = NULL, *gid = NULL, *sgnl = NULL,
			    *exe = NULL, *comm = NULL, *cmdline = NULL,
			    *filename = NULL;
	const void *d;
	size_t l;
	usec_t t;
	...
	if (!had_legend && !arg_no_legend)
		fprintf(file, "%-*s %*s %*s %*s %*s %*s %s\n",
			FORMAT_TIMESTAMP_WIDTH, "TIME", 6, "PID", 5, "UID", 5,
			"GID", 3, "SIG", 1, "PRESENT", "EXE");

	fprintf(file, "%-*s %*s %*s %*s %*s %*s %s\n", FORMAT_TIMESTAMP_WIDTH,
		buf, 6, strna(pid), 5, strna(uid), 5, strna(gid), 3,
		strna(sgnl), 1, present ? "*" : "",
...
static int
print_info(FILE *file, sd_journal *j, bool need_space)
{
	_cleanup_free_ char *pid = NULL, *uid = NULL, *gid = NULL, *sgnl = NULL,
			    *exe = NULL, *comm = NULL, *cmdline = NULL,
			    *unit = NULL, *user_unit = NULL, *session = NULL,
			    *boot_id = NULL, *machine_id = NULL,
			    *hostname = NULL, *slice = NULL, *cgroup = NULL,
	...
	if (comm)
		fprintf(file, "           PID: %s%s%s (%s)\n", ansi_highlight(),
			strna(pid), ansi_highlight_off(), comm);
	else
		fprintf(file, "           PID: %s%s%s\n", ansi_highlight(),
			strna(pid), ansi_highlight_off());
	...
	if (timestamp) {
		...
		if (r >= 0) {
			char absolute[FORMAT_TIMESTAMP_MAX],
				relative[FORMAT_TIMESPAN_MAX];

			fprintf(file, "     Timestamp: %s (%s)\n",
				format_timestamp(absolute, sizeof(absolute), u),
				format_timestamp_relative(relative,
					sizeof(relative), u));
...
static void
print_entry(sd_journal *j, unsigned n_found)
{
	assert(j);
...
static int
save_core(sd_journal *j, int fd, char **path, bool *unlink_temp)
{
	const char *data;
	...
	r = sd_journal_get_data(j, "COREDUMP_FILENAME", (const void **)&data,
		&len);
	if (r < 0 && r != -ENOENT)
		log_warning_errno(r,
...
dump_core(sd_journal *j)
{
	...
	r = save_core(j, arg_output ? fileno(arg_output) : STDOUT_FILENO, NULL,
		NULL);
	if (r < 0)
