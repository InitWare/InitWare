...
#define DEFAULT_MAX_USE_UPPER                                                  \
	(off_t)(4ULL * 1024ULL * 1024ULL * 1024ULL) /* 4 GiB */
#define DEFAULT_KEEP_FREE_UPPER                                                \
	(off_t)(4ULL * 1024ULL * 1024ULL * 1024ULL) /* 4 GiB */
...
static void
vacuum_candidate_free(struct vacuum_candidate *c)
{
	if (!c)
		return;
...
static void
vacuum_candidate_hasmap_free(Hashmap *h)
{
...
static int
uid_from_file_name(const char *filename, uid_t *uid)
{
	const char *p, *e, *u;
...
static bool
vacuum_necessary(int fd, off_t sum, off_t keep_free, off_t max_use)
{
...
int
coredump_vacuum(int exclude_fd, off_t keep_free, off_t max_use)
{
	...
	for (;;) {
		...
		FOREACH_DIRENT (de, d, goto fail) {
			...
			if (exclude_fd >= 0 && exclude_st.st_dev == st.st_dev &&
			...
			if (c) {
				...
				_cleanup_(
					vacuum_candidate_freep) struct vacuum_candidate
					*n = NULL;
			...
			if (!worst || worst->n_files < c->n_files ||
				(worst->n_files == c->n_files &&
					c->oldest_mtime < worst->oldest_mtime))
				worst = c;
		...
		if (unlinkat(dirfd(d), worst->oldest_file, 0) < 0) {
			...
			log_error_errno(errno, "Failed to remove file %s: %m",
				worst->oldest_file);
			return -errno;
		} else
			log_info("Removed old coredump %s.",
				worst->oldest_file);
	}
