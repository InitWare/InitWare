...
static int add_match(Set *set, const char *match)
{
	...
	if (r < 0) {
		log_error("Failed to add pattern '%s': %s", pattern, strerror(-r));
		free(pattern);
		goto fail;
	}
...
static int parse_argv(int argc, char *argv[], Set *matches)
{
	...
	static const struct option options[] = { { "help", no_argument, NULL, 'h' },
		...
		{ "field", required_argument, NULL, 'F' }, { NULL, 0, NULL, 0 } };
...
static int retrieve(const void *data, size_t len, const char *name, const char **var)
{
...
{
	_cleanup_free_ const char *pid = NULL, *uid = NULL, *gid = NULL, *sgnl = NULL, *exe = NULL;
	const void *d;
	size_t l;
	usec_t t;
	...
	if (!had_legend && !arg_no_legend)
		fprintf(file, "%-*s %*s %*s %*s %*s %s\n", FORMAT_TIMESTAMP_MAX - 1, "TIME", 6,
		    "PID", 5, "UID", 5, "GID", 3, "SIG", "EXE");

	fprintf(file, "%*s %*s %*s %*s %*s %s\n", FORMAT_TIMESTAMP_MAX - 1, buf, 6, pid, 5, uid, 5,
	    gid, 3, sgnl, exe);
...
int main(int argc, char *argv[])
{
	...
	SET_FOREACH (match, matches, it) {
		...
		if (r != 0) {
			log_error("Failed to add match '%s': %s", match, strerror(-r));
			goto end;
		}
	}
