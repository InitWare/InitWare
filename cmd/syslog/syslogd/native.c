...
bool
valid_user_field(const char *p, size_t l, bool allow_protected)
{
	const char *a;
	...
	const char *label, size_t label_len)
{
	...
	while (remaining > 0) {
		...
		if (e == p) {
			...
			if (entry_size + n + 1 >
				ENTRY_SIZE_MAX) { /* data + separators + trailer */
				log_debug(
		...
		if (q) {
			...
			if (l > DATA_SIZE_MAX) {
				log_debug(
					"Received binary data block of %" PRIu64
					" bytes is too large, ignoring.",
					l);
				break;
			}

			if ((uint64_t)remaining <
					e - p + 1 + sizeof(uint64_t) + l + 1 ||
	...
	if (message) {
		if (s->forward_to_syslog)
			server_forward_syslog(s, priority, identifier, message,
				ucred, tv);
		...
		if (s->forward_to_console)
			server_forward_console(s, priority, identifier, message,
				ucred);
...
finish:
	for (j = 0; j < n; j++) {
		...
		if (iovec[j].iov_base < buffer ||
			(const uint8_t *)iovec[j].iov_base >=
	...
	const struct timeval *tv, const char *label, size_t label_len)
{
	...
	if (sealed) {
		...
		server_process_native_message(s, p, st.st_size, ucred, tv,
			label, label_len);
...
int
server_open_native_socket(Server *s)
{
...
#ifdef HAVE_SELINUX
	if (mac_selinux_use()) {
		r = setsockopt(s->native_fd, SOL_SOCKET, SO_PASSSEC, &one,
			sizeof(one));
		if (r < 0)
	...
	r = setsockopt(s->native_fd, SOL_SOCKET, SO_TIMESTAMP, &one,
		sizeof(one));
	if (r < 0)
