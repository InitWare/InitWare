...
prefix_timestamp(void)
{
	...
	if (_unlikely_(cached_printk_time < 0)) {
		...
		cached_printk_time =
			read_one_line_file("/sys/module/printk/parameters/time",
				&p) >= 0 &&
			parse_boolean(p) > 0;
	}
	...
	const char *message, const struct socket_ucred *ucred)
{
	...
	struct timespec ts;
	char tbuf[sizeof("[] ") - 1 + DECIMAL_STR_MAX(ts.tv_sec) +
	...
	if (prefix_timestamp()) {
		...
		xsprintf(tbuf, "[%5" PRI_TIME ".%06ld] ", ts.tv_sec,
			ts.tv_nsec / 1000);
	...
	if (writev(fd, iovec, n) < 0)
		log_debug_errno(errno, "Failed to write to %s for logging: %m",
			tty);
