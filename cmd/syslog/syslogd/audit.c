...
typedef struct MapField {
	...
	int (*map)(const char *field, const char **p, struct iovec **iov,
		size_t *n_iov_allocated, unsigned *n_iov);
} MapField;

static int
map_simple_field(const char *field, const char **p, struct iovec **iov,
	size_t *n_iov_allocated, unsigned *n_iov)
{
...
static int
map_string_field_internal(const char *field, const char **p, struct iovec **iov,
	size_t *n_iov_allocated, unsigned *n_iov, bool filter_printable)
{
	_cleanup_free_ char *c = NULL;
	const char *s, *e;
	size_t l;
...
static int
map_string_field(const char *field, const char **p, struct iovec **iov,
	size_t *n_iov_allocated, unsigned *n_iov)
{
...
static int
map_string_field_printable(const char *field, const char **p,
	struct iovec **iov, size_t *n_iov_allocated, unsigned *n_iov)
{
	return map_string_field_internal(field, p, iov, n_iov_allocated, n_iov,
		true);
}
...
static int
map_generic_field(const char *prefix, const char **p, struct iovec **iov,
	size_t *n_iov_allocated, unsigned *n_iov)
{
	const char *e, *f;
	char *c, *t;
...
static int
map_all_fields(const char *p, const MapField map_fields[], const char *prefix,
	bool handle_msg, struct iovec **iov, size_t *n_iov_allocated,
	unsigned *n_iov)
{
	int r;
	...
	for (;;) {
		...
		if (handle_msg) {
			...
			if (v) {
				...
				c = strndupa(v, e - v);
				return map_all_fields(c, map_fields_userspace,
		...
		for (m = map_fields; m->audit_field; m++) {
			...
			r = m->map(m->journal_field, &v, iov, n_iov_allocated,
				n_iov);
			if (r < 0)
		...
		if (!mapped) {
			r = map_generic_field(prefix, &p, iov, n_iov_allocated,
				n_iov);
			if (r < 0)
				return log_debug_errno(r,
					"Failed to parse audit array: %m");
...
static void
process_audit_string(Server *s, int type, const char *data, size_t size)
{
	...
	if (sscanf(p, "(%" PRIu64 ".%" PRIu64 ":%" PRIu64 "):%n", &seconds,
		    &msec, &id, &k) != 3)
		return;
	...
	if (!GREEDY_REALLOC(iov, n_iov_allocated,
		    n_iov + N_IOVEC_META_FIELDS)) {
		log_oom();
		goto finish;
	}
...
server_open_audit(Server *s)
{
	...
	if (s->audit_fd < 0) {
		...
		s->audit_fd = socket(AF_NETLINK,
			SOCK_RAW | SOCK_CLOEXEC | SOCK_NONBLOCK, NETLINK_AUDIT);
		if (s->audit_fd < 0) {
	...
	r = sd_event_add_io(s->event, &s->audit_event_source, s->audit_fd,
