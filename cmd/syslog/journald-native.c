    ...
    struct socket_ucred *ucred, struct timeval *tv, const char *label, size_t label_len)
{
	...
	while (remaining > 0) {
		...
		if (e == p) {
			/* Entry separator */
			server_dispatch_message(s, iovec, n, m, ucred, tv, label, label_len, NULL,
			    priority, object_pid);
			n = 0;
			priority = LOG_INFO;
		...
		if (!GREEDY_REALLOC(iovec, m,
			n + 1 + N_IOVEC_META_FIELDS + !!object_pid * N_IOVEC_OBJECT_FIELDS)) {
			log_oom();
			break;
		}
		...
		if (q) {
			if (valid_user_field(p, q - p)) {
				 ...
				 * logic */
				if (l == 10 && startswith(p, "PRIORITY=") && p[9] >= '0' &&
				    p[9] <= '9')
					priority = (priority & LOG_FACMASK) | (p[9] - '0');

				else if (l == 17 && startswith(p, "SYSLOG_FACILITY=") &&
				    p[16] >= '0' && p[16] <= '9')
					priority = (priority & LOG_PRIMASK) | ((p[16] - '0') << 3);

				else if (l == 18 && startswith(p, "SYSLOG_FACILITY=") &&
				    p[16] >= '0' && p[16] <= '9' && p[17] >= '0' && p[17] <= '9')
...
int server_open_native_socket(Server *s)
{
	...
	if (s->native_watch.fd < 0) {
		...
		strncpy(sa.un.sun_path, INSTALL_PKGRUNSTATE_DIR "/journal/socket",
