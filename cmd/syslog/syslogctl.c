...
typedef struct BootId {
	...
	uint64_t first;
	uint64_t last;
	IWLIST_FIELDS(struct BootId, boot_list);
} BootId;
...
add_matches_for_device(sd_journal *j, const char *devpath)
{
	...
	d = device = udev_device_new_from_devnum(udev,
		...
		if (devnode) {
			...
			if (r < 0)
				return log_error_errno(r,
					"Failed to stat() device node \"%s\": %m",
					devnode);
...
static char *
format_timestamp_maybe_utc(char *buf, size_t l, usec_t t)
{
	if (arg_utc)
...
static int
parse_boot_descriptor(const char *x, sd_id128_t *boot_id, int *offset)
{
...
parse_argv(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv,
		...
		case 'b':
			...
			if (optarg) {
				...
				if (r < 0) {
					log_error(
						"Failed to parse boot descriptor '%s'",
						optarg);
					return -EINVAL;
				}
			} else {
				...
				if (optind < argc &&
					parse_boot_descriptor(argv[optind],
						&arg_boot_id,
						&arg_boot_offset) >= 0)
					optind++;
			}
		...
		case ARG_VACUUM_SIZE:
			...
			if (r < 0) {
				log_error("Failed to parse vacuum size: %s",
					optarg);
				return r;
			}
		...
		case ARG_VACUUM_TIME:
			...
			if (r < 0) {
				log_error("Failed to parse vacuum time: %s",
					optarg);
				return r;
			}
		...
		case ARG_INTERVAL:
			...
			if (r < 0 || arg_interval <= 0) {
				log_error(
					"Failed to parse sealing key change interval: %s",
					optarg);
				return -EINVAL;
			}
			break;
		...
		case 'p': {
			...
			if (dots) {
				...
				if (from < 0 || to < 0) {
					log_error(
						"Failed to parse log level range %s",
						optarg);
					return -EINVAL;
				}
		...
		case 'S':
			...
			if (r < 0) {
				log_error("Failed to parse timestamp: %s",
					optarg);
				return -EINVAL;
			}
			arg_since_set = true;
	...
	if (!!arg_directory + !!arg_file + !!arg_machine > 1) {
		log_error(
			"Please specify either -D/--directory= or --file= or -M/--machine=, not more than one.");
		return -EINVAL;
	}
	...
	if (arg_action != ACTION_SHOW && optind < argc) {
		log_error("Extraneous arguments starting with '%s'",
			argv[optind]);
		return -EINVAL;
	}
...
static int
add_matches(sd_journal *j, char **args)
{
	char **i;
	...
	STRV_FOREACH (i, args) {
		...
		if (streq(*i, "+")) {
			...
			if (S_ISREG(st.st_mode) && (0111 & st.st_mode)) {
				if (executable_is_script(path, &interpreter) >
					0) {
...
static void
boot_id_free_all(BootId *l)
{
	while (l) {
		BootId *i = l;
		IWLIST_REMOVE(boot_list, l, i);
		free(i);
	}
}

static int
discover_next_boot(sd_journal *j, sd_id128_t previous_boot_id,
...
static int
get_boots(sd_journal *j, BootId **boots, BootId *query_ref_boot,
	int ref_boot_offset)
{
	bool skip_once;
	int r, count = 0;
	...
	skip_once = query_ref_boot && sd_id128_is_null(query_ref_boot->id) &&
		ref_boot_offset <= 0;
	...
	for (;;) {
		...
		r = discover_next_boot(j, previous_boot_id, advance_older,
			&current);
		if (r < 0) {
		...
		if (query_ref_boot) {
			...
			IWLIST_INSERT_AFTER(boot_list, head, tail, current);
			tail = current;
			current = NULL;
			count++;
		}
...
add_boot(sd_journal *j)
{
	...
	if (arg_boot_offset == 0 &&
		sd_id128_equal(arg_boot_id, SD_ID128_NULL) && !arg_directory &&
		!arg_file)
	...
	assert(r <= 1);
	if (r <= 0) {
		const char *reason = (r == 0) ? "No such boot ID in journal" :
						      strerror(-r);

		if (sd_id128_is_null(arg_boot_id))
			log_error("Failed to look up boot %+i: %s",
...
static int
get_possible_units(sd_journal *j, const char *fields, char **patterns,
	Set **units)
{
...
static int
add_syslog_identifier(sd_journal *j)
{
	int r;
	char **i;
...
setup_keys(void)
{
	...
	if (access(p, F_OK) >= 0) {
		if (arg_force) {
			...
			log_error(
				"Sealing key file %s exists already. (--force to recreate)",
				p);
			r = -EEXIST;
			goto finish;
		}
	...
	if (on_tty()) {
		fprintf(stderr,
			"\n"
			"The new key pair has been generated. The " ANSI_HIGHLIGHT_ON
			...
			"\t%s\n"
			"\n"
			"Please write down the following " ANSI_HIGHLIGHT_ON
...
verify(sd_journal *j)
{
	...
	ORDERED_HASHMAP_FOREACH (f, j->files, i) {
		...
		k = journal_file_verify(f, arg_verify_key, &first, &validated,
			&last, true);
		if (k == -EINVAL) {
			...
			if (arg_verify_key &&
				...
				if (validated > 0) {
					log_info(
						...
						format_timestamp_maybe_utc(a,
							sizeof(a), first),
						format_timestamp_maybe_utc(b,
							sizeof(b), validated),
...
static int
access_check_var_log_journal(sd_journal *j)
{
#ifdef HAVE_ACL
	...
	HASHMAP_FOREACH_KEY (path, code, j->errors, it) {
		...
		case ENODATA:
			log_warning_errno(err,
				"Journal file %s is truncated, ignoring file.",
				path);
			break;

		case EPROTONOSUPPORT:
			log_warning_errno(err,
				"Journal file %s uses an unsupported feature, ignoring file.",
				path);
			break;
		...
		default:
			log_warning_errno(err,
				"An error was encountered while opening journal file %s, ignoring file.",
				path);
			break;
		}
	}
...
flush_to_var(void)
{
	...
	r = sd_bus_call_method(bus, SVC_DBUS_BUSNAME,
		...
		SIGUSR1);
	if (r < 0) {
		log_error("Failed to kill journal service: %s",
...
main(int argc, char *argv[])
{
	...
	if (arg_action == ACTION_UPDATE_CATALOG ||
		...
		if (arg_action == ACTION_UPDATE_CATALOG) {
			r = catalog_update(database, arg_root,
				catalog_file_dirs);
			if (r < 0)
				log_error_errno(r,
	...
	if (arg_directory)
		r = sd_journal_open_directory(&j, arg_directory,
			arg_journal_type);
	else if (arg_file)
		...
		r = sd_journal_open(&j,
			!arg_merge * SD_JOURNAL_LOCAL_ONLY + arg_journal_type);
	if (r < 0) {
	...
	if (!arg_quiet) {
		usec_t start, end;
		char start_buf[FORMAT_TIMESTAMP_MAX],
			end_buf[FORMAT_TIMESTAMP_MAX];
		...
		if (r > 0) {
			if (arg_follow)
				printf("-- Logs begin at %s. --\n",
					format_timestamp_maybe_utc(start_buf,
						sizeof(start_buf), start));
			else
				printf("-- Logs begin at %s, end at %s. --\n",
					format_timestamp_maybe_utc(start_buf,
						sizeof(start_buf), start),
					format_timestamp_maybe_utc(end_buf,
						sizeof(end_buf), end));
		}
	}
	...
	for (;;) {
		while (arg_lines < 0 || n_shown < arg_lines ||
			...
			if (!arg_merge && !arg_quiet) {
				...
				r = sd_journal_get_monotonic_usec(j, NULL,
					&boot_id);
				if (r >= 0) {
