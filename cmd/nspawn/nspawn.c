...
typedef struct ExposePort {
	...
	IWLIST_FIELDS(struct ExposePort, ports);
} ExposePort;
...
static uint64_t arg_retain = (1ULL << CAP_CHOWN) | (1ULL << CAP_DAC_OVERRIDE) |
	(1ULL << CAP_DAC_READ_SEARCH) | (1ULL << CAP_FOWNER) |
	(1ULL << CAP_FSETID) | (1ULL << CAP_IPC_OWNER) | (1ULL << CAP_KILL) |
	(1ULL << CAP_LEASE) | (1ULL << CAP_LINUX_IMMUTABLE) |
	(1ULL << CAP_NET_BIND_SERVICE) | (1ULL << CAP_NET_BROADCAST) |
	(1ULL << CAP_NET_RAW) | (1ULL << CAP_SETGID) | (1ULL << CAP_SETFCAP) |
	(1ULL << CAP_SETPCAP) | (1ULL << CAP_SETUID) | (1ULL << CAP_SYS_ADMIN) |
	(1ULL << CAP_SYS_CHROOT) | (1ULL << CAP_SYS_NICE) |
	(1ULL << CAP_SYS_PTRACE) | (1ULL << CAP_SYS_TTY_CONFIG) |
	(1ULL << CAP_SYS_RESOURCE) | (1ULL << CAP_SYS_BOOT) |
	(1ULL << CAP_AUDIT_WRITE) | (1ULL << CAP_AUDIT_CONTROL) |
	(1ULL << CAP_MKNOD);
...
static int
set_sanitized_path(char **b, const char *path)
{
	char *p;
...
parse_argv(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "help", no_argument, NULL,
		...
		{ "drop-capability", required_argument, NULL,
			ARG_DROP_CAPABILITY },
		...
		{ "network-interface", required_argument, NULL,
			ARG_NETWORK_INTERFACE },
		{ "network-macvlan", required_argument, NULL,
			ARG_NETWORK_MACVLAN },
		{ "network-ipvlan", required_argument, NULL,
			ARG_NETWORK_IPVLAN },
		{ "network-veth", no_argument, NULL, 'n' },
		{ "network-bridge", required_argument, NULL,
			ARG_NETWORK_BRIDGE },
	...
	while ((c = getopt_long(argc, argv, "+hD:u:abL:M:jS:Z:qi:xp:n", options,
			NULL)) >= 0)
		...
		case 'M':
			if (isempty(optarg)) {
				...
				if (!machine_name_is_valid(optarg)) {
					log_error("Invalid machine name: %s",
						optarg);
					return -EINVAL;
				}
		...
		case ARG_LINK_JOURNAL:
			if (streq(optarg, "auto")) {
				...
				log_error(
					"Failed to parse link journal mode %s",
					optarg);
				return -EINVAL;
			}
		...
		case ARG_BIND_RO: {
			...
			if (!path_is_absolute(a) || !path_is_absolute(b)) {
				log_error(
					"Invalid bind mount specification: %s",
					optarg);
				return -EINVAL;
			}
		...
		case ARG_REGISTER:
			...
			if (r < 0) {
				log_error(
					"Failed to parse --register= argument: %s",
					optarg);
				return r;
			}
		...
		case ARG_PERSONALITY:
			...
			if (arg_personality == 0xffffffffLU) {
				log_error(
					"Unknown or unsupported personality '%s'.",
					optarg);
				return -EINVAL;
			}
			...
			if (!optarg)
				...
				if (r < 0) {
					if (streq(optarg, "state"))
						...
						log_error(
							"Failed to parse --volatile= argument: %s",
							optarg);
						return r;
					}
				} else
					arg_volatile =
						r ? VOLATILE_YES : VOLATILE_NO;
			}
		...
		case 'p': {
			...
			if (r < 0 || container_port <= 0) {
				log_error("Failed to parse host port: %s",
					optarg);
				return -EINVAL;
			}
			...
			IWLIST_FOREACH (ports, p, arg_expose_ports) {
				if (p->protocol == protocol &&
					...
					log_error(
						"Duplicate port specification: %s",
						optarg);
					return -EINVAL;
				}
			}
			...
			IWLIST_PREPEND(ports, arg_expose_ports, p);
	...
	if (arg_keep_unit && cg_pid_get_owner_uid(0, NULL) >= 0) {
		log_error(
			"--keep-unit may not be used when invoked from a user session.");
		return -EINVAL;
	}
	...
	arg_retain =
		(arg_retain | plus |
			(arg_private_network ? 1ULL << CAP_NET_ADMIN : 0)) &
		~minus;
...
mount_all(const char *dest)
{
	...
	static const MountPoint mount_table[] = {
		...
		{ "devpts", "/dev/pts", "devpts",
			"newinstance,ptmxmode=0666,mode=620,gid=" STRINGIFY(
				TTY_GID),
	...
	for (k = 0; k < ELEMENTSOF(mount_table); k++) {
		...
		if (t < 0 && t != -ENOENT) {
			log_error_errno(t,
				"Failed to detect whether %s is a mount point: %m",
				where);
		...
		if (arg_selinux_apifs_context &&
			(streq_ptr(mount_table[k].what, "tmpfs") ||
				streq_ptr(mount_table[k].what, "devpts"))) {
			options = strjoin(mount_table[k].options, ",context=\"",
		...
		if (mount(mount_table[k].what, where, mount_table[k].type,
			    mount_table[k].flags, o) < 0) {
			if (mount_table[k].fatal) {
				log_error_errno(errno, "mount(%s) failed: %m",
					where);

				if (r == 0)
					r = -errno;
			} else
				log_warning_errno(errno, "mount(%s) failed: %m",
					where);
		}
	}
...
static int
mount_binds(const char *dest, char **l, bool ro)
{
	char **x, **y;
	...
	STRV_FOREACH_PAIR (x, y, l) {
		...
		if (r == 0) {
			...
			log_error_errno(errno, "Failed to bind mount %s: %m",
				*x);
			return -errno;
		}
		...
		if (S_ISDIR(source_st.st_mode)) {
			...
			if (r < 0 && errno != EEXIST)
				return log_error_errno(r,
					"Failed to create mount point %s: %m",
					where);
		} else {
			r = touch(where);
			if (r < 0)
...
static int
mount_cgroup_hierarchy(const char *dest, const char *controller,
	const char *hierarchy, bool read_only)
{
	...
	if (mount("cgroup", to, "cgroup", MS_NOSUID | MS_NOEXEC | MS_NODEV,
		    controller) < 0)
...
mount_cgroup(const char *dest)
{
	...
	for (;;) {
		...
		if (r == -EINVAL) {
			...
			r = mount_cgroup_hierarchy(dest, controller, controller,
				true);
			if (r < 0)
				return r;

		} else if (r < 0)
			return log_error_errno(r, "Failed to read link %s: %m",
				origin);
		else {
			...
			if (!filename_is_valid(combined)) {
				log_warning(
					"Ignoring invalid combined hierarchy %s.",
					combined);
				continue;
			}

			r = mount_cgroup_hierarchy(dest, combined, combined,
				true);
			if (r < 0)
				return r;
	...
	r = mount_cgroup_hierarchy(dest, "name=systemd,xattr", "systemd",
		false);
	if (r < 0)
		return r;
...
mount_tmpfs(const char *dest)
{
	...
	STRV_FOREACH_PAIR (i, o, arg_tmpfs) {
		...
		if (r < 0 && r != -EEXIST)
			return log_error_errno(r,
				"Creating mount point for tmpfs %s failed: %m",
				where);

		if (mount("tmpfs", where, "tmpfs", MS_NODEV | MS_STRICTATIME,
			    *o) < 0)
			return log_error_errno(errno,
...
static int
setup_timezone(const char *dest)
{
	_cleanup_free_ char *where = NULL, *p = NULL, *q = NULL, *check = NULL,
			    *what = NULL;
	char *z, *y;
	int r;
	...
	if (access(check, F_OK) < 0) {
		log_warning(
			"Timezone %s does not exist in container, not updating container timezone.",
			z);
		return 0;
	}
	...
	if (r < 0) {
		log_error_errno(r,
			"Failed to create directory for timezone info %s in container: %m",
			where);
	...
	if (r < 0 && errno != ENOENT) {
		log_error_errno(errno,
			"Failed to remove existing timezone info %s in container: %m",
			where);
...
static int
setup_resolv_conf(const char *dest)
{
	_cleanup_free_ char *where = NULL;
	int r;
	...
	if (r < 0) {
		log_warning_errno(r,
			"Failed to create parent directory for resolv.conf %s: %m",
			where);
...
static int
setup_volatile_state(const char *directory)
{
	const char *p;
	int r;
...
static int
setup_volatile(const char *directory)
{
...
static char *
id128_format_as_uuid(sd_id128_t id, char s[37])
{
	snprintf(s, 37,
...
setup_boot_id(const char *dest)
{
	...
	if (mount(from, to, "bind", MS_BIND, NULL) < 0) {
		...
		r = -errno;
	} else if (mount(from, to, "bind", MS_BIND | MS_REMOUNT | MS_RDONLY,
			   NULL))
...
copy_devnodes(const char *dest)
{
	...
	NULSTR_FOREACH (d, devnodes) {
		...
		if (stat(from, &st) < 0) {
			...
			log_error(
				"%s is not a char or block device, cannot copy",
				from);
			return -EIO;
			...
			if (r < 0) {
				log_error_errno(r,
					"Failed to create parent directory of %s: %m",
					to);
				return -r;
			}
...
static int
setup_dev_console(const char *dest, const char *console)
{
	_cleanup_umask_ mode_t u;
	const char *to;
	struct stat st;
...
static int
setup_kmsg(const char *dest, int kmsg_socket)
{
	_cleanup_free_ char *from = NULL, *to = NULL;
	_cleanup_umask_ mode_t u;
	int r, fd, k;
...
static int
flush_ports(union in_addr_union *exposed)
{
	ExposePort *p;
	int r, af = AF_INET;
...
static int
expose_ports(sd_rtnl *rtnl, union in_addr_union *exposed)
{
...
static int
on_address_change(sd_rtnl *rtnl, sd_rtnl_message *m, void *userdata)
{
...
static int
watch_rtnl(sd_event *event, int recv_fd, union in_addr_union *exposed,
	sd_rtnl **ret)
{
	union {
		struct cmsghdr cmsghdr;
...
static int
setup_journal(const char *directory)
{
	...
	if (r >= 0) {
		...
		if (arg_link_journal == LINK_GUEST && rmdir(p) < 0) {
			if (errno == ENOTDIR) {
				log_error(
					"%s already exists and is neither a symlink nor a directory",
					p);
				return r;
			} else {
				log_error_errno(errno,
		...
		if (symlink(q, p) < 0) {
			if (arg_link_journal_try) {
				log_debug_errno(errno,
					"Failed to symlink %s to %s, skipping journal setup: %m",
					q, p);
				return 0;
			} else {
				log_error_errno(errno,
					"Failed to symlink %s to %s: %m", q, p);
				return -errno;
			}
		}
	...
	if (arg_link_journal == LINK_HOST) {
		...
		if (r < 0) {
			if (arg_link_journal_try) {
				log_debug_errno(errno,
					"Failed to create %s, skipping journal setup: %m",
					p);
				return 0;
			} else {
				log_error_errno(errno,
...
static int
register_machine(pid_t pid, int local_ifindex)
{
	...
	if (arg_keep_unit) {
		r = sd_bus_call_method(bus, SVC_MACHINED_DBUS_BUSNAME,
			...
			strempty(arg_directory), local_ifindex > 0 ? 1 : 0,
			local_ifindex);
	} else {
		...
		r = sd_bus_message_append(m, "sayssusai", arg_machine,
			SD_BUS_MESSAGE_APPEND_ID128(arg_uuid), "nspawn",
			"container", (uint32_t)pid, strempty(arg_directory),
		...
		if (!isempty(arg_slice)) {
			r = sd_bus_message_append(m, "(sv)", "Slice", "s",
				arg_slice);
			if (r < 0)
				return log_error_errno(r,
		...
		r = sd_bus_message_append(m, "(sv)", "DevicePolicy", "s",
			"strict");
		if (r < 0)
			return log_error_errno(r,
		...
		r = sd_bus_message_append(m, "(sv)", "DeviceAllow", "a(ss)", 9,
			/* Allow the container to
                                           ...
                                           * container can work
                                           * fine */
			"/dev/null", "rwm", "/dev/zero", "rwm", "/dev/full",
			"rwm", "/dev/random", "rwm", "/dev/urandom", "rwm",
			"/dev/tty", "rwm", "/dev/net/tun", "rwm",
			/* Allow the container
                                           * access to ptys. However,
                                           * do not permit the
                                           * container to ever create
                                           * these device nodes. */
			"/dev/pts/ptmx", "rw", "char-pts", "rw");
		if (r < 0)
			return log_error_errno(r,
	...
	if (r < 0) {
		log_error("Failed to register machine: %s",
...
terminate_machine(pid_t pid)
{
	...
	if (r < 0) {
		...
		log_debug("Failed to get machine: %s",
			bus_error_message(&error, r));
		return 0;
	}
...
#define HOST_HASH_KEY                                                          \
	SD_ID128_MAKE(1a, 37, 6f, c7, 46, ec, 45, 0b, ad, a3, d5, 31, 06, 60,  \
		5d, b1)
#define CONTAINER_HASH_KEY                                                     \
	SD_ID128_MAKE(c3, c4, f9, 19, b5, 57, b2, 1c, e6, cf, 14, 27, 03, 9c,  \
		ee, a2)
#define MACVLAN_HASH_KEY                                                       \
	SD_ID128_MAKE(00, 13, 6d, bc, 66, 83, 44, 81, bb, 0c, f9, 51, 1f, 24,  \
		a6, 6f)

static int
generate_mac(struct ether_addr *mac, sd_id128_t hash_key, uint64_t idx)
{
	uint8_t result[8];
	size_t l, sz;
	uint8_t *v, *i;
	...
	mac->ether_addr_octet[0] |=
		0x02; /* set local assignment bit (IEEE802) */

	return 0;
}

static int
setup_veth(pid_t pid, char iface_name[IFNAMSIZ], int *ifi)
{
...
static int
setup_bridge(const char veth_name[], int *ifi)
{
	_cleanup_rtnl_message_unref_ sd_rtnl_message *m = NULL;
	_cleanup_rtnl_unref_ sd_rtnl *rtnl = NULL;
	int r, bridge;
	...
	if (bridge <= 0)
		return log_error_errno(errno,
			"Failed to resolve interface %s: %m",
			arg_network_bridge);
...
static int
parse_interface(struct udev *udev, const char *name)
{
...
static int
move_network_interfaces(pid_t pid)
{
	...
	STRV_FOREACH (i, arg_network_interfaces) {
		...
		if (r < 0)
			return log_error_errno(r,
				"Failed to move interface %s to namespace: %m",
				*i);
	}
...
setup_macvlan(pid_t pid)
{
	...
	STRV_FOREACH (i, arg_network_macvlan) {
		...
		r = sd_rtnl_message_append_u32(m, IFLA_MACVLAN_MODE,
			MACVLAN_MODE_BRIDGE);
		if (r < 0)
			return log_error_errno(r,
...
setup_ipvlan(pid_t pid)
{
	...
	STRV_FOREACH (i, arg_network_ipvlan) {
		...
		r = sd_rtnl_message_append_u16(m, IFLA_IPVLAN_MODE,
			IPVLAN_MODE_L2);
		if (r < 0)
			return log_error_errno(r,
...
setup_seccomp(void)
{
	...
	r = seccomp_rule_add(seccomp, SCMP_ACT_ERRNO(EAFNOSUPPORT),
		SCMP_SYS(socket), 2, SCMP_A0(SCMP_CMP_EQ, AF_NETLINK),
...
static int
setup_propagate(const char *root)
{
	const char *p, *q;
...
static int
setup_image(char **device_path, int *loop_nr)
{
	...
	fd = open(arg_image,
		O_CLOEXEC | (arg_read_only ? O_RDONLY : O_RDWR) | O_NONBLOCK |
			O_NOCTTY);
	if (fd < 0)
	...
	if (!S_ISREG(st.st_mode)) {
		log_error_errno(errno,
			"%s is not a regular file or block device: %m",
			arg_image);
		return -EINVAL;
	}
	...
	if (ioctl(loop, LOOP_SET_FD, fd) < 0)
		return log_error_errno(errno,
			"Failed to set loopback file descriptor on %s: %m",
			loopdev);
...
static int
dissect_image(int fd, char **root_device, bool *root_device_rw,
	char **home_device, bool *home_device_rw, char **srv_device,
	bool *srv_device_rw, bool *secondary)
{
#ifdef HAVE_BLKID
	...
	bool home_rw = true, root_rw = true, secondary_root_rw = true,
	     srv_rw = true, generic_rw = true;
	...
	for (i = 0;; i++) {
		...
		if (r < 0)
			return log_error_errno(r,
				"Failed to scan for partition devices of %s: %m",
				arg_image);

		/* Count the partitions enumerated by the kernel */
		n = 0;
		first = udev_enumerate_get_list_entry(e);
		udev_list_entry_foreach(item, first) n++;
	...
	udev_list_entry_foreach(item, first)
	{
		...
		if (!q) {
			...
			log_error_errno(errno,
				"Failed to get partition device of %s: %m",
				arg_image);
			return -errno;
		}
	...
	if (root) {
		...
		*secondary = true;
		...
		if (multiple_generic) {
			log_error(
				"Identified multiple bootable Linux partitions on\n"
				"    %s\n" PARTITION_TABLE_BLURB,
				arg_image);
			return -EINVAL;
		}
...
static int
mount_device(const char *what, const char *where, const char *directory,
	bool rw)
{
#ifdef HAVE_BLKID
	_cleanup_blkid_free_probe_ blkid_probe b = NULL;
	...
	if (!b) {
		if (errno == 0)
			return log_oom();
		log_error_errno(errno, "Failed to allocate prober for %s: %m",
			what);
		return -errno;
	}
	...
	if (streq(fstype, "crypto_LUKS")) {
		log_error(
			"nspawn currently does not support LUKS disk images.");
		return -ENOTSUP;
	}
...
static int
mount_devices(const char *where, const char *root_device, bool root_device_rw,
	const char *home_device, bool home_device_rw, const char *srv_device,
	bool srv_device_rw)
{
	int r;
...
static void
loop_remove(int nr, int *image_fd)
{
...
static int
spawn_getent(const char *database, const char *key, pid_t *rpid)
{
	int pipe_fds[2];
	pid_t pid;
 ...
 */
static int
wait_for_container(pid_t pid, ContainerStatus *container)
{
	siginfo_t status;
	int r;
	...
	case CLD_EXITED:
		if (status.si_status == 0) {
			log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
				"Container %s exited successfully.",
				arg_machine);

		} else
			log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
				"Container %s failed with error code %i.",
	...
	case CLD_KILLED:
		if (status.si_status == SIGINT) {
			log_full(arg_quiet ? LOG_DEBUG : LOG_INFO,
				"Container %s has been shut down.",
				arg_machine);
			*container = CONTAINER_TERMINATED;
			return 0;
	...
	case CLD_DUMPED:
		log_error("Container %s terminated by signal %s.", arg_machine,
...
static int
on_orderly_shutdown(sd_event_source *s, const struct signalfd_siginfo *si,
	void *userdata)
{
	pid_t pid;
...
determine_names(void)
{
	...
	if (!arg_image && !arg_directory) {
		if (arg_machine) {
			...
			if (r < 0)
				return log_error_errno(r,
					"Failed to find image for machine '%s': %m",
					arg_machine);
			else if (r == 0) {
				log_error("No image for machine '%s': %m",
					arg_machine);
				return -ENOENT;
			}
	...
	if (!arg_machine) {
		...
		if (arg_ephemeral) {
			...
			if (asprintf(&b, "%s-%016" PRIx64, arg_machine,
				    random_u64()) < 0)
				return log_oom();
...
{
	_cleanup_free_ char *device_path = NULL, *root_device = NULL,
	...
	_cleanup_release_lock_file_ LockFile tree_global_lock = LOCK_FILE_INIT,
	...
	if (arg_directory) {
		...
		if (arg_ephemeral) {
			...
			if (r < 0) {
				log_error_errno(r,
					"Failed to determine whether directory %s is mount point: %m",
					arg_directory);
				goto finish;
			}
			if (r > 0)
			...
			if (r < 0) {
				log_error_errno(r,
					"Failed to create snapshot %s from %s: %m",
					np, arg_directory);
				goto finish;
			}
			...
			if (r == -EBUSY) {
				log_error_errno(r,
					"Directory tree %s is currently busy.",
					arg_directory);
				goto finish;
			}
			if (r < 0) {
				log_error_errno(r, "Failed to lock %s: %m",
					arg_directory);
				return r;
			}

			if (arg_template) {
				r = btrfs_subvol_snapshot(arg_template,
					arg_directory, arg_read_only, true);
				if (r == -EEXIST) {
					if (!arg_quiet)
						log_info(
							"Directory %s already exists, not populating from template %s.",
							arg_directory,
							arg_template);
				} else if (r < 0) {
					log_error_errno(r,
		...
		if (arg_start_mode == START_BOOT) {
			if (path_is_os_tree(arg_directory) <= 0) {
				log_error(
					"Directory %s doesn't look like an OS root directory (os-release file is missing). Refusing.",
					arg_directory);
				r = -EINVAL;
				goto finish;
			}
			...
			p = strjoina(arg_directory,
				argc > optind &&
						path_is_absolute(argv[optind]) ?
					      argv[optind] :
					      "/usr/bin/");
			if (access(p, F_OK) < 0) {
				log_error(
					"Directory %s lacks the binary to execute or doesn't look like a binary tree. Refusing.",
					arg_directory);
				r = -EINVAL;
				goto finish;
			}
		...
		r = dissect_image(image_fd, &root_device, &root_device_rw,
			&home_device, &home_device_rw, &srv_device,
			&srv_device_rw, &secondary);
		if (r < 0)
			goto finish;
	}
	...
	for (;;) {
		...
		if (pid == 0) {
			...
			_cleanup_free_ char *home = NULL;
			unsigned n_env = 2;
			const char *envp[] = { "PATH=" DEFAULT_PATH_SPLIT_USR,
			...
			if (mount_devices(arg_directory, root_device,
				    root_device_rw, home_device, home_device_rw,
				    srv_device, srv_device_rw) < 0)
				_exit(EXIT_FAILURE);

			/* Turn directory into bind mount */
			if (mount(arg_directory, arg_directory, "bind",
			...
			if (chdir(arg_directory) < 0) {
				log_error_errno(errno, "chdir(%s) failed: %m",
					arg_directory);
				_exit(EXIT_FAILURE);
			}

			if (mount(arg_directory, "/", NULL, MS_MOVE, NULL) <
				0) {
				log_error_errno(errno,
			...
			if ((asprintf((char **)(envp + n_env++), "HOME=%s",
				     home ? home : "/root") < 0) ||
			...
			if (arg_selinux_context)
				if (setexeccon((security_context_t)
						    arg_selinux_context) < 0) {
					log_error_errno(errno,
						"setexeccon(\"%s\") failed: %m",
						arg_selinux_context);
					_exit(EXIT_FAILURE);
				}
#endif
		...
		if (barrier_sync_next(&barrier)) {
			...
			if (barrier_place_and_sync(&barrier)) {
				...
				_cleanup_(pty_forward_freep)
					PTYForward *forward = NULL;
				_cleanup_rtnl_unref_ sd_rtnl *rtnl = NULL;
				char last_char = 0;
				...
				if (arg_start_mode == START_BOOT) {
					...
					sd_event_add_signal(event, NULL, SIGINT,
						on_orderly_shutdown,
						UINT32_TO_PTR(pid));
					sd_event_add_signal(event, NULL,
						SIGTERM, on_orderly_shutdown,
						UINT32_TO_PTR(pid));
				} else {
					/* Immediately exit */
					sd_event_add_signal(event, NULL, SIGINT,
				...
				sd_event_add_signal(event, NULL, SIGCHLD, NULL,
					NULL);

				if (arg_expose_ports) {
					r = watch_rtnl(event,
						rtnl_socket_pair[0], &exposed,
						&rtnl);
					if (r < 0)
						goto finish;
				...
				rtnl_socket_pair[0] =
					safe_close(rtnl_socket_pair[0]);

				r = pty_forward_new(event, master, true,
					!interactive, &forward);
				if (r < 0) {
					log_error_errno(r,
...
finish:
	...
	if (remove_subvol && arg_directory) {
		...
		if (k < 0)
			log_warning_errno(k,
				"Cannot remove subvolume '%s', ignoring: %m",
				arg_directory);
	}
	...
	while (arg_expose_ports) {
		...
		IWLIST_REMOVE(ports, arg_expose_ports, p);
		free(p);
	}
