...
static int
reset_environ(const char *new_environment, size_t length)
{
...
stub_pid1(sd_id128_t uuid)
{
	...
	sd_id128_to_string(uuid,
		new_environment + sizeof(new_environment) -
			SD_ID128_STRING_MAX);
	...
	sigset_add_many(&waitmask, SIGCHLD, /* posix: process died */
