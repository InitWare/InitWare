...
const sd_bus_vtable bus_scope_vtable[] = { SD_BUS_VTABLE_START(0),
...
static int
bus_scope_set_transient_property(Scope *s, const char *name,
	...
	sd_bus_error *error)
{
	...
	if (streq(name, "PIDs")) {
		...
		if (mode != UNIT_CHECK) {
			r = sd_bus_message_read(message, "t",
				&s->timeout_stop_usec);
			if (r < 0)
				return r;

			unit_write_drop_in_format(UNIT(s), mode, name,
...
int
bus_scope_set_property(Unit *u, const char *name, sd_bus_message *message,
	UnitSetPropertiesMode mode, sd_bus_error *error)
{
	...
	if (u->load_state == UNIT_STUB) {
		...
		r = bus_scope_set_transient_property(s, name, message, mode,
			error);
		if (r != 0)
			return r;
...
int
bus_scope_commit_properties(Unit *u)
{
	assert(u);
