...
static int
property_get_monotonic_timers(sd_bus *bus, const char *path,
...
static int
property_get_calendar_timers(sd_bus *bus, const char *path,
	const char *interface, const char *property, sd_bus_message *reply,
	void *userdata, sd_bus_error *error)
{
	Timer *t = userdata;
	...
	IWLIST_FOREACH (value, v, t->values) {
		...
		r = sd_bus_message_append(reply, "(sst)",
			timer_base_to_string(v->base), buf, v->next_elapse);
		if (r < 0)
			return r;
	}
...
static int
property_get_next_elapse_monotonic(sd_bus *bus, const char *path,
	const char *interface, const char *property, sd_bus_message *reply,
	void *userdata, sd_bus_error *error)
{
	Timer *t = userdata;
...
const sd_bus_vtable bus_timer_vtable[] = { SD_BUS_VTABLE_START(0),
	SD_BUS_PROPERTY("Unit", "s", property_get_unit, 0,
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("TimersMonotonic", "a(stt)",
		property_get_monotonic_timers, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_INVALIDATION),
	SD_BUS_PROPERTY("TimersCalendar", "a(sst)",
		property_get_calendar_timers, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_INVALIDATION),
	...
	SD_BUS_PROPERTY("Result", "s", property_get_result,
		offsetof(Timer, result), SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("AccuracyUSec", "t", bus_property_get_usec,
		offsetof(Timer, accuracy_usec), SD_BUS_VTABLE_PROPERTY_CONST),
...
static int
bus_timer_set_transient_property(Timer *t, const char *name,
	...
	sd_bus_error *error)
{
	...
	if (STR_IN_SET(name, "OnActiveSec", "OnBootSec", "OnStartupSec",
		...
		if (mode != UNIT_CHECK) {
			...
			unit_write_drop_in_private_format(UNIT(t), mode, name,
				"%s=%s\n", name,
				format_timespan(time, sizeof(time), u,
					USEC_PER_MSEC));
...
int
bus_timer_set_property(Unit *u, const char *name, sd_bus_message *message,
