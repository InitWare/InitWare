...
static const MountPoint mount_table[] = {
	...
	{ "tmpfs", "/dev/shm", "tmpfs", "mode=1777,smackfsroot=*",
		MS_NOSUID | MS_NODEV | MS_STRICTATIME, mac_smack_use,
		MNT_FATAL },
#endif
	{ "tmpfs", "/dev/shm", "tmpfs", "mode=1777",
		MS_NOSUID | MS_NODEV | MS_STRICTATIME, NULL,
		MNT_FATAL | MNT_IN_CONTAINER },
	...
	{ "tmpfs", "/sys/fs/cgroup", "tmpfs", "mode=755",
		MS_NOSUID | MS_NOEXEC | MS_NODEV | MS_STRICTATIME, NULL,
		MNT_FATAL | MNT_IN_CONTAINER },
...
bool
mount_point_ignore(const char *path)
{
	const char *i;
...
static int
mount_one(const MountPoint *p, bool relabel)
{
	int r;
	...
	if (mount(p->what, p->where, p->type, p->flags, p->options) < 0) {
		log_full((p->mode & MNT_FATAL) ? LOG_ERR : LOG_DEBUG,
			"Failed to mount %s at %s: %m", p->type, p->where);
...
mount_cgroup_controllers(char ***join_controllers)
{
	...
	for (;;) {
		_cleanup_free_ char *options = NULL, *controller = NULL,
				    *where = NULL;
		MountPoint p = {
			.what = "cgroup",
			.type = "cgroup",
...
static int
relabel_cgroup_filesystems(void)
{
	int r;
	struct statfs st;
	...
	nftw("/sys/fs/cgroup", nftw_cb, 64,
		FTW_MOUNT | FTW_PHYS | FTW_ACTIONRETVAL);

	if (st.f_flags & ST_RDONLY)
		(void)mount(NULL, "/sys/fs/cgroup", NULL,
...
mount_setup(bool loaded_policy)
{
	...
	if (loaded_policy) {
		...
		mkdir_label(SVC_PKGRUNSTATEDIR "/policy-relabelling", 0755);
		...
		mkdir_label(SVC_PKGRUNSTATEDIR "/policy-relabelled", 0755);
	...
	/* Create a few directories we always want around, Note that
         * sd_booted() checks for system, so this mkdir
         * really needs to stay for good, otherwise software that
         * copied sd-daemon.c into their sources will misdetect
         * systemd. */
	mkdir_label(SVC_PKGRUNSTATEDIR, 0755);
	mkdir_label(SVC_PKGRUNSTATEDIR "/system", 0755);
	mkdir_label(SVC_PKGRUNSTATEDIR "/inaccessible", 0000);
