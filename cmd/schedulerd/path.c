...
int
path_spec_watch(PathSpec *s, sd_event_io_handler_t handler)
{
	static const int flags_table[_PATH_TYPE_MAX] = {
		...
		[PATH_DIRECTORY_NOT_EMPTY] = IN_DELETE_SELF | IN_MOVE_SELF |
	...
	for (slash = strchr(s->path, '/');; slash = strchr(slash + 1, '/')) {
		...
		if (slash) {
			...
			*cut = '\0';
			...
			flags = IN_MOVE_SELF | IN_DELETE_SELF | IN_ATTRIB |
		...
		if (r < 0) {
			...
			if (oldslash) {
				...
				*cut2 = '\0';
				...
				inotify_add_watch(s->inotify_fd, s->path,
					IN_MOVE_SELF);
...
int
path_spec_fd_event(PathSpec *s, uint32_t revents)
{
...
static bool
path_spec_check_good(PathSpec *s, bool initial)
{
	bool good = false;
...
static void
path_spec_mkdir(PathSpec *s, mode_t mode)
{
	int r;
...
static void
path_spec_dump(PathSpec *s, FILE *f, const char *prefix)
{
	fprintf(f, "%s%s: %s\n", prefix, path_type_to_string(s->type), s->path);
}

void
path_spec_done(PathSpec *s)
{
	assert(s);
...
path_verify(Path *p)
{
	...
	if (!p->specs) {
		log_unit_error(UNIT(p)->id, "%s lacks path setting. Refusing.",
			UNIT(p)->id);
		return -EINVAL;
	}
...
static int
path_add_default_dependencies(Path *p)
{
	int r;
	...
	return unit_add_two_dependencies_by_name(UNIT(p), UNIT_BEFORE,
...
static void
path_dump(Unit *u, FILE *f, const char *prefix)
{
	Path *p = PATH(u);
	Unit *trigger;
	PathSpec *s;
...
static void
path_set_state(Path *p, PathState state)
{
	PathState old_state;
	assert(p);
	...
	if (state != old_state)
		log_debug("%s changed %s -> %s", UNIT(p)->id,
...
static int
path_enter_waiting_coldplug(Unit *u)
{
...
static int
path_coldplug(Unit *u, Hashmap *deferred_work)
{
	Path *p = PATH(u);
...
static void
path_enter_dead(Path *p, PathResult f)
{
	assert(p);
...
path_enter_running(Path *p)
{
	...
	r = manager_add_job(UNIT(p)->manager, JOB_START, trigger, JOB_REPLACE,
		true, &error, NULL);
	if (r < 0)
		goto fail;
...
fail:
	log_warning("%s failed to queue unit startup job: %s", UNIT(p)->id,
		bus_error_message(&error, r));
	path_enter_dead(p, PATH_FAILURE_RESOURCES);
}

static bool
path_check_good(Path *p, bool initial)
{
	PathSpec *s;
	bool good = false;
...
static void
path_enter_waiting(Path *p, bool initial, bool recheck)
{
	int r;
...
fail:
	log_warning_errno(r, "%s failed to enter waiting state: %m",
		UNIT(p)->id);
	path_enter_dead(p, PATH_FAILURE_RESOURCES);
}
...
static int
path_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Path *p = PATH(u);
...
static int
path_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Path *p = PATH(u);
...
path_sub_state_to_string(Unit *u)
{
	assert(u);
...
static int
path_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	PathSpec *s = userdata;
	Path *p;
	...
	if (p->state != PATH_WAITING && p->state != PATH_RUNNING)
		return 0;
...
static void
path_trigger_notify(Unit *u, Unit *other)
{
	Path *p = PATH(u);
...
static const char *const path_state_table[_PATH_STATE_MAX] = { [PATH_DEAD] =
								       "dead",
...
const UnitVTable path_vtable = { .object_size = sizeof(Path),
