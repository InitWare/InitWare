 ...
 *   interested in, such that you are notified when those processes fork or
 *   exit.
 ...
 * impossible to record which processes we are actually tracking.
 */
 ...
 * @param ignore_self Whether to avoid sending the signal to this process
 *      (i.e. the manager itself)
 ...
 */
int cg_kill(const char *unused, const PTGroup *grp, int sig, bool sigcont,
	bool ignore_self, Set *ignore_set);
 ...
 */
int cg_kill_recursive(const char *unused, const PTGroup *grp, int sig,
	bool sigcont, bool ignore_self, bool rem, Set *ignore_set);
