...
static int
swap_set_devnode(Swap *s, const char *devnode)
{
	Hashmap *swaps;
	Swap *first;
	int r;
	...
	if (s->devnode) {
		...
		IWLIST_REMOVE(same_devnode, first, s);
		if (first)
	...
	if (devnode) {
		...
		first = hashmap_get(swaps, s->devnode);
		IWLIST_PREPEND(same_devnode, first, s);
...
swap_init(Unit *u)
{
	...
	s->parameters_proc_swaps.priority = s->parameters_fragment.priority =
		-1;
...
static void
swap_unwatch_control_pid(Swap *s)
{
	assert(s);
...
swap_arm_timer(Swap *s)
{
	...
	return sd_event_add_time(UNIT(s)->manager->event,
		...
		now(CLOCK_MONOTONIC) + s->timeout_usec, 0, swap_dispatch_timer,
		s);
}

static int
swap_add_device_links(Swap *s)
{
	assert(s);
	...
	if (is_device_path(s->what))
		...
		return unit_add_dependency_by_name(UNIT(s), UNIT_AFTER,
			SPECIAL_REMOUNT_FS_SERVICE, NULL, true);
}

static int
swap_add_default_dependencies(Swap *s)
{
	int r;
...
swap_load_devnode(Swap *s)
{
	...
	d = udev_device_new_from_devnum(UNIT(s)->manager->udev, 'b',
		st.st_rdev);
	if (!d)
		return 0;
...
static int
swap_setup_unit(Manager *m, const char *what, const char *what_proc_swaps,
	int priority, bool set_flags)
{
	_cleanup_free_ char *e = NULL;
	bool delete = false;
	Unit *u = NULL;
	...
	if (u && SWAP(u)->from_proc_swaps &&
		!path_equal(SWAP(u)->parameters_proc_swaps.what,
			what_proc_swaps)) {
		log_error(
			"Swap %s appeared twice with different device paths %s and %s",
			e, SWAP(u)->parameters_proc_swaps.what,
			what_proc_swaps);
		return -EEXIST;
	}
...
static int
swap_process_new(Manager *m, const char *device, int prio, bool set_flags)
{
	_cleanup_udev_device_unref_ struct udev_device *d = NULL;
...
static void
swap_set_state(Swap *s, SwapState state)
{
	SwapState old_state;
	Swap *other;
	...
	if (state != SWAP_ACTIVATING && state != SWAP_ACTIVATING_DONE &&
...
static int
swap_coldplug(Unit *u, Hashmap *deferred_work)
{
	Swap *s = SWAP(u);
	...
	if (new_state == SWAP_ACTIVATING || new_state == SWAP_ACTIVATING_DONE ||
...
static void
swap_dump(Unit *u, FILE *f, const char *prefix)
{
	Swap *s = SWAP(u);
	SwapParameters *p;
...
static int
swap_spawn(Swap *s, ExecCommand *c, pid_t *_pid)
{
	pid_t pid;
	int r;
...
static void
swap_enter_dead(Swap *s, SwapResult f)
{
	assert(s);
...
static void
swap_enter_active(Swap *s, SwapResult f)
{
	assert(s);
...
static void
swap_enter_dead_or_active(Swap *s, SwapResult f)
{
	assert(s);
...
static void
swap_enter_signal(Swap *s, SwapState state, SwapResult f)
{
	int r;
...
swap_enter_activating(Swap *s)
{
	...
	if (s->from_fragment) {
		...
		if (priority < 0)
			fstab_find_pri(s->parameters_fragment.options,
				&priority);
	}
...
fail:
	log_unit_warning_errno(UNIT(s)->id, r,
		"%s failed to run 'swapon' task: %m", UNIT(s)->id);
	swap_enter_dead_or_active(s, SWAP_FAILURE_RESOURCES);
}

static void
swap_enter_deactivating(Swap *s)
{
	int r;
	...
	r = exec_command_set(s->control_command, "/sbin/swapoff", s->what,
		NULL);
	if (r < 0)
		goto fail;
...
fail:
	log_unit_warning_errno(UNIT(s)->id, r,
		"%s failed to run 'swapoff' task: %m", UNIT(s)->id);
	swap_enter_dead_or_active(s, SWAP_FAILURE_RESOURCES);
}
...
static int
swap_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Swap *s = SWAP(u);
...
static int
swap_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Swap *s = SWAP(u);
...
swap_sub_state_to_string(Unit *u)
{
	assert(u);
...
static void
swap_sigchld_event(Unit *u, pid_t pid, int code, int status)
{
	Swap *s = SWAP(u);
	SwapResult f;
	...
	log_unit_full(u->id, f == SWAP_SUCCESS ? LOG_DEBUG : LOG_NOTICE,
		"%s swap process exited, code=%s status=%i", u->id,
...
static int
swap_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata)
{
	...
	case SWAP_ACTIVATING_DONE:
		log_unit_warning(UNIT(s)->id,
			"%s activation timed out. Stopping.", UNIT(s)->id);
	...
	case SWAP_DEACTIVATING:
		log_unit_warning(UNIT(s)->id,
			"%s deactivation timed out. Stopping.", UNIT(s)->id);
		swap_enter_signal(s, SWAP_DEACTIVATING_SIGTERM,
			SWAP_FAILURE_TIMEOUT);
		break;
	...
	case SWAP_DEACTIVATING_SIGKILL:
		log_unit_warning(UNIT(s)->id,
			"%s swap process still around after SIGKILL. Ignoring.",
			UNIT(s)->id);
		swap_enter_dead_or_active(s, SWAP_FAILURE_TIMEOUT);
		break;
...
static int
swap_load_proc_swaps(Manager *m, bool set_flags)
{
	unsigned i;
	int r = 0;
...
static int
swap_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	Manager *m = userdata;
	Unit *u;
	...
	IWLIST_FOREACH (units_by_type, u, m->units_by_type[UNIT_SWAP]) {
		...
		if (!swap->is_active) {
			...
			if (swap->what)
				device_found_node(m, swap->what, false,
					DEVICE_FOUND_SWAP, true);
...
static int
swap_following_set(Unit *u, Set **_set)
{
	Swap *s = SWAP(u), *other;
	Set *set;
	int r;
	...
	if (IWLIST_JUST_US(same_devnode, s)) {
		*_set = NULL;
		return 0;
	}
...
swap_enumerate(Manager *m)
{
	...
	if (!m->proc_swaps) {
		...
		r = sd_event_add_io(m->event, &m->swap_event_source,
			fileno(m->proc_swaps), EPOLLPRI, swap_dispatch_io, m);
		if (r < 0)
			goto fail;
...
int
swap_process_device_new(Manager *m, struct udev_device *dev)
{
	struct udev_list_entry *item = NULL, *first = NULL;
	_cleanup_free_ char *e = NULL;
	const char *dn;
	...
	udev_list_entry_foreach(item, first)
	{
		...
		n = unit_name_from_path(udev_list_entry_get_name(item),
			".swap");
		if (!n)
			return -ENOMEM;
...
int
swap_process_device_remove(Manager *m, struct udev_device *dev)
{
	const char *dn;
	int r = 0;
	Swap *s;
...
static int
swap_kill(Unit *u, KillWho who, int signo, sd_bus_error *error)
{
	return unit_kill_common(u, who, signo, -1, SWAP(u)->control_pid, error);
}

static int
swap_get_timeout(Unit *u, uint64_t *timeout)
{
	Swap *s = SWAP(u);
	int r;
...
swap_supported(Manager *m)
{
	...
	if (supported < 0)
		supported = access("/proc/swaps", F_OK) >= 0 &&
			detect_container(NULL) <= 0;

	return supported;
}

static const char *const swap_state_table[_SWAP_STATE_MAX] = { [SWAP_DEAD] =
								       "dead",
