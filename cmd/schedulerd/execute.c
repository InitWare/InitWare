...
static int
shift_fds(int fds[], unsigned n_fds)
{
...
static int
flags_fds(const int fds[], unsigned n_fds, bool nonblock)
{
	unsigned i;
	int r;
...
tty_path(const ExecContext *context)
{
	assert(context);
...
static void
exec_context_tty_reset(const ExecContext *context)
{
	assert(context);
...
static bool
is_terminal_output(ExecOutput o)
{
	return o == EXEC_OUTPUT_TTY || o == EXEC_OUTPUT_SYSLOG_AND_CONSOLE ||
		o == EXEC_OUTPUT_KMSG_AND_CONSOLE ||
		o == EXEC_OUTPUT_JOURNAL_AND_CONSOLE;
}

static int
open_null_as(int flags, int nfd)
{
	int fd, r;
...
static int
connect_journal_socket(int fd, uid_t uid, gid_t gid)
{
...
static int
connect_logger_as(const ExecContext *context, ExecOutput output,
	const char *ident, const char *unit_id, int nfd, uid_t uid, gid_t gid)
{
	int fd, r;
	...
	dprintf(fd,
		...
		unit_id, context->syslog_priority,
		!!context->syslog_level_prefix,
		output == EXEC_OUTPUT_SYSLOG ||
			output == EXEC_OUTPUT_SYSLOG_AND_CONSOLE,
...
static int
open_terminal_as(const char *path, mode_t mode, int nfd)
{
	int fd, r;
...
static bool
is_terminal_input(ExecInput i)
{
	return i == EXEC_INPUT_TTY || i == EXEC_INPUT_TTY_FORCE ||
		i == EXEC_INPUT_TTY_FAIL;
}

static int
fixup_input(ExecInput std_input, int socket_fd, bool apply_tty_stdin)
{
...
static int
fixup_output(ExecOutput std_output, int socket_fd)
{
...
static int
setup_input(const ExecContext *context, int socket_fd, bool apply_tty_stdin)
{
	ExecInput i;
	...
	case EXEC_INPUT_TTY_FAIL: {
		...
		fd = acquire_terminal(tty_path(context),
			i == EXEC_INPUT_TTY_FAIL, i == EXEC_INPUT_TTY_FORCE,
			false, USEC_INFINITY);
		if (fd < 0)
			return fd;
	...
	case EXEC_INPUT_SOCKET:
		return dup2(socket_fd, STDIN_FILENO) < 0 ? -errno :
								 STDIN_FILENO;
...
static int
setup_output(const ExecContext *context, int fileno, int socket_fd,
	const char *ident, const char *unit_id, bool apply_tty_stdin, uid_t uid,
	gid_t gid)
{
	ExecOutput o;
	ExecInput i;
	...
	if (fileno == STDERR_FILENO) {
		...
		if (i == EXEC_INPUT_NULL &&
			is_terminal_input(context->std_input))
			return open_terminal_as(tty_path(context), O_WRONLY,
				fileno);
	...
	case EXEC_OUTPUT_JOURNAL_AND_CONSOLE:
		r = connect_logger_as(context, o, ident, unit_id, fileno, uid,
			gid);
		if (r < 0) {
			log_unit_struct(unit_id, LOG_ERR,
				LOG_MESSAGE(
					"Failed to connect %s of %s to the journal socket: %s",
					fileno == STDOUT_FILENO ? "stdout" :
									"stderr",
					unit_id, strerror(-r)),
				LOG_ERRNO(-r), NULL);
...
static int
chown_terminal(int fd, uid_t uid)
{
	struct stat st;
...
static int
setup_confirm_stdio(int *_saved_stdin, int *_saved_stdout)
{
	...
	fd = acquire_terminal("/dev/console", false, false, false,
...
static int
restore_confirm_stdio(int *saved_stdin, int *saved_stdout)
{
	int r = 0;
...
static int
ask_for_confirmation(char *response, char **argv)
{
...
static int
enforce_groups(const ExecContext *context, const char *username, gid_t gid)
{
...
static int
enforce_user(const ExecContext *context, uid_t uid)
{
	assert(context);
	...
	if (context->capabilities || context->capability_ambient_set != 0) {
		...
		if (context->capabilities) {
			...
			if (cap_set_flag(d, CAP_EFFECTIVE, ELEMENTSOF(bits),
				    bits, CAP_SET) < 0 ||
				cap_set_flag(d, CAP_PERMITTED, ELEMENTSOF(bits),
					bits, CAP_SET) < 0)
				return -errno;
...
static int
setup_pam(const char *name, const char *user, uid_t uid, const char *tty,
	char ***pam_env, int fds[], unsigned n_fds)
{
...
static void
rename_process_from_path(const char *path)
{
...
static int
apply_seccomp(const ExecContext *c)
{
	...
	seccomp = seccomp_init(
		c->syscall_whitelist ? negative_action : SCMP_ACT_ALLOW);
	if (!seccomp)
		return -ENOMEM;
...
static int
apply_address_families(const ExecContext *c)
{
	scmp_filter_ctx *seccomp;
	Iterator i;
	int r;
	...
	if (c->address_families_whitelist) {
		...
		if (first == 0) {
			...
			for (af = 1; af < af_max(); af++) {
				if (set_contains(c->address_families,
					    INT_TO_PTR(af)))
					continue;
...
static void
do_idle_pipe_dance(int idle_pipe[4])
{
	assert(idle_pipe);
	...
	if (idle_pipe[0] >= 0) {
		...
		if (idle_pipe[3] >= 0 && r == 0 /* timeout */) {
			...
			fd_wait_for_event(idle_pipe[0], POLLHUP,
				IDLE_TIMEOUT2_USEC);
		}
...
static int
build_environment(const ExecContext *c, unsigned n_fds, usec_t watchdog_usec,
	const char *home, const char *username, const char *shell, char ***ret)
{
	...
	if (is_terminal_input(c->std_input) ||
		...
		c->std_error == EXEC_OUTPUT_TTY || c->tty_path) {
...
static bool
exec_needs_mount_namespace(const ExecContext *context,
...
static int
exec_child(ExecCommand *command, const ExecContext *context,
	const ExecParameters *params, ExecRuntime *runtime, char **argv,
	int socket_fd, int *fds, unsigned n_fds, char **files_env,
	int *exit_status)
{
	_cleanup_strv_free_ char **our_env = NULL, **pass_env = NULL,
				 **pam_env = NULL, **final_env = NULL,
				 **final_argv = NULL;
	...
	if (runtime) {
		if (runtime->netns_storage_socket[0] >= 0)
			dont_close[n_dont_close++] =
				runtime->netns_storage_socket[0];
		if (runtime->netns_storage_socket[1] >= 0)
			dont_close[n_dont_close++] =
				runtime->netns_storage_socket[1];
	}
	...
	if (params->confirm_spawn) {
		...
		if (r == -ETIMEDOUT)
			...
			write_confirm_message(
				"Couldn't ask confirmation question, assuming positive response: %s\n",
				strerror(-r));
		else if (response == 's') {
	...
	if (params->cgroup_path) {
		r = cg_attach_everywhere(params->cgroup_supported,
			params->cgroup_path, 0, NULL, NULL);
		if (r < 0) {
	...
	if (context->oom_score_adjust_set) {
		...
		if (r == -EPERM || r == -EACCES) {
			log_open();
			log_unit_debug_errno(params->unit_id, r,
	...
	if (params->cgroup_path && context->user && params->cgroup_delegate) {
		r = cg_set_task_access(SYSTEMD_CGROUP_CONTROLLER,
			params->cgroup_path, 0644, uid, gid);
		if (r < 0) {
			*exit_status = EXIT_CGROUP;
			return r;
		}
		...
		r = cg_set_group_access(SYSTEMD_CGROUP_CONTROLLER,
			params->cgroup_path, 0755, uid, gid);
		if (r < 0) {
			*exit_status = EXIT_CGROUP;
			return r;
		}
	...
	if (!strv_isempty(context->runtime_directory) &&
		params->runtime_prefix) {
		char **rt;
		...
		STRV_FOREACH (rt, context->runtime_directory) {
			...
			r = mkdir_safe_label(p, context->runtime_directory_mode,
				uid, gid);
			if (r < 0) {
				*exit_status = EXIT_RUNTIME_DIRECTORY;
				return r;
	...
	if (needs_mount_namespace) {
		...
		r = setup_namespace(
			...
			context->read_write_dirs, context->read_only_dirs,
			context->inaccessible_dirs, tmp, var,
			params->bus_endpoint_path, context->private_devices,
			context->protect_home, context->protect_system,
			context->mount_flags);
	...
	if (params->apply_chroot) {
		...
		if (asprintf(&d, "%s/%s", context->root_directory ?: "",
	...
	if (params->apply_permissions && mac_selinux_use() &&
		...
		r = mac_selinux_get_child_mls_label(socket_fd, command->path,
			context->selinux_context, &mac_selinux_context_net);
		if (r < 0) {
	...
	if (params->apply_permissions) {
		...
		if (!cap_test_all(context->capability_bounding_set)) {
			r = capability_bounding_set_drop(
				context->capability_bounding_set, false);
			if (r < 0) {
		...
		if (context->smack_process_label) {
			r = mac_smack_apply_pid(0,
				context->smack_process_label);
			if (r < 0) {
		...
		if (context->capability_ambient_set != 0) {
			r = capability_ambient_set_apply(
				context->capability_ambient_set, true);
			if (r < 0) {
				*exit_status = EXIT_CAPABILITIES;
				return r;
			}
			...
			if (context->capabilities) {
				...
				r = capability_update_inherited_set(
	...
	final_env = strv_env_merge(6, params->environment, our_env, pass_env,
		context->environment, files_env, pam_env, NULL);
	if (!final_env) {
		*exit_status = EXIT_MEMORY;
		return -ENOMEM;
	}
	...
	if (_unlikely_(log_get_max_level() >= LOG_DEBUG)) {
		...
		if (line) {
			log_open();
			log_unit_struct(params->unit_id, LOG_DEBUG,
				"EXECUTABLE=%s", command->path,
				LOG_MESSAGE("Executing: %s", line), NULL);
			log_close();
		}
	}
...
int
exec_spawn(ExecCommand *command, const ExecContext *context,
	const ExecParameters *params, ExecRuntime *runtime, pid_t *ret)
{
	...
	if (r < 0)
		return log_unit_error_errno(params->unit_id, r,
	...
	pid = fork();
	if (pid < 0)
		return log_unit_error_errno(params->unit_id, errno,
			"Failed to fork: %m");
	...
	if (pid == 0) {
		...
		if (r < 0) {
			log_open();
			log_unit_struct(params->unit_id, LOG_ERR,
...
void
exec_context_init(ExecContext *c)
{
	assert(c);
...
void
exec_context_done(ExecContext *c)
{
	unsigned l;
...
int
exec_context_destroy_runtime_directory(ExecContext *c,
...
void
exec_command_done(ExecCommand *c)
{
	assert(c);
...
void
exec_command_done_array(ExecCommand *c, unsigned n)
{
	unsigned i;
...
ExecCommand *
exec_command_free_list(ExecCommand *c)
{
	ExecCommand *i;
...
void
exec_command_free_array(ExecCommand **c, unsigned n)
{
	unsigned i;
...
static void
invalid_env(const char *p, void *userdata)
{
	InvalidEnvInfo *info = userdata;

	log_unit_error(info->unit_id,
		"Ignoring invalid environment assignment '%s': %s", p,
		info->path);
}

int
exec_context_load_environment(const ExecContext *c, const char *unit_id,
	char ***l)
{
	char **i, **r = NULL;
	...
	STRV_FOREACH (i, c->environment_files) {
		...
		for (n = 0; n < count; n++) {
			...
			if (p) {
				InvalidEnvInfo info = { .unit_id = unit_id,
					.path = pglob.gl_pathv[n] };

				p = strv_env_clean_with_callback(p, invalid_env,
					&info);
			}
...
static bool
tty_may_match_dev_console(const char *tty)
{
	...
	return streq(console, tty) ||
		(streq(console, "tty0") && tty_is_vc(tty));
}

bool
exec_context_may_touch_console(ExecContext *ec)
{
...
void
exec_context_dump(ExecContext *c, FILE *f, const char *prefix)
{
	char **e;
	unsigned i;
	...
	if (c->tty_path)
		fprintf(f,
			...
			"%sTTYVHangup: %s\n"
			"%sTTYVTDisallocate: %s\n",
			prefix, c->tty_path, prefix, yes_no(c->tty_reset),
			prefix, yes_no(c->tty_vhangup), prefix,
			yes_no(c->tty_vt_disallocate));
	...
	if (c->std_output == EXEC_OUTPUT_SYSLOG ||
		...
		log_facility_unshifted_to_string_alloc(c->syslog_priority >> 3,
			&fac_str);
		log_level_to_string_alloc(LOG_PRI(c->syslog_priority),
			&lvl_str);
	...
	if (c->syscall_filter) {
		...
		SET_FOREACH (id, c->syscall_filter, j) {
			...
			name = seccomp_syscall_resolve_num_arch(
				SCMP_ARCH_NATIVE, PTR_TO_INT(id) - 1);
			fputs(strna(name), f);
		}
#endif
	...
	if (c->syscall_errno != 0)
		fprintf(f, "%sSystemCallErrorNumber: %s\n", prefix,
			strna(errno_to_name(c->syscall_errno)));
...
bool
exec_context_maintains_privileges(ExecContext *c)
{
	assert(c);
...
void
exec_status_start(ExecStatus *s, pid_t pid)
{
	assert(s);
...
void
exec_status_exit(ExecStatus *s, ExecContext *context, pid_t pid, int code,
	int status)
{
	assert(s);
	...
	if (context) {
		if (context->utmp_id)
			utmp_put_dead_process(context->utmp_id, pid, code,
				status);
...
void
exec_status_dump(ExecStatus *s, FILE *f, const char *prefix)
{
	...
	if (s->exit_timestamp.realtime > 0)
		fprintf(f,
			...
			prefix,
			format_timestamp(buf, sizeof(buf),
				s->exit_timestamp.realtime),
			prefix, sigchld_code_to_string(s->code), prefix,
			s->status);
}

char *
exec_command_line(char **argv)
{
	size_t k;
	char *n, *p, **a;
	bool first = true;
...
void
exec_command_dump(ExecCommand *c, FILE *f, const char *prefix)
{
	...
	fprintf(f, "%sCommand Line: %s\n", prefix,
		cmd ? cmd : strerror(ENOMEM));

	exec_status_dump(&c->exec_status, f, prefix2);
}

void
exec_command_dump_list(ExecCommand *c, FILE *f, const char *prefix)
{
	assert(f);
...
void
exec_command_append_list(ExecCommand **l, ExecCommand *e)
{
	ExecCommand *end;
...
int
exec_command_set(ExecCommand *c, const char *path, ...)
{
	va_list ap;
	char **l, *p;
...
int
exec_command_append(ExecCommand *c, const char *path, ...)
{
...
static int
exec_runtime_allocate(ExecRuntime **rt)
{
	if (*rt)
		return 0;
...
int
exec_runtime_make(ExecRuntime **rt, ExecContext *c, const char *id)
{
	int r;
...
ExecRuntime *
exec_runtime_ref(ExecRuntime *r)
{
	assert(r);
	assert(r->n_ref > 0);
...
ExecRuntime *
exec_runtime_unref(ExecRuntime *r)
{
	if (!r)
		return NULL;
...
int
exec_runtime_serialize(ExecRuntime *rt, Unit *u, FILE *f, FDSet *fds)
{
	assert(u);
	assert(f);
	assert(fds);
...
int
exec_runtime_deserialize_item(ExecRuntime **rt, Unit *u, const char *key,
...
void
exec_runtime_destroy(ExecRuntime *rt)
{
	int r;
	...
	if (rt->tmp_dir) {
		...
		if (r < 0) {
			log_warning_errno(r, "Failed to nuke %s: %m",
				rt->tmp_dir);
			free(rt->tmp_dir);
		}
