...
static int
property_get_blockio_device_weight(sd_bus *bus, const char *path,
...
static int
property_get_blockio_device_bandwidths(sd_bus *bus, const char *path,
	const char *interface, const char *property, sd_bus_message *reply,
	void *userdata, sd_bus_error *error)
{
	CGroupContext *c = userdata;
...
static int
property_get_device_allow(sd_bus *bus, const char *path, const char *interface,
...
const sd_bus_vtable bus_cgroup_vtable[] = { SD_BUS_VTABLE_START(0),
	...
	SD_BUS_PROPERTY("CPUShares", "t", NULL,
		offsetof(CGroupContext, cpu_shares), 0),
	...
	SD_BUS_PROPERTY("DeviceAllow", "a(ss)", property_get_device_allow, 0,
		0),
	SD_BUS_PROPERTY("TasksAccounting", "b", bus_property_get_bool,
		offsetof(CGroupContext, tasks_accounting), 0),
	SD_BUS_PROPERTY("TasksMax", "t", NULL,
		offsetof(CGroupContext, tasks_max), 0),
	SD_BUS_VTABLE_END };

static int
bus_cgroup_set_transient_property(Unit *u, CGroupContext *c, const char *name,
	...
	sd_bus_error *error)
{
	...
	if (streq(name, "Delegate")) {
		...
		if (mode != UNIT_CHECK) {
			c->delegate = b;
			unit_write_drop_in_private(u, mode, name,
				b ? "Delegate=yes" : "Delegate=no");
		}
...
int
bus_cgroup_set_property(Unit *u, CGroupContext *c, const char *name,
	sd_bus_message *message, UnitSetPropertiesMode mode,
	sd_bus_error *error)
{
	int r;
	...
	if (streq(name, "CPUAccounting")) {
		...
		if (mode != UNIT_CHECK) {
			...
			unit_write_drop_in_private(u, mode, name,
				b ? "CPUAccounting=yes" : "CPUAccounting=no");
		}
		...
		if (mode != UNIT_CHECK) {
			...
			if (shares == CGROUP_CPU_SHARES_INVALID)
				...
				unit_write_drop_in_private_format(u, mode, name,
					"StartupCPUShares=%" PRIu64, shares);
		}
	...
	} else if (streq(name, "BlockIOReadBandwidth") ||
		...
		while ((r = sd_bus_message_read(message, "(st)", &path, &u64)) >
			0) {
			if (mode != UNIT_CHECK) {
		...
		while ((r = sd_bus_message_read(message, "(st)", &path,
				&weight)) > 0) {
			if (!CGROUP_BLKIO_WEIGHT_IS_OK(weight) ||
		...
		while ((r = sd_bus_message_read(message, "(ss)", &path, &rwm)) >
			0) {
		...
		if (mode != UNIT_CHECK) {
			...
			if (n == 0) {
				while (c->device_allow)
					cgroup_context_free_device_allow(c,
						c->device_allow);
			}
	...
	if (u->transient && u->load_state == UNIT_STUB) {
		r = bus_cgroup_set_transient_property(u, c, name, message, mode,
			error);
		if (r != 0)
			return r;
	}
