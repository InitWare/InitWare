...
static inline void
expire_data_free(struct expire_data *data)
{
	if (!data)
		return;
...
static void
repeat_unmount(const char *path)
{
	assert(path);
...
static int
automount_add_mount_links(Automount *a)
{
...
static int
automount_add_default_dependencies(Automount *a)
{
	int r;
...
static int
automount_set_where(Automount *a)
{
	assert(a);
...
automount_load(Unit *u)
{
	...
	if (u->load_state == UNIT_LOADED) {
		...
		r = unit_add_two_dependencies(u, UNIT_BEFORE, UNIT_TRIGGERS, x,
			true);
		if (r < 0)
			return r;
...
static void
automount_set_state(Automount *a, AutomountState state)
{
	...
	if (state != AUTOMOUNT_WAITING && state != AUTOMOUNT_RUNNING)
		unmount_autofs(a);

	if (state != old_state)
		log_unit_debug(UNIT(a)->id, "%s changed %s -> %s", UNIT(a)->id,
...
static int
automount_coldplug(Unit *u, Hashmap *deferred_work)
{
	Automount *a = AUTOMOUNT(u);
	int r;
	...
	if (IN_SET(a->deserialized_state, AUTOMOUNT_WAITING,
		...
		if (a->deserialized_state == AUTOMOUNT_RUNNING) {
			...
			if (r < 0)
				log_unit_warning_errno(UNIT(a)->id, r,
...
static void
automount_dump(Unit *u, FILE *f, const char *prefix)
{
...
static void
automount_enter_dead(Automount *a, AutomountResult f)
{
	assert(a);
	...
	automount_set_state(a,
		a->result != AUTOMOUNT_SUCCESS ? AUTOMOUNT_FAILED :
						       AUTOMOUNT_DEAD);
}

static int
open_dev_autofs(Manager *m)
{
	...
	log_debug("Autofs kernel version %i.%i", param.ver_major,
		param.ver_minor);
...
static int
open_ioctl_fd(int dev_autofs_fd, const char *where, dev_t devid)
{
	struct autofs_dev_ioctl *param;
	size_t l;
...
static int
autofs_protocol(int dev_autofs_fd, int ioctl_fd)
{
...
static int
autofs_set_timeout(int dev_autofs_fd, int ioctl_fd, usec_t usec)
{
	struct autofs_dev_ioctl param;
...
static int
autofs_send_ready(int dev_autofs_fd, int ioctl_fd, uint32_t token, int status)
{
	struct autofs_dev_ioctl param;
...
static int
automount_send_ready(Automount *a, Set *tokens, int status)
{
	...
	ioctl_fd = open_ioctl_fd(UNIT(a)->manager->dev_autofs_fd, a->where,
		a->dev_id);
	if (ioctl_fd < 0)
		return ioctl_fd;

	if (status)
		log_unit_debug_errno(UNIT(a)->id, status,
			"Sending failure: %m");
	else
	...
	while ((token = PTR_TO_UINT(set_steal_first(tokens)))) {
		...
		k = autofs_send_ready(UNIT(a)->manager->dev_autofs_fd, ioctl_fd,
			token, status);
		if (k < 0)
			r = k;
	}
...
int
automount_update_mount(Automount *a, MountState old_state, MountState state)
{
	_cleanup_close_ int ioctl_fd = -1;
	int r;
...
static void
automount_enter_waiting(Automount *a)
{
	_cleanup_close_ int ioctl_fd = -1;
	int p[2] = { -1, -1 };
	...
	r = sd_event_add_io(UNIT(a)->manager->event, &a->pipe_event_source,
		p[0], EPOLLIN, automount_dispatch_io, a);
	if (r < 0)
		goto fail;
...
fail:
	...
	log_unit_error(UNIT(a)->id, "Failed to initialize automounter: %s",
		strerror(-r));
	automount_enter_dead(a, AUTOMOUNT_FAILURE_RESOURCES);
}

static void *
expire_thread(void *p)
{
	struct autofs_dev_ioctl param;
...
static int
automount_dispatch_expire(sd_event_source *source, usec_t usec, void *userdata)
{
	...
	if (data->dev_autofs_fd < 0)
		return log_unit_error_errno(UNIT(a)->id, errno,
			"Failed to duplicate autofs fd: %m");

	data->ioctl_fd = open_ioctl_fd(UNIT(a)->manager->dev_autofs_fd,
		a->where, a->dev_id);
	if (data->ioctl_fd < 0)
		return log_unit_error_errno(UNIT(a)->id, data->ioctl_fd,
			"Couldn't open autofs ioctl fd: %m");

	r = asynchronous_job(expire_thread, data);
	if (r < 0)
		return log_unit_error_errno(UNIT(a)->id, r,
...
static int
automount_start_expire(Automount *a)
{
	int r;
	usec_t timeout;
	...
	timeout = now(CLOCK_MONOTONIC) +
		MAX(a->timeout_idle_usec / 10, USEC_PER_SEC);
	...
	return sd_event_add_time(UNIT(a)->manager->event,
		&a->expire_event_source, CLOCK_MONOTONIC, timeout, 0,
		automount_dispatch_expire, a);
}

static void
automount_enter_running(Automount *a)
{
	...
	if (unit_stop_pending(UNIT(a))) {
		log_unit_debug(UNIT(a)->id,
			"Suppressing automount request on %s since unit stop is scheduled.",
			UNIT(a)->id);
		automount_send_ready(a, a->tokens, -EHOSTDOWN);
		automount_send_ready(a, a->expire_tokens, -EHOSTDOWN);
		return;
	...
	r = manager_add_job(UNIT(a)->manager, JOB_START, trigger, JOB_REPLACE,
		true, &error, NULL);
	if (r < 0) {
		log_unit_warning(UNIT(a)->id,
			"%s failed to queue mount startup job: %s", UNIT(a)->id,
...
static int
automount_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Automount *a = AUTOMOUNT(u);
	void *p;
	Iterator i;
	...
	SET_FOREACH (p, a->expire_tokens, i)
		unit_serialize_item_format(u, f, "expire-token", "%u",
			PTR_TO_UINT(p));
...
static int
automount_deserialize_item(Unit *u, const char *key, const char *value,
	FDSet *fds)
{
	Automount *a = AUTOMOUNT(u);
	int r;
	...
	if (streq(key, "state")) {
		...
		if (safe_atou(value, &d) < 0)
			log_unit_debug(u->id, "Failed to parse dev-id value %s",
				value);
		else
			a->dev_id = (unsigned)d;
		...
		if (safe_atou(value, &token) < 0)
			log_unit_debug(u->id, "Failed to parse token value %s",
				value);
		else {
			if (!a->tokens)
...
static const char *
automount_sub_state_to_string(Unit *u)
{
	assert(u);
...
static int
automount_dispatch_io(sd_event_source *s, int fd, uint32_t events,
	void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	...
	if (l != sizeof(packet)) {
		if (l < 0)
			log_unit_error_errno(UNIT(a)->id, l,
				"Invalid read from pipe: %m");
		else
			log_unit_error(UNIT(a)->id,
				"Invalid read from pipe: short read");
		goto fail;
	}
	...
	case autofs_ptype_missing_direct:
		...
		if (packet.v5_packet.pid > 0) {
			...
			log_unit_debug(UNIT(a)->id,
				"Got direct mount request on %s", a->where);
		...
		if (r < 0) {
			log_unit_error_errno(UNIT(a)->id, r,
	...
	case autofs_ptype_expire_direct:
		log_unit_debug(UNIT(a)->id, "Got direct umount request on %s",
			a->where);
	...
	default:
		log_unit_error(UNIT(a)->id,
			"Received unknown automount request %i",
			packet.hdr.type);
		break;
	}
