...
scope_arm_timer(Scope *s)
{
	...
	if (s->timer_event_source) {
		r = sd_event_source_set_time(s->timer_event_source,
			now(CLOCK_MONOTONIC) + s->timeout_stop_usec);
		if (r < 0)
			return r;
...
static void
scope_set_state(Scope *s, ScopeState state)
{
...
static int
scope_add_default_dependencies(Scope *s)
{
	int r;
...
scope_verify(Scope *s)
{
	...
	if (set_isempty(UNIT(s)->pids) && UNIT(s)->manager->n_reloading <= 0) {
		log_unit_error(UNIT(s)->id, "Scope %s has no PIDs. Refusing.",
			UNIT(s)->id);
		return -EINVAL;
	}
...
static int
scope_coldplug(Unit *u, Hashmap *deferred_work)
{
	Scope *s = SCOPE(u);
	int r;
	...
	if (s->deserialized_state != s->state) {
		if (IN_SET(s->deserialized_state, SCOPE_STOP_SIGKILL,
			    SCOPE_STOP_SIGTERM)) {
			r = scope_arm_timer(s);
			if (r < 0)
				return r;
...
static void
scope_dump(Unit *u, FILE *f, const char *prefix)
{
	Scope *s = SCOPE(u);
...
static void
scope_enter_dead(Scope *s, ScopeResult f)
{
	assert(s);
	...
	scope_set_state(s,
		s->result != SCOPE_SUCCESS ? SCOPE_FAILED : SCOPE_DEAD);
}

static void
scope_enter_signal(Scope *s, ScopeState state, ScopeResult f)
{
...
fail:
	log_unit_warning(UNIT(s)->id, "%s failed to kill processes: %s",
...
scope_start(Unit *u)
{
	...
	if (s->state == SCOPE_STOP_SIGTERM || s->state == SCOPE_STOP_SIGKILL)
		return -EAGAIN;
...
scope_stop(Unit *u)
{
	...
	assert(s->state == SCOPE_RUNNING || s->state == SCOPE_ABANDONED);
...
static int
scope_kill(Unit *u, KillWho who, int signo, sd_bus_error *error)
{
	return unit_kill_common(u, who, signo, -1, -1, error);
}

static int
scope_get_timeout(Unit *u, uint64_t *timeout)
{
	Scope *s = SCOPE(u);
	int r;
...
static int
scope_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Scope *s = SCOPE(u);
...
static int
scope_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Scope *s = SCOPE(u);
...
static void
scope_notify_cgroup_empty_event(Unit *u)
{
	Scope *s = SCOPE(u);
	assert(u);
...
static void
scope_sigchld_event(Unit *u, pid_t pid, int code, int status)
{
...
static int
scope_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata)
{
	...
	case SCOPE_STOP_SIGTERM:
		if (s->kill_context.send_sigkill) {
			...
			log_unit_warning(UNIT(s)->id,
				"%s stopping timed out. Skipping SIGKILL.",
				UNIT(s)->id);
...
scope_sub_state_to_string(Unit *u)
{
	assert(u);
...
const UnitVTable scope_vtable = { .object_size = sizeof(Scope),
	...
	.bus_interface = SVC_DBUS_INTERFACE ".Scope",
