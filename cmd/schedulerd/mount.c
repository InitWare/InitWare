...
static bool
mount_needs_network(const char *options, const char *fstype)
{
...
static bool
mount_is_network(const MountParameters *p)
{
	assert(p);
...
static bool
mount_is_bind(const MountParameters *p)
{
	assert(p);
...
static bool
mount_is_auto(const MountParameters *p)
{
	assert(p);
...
static bool
needs_quota(const MountParameters *p)
{
	assert(p);
...
const char *
mount_get_fstype(const Mount *m)
{
	...
	if (m->from_proc_self_mountinfo &&
		m->parameters_proc_self_mountinfo.fstype)
...
mount_arm_timer(Mount *m)
{
	...
	if (m->timeout_usec <= 0) {
		m->timer_event_source =
			sd_event_source_unref(m->timer_event_source);
		return 0;
	}

	if (m->timer_event_source) {
		r = sd_event_source_set_time(m->timer_event_source,
			now(CLOCK_MONOTONIC) + m->timeout_usec);
		if (r < 0)
			return r;
	...
	return sd_event_add_time(UNIT(m)->manager->event,
		&m->timer_event_source, CLOCK_MONOTONIC,
		now(CLOCK_MONOTONIC) + m->timeout_usec, 0, mount_dispatch_timer,
		m);
}

static void
mount_unwatch_control_pid(Mount *m)
{
	assert(m);
...
static void
mount_parameters_done(MountParameters *p)
{
	assert(p);
...
_pure_ static MountParameters *
get_mount_parameters_fragment(Mount *m)
{
	assert(m);
...
mount_add_mount_links(Mount *m)
{
	...
	SET_FOREACH (other, s, i) {
		...
		if (UNIT(m)->fragment_path &&
			!streq(UNIT(m)->id, "tmp.mount")) {
			/* If we have fragment configuration, then make this dependency required */
			r = unit_add_dependency(other, UNIT_REQUIRES, UNIT(m),
				true);
			if (r < 0)
				return r;
		}
...
static int
mount_add_device_links(Mount *m)
{
	MountParameters *p;
	...
	r = unit_add_node_link(UNIT(m), p->what, device_wants_mount,
		m->from_fragment ? UNIT_BINDS_TO : UNIT_REQUIRES);
	if (r < 0)
		return r;
...
mount_add_quota_links(Mount *m)
{
	...
	r = unit_add_two_dependencies_by_name(UNIT(m), UNIT_BEFORE, UNIT_WANTS,
		SPECIAL_QUOTACHECK_SERVICE, NULL, true);
	if (r < 0)
		return r;

	r = unit_add_two_dependencies_by_name(UNIT(m), UNIT_BEFORE, UNIT_WANTS,
		SPECIAL_QUOTAON_SERVICE, NULL, true);
	if (r < 0)
		return r;
...
static bool
should_umount(Mount *m)
{
	MountParameters *p;

	if (path_equal(m->where, "/") || path_equal(m->where, "/usr"))
		return false;
...
static int
mount_add_default_dependencies(Mount *m)
{
	...
	if (after2) {
		r = unit_add_dependency_by_name(UNIT(m), UNIT_AFTER, after2,
			NULL, true);
		if (r < 0)
			return r;
	}
...
static int
mount_notify_automount(Mount *m, MountState old_state, MountState state)
{
	Unit *p;
	int r;
	Iterator i;
	...
	SET_FOREACH (p, UNIT(m)->dependencies[UNIT_TRIGGERED_BY], i)
		if (p->type == UNIT_AUTOMOUNT) {
			r = automount_update_mount(AUTOMOUNT(p), old_state,
				state);
			if (r < 0)
				return r;
		}
...
static void
mount_set_state(Mount *m, MountState state)
{
	...
	if (state != MOUNT_MOUNTING && state != MOUNT_MOUNTING_DONE &&
		state != MOUNT_REMOUNTING && state != MOUNT_UNMOUNTING &&
	...
	if (state != old_state)
		log_unit_debug(UNIT(m)->id, "%s changed %s -> %s", UNIT(m)->id,
...
static int
mount_coldplug(Unit *u, Hashmap *deferred_work)
{
	Mount *m = MOUNT(u);
	...
	if (new_state == MOUNT_MOUNTING || new_state == MOUNT_MOUNTING_DONE ||
...
static void
mount_dump(Unit *u, FILE *f, const char *prefix)
{
	Mount *m = MOUNT(u);
	MountParameters *p;
...
static int
mount_spawn(Mount *m, ExecCommand *c, pid_t *_pid)
{
	pid_t pid;
	int r;
	...
	r = exec_spawn(c, &m->exec_context, &exec_params, m->exec_runtime,
		&pid);
	if (r < 0)
		goto fail;
...
static void
mount_enter_dead(Mount *m, MountResult f)
{
	assert(m);
	...
	mount_set_state(m,
		m->result != MOUNT_SUCCESS ? MOUNT_FAILED : MOUNT_DEAD);
}

static void
mount_enter_mounted(Mount *m, MountResult f)
{
	assert(m);
...
static void
mount_enter_signal(Mount *m, MountState state, MountResult f)
{
	int r;
...
fail:
	log_unit_warning(UNIT(m)->id, "%s failed to kill processes: %s",
		UNIT(m)->id, strerror(-r));

	if (state == MOUNT_REMOUNTING_SIGTERM ||
		state == MOUNT_REMOUNTING_SIGKILL)
...
void
warn_if_dir_nonempty(const char *unit, const char *where)
{
	int r;
	...
	if (r > 0)
		...
		log_unit_struct(unit, LOG_NOTICE,
			LOG_MESSAGE_ID(SD_MESSAGE_OVERMOUNTING),
			LOG_MESSAGE(
				"%s: Directory %s to mount over is not empty, mounting anyway.",
				unit, where),
			"WHERE=%s", where, NULL);
	else
		log_unit_warning(unit,
			"MESSAGE=Failed to check directory %s: %s", where,
			strerror(-r));
}

static int
fail_if_symlink(const char *unit, const char *where)
{
	assert(where);

	if (is_symlink(where) > 0) {
		log_unit_struct(unit, LOG_ERR,
			LOG_MESSAGE_ID(SD_MESSAGE_OVERMOUNTING),
...
static void
mount_enter_unmounting(Mount *m)
{
	int r;
...
fail:
	log_unit_warning(UNIT(m)->id, "%s failed to run 'umount' task: %s",
		UNIT(m)->id, strerror(-r));
	mount_enter_mounted(m, MOUNT_FAILURE_RESOURCES);
}

static void
mount_enter_mounting(Mount *m)
{
	int r;
	MountParameters *p;
	...
	if (m->from_fragment) {
		...
		if (r >= 0 && m->parameters_fragment.fstype)
			r = exec_command_append(m->control_command, "-t",
				m->parameters_fragment.fstype, NULL);
		if (r >= 0 && !isempty(opts))
			r = exec_command_append(m->control_command, "-o", opts,
				NULL);
	} else
		r = -ENOENT;
...
fail:
	log_unit_warning(UNIT(m)->id, "%s failed to run 'mount' task: %s",
		UNIT(m)->id, strerror(-r));
	mount_enter_dead(m, MOUNT_FAILURE_RESOURCES);
}

static void
mount_enter_remounting(Mount *m)
{
	int r;
	...
	if (m->from_fragment) {
		...
		r = exec_command_set(m->control_command, "/bin/mount",
			m->parameters_fragment.what, m->where, "-o", o, NULL);
		if (r >= 0 && m->sloppy_options)
			r = exec_command_append(m->control_command, "-s", NULL);
		if (r >= 0 && m->parameters_fragment.fstype)
			r = exec_command_append(m->control_command, "-t",
...
fail:
	log_unit_warning(UNIT(m)->id, "%s failed to run 'remount' task: %s",
		UNIT(m)->id, strerror(-r));
...
mount_stop(Unit *u)
{
	...
	assert(m->state == MOUNT_MOUNTING || m->state == MOUNT_MOUNTING_DONE ||
		m->state == MOUNT_MOUNTED || m->state == MOUNT_REMOUNTING ||
...
static int
mount_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Mount *m = MOUNT(u);
...
static int
mount_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds)
{
	Mount *m = MOUNT(u);
	...
	if (streq(key, "state")) {
		...
		log_unit_debug(UNIT(m)->id, "Unknown serialization key '%s'",
			key);
...
mount_sub_state_to_string(Unit *u)
{
	assert(u);
...
static void
mount_sigchld_event(Unit *u, pid_t pid, int code, int status)
{
	Mount *m = MOUNT(u);
	MountResult f;
	...
	if (m->control_command) {
		exec_status_exit(&m->control_command->exec_status,
			&m->exec_context, pid, code, status);

		m->control_command = NULL;
		m->control_command_id = _MOUNT_EXEC_COMMAND_INVALID;
	}

	log_unit_full(u->id, f == MOUNT_SUCCESS ? LOG_DEBUG : LOG_NOTICE,
		"%s mount process exited, code=%s status=%i", u->id,
		...
		if (f == MOUNT_SUCCESS) {
			if (m->from_proc_self_mountinfo) {
				...
				if (m->n_retry_umount < RETRY_UMOUNT_MAX) {
					log_unit_debug(u->id,
						"%s: mount still present, trying again.",
						u->id);
					m->n_retry_umount++;
					mount_enter_unmounting(m);
				} else {
					log_unit_debug(u->id,
						"%s: mount still present after %u attempts to unmount, giving up.",
						u->id, m->n_retry_umount);
					mount_enter_mounted(m, f);
				}
			} else
				mount_enter_dead(m, f);
...
static int
mount_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata)
{
	...
	case MOUNT_MOUNTING_DONE:
		...
		mount_enter_signal(m, MOUNT_MOUNTING_SIGTERM,
	...
	case MOUNT_MOUNTING_SIGTERM:
		if (m->kill_context.send_sigkill) {
			...
			mount_enter_signal(m, MOUNT_MOUNTING_SIGKILL,
				MOUNT_FAILURE_TIMEOUT);
		} else {
			log_unit_warning(UNIT(m)->id,
	...
	case MOUNT_REMOUNTING_SIGTERM:
		if (m->kill_context.send_sigkill) {
			log_unit_warning(UNIT(m)->id,
				"%s remounting timed out. Killing.",
				UNIT(m)->id);
			mount_enter_signal(m, MOUNT_REMOUNTING_SIGKILL,
				MOUNT_FAILURE_TIMEOUT);
		} else {
			log_unit_warning(UNIT(m)->id,
	...
	case MOUNT_UNMOUNTING_SIGTERM:
		if (m->kill_context.send_sigkill) {
			log_unit_warning(UNIT(m)->id,
				"%s unmounting timed out. Killing.",
				UNIT(m)->id);
...
static int
mount_setup_unit(Manager *m, const char *what, const char *where,
	const char *options, const char *fstype, bool set_flags)
{
	...
	if (!u) {
		...
		if (m->running_as == SYSTEMD_SYSTEM) {
			...
			r = unit_add_dependency_by_name(u, UNIT_BEFORE, target,
				NULL, true);
			if (r < 0)
				goto fail;
			...
			if (should_umount(MOUNT(u))) {
				r = unit_add_dependency_by_name(u,
					UNIT_CONFLICTS, SPECIAL_UMOUNT_TARGET,
					NULL, true);
				if (r < 0)
					goto fail;
			}
		...
		if (m->running_as == SYSTEMD_SYSTEM &&
			...
			unit_add_dependency_by_name(u, UNIT_BEFORE,
				SPECIAL_REMOTE_FS_TARGET, NULL, true);
			load_extras = true;
		}
		...
		just_mounted = !MOUNT(u)->from_proc_self_mountinfo ||
	...
	changed = changed || !streq_ptr(p->options, options) ||
		!streq_ptr(p->what, what) || !streq_ptr(p->fstype, fstype);
...
static int
mount_load_proc_self_mountinfo(Manager *m, bool set_flags)
{
	...
	for (;;) {
		...
		(void)device_found_node(m, d, true, DEVICE_FOUND_MOUNT,
			set_flags);
...
static int
mount_get_timeout(Unit *u, uint64_t *timeout)
{
	Mount *m = MOUNT(u);
	int r;
...
mount_enumerate(Manager *m)
{
	...
	if (!m->proc_self_mountinfo) {
		...
		r = sd_event_add_io(m->event, &m->mount_event_source,
			fileno(m->proc_self_mountinfo), EPOLLPRI,
			mount_dispatch_io, m);
		if (r < 0)
			goto fail;
...
static int
mount_dispatch_io(sd_event_source *source, int fd, uint32_t revents,
	void *userdata)
{
	...
	IWLIST_FOREACH (units_by_type, u, m->units_by_type[UNIT_MOUNT]) {
		...
		if (!mount->is_mounted) {
			...
			if (mount->from_proc_self_mountinfo &&
				...
				if (set_ensure_allocated(&gone,
					    &string_hash_ops) < 0 ||
					set_put(gone,
						mount->parameters_proc_self_mountinfo
							.what) < 0)
		...
		if (mount->is_mounted && mount->from_proc_self_mountinfo &&
			mount->parameters_proc_self_mountinfo.what) {
			if (set_ensure_allocated(&around, &string_hash_ops) <
					0 ||
				set_put(around,
					mount->parameters_proc_self_mountinfo
						.what) < 0)
		...
		mount->is_mounted = mount->just_mounted = mount->just_changed =
			false;
	}
	...
	SET_FOREACH (what, gone, i) {
		...
		(void)device_found_node(m, what, false, DEVICE_FOUND_MOUNT,
			true);
	}
...
static int
mount_kill(Unit *u, KillWho who, int signo, sd_bus_error *error)
{
	return unit_kill_common(u, who, signo, -1, MOUNT(u)->control_pid,
		error);
}

static const char *const mount_state_table[_MOUNT_STATE_MAX] = { [MOUNT_DEAD] =
									 "dead",
