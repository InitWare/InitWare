...
crash(int sig)
{
	if (getpid() != 1)
		...
		log_emergency("Caught <%s>, not dumping core.",
		...
		if (pid < 0)
			...
			if (r < 0)
				...
				log_emergency(
					"Caught <%s>, dumped core as pid " PID_FMT
					".",
					signal_to_string(sig), pid);
		}
#else
	...
	if (arg_crash_shell) {
		...
		if (pid < 0)
			...
			log_info(
				"Successfully spawned crash shell as PID " PID_FMT
				".",
				pid);
	}
#endif
...
install_crash_handler(void)
{
	static const struct sigaction sa = {
		.sa_handler = crash,
		.sa_flags =
			SA_NODEFER, /* So that we can raise the signal again from the signal handler */
	};
	int r;
...
parse_proc_cmdline_item(const char *key, const char *value)
{
	...
	if (streq(key, "systemd.unit") && value) {
		...
		if (r < 0)
			log_warning(
				"Failed to parse dump core switch %s. Ignoring.",
				value);
		else
			arg_dump_core = r;
		...
		if (r < 0)
			log_warning(
				"Failed to parse crash shell switch %s. Ignoring.",
				value);
		else
			arg_crash_shell = r;
		...
		if (safe_atoi(value, &r) < 0)
			log_warning(
				"Failed to parse crash chvt switch %s. Ignoring.",
				value);
		else
			arg_crash_chvt = r;
		...
		if (r < 0)
			log_warning(
				"Failed to parse confirm spawn switch %s. Ignoring.",
				value);
		else
			arg_confirm_spawn = r;
		...
		if (r < 0)
			log_warning(
				"Failed to parse show status switch %s. Ignoring.",
				value);
		...
		if (r < 0)
			log_warning(
				"Failed to parse default standard output switch %s. Ignoring.",
				value);
		else
		...
		if (r < 0)
			log_warning(
				"Failed to parse default standard error switch %s. Ignoring.",
				value);
		else
		...
		if (env_assignment_is_valid(value)) {
			...
			if (env)
				...
				log_warning_errno(ENOMEM,
					"Setting environment variable '%s' failed, ignoring: %m",
					value);
		} else
			log_warning(
	...
	void *userdata)
{
	...
	ncpus = parse_cpu_set_and_warn(rvalue, &c, unit, filename, line,
		lvalue);
	if (ncpus < 0)
		return ncpus;
...
static int
config_parse_join_controllers(const char *unit, const char *filename,
	unsigned line, const char *section, unsigned section_line,
	const char *lvalue, int ltype, const char *rvalue, void *data,
	void *userdata)
{
...
parse_config_file(void)
{
	const ConfigTableItem items[] = {
		...
		{ "Manager", "CrashShell", config_parse_bool, 0,
			&arg_crash_shell },
		{ "Manager", "ShowStatus", config_parse_show_status, 0,
			&arg_show_status },
		...
		{ "Manager", "DefaultStandardError", config_parse_output, 0,
		...
		{ "Manager", "DefaultLimitCPU", config_parse_limit, 0,
			&arg_default_rlimit[RLIMIT_CPU] },
		{ "Manager", "DefaultLimitFSIZE", config_parse_bytes_limit, 0,
			&arg_default_rlimit[RLIMIT_FSIZE] },
		{ "Manager", "DefaultLimitDATA", config_parse_bytes_limit, 0,
			&arg_default_rlimit[RLIMIT_DATA] },
		{ "Manager", "DefaultLimitSTACK", config_parse_bytes_limit, 0,
			&arg_default_rlimit[RLIMIT_STACK] },
		{ "Manager", "DefaultLimitCORE", config_parse_bytes_limit, 0,
			&arg_default_rlimit[RLIMIT_CORE] },
		{ "Manager", "DefaultLimitRSS", config_parse_bytes_limit, 0,
		...
		{ "Manager", "DefaultLimitSIGPENDING", config_parse_limit, 0,
			&arg_default_rlimit[RLIMIT_SIGPENDING] },
		{ "Manager", "DefaultLimitMSGQUEUE", config_parse_bytes_limit,
			0, &arg_default_rlimit[RLIMIT_MSGQUEUE] },
		{ "Manager", "DefaultLimitNICE", config_parse_nice_limit, 0,
			&arg_default_rlimit[RLIMIT_NICE] },
		{ "Manager", "DefaultLimitRTPRIO", config_parse_limit, 0,
		...
		{ "Manager", "CtrlAltDelBurstAction",
			config_parse_emergency_action, 0,
	...
	while ((c = getopt_long(argc, argv, "hDbsz:", options, NULL)) >= 0)
		...
		case ARG_LOG_LOCATION:
			if (optarg) {
				...
				if (r < 0) {
					log_error(
						"Failed to parse log location setting %s.",
						optarg);
					return r;
				}
			} else
		...
		case ARG_DEFAULT_STD_OUTPUT:
			...
			if (r < 0) {
				log_error(
					"Failed to parse default standard output setting %s.",
					optarg);
				return r;
			} else
				arg_default_std_output = r;
		...
		case ARG_DEFAULT_STD_ERROR:
			...
			if (r < 0) {
				log_error(
					"Failed to parse default standard error output setting %s.",
					optarg);
				return r;
			} else
				arg_default_std_error = r;
		...
		case ARG_DUMP_CORE:
			...
			if (r < 0) {
				log_error(
					"Failed to parse dump core boolean %s.",
					optarg);
				return r;
			}
			arg_dump_core = r;
		...
		case ARG_CRASH_SHELL:
			...
			if (r < 0) {
				log_error(
					"Failed to parse crash shell boolean %s.",
					optarg);
				return r;
			}
			arg_crash_shell = r;
		...
		case ARG_CONFIRM_SPAWN:
			...
			if (r < 0) {
				log_error(
					"Failed to parse confirm spawn boolean %s.",
					optarg);
				return r;
			}
			arg_confirm_spawn = r;
		...
		case ARG_SHOW_STATUS:
			if (optarg) {
				...
				if (r < 0) {
					log_error(
						"Failed to parse show status boolean %s.",
						optarg);
					return r;
				}
			} else
		...
		case ARG_DESERIALIZE: {
			...
			if (r < 0 || fd < 0) {
				log_error(
					"Failed to parse deserialize option %s.",
					optarg);
				return r < 0 ? r : -EINVAL;
			}
...
static int
prepare_reexecute(Manager *m, FILE **_f, FDSet **_fds, bool switching_root)
{
	FILE *f = NULL;
	FDSet *fds = NULL;
	int r;
...
static int
bump_rlimit_nofile(struct rlimit *saved_rlimit)
{
	struct rlimit nl;
	int r;
	int min_max;
...
test_mtab(void)
{
	static const char ok[] = "/proc/self/mounts\0"
				 "/proc/mounts\0"
				 "../proc/self/mounts\0"
				 "../proc/mounts\0";
...
static int
initialize_join_controllers(void)
{
...
static int
enforce_syscall_archs(Set *archs)
{
#ifdef HAVE_SECCOMP
...
status_welcome(void)
{
	...
	r = parse_env_file("/etc/os-release", NEWLINE, "PRETTY_NAME",
		&pretty_name, "ANSI_COLOR", &ansi_color, NULL);
	if (r == -ENOENT) {
		r = parse_env_file("/usr/lib/os-release", NEWLINE,
			"PRETTY_NAME", &pretty_name, "ANSI_COLOR", &ansi_color,
			NULL);
	}
...
main(int argc, char *argv[])
{
		...
		if (!skip_setup) {
			...
			if (clock_is_localtime() > 0) {
				...
				if (r < 0)
					...
					log_info(
						"RTC configured in localtime, applying delta of %i minutes to system time.",
						min);
			} else if (!in_initrd()) {
				/*
	...
	if (r < 0) {
		log_emergency_errno(r, "Failed to set default unit %s: %m",
	...
	if (arg_running_as == SYSTEMD_USER && !getenv("XDG_RUNTIME_DIR")) {
		log_error(
	...
	if (queue_default_job) {
		...
		r = manager_load_unit(m, arg_default_unit, NULL, &error,
			&target);
		if (r < 0)
			...
			log_error_errno(target->load_error,
		...
		if (!target || target->load_state != UNIT_LOADED) {
			...
			r = manager_load_unit(m, SPECIAL_RESCUE_TARGET, NULL,
				&error, &target);
			if (r < 0) {
				log_emergency(
					"Failed to load rescue target: %s",
					bus_error_message(&error, r));
				error_message = "Failed to load rescue target";
				goto finish;
			} else if (target->load_state == UNIT_ERROR ||
				target->load_state == UNIT_NOT_FOUND) {
				log_emergency_errno(target->load_error,
		...
		if (r == -EPERM) {
			...
			if (r < 0) {
				log_emergency(
					"Failed to start default target: %s",
					bus_error_message(&error, r));
				error_message =
			...
			log_emergency("Failed to isolate default target: %s",
				bus_error_message(&error, r));
	...
	for (;;) {
		...
		case MANAGER_REEXECUTE:
			...
			if (prepare_reexecute(m, &arg_serialization, &fds,
				    false) < 0) {
				error_message =
		...
		case MANAGER_SWITCH_ROOT:
			...
			if (!switch_root_init)
				if (prepare_reexecute(m, &arg_serialization,
					    &fds, true) < 0) {
					error_message =
						"Failed to prepare for reexection";
					goto finish;
...
finish:
	...
	if (shutdown_verb) {
		...
		if (arm_reboot_watchdog && arg_shutdown_watchdog > 0) {
			...
			if (asprintf(&e, "WATCHDOG_USEC=" USEC_FMT,
