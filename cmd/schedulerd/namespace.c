...
static int
append_mounts(BindMount **p, char **strv, MountMode mode)
{
	char **i;
	...
	STRV_FOREACH (i, strv) {
		...
		if ((mode == INACCESSIBLE || mode == READONLY ||
			    mode == READWRITE) &&
			(*i)[0] == '-') {
			(*p)->ignore = true;
			(*i)++;
...
static int
mount_path_compare(const void *a, const void *b)
{
...
static void
drop_duplicates(BindMount *m, unsigned *n)
{
...
static int
mount_move_root(const char *path)
{
	if (chdir(path) < 0)
		return -errno;
...
make_read_only(BindMount *m)
{
	...
	else if (IN_SET(m->mode, READWRITE, PRIVATE_TMP, PRIVATE_VAR_TMP,
			 PRIVATE_DEV))
...
int
setup_namespace(const char *root_directory, char **read_write_dirs,
	char **read_only_dirs, char **inaccessible_dirs, const char *tmp_dir,
	const char *var_tmp_dir, const char *bus_endpoint_path,
	bool private_dev, ProtectHome protect_home,
	ProtectSystem protect_system, unsigned long mount_flags)
{
	...
	n = !!tmp_dir + !!var_tmp_dir + !!bus_endpoint_path +
		strv_length(read_write_dirs) + strv_length(read_only_dirs) +
		strv_length(inaccessible_dirs) + private_dev +
	...
	if (n > 0) {
		...
		if (bus_endpoint_path) {
			m->path =
				prefix_roota(root_directory, bus_endpoint_path);
	...
	if (root_directory) {
		...
		if (mount(root_directory, root_directory, NULL,
...
static int
setup_one_tmp_dir(const char *id, const char *prefix, char **path)
{
...
int
setup_tmp_dirs(const char *id, char **tmp_dir, char **var_tmp_dir)
{
	char *a, *b;
	int r;
...
int
setup_netns(int netns_storage_socket[2])
{
	...
	if (recvmsg(netns_storage_socket[0], &mh,
	...
	if (sendmsg(netns_storage_socket[1], &mh, MSG_DONTWAIT | MSG_NOSIGNAL) <
		0) {
		r = -errno;
		goto fail;
	}
