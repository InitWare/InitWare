...
static void
snapshot_set_state(Snapshot *s, SnapshotState state)
{
...
static int
snapshot_coldplug(Unit *u, Hashmap *deferred_work)
{
	Snapshot *s = SNAPSHOT(u);
...
static void
snapshot_dump(Unit *u, FILE *f, const char *prefix)
{
	Snapshot *s = SNAPSHOT(u);
	...
	fprintf(f,
		"%sSnapshot State: %s\n"
		"%sClean Up: %s\n",
		prefix, snapshot_state_to_string(s->state), prefix,
		yes_no(s->cleanup));
}

static int
snapshot_start(Unit *u)
{
	Snapshot *s = SNAPSHOT(u);
...
static int
snapshot_serialize(Unit *u, FILE *f, FDSet *fds)
{
	Snapshot *s = SNAPSHOT(u);
	Unit *other;
	Iterator i;
...
static int
snapshot_deserialize_item(Unit *u, const char *key, const char *value,
	FDSet *fds)
{
	Snapshot *s = SNAPSHOT(u);
	int r;
	...
	if (streq(key, "state")) {
		...
		r = unit_add_two_dependencies_by_name(u, UNIT_AFTER, UNIT_WANTS,
			value, NULL, true);
		if (r < 0)
			return r;
	} else
...
snapshot_sub_state_to_string(Unit *u)
{
	assert(u);
...
int
snapshot_create(Manager *m, const char *name, bool cleanup, sd_bus_error *e,
	Snapshot **_s)
{
	...
	if (name) {
		...
		if (manager_get_unit(m, name))
			return sd_bus_error_setf(e, BUS_ERROR_UNIT_EXISTS,
				"Snapshot %s exists already.", name);

	} else {
		for (;;) {
			if (asprintf(&n, "snapshot-%u.snapshot",
				    ++m->n_snapshots) < 0)
				return -ENOMEM;
	...
	HASHMAP_FOREACH_KEY (other, k, m->units, i) {
		if (other->ignore_on_snapshot || other->transient)
			continue;
		...
		r = unit_add_two_dependencies(u, UNIT_AFTER, UNIT_WANTS, other,
			true);
		if (r < 0)
			goto fail;
	}
...
const UnitVTable snapshot_vtable = { .object_size = sizeof(Snapshot),
	...
	.bus_interface = SVC_DBUS_INTERFACE ".Snapshot",
