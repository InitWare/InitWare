...
static int
session_device_notify(SessionDevice *sd, enum SessionDeviceNotifications type)
{
...
static int
session_device_open(SessionDevice *sd, bool active)
{
	int fd, r;
...
static int
session_device_start(SessionDevice *sd)
{
	int r;
...
static void
session_device_stop(SessionDevice *sd)
{
	assert(sd);
...
static DeviceType
detect_device_type(struct udev_device *dev)
{
...
static int
session_device_verify(SessionDevice *sd)
{
	struct udev_device *dev, *p = NULL;
	const char *sp, *node;
	int r;

	dev = udev_device_new_from_devnum(sd->session->manager->udev, 'c',
		sd->dev);
	if (!dev)
		return -ENODEV;
	...
	if (sd->type == DEVICE_TYPE_UNKNOWN) {
		...
		p = dev;
		dev = udev_device_get_parent_with_subsystem_devtype(p, "input",
			NULL);
		if (!dev) {
			r = -ENODEV;
			goto err_dev;
...
int
session_device_new(Session *s, dev_t dev, SessionDevice **out)
{
	SessionDevice *sd;
	int r;
	...
	IWLIST_PREPEND(sd_by_device, sd->device->session_devices, sd);
...
void
session_device_free(SessionDevice *sd)
{
	assert(sd);
	...
	IWLIST_REMOVE(sd_by_device, sd->device->session_devices, sd);
...
void
session_device_complete_pause(SessionDevice *sd)
{
	SessionDevice *iter;
	Iterator i;
...
void
session_device_resume_all(Session *s)
{
	SessionDevice *sd;
	Iterator i;
	int r;
...
void
session_device_pause_all(Session *s)
{
	SessionDevice *sd;
	Iterator i;
...
unsigned int
session_device_try_pause_all(Session *s)
{
	SessionDevice *sd;
	Iterator i;
