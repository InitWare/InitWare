...
Button *
button_new(Manager *m, const char *name)
{
	Button *b;
...
int
button_set_seat(Button *b, const char *sn)
{
	char *s;
...
static void
button_lid_switch_handle_action(Manager *manager, bool is_edge)
{
	...
	manager_handle_action(manager, INHIBIT_HANDLE_LID_SWITCH, handle_action,
		manager->lid_switch_ignore_inhibited, is_edge);
}

static int
button_recheck(sd_event_source *e, void *userdata)
{
	Button *b = userdata;
...
static int
button_install_check_event_source(Button *b)
{
	int r;
	assert(b);
	...
	r = sd_event_add_post(b->manager->event, &b->check_event_source,
		button_recheck, b);
	if (r < 0)
		return r;

	return sd_event_source_set_priority(b->check_event_source,
		SD_EVENT_PRIORITY_IDLE + 1);
}

static int
button_dispatch(sd_event_source *s, int fd, uint32_t revents, void *userdata)
{
	Button *b = userdata;
	...
	if (ev.type == EV_KEY && ev.value > 0) {
		...
		case KEY_POWER:
		case KEY_POWER2:
			log_struct(LOG_INFO, LOG_MESSAGE("Power key pressed."),
				LOG_MESSAGE_ID(SD_MESSAGE_POWER_KEY), NULL);

			manager_handle_action(b->manager,
				INHIBIT_HANDLE_POWER_KEY,
				b->manager->handle_power_key,
				b->manager->power_key_ignore_inhibited, true);
			break;
		...
		case KEY_SLEEP:
			log_struct(LOG_INFO,
				...
				LOG_MESSAGE_ID(SD_MESSAGE_SUSPEND_KEY), NULL);

			manager_handle_action(b->manager,
				INHIBIT_HANDLE_SUSPEND_KEY,
				b->manager->handle_suspend_key,
				b->manager->suspend_key_ignore_inhibited, true);
			break;

		case KEY_SUSPEND:
			log_struct(LOG_INFO,
				LOG_MESSAGE("Hibernate key pressed."),
				LOG_MESSAGE_ID(SD_MESSAGE_HIBERNATE_KEY), NULL);
		...
		if (ev.code == SW_LID) {
			log_struct(LOG_INFO, LOG_MESSAGE("Lid closed."),
				LOG_MESSAGE_ID(SD_MESSAGE_LID_CLOSED), NULL);
			...
			log_struct(LOG_INFO, LOG_MESSAGE("System docked."),
				LOG_MESSAGE_ID(SD_MESSAGE_SYSTEM_DOCKED), NULL);
		...
		if (ev.code == SW_LID) {
			log_struct(LOG_INFO, LOG_MESSAGE("Lid opened."),
...
button_open(Button *b)
{
	...
	if (b->fd < 0)
		return log_warning_errno(errno, "Failed to open %s: %m",
			b->name);
	...
	log_info("Watching system buttons on /dev/input/%s (%s)", b->name,
		name);
