...
int
manager_get_session_from_creds(Manager *m, sd_bus_message *message,
	const char *name, sd_bus_error *error, Session **ret)
{
	...
	if (isempty(name)) {
		r = sd_bus_query_sender_creds(message,
			SD_BUS_CREDS_SESSION | SD_BUS_CREDS_AUGMENT, &creds);
		if (r < 0)
			return r;
	...
	if (!session)
		return sd_bus_error_setf(error, BUS_ERROR_NO_SUCH_SESSION,
...
int
manager_get_user_from_creds(Manager *m, sd_bus_message *message, uid_t uid,
	sd_bus_error *error, User **ret)
{
	...
	if (!user)
		return sd_bus_error_setf(error, BUS_ERROR_NO_SUCH_USER,
			"No user " UID_FMT " known or logged in", uid);
...
int
manager_get_seat_from_creds(Manager *m, sd_bus_message *message,
	const char *name, sd_bus_error *error, Seat **ret)
{
	...
	if (isempty(name)) {
		...
		r = manager_get_session_from_creds(m, message, NULL, error,
			&session);
		if (r < 0)
			return r;
...
static int
property_get_idle_hint(sd_bus *bus, const char *path, const char *interface,
	...
	sd_bus_error *error)
{
	...
	return sd_bus_message_append(reply, "b",
		manager_get_idle_hint(m, NULL) > 0);
}

static int
property_get_idle_since_hint(sd_bus *bus, const char *path,
	...
	void *userdata, sd_bus_error *error)
{
	...
	return sd_bus_message_append(reply, "t",
		streq(property, "IdleSinceHint") ? t.realtime : t.monotonic);
}

static int
property_get_inhibited(sd_bus *bus, const char *path, const char *interface,
	const char *property, sd_bus_message *reply, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	...
	w = manager_inhibit_what(m,
		streq(property, "BlockInhibited") ? INHIBIT_BLOCK :
							  INHIBIT_DELAY);
...
static int
property_get_preparing(sd_bus *bus, const char *path, const char *interface,
	const char *property, sd_bus_message *reply, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static BUS_DEFINE_PROPERTY_GET_ENUM(property_get_handle_action, handle_action,
	HandleAction);

static int
method_get_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
static int
method_get_session_by_pid(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_free_ char *p = NULL;
...
static int
method_get_user(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_free_ char *p = NULL;
	Manager *m = userdata;
...
static int
method_get_user_by_pid(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_free_ char *p = NULL;
	Manager *m = userdata;
	...
	if (pid <= 0) {
		r = manager_get_user_from_creds(m, message, UID_INVALID, error,
			&user);
		if (r < 0)
			return r;
	} else {
		...
		if (!user)
			return sd_bus_error_setf(error,
				...
				"PID " PID_FMT
				" does not belong to any known or logged in user",
				pid);
	}
...
static int
method_get_seat(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_free_ char *p = NULL;
	Manager *m = userdata;
...
static int
method_list_sessions(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	...
	HASHMAP_FOREACH (session, m->sessions, i) {
		...
		r = sd_bus_message_append(reply, "(susso)", session->id,
			(uint32_t)session->user->uid, session->user->name,
			session->seat ? session->seat->id : "", p);
		if (r < 0)
			return r;
	}
...
static int
method_list_users(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	Manager *m = userdata;
	...
	HASHMAP_FOREACH (user, m->users, i) {
		...
		r = sd_bus_message_append(reply, "(uso)", (uint32_t)user->uid,
			user->name, p);
		if (r < 0)
			return r;
	}
...
static int
method_list_seats(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	Manager *m = userdata;
...
static int
method_list_inhibitors(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	Manager *m = userdata;
	...
	HASHMAP_FOREACH (inhibitor, m->inhibitors, i) {
		r = sd_bus_message_append(reply, "(ssssuu)",
			...
			strempty(inhibitor->who), strempty(inhibitor->why),
			strempty(inhibit_mode_to_string(inhibitor->mode)),
			(uint32_t)inhibitor->uid, (uint32_t)inhibitor->pid);
		if (r < 0)
			return r;
	}
...
static int
method_create_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	const char *service, *type, *class, *cseat, *tty, *display,
		*remote_user, *remote_host, *desktop;
	...
	r = sd_bus_message_read(message, "uusssssussbss", &uid, &leader,
		&service, &type, &class, &desktop, &cseat, &vtnr, &tty,
		&display, &remote, &remote_user, &remote_host);
	if (r < 0)
		return r;
	...
	if (session) {
		...
		log_debug("Sending reply about an existing session: "
			...
			session->id, path, (uint32_t)session->user->uid,
			session->user->runtime_path, fifo_fd,
			session->seat ? session->seat->id : "",
			(uint32_t)session->vtnr);

		return sd_bus_reply_method_return(message, "soshusub",
			session->id, path, session->user->runtime_path, fifo_fd,
			(uint32_t)session->user->uid,
			session->seat ? session->seat->id : "",
			(uint32_t)session->vtnr, true);
	}
	...
	if (audit_id > 0) {
		...
		if (hashmap_get(m->sessions, id)) {
			log_warning(
				"Existing logind session ID %s used by new audit session, ignoring",
				id);
			audit_id = 0;
...
static int
method_release_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	Session *session;
...
static int
method_activate_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	Session *session;
...
static int
method_activate_session_on_seat(sd_bus *bus, sd_bus_message *message,
	void *userdata, sd_bus_error *error)
{
	...
	r = manager_get_session_from_creds(m, message, session_name, error,
		&session);
	if (r < 0)
		return r;
...
static int
method_lock_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	Session *session;
	...
	r = session_send_lock(session,
		streq(sd_bus_message_get_member(message), "LockSession"));
	if (r < 0)
		return r;
...
static int
method_lock_sessions(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	int r;
	...
	r = session_send_lock_all(m,
		streq(sd_bus_message_get_member(message), "LockSessions"));
	if (r < 0)
		return r;
...
static int
method_kill_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
static int
method_kill_user(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	uint32_t uid;
...
static int
method_terminate_session(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	const char *name;
...
static int
method_terminate_user(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	uint32_t uid;
...
static int
method_terminate_seat(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	const char *name;
...
static int
method_set_user_linger(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
attach_device(Manager *m, const char *seat, const char *sysfs)
{
	...
	if (asprintf(&file, "/etc/udev/rules.d/72-seat-%s.rules", id_for_seat) <
		0)
		return -ENOMEM;
	...
	if (!d) {
		...
		while ((de = readdir(d))) {
			...
			if (unlinkat(dirfd(d), de->d_name, 0) < 0)
				log_warning_errno(errno,
					"Failed to unlink %s: %m", de->d_name);
		}
	}
...
static int
method_attach_device(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
static int
method_flush_devices(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	int interactive, r;
...
static int
have_multiple_sessions(Manager *m, uid_t uid)
{
	Session *session;
	Iterator i;
...
static int
bus_manager_log_shutdown(Manager *m, InhibitWhat w, const char *unit_name)
{
	const char *p, *q;
	...
	return log_struct(LOG_NOTICE, LOG_MESSAGE_ID(SD_MESSAGE_SHUTDOWN), p, q,
		NULL);
}

static int
lid_switch_ignore_handler(sd_event_source *e, uint64_t usec, void *userdata)
{
	Manager *m = userdata;
...
int
manager_set_lid_switch_ignore(Manager *m, usec_t until)
{
	int r;
	...
	if (m->lid_switch_ignore_event_source) {
		...
		r = sd_event_source_get_time(m->lid_switch_ignore_event_source,
			&u);
		if (r < 0)
			return r;
		...
		r = sd_event_source_set_time(m->lid_switch_ignore_event_source,
			until);
	} else
		r = sd_event_add_time(m->event,
			&m->lid_switch_ignore_event_source, CLOCK_MONOTONIC,
			until, 0, lid_switch_ignore_handler, m);

	return r;
}

static int
execute_shutdown_or_sleep(Manager *m, InhibitWhat w, const char *unit_name,
	sd_bus_error *error)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	const char *p;
	...
	manager_set_lid_switch_ignore(m,
		now(CLOCK_MONOTONIC) + IGNORE_LID_SWITCH_SUSPEND_USEC);

	return 0;
}

static int
delay_shutdown_or_sleep(Manager *m, InhibitWhat w, const char *unit_name)
{
	assert(m);
	assert(w >= 0);
	assert(w < _INHIBIT_WHAT_MAX);
...
static int
send_prepare_for(Manager *m, InhibitWhat w, bool _active)
{
...
int
bus_manager_shutdown_or_sleep_now_or_later(Manager *m, const char *unit_name,
	InhibitWhat w, sd_bus_error *error)
{
	bool delayed;
	int r;
...
static int
method_do_shutdown_or_sleep(Manager *m, sd_bus_message *message,
	const char *unit_name, InhibitWhat w, const char *action,
	const char *action_multiple_sessions, const char *action_ignore_inhibit,
	const char *sleep_verb, sd_bus_message_handler_t method,
	sd_bus_error *error)
{
	_cleanup_bus_creds_unref_ sd_bus_creds *creds = NULL;
	...
	blocked = manager_is_inhibited(m, w, INHIBIT_BLOCK, NULL, false, true,
		uid, NULL);

	if (multiple_sessions) {
		r = bus_verify_polkit_async(message, CAP_SYS_BOOT,
			action_multiple_sessions, interactive,
			&m->polkit_registry, error);
		if (r < 0)
			return r;
		if (r == 0)
	...
	if (blocked) {
		r = bus_verify_polkit_async(message, CAP_SYS_BOOT,
			action_ignore_inhibit, interactive, &m->polkit_registry,
			error);
		if (r < 0)
			return r;
		if (r == 0)
	...
	if (!multiple_sessions && !blocked) {
		r = bus_verify_polkit_async(message, CAP_SYS_BOOT, action,
			interactive, &m->polkit_registry, error);
		if (r < 0)
			return r;
		if (r == 0)
			return 1; /* No authorization for now, but the async polkit stuff will call us again when it has it */
...
static int
method_poweroff(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_reboot(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_suspend(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_hibernate(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_hybrid_sleep(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_can_shutdown_or_sleep(Manager *m, sd_bus_message *message, InhibitWhat w,
	const char *action, const char *action_multiple_sessions,
	const char *action_ignore_inhibit, const char *sleep_verb,
	sd_bus_error *error)
{
	_cleanup_bus_creds_unref_ sd_bus_creds *creds = NULL;
	...
	if (multiple_sessions) {
		r = bus_verify_polkit(message, CAP_SYS_BOOT,
			action_multiple_sessions, false, &challenge, error);
		if (r < 0)
			return r;
	...
	if (blocked) {
		r = bus_verify_polkit(message, CAP_SYS_BOOT,
			action_ignore_inhibit, false, &challenge, error);
		if (r < 0)
			return r;
	...
	if (!multiple_sessions && !blocked) {
		...
		r = bus_verify_polkit(message, CAP_SYS_BOOT, action, false,
			&challenge, error);
		if (r < 0)
			return r;
...
static int
method_can_poweroff(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_can_reboot(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_can_suspend(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_can_hibernate(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_can_hybrid_sleep(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
...
static int
method_inhibit(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	_cleanup_bus_creds_unref_ sd_bus_creds *creds = NULL;
	...
	r = sd_bus_query_sender_creds(message,
		SD_BUS_CREDS_EUID | SD_BUS_CREDS_PID, &creds);
	if (r < 0)
		return r;
...
const sd_bus_vtable manager_vtable[] = { SD_BUS_VTABLE_START(0),
	...
	SD_BUS_PROPERTY("NAutoVTs", "u", NULL, offsetof(Manager, n_autovts),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("KillOnlyUsers", "as", NULL,
		offsetof(Manager, kill_only_users),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("KillExcludeUsers", "as", NULL,
		offsetof(Manager, kill_exclude_users),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("KillUserProcesses", "b", NULL,
		offsetof(Manager, kill_user_processes),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("IdleHint", "b", property_get_idle_hint, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("IdleSinceHint", "t", property_get_idle_since_hint, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("IdleSinceHintMonotonic", "t",
		property_get_idle_since_hint, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("BlockInhibited", "s", property_get_inhibited, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("DelayInhibited", "s", property_get_inhibited, 0,
		SD_BUS_VTABLE_PROPERTY_EMITS_CHANGE),
	SD_BUS_PROPERTY("InhibitDelayMaxUSec", "t", NULL,
		offsetof(Manager, inhibit_delay_max),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("HandlePowerKey", "s", property_get_handle_action,
	...
	SD_BUS_PROPERTY("HandleLidSwitch", "s", property_get_handle_action,
		...
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("HandleLidSwitchDocked", "s",
	...
	SD_BUS_PROPERTY("IdleActionUSec", "t", NULL,
		offsetof(Manager, idle_action_usec),
		SD_BUS_VTABLE_PROPERTY_CONST),
	SD_BUS_PROPERTY("PreparingForShutdown", "b", property_get_preparing, 0,
		0),
	SD_BUS_PROPERTY("PreparingForSleep", "b", property_get_preparing, 0, 0),

	SD_BUS_METHOD("GetSession", "s", "o", method_get_session,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("GetSessionByPID", "u", "o", method_get_session_by_pid,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("GetUser", "u", "o", method_get_user,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("GetUserByPID", "u", "o", method_get_user_by_pid,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("GetSeat", "s", "o", method_get_seat,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("ListSessions", NULL, "a(susso)", method_list_sessions,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("ListUsers", NULL, "a(uso)", method_list_users,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("ListSeats", NULL, "a(so)", method_list_seats,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("ListInhibitors", NULL, "a(ssssuu)",
		method_list_inhibitors, SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("CreateSession", "uusssssussbssa(sv)", "soshusub",
		method_create_session, 0),
	SD_BUS_METHOD("ReleaseSession", "s", NULL, method_release_session, 0),
	SD_BUS_METHOD("ActivateSession", "s", NULL, method_activate_session,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("ActivateSessionOnSeat", "ss", NULL,
		method_activate_session_on_seat, SD_BUS_VTABLE_UNPRIVILEGED),
	...
	SD_BUS_METHOD("KillSession", "ssi", NULL, method_kill_session,
		SD_BUS_VTABLE_CAPABILITY(CAP_KILL)),
	SD_BUS_METHOD("KillUser", "ui", NULL, method_kill_user,
		SD_BUS_VTABLE_CAPABILITY(CAP_KILL)),
	...
	SD_BUS_METHOD("CanSuspend", NULL, "s", method_can_suspend,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("CanHibernate", NULL, "s", method_can_hibernate,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("CanHybridSleep", NULL, "s", method_can_hybrid_sleep,
		SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("Inhibit", "ssss", "h", method_inhibit,
		SD_BUS_VTABLE_UNPRIVILEGED),
...
static int
session_jobs_reply(Session *s, const char *unit, const char *result)
{
	int r = 0;
...
int
match_job_removed(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
int
match_unit_removed(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
int
match_properties_changed(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
int
match_reloading(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
	Manager *m = userdata;
	Session *session;
...
int
match_name_owner_changed(sd_bus *bus, sd_bus_message *message, void *userdata,
	sd_bus_error *error)
{
...
int
manager_send_changed(Manager *manager, const char *property, ...)
{
	char **l;
...
int
manager_dispatch_delayed(Manager *manager)
{
	...
	/* Continue delay? */
	if (manager_is_inhibited(manager, manager->action_what, INHIBIT_DELAY,
		    NULL, false, false, 0, &offending)) {
		_cleanup_free_ char *comm = NULL, *u = NULL;

		get_process_comm(offending->pid, &comm);
		u = uid_to_name(offending->uid);

		if (manager->action_timestamp + manager->inhibit_delay_max >
			now(CLOCK_MONOTONIC))
			return 0;

		log_info("Delay lock is active (UID " UID_FMT
			 "/%s, PID " PID_FMT
			 "/%s) but inhibitor timeout is reached.",
			offending->uid, strna(u), offending->pid, strna(comm));
	}

	/* Actually do the operation */
	r = execute_shutdown_or_sleep(manager, manager->action_what,
		manager->action_unit, &error);
	if (r < 0) {
		log_warning("Failed to send delayed message: %s",
...
int
manager_start_slice(Manager *manager, const char *slice,
...
int
manager_start_scope(Manager *manager, const char *scope, pid_t pid,
	const char *slice, const char *description, const char *after,
	const char *after2, uint64_t tasks_max, sd_bus_error *error, char **job)
{
	_cleanup_bus_message_unref_ sd_bus_message *m = NULL, *reply = NULL;
	int r;
...
int
manager_start_unit(Manager *manager, const char *unit, sd_bus_error *error,
	char **job)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	int r;
...
int
manager_stop_unit(Manager *manager, const char *unit, sd_bus_error *error,
	char **job)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	int r;
...
int
manager_abandon_scope(Manager *manager, const char *scope, sd_bus_error *error)
{
	_cleanup_free_ char *path = NULL;
	int r;
...
int
manager_kill_unit(Manager *manager, const char *unit, KillWho who, int signo,
	sd_bus_error *error)
{
	assert(manager);
	assert(unit);
...
int
manager_unit_is_active(Manager *manager, const char *unit)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	_cleanup_free_ char *path = NULL;
...
int
manager_job_is_active(Manager *manager, const char *path)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	int r;
