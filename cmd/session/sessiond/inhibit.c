...
Inhibitor *
inhibitor_new(Manager *m, const char *id)
{
	Inhibitor *i;
...
inhibitor_save(Inhibitor *i)
{
	...
	fprintf(f,
		...
		inhibit_mode_to_string(i->mode), i->uid, i->pid);
...
finish:
	if (r < 0)
		log_error_errno(r, "Failed to save inhibit data %s: %m",
			i->state_file);
...
inhibitor_start(Inhibitor *i)
{
	...
	log_debug("Inhibitor %s (%s) pid=" PID_FMT " uid=" UID_FMT
		  " mode=%s started.",
		strna(i->who), strna(i->why), i->pid, i->uid,
	...
	manager_send_changed(i->manager,
		i->mode == INHIBIT_BLOCK ? "BlockInhibited" : "DelayInhibited",
		NULL);
...
int
inhibitor_load(Inhibitor *i)
{
	_cleanup_free_ char *what = NULL, *uid = NULL, *pid = NULL, *who = NULL,
			    *why = NULL, *mode = NULL;
	...
	r = parse_env_file(i->state_file, NEWLINE, "WHAT", &what, "UID", &uid,
		"PID", &pid, "WHO", &who, "WHY", &why, "MODE", &mode, "FIFO",
		&i->fifo_path, NULL);
	if (r < 0)
		return r;
...
static int
inhibitor_dispatch_fifo(sd_event_source *s, int fd, uint32_t revents,
	void *userdata)
{
...
int
inhibitor_create_fifo(Inhibitor *i)
{
	int r;
	...
	if (!i->event_source) {
		r = sd_event_add_io(i->manager->event, &i->event_source,
			i->fifo_fd, 0, inhibitor_dispatch_fifo, i);
		if (r < 0)
			return r;

		r = sd_event_source_set_priority(i->event_source,
...
void
inhibitor_remove_fifo(Inhibitor *i)
{
	assert(i);
...
InhibitWhat
manager_inhibit_what(Manager *m, InhibitMode mm)
{
	Inhibitor *i;
	Iterator j;
	InhibitWhat what = 0;
...
static int
pid_is_active(Manager *m, pid_t pid)
{
	Session *s;
	int r;
...
bool
manager_is_inhibited(Manager *m, InhibitWhat w, InhibitMode mm,
	dual_timestamp *since, bool ignore_inactive, bool ignore_uid, uid_t uid,
	Inhibitor **offending)
{
	Inhibitor *i;
	Iterator j;
...
const char *
inhibit_what_to_string(InhibitWhat w)
{
