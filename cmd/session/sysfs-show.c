	...
	while (*item) {
		...
		while (lookahead) {
			...
			if (path_startswith(lookahead_sysfs, sub) &&
				...
				lookahead_d = udev_device_new_from_syspath(udev,
					lookahead_sysfs);
				if (lookahead_d) {
		...
		printf("%s%s%s\n", prefix,
			draw_special_char(
				lookahead ? DRAW_TREE_BRANCH : DRAW_TREE_RIGHT),
			k);

		if (asprintf(&l, "%s%s:%s%s%s%s", is_master ? "[MASTER] " : "",
			    subsystem, sysname, name ? " \"" : "",
			    name ? name : "", name ? "\"" : "") < 0)
			return -ENOMEM;
		...
		printf("%s%s%s\n", prefix,
			lookahead ? draw_special_char(DRAW_TREE_VERTICAL) :
					  "  ",
			k);
