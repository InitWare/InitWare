	...
	const char **class, const char **type, bool *debug)
{
	...
	for (i = 0; i < (unsigned)argc; i++) {
		if (startswith(argv[i], "class=")) {
			...
			pam_syslog(handle, LOG_WARNING,
				"Unknown parameter '%s', ignoring", argv[i]);
	}
...
static int
get_user_data(pam_handle_t *handle, const char **ret_username,
...
static int
get_seat_from_display(const char *display, const char **seat, uint32_t *vtnr)
{
	...
	const char **argv)
{
	...
	const char *username, *id, *object_path, *runtime_path,
		*service = NULL, *tty = NULL, *display = NULL,
		*remote_user = NULL, *remote_host = NULL, *seat = NULL,
		*type = NULL, *class = NULL, *class_pam = NULL,
		*type_pam = NULL, *cvtnr = NULL, *desktop = NULL;
	...
	if (seat && !streq(seat, "seat0") && vtnr != 0) {
		pam_syslog(handle, LOG_DEBUG,
			"Ignoring vtnr %" PRIu32 " for %s which is not seat0",
			vtnr, seat);
		vtnr = 0;
	}
	...
	if (debug)
		pam_syslog(handle, LOG_DEBUG,
			...
			"uid=" UID_FMT " pid=" PID_FMT
			" service=%s type=%s class=%s desktop=%s seat=%s vtnr=%" PRIu32
			" tty=%s display=%s remote=%s remote_user=%s remote_host=%s",
			pw->pw_uid, getpid(), strempty(service), type, class,
			strempty(desktop), strempty(seat), vtnr, strempty(tty),
	...
	r = sd_bus_message_read(reply, "soshusub", &id, &object_path,
		&runtime_path, &session_fd, &original_uid, &seat, &vtnr,
		&existing);
	if (r < 0) {
		pam_syslog(handle, LOG_ERR, "Failed to parse message: %s",
			strerror(-r));
		return PAM_SESSION_ERR;
	}
	...
	r = pam_set_data(handle, "systemd.existing", INT_TO_PTR(!!existing),
		NULL);
	if (r != PAM_SUCCESS) {
...
_public_ PAM_EXTERN int
pam_sm_close_session(pam_handle_t *handle, int flags, int argc,
	const char **argv)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_close_unref_ sd_bus *bus = NULL;
