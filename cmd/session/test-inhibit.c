...
static int
inhibit(sd_bus *bus, const char *what)
{
	...
	r = sd_bus_call_method(bus, SVC_SESSIOND_DBUS_BUSNAME,
		...
		&reply, "ssss", what, who, reason, mode);
	assert_se(r >= 0);
...
print_inhibitors(sd_bus *bus)
{
	...
	r = sd_bus_call_method(bus, SVC_SESSIOND_DBUS_BUSNAME,
		"/org/freedesktop/login1",
		SVC_SESSIOND_DBUS_INTERFACE ".Manager", "ListInhibitors",
		&error, &reply, "");
	assert_se(r >= 0);
	...
	while ((r = sd_bus_message_read(reply, "(ssssuu)", &what, &who, &why,
			&mode, &uid, &pid)) > 0) {
		printf("what=<%s> who=<%s> why=<%s> mode=<%s> uid=<%" PRIu32
		       "> pid=<%" PRIu32 ">\n",
