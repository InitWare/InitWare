...
static int
list_sessions(int argc, char *argv[], void *userdata)
{
	...
	while ((r = sd_bus_message_read(reply, "(susso)", &id, &uid, &user,
			&seat, &object)) > 0) {
		printf("%10s %10u %-16s %-16s\n", id, (unsigned)uid, user,
			seat);
		k++;
	}
	if (r < 0)
...
static int
list_users(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	...
	while ((r = sd_bus_message_read(reply, "(uso)", &uid, &user, &object)) >
		0) {
		printf("%10u %-16s\n", (unsigned)uid, user);
		k++;
	}
...
static int
list_seats(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
...
static int
show_unit_cgroup(sd_bus *bus, const char *interface, const char *unit,
	pid_t leader)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
...
static int
prop_map_first_of_struct(sd_bus *bus, const char *member, sd_bus_message *m,
...
static int
prop_map_sessions_strv(sd_bus *bus, const char *member, sd_bus_message *m,
	sd_bus_error *error, void *userdata)
{
	const char *name;
	int r;
...
static int
print_session_status_info(sd_bus *bus, const char *path, bool *new_line)
{
	static const struct bus_properties_map map[] = {
		...
		{ "RemoteHost", "s", NULL,
			offsetof(SessionStatusInfo, remote_host) },
		{ "RemoteUser", "s", NULL,
			offsetof(SessionStatusInfo, remote_user) },
		...
		{ "Timestamp", "t", NULL,
			offsetof(SessionStatusInfo, timestamp.realtime) },
		{ "TimestampMonotonic", "t", NULL,
			offsetof(SessionStatusInfo, timestamp.monotonic) },
	...
	*new_line = true;
	...
	if (i.scope) {
		...
		if (arg_transport == BUS_TRANSPORT_LOCAL) {
			show_journal_by_unit(stdout, i.scope, arg_output, 0,
...
static int
print_user_status_info(sd_bus *bus, const char *path, bool *new_line)
{
	static const struct bus_properties_map map[] = {
		...
		{ "Timestamp", "t", NULL,
			offsetof(UserStatusInfo, timestamp.realtime) },
		{ "TimestampMonotonic", "t", NULL,
			offsetof(UserStatusInfo, timestamp.monotonic) },
		{ "Display", "(so)", prop_map_first_of_struct,
			offsetof(UserStatusInfo, display) },
		{ "Sessions", "a(so)", prop_map_sessions_strv,
			offsetof(UserStatusInfo, sessions) },
		{}
	};
...
static int
print_seat_status_info(sd_bus *bus, const char *path, bool *new_line)
{
	static const struct bus_properties_map map[] = {
		{ "Id", "s", NULL, offsetof(SeatStatusInfo, id) },
		{ "ActiveSession", "(so)", prop_map_first_of_struct,
			offsetof(SeatStatusInfo, active_session) },
		{ "Sessions", "a(so)", prop_map_sessions_strv,
			offsetof(SeatStatusInfo, sessions) },
		{}
	};
...
print_property(const char *name, sd_bus_message *m, const char *contents)
{
	...
	case SD_BUS_TYPE_STRUCT_BEGIN:
		...
		if (contents[1] == SD_BUS_TYPE_STRING &&
		...
		} else if (contents[1] == SD_BUS_TYPE_UINT32 &&
			streq(name, "User")) {
			uint32_t uid;
	...
	case SD_BUS_TYPE_ARRAY:
		...
		if (contents[1] == SD_BUS_TYPE_STRUCT_BEGIN &&
			...
			while ((r = sd_bus_message_read(m, "(so)", &s, NULL)) >
				0) {
				printf("%s%s", space ? " " : "", s);
				space = true;
			}
...
static int
show_properties(sd_bus *bus, const char *path, bool *new_line)
{
	_cleanup_bus_message_unref_ sd_bus_message *reply = NULL;
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	int r;
...
static int
show_session(int argc, char *argv[], void *userdata)
{
	...
	if (argc <= 1) {
		...
		return print_session_status_info(bus,
...
static int
show_user(int argc, char *argv[], void *userdata)
{
	bool properties, new_line = false;
	sd_bus *bus = userdata;
	int r, i;
	...
	if (argc <= 1) {
		...
		return print_user_status_info(bus,
	...
	for (i = 1; i < argc; i++) {
		...
		r = get_user_creds((const char **)(argv + i), &uid, NULL, NULL,
			NULL);
		if (r < 0)
...
static int
show_seat(int argc, char *argv[], void *userdata)
{
	bool properties, new_line = false;
	sd_bus *bus = userdata;
	int r, i;
	...
	if (argc <= 1) {
		...
		return print_seat_status_info(bus,
...
static int
activate(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	char *short_argv[3];
		...
		r = sd_bus_call_method(bus, SVC_SESSIOND_DBUS_BUSNAME,
			...
			streq(argv[0], "lock-session") ? "LockSession" :
				streq(argv[0], "unlock-session") ?
							       "UnlockSession" :
				streq(argv[0], "terminate-session") ?
							       "TerminateSession" :
							       "ActivateSession",
			&error, NULL, "s", argv[i]);
		if (r < 0) {
...
static int
kill_session(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r, i;
...
static int
enable_linger(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	char *short_argv[3];
	...
	for (i = 1; i < argc; i++) {
		...
		if (isempty(argv[i]))
			...
			r = get_user_creds((const char **)(argv + i), &uid,
				NULL, NULL, NULL);
			if (r < 0)
				return log_error_errno(r,
...
static int
terminate_user(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r, i;
...
static int
kill_user(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r, i;
...
static int
attach(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r, i;
	...
	for (i = 2; i < argc; i++) {
		...
		if (r < 0) {
			log_error("Could not attach device: %s",
				bus_error_message(&error, -r));
			return r;
		}
	}
...
static int
flush_devices(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r;
...
static int
lock_sessions(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r;
	...
	r = sd_bus_call_method(bus, SVC_SESSIOND_DBUS_BUSNAME,
		...
		&error, NULL, NULL);
	if (r < 0)
		log_error("Could not lock sessions: %s",
			bus_error_message(&error, -r));
...
static int
terminate_seat(int argc, char *argv[], void *userdata)
{
	_cleanup_bus_error_free_ sd_bus_error error = SD_BUS_ERROR_NULL;
	sd_bus *bus = userdata;
	int r, i;
...
parse_argv(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "hp:als:H:M:n:o:", options,
			NULL)) >= 0)
...
static int
loginctl_main(int argc, char *argv[], sd_bus *bus)
{
