...
static void
print_device(struct udev_device *device)
{
	const char *str;
	dev_t devnum;
	int count;
...
static int
test_device(struct udev *udev, const char *syspath)
{
...
static int
test_device_parents(struct udev *udev, const char *syspath)
{
	_cleanup_udev_device_unref_ struct udev_device *device;
...
static int
test_device_devnum(struct udev *udev)
{
...
static int
test_device_subsys_name(struct udev *udev)
{
	struct udev_device *device;
	...
	device = udev_device_new_from_subsystem_sysname(udev, "subsystem",
		"pci");
	if (device == NULL)
		return -1;
	print_device(device);
	...
	device = udev_device_new_from_subsystem_sysname(udev, "drivers",
		"scsi:sd");
	if (device == NULL)
		return -1;
	print_device(device);
	...
	device = udev_device_new_from_subsystem_sysname(udev, "module",
		"printk");
	if (device == NULL)
		return -1;
	print_device(device);
...
static int
test_enumerate_print_list(struct udev_enumerate *enumerate)
{
	struct udev_list_entry *list_entry;
	int count = 0;
		...
		udev_enumerate_get_list_entry(enumerate))
	{
		...
		device = udev_device_new_from_syspath(udev_enumerate_get_udev(
							      enumerate),
			udev_list_entry_get_name(list_entry));
		if (device != NULL) {
			printf("device: '%s' (%s)\n",
	...
	for (;;) {
		...
		for (i = 0; i < fdcount; i++) {
			if (ev[i].data.fd == fd_udev &&
				ev[i].events & EPOLLIN) {
				device = udev_monitor_receive_device(
					udev_monitor);
				if (device == NULL) {
				...
				print_device(device);
				udev_device_unref(device);
			} else if (ev[i].data.fd == STDIN_FILENO &&
				ev[i].events & EPOLLIN) {
				printf("exiting loop\n");
				goto out;
			}
...
static int
test_enumerate(struct udev *udev, const char *subsystem)
{
	...
	if (udev_enumerate == NULL)
		return -1;
	udev_enumerate_add_match_property(udev_enumerate, "ID_FS*",
		"filesystem");
	udev_enumerate_scan_devices(udev_enumerate);
	test_enumerate_print_list(udev_enumerate);
	udev_enumerate_unref(udev_enumerate);
...
main(int argc, char *argv[])
{
	...
	static const struct option options[] = { { "syspath", required_argument,
							 NULL, 'p' },
