...
fake_filesystems(void)
{
	static const struct fakefs {
		...
		{ "test/run", "/etc/udev/rules.d",
			"failed to mount empty /etc/udev/rules.d" },
		{ "test/run", "/usr/lib/udev/rules.d",
			"failed to mount empty /usr/lib/udev/rules.d" },
	};
	unsigned int i;
	int err;
	...
	for (i = 0; i < ELEMENTSOF(fakefss); i++) {
		err = mount(fakefss[i].src, fakefss[i].target, NULL, MS_BIND,
			NULL);
		if (err < 0) {
			err = -errno;
...
main(int argc, char *argv[])
{
	...
	if (udev_device_get_devnode(dev) != NULL) {
		...
		if (!streq(action, "remove")) {
			...
			mknod(udev_device_get_devnode(dev), mode,
	...
	udev_event_execute_rules(event, 3 * USEC_PER_SEC, USEC_PER_SEC, NULL,
		rules, &sigmask_orig);
