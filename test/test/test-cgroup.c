...
main(int argc, char *argv[])
{
	...
	assert_se(cg_create_and_attach(SYSTEMD_CGROUP_CONTROLLER, "/test-b",
			  0) == 0);

	assert_se(cg_pid_get_path(SYSTEMD_CGROUP_CONTROLLER, getpid(), &path) ==
		0);
	...
	assert_se(cg_create_and_attach(SYSTEMD_CGROUP_CONTROLLER,
			  "/test-b/test-d", 0) == 0);
	...
	assert_se(cg_get_path(SYSTEMD_CGROUP_CONTROLLER, "/test-b/test-d", NULL,
			  &path) == 0);
	...
	assert_se(cg_is_empty_recursive(SYSTEMD_CGROUP_CONTROLLER, "/test-a",
			  false) > 0);
	assert_se(cg_is_empty_recursive(SYSTEMD_CGROUP_CONTROLLER, "/test-b",
			  false) == 0);

	assert_se(cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, "/test-a", 0,
			  false, false, false, NULL) == 0);
	assert_se(cg_kill_recursive(SYSTEMD_CGROUP_CONTROLLER, "/test-b", 0,
			  false, false, false, NULL) > 0);

	assert_se(cg_migrate_recursive(SYSTEMD_CGROUP_CONTROLLER, "/test-b",
			  SYSTEMD_CGROUP_CONTROLLER, "/test-a", false,
			  false) > 0);
