...
static void
test_strv_env_delete(void)
{
	_cleanup_strv_free_ char **a = NULL, **b = NULL, **c = NULL, **d = NULL;

	a = strv_new("FOO=BAR", "WALDO=WALDO", "WALDO=", "PIEP",
		"SCHLUMPF=SMURF", NULL);
	assert_se(a);
...
test_strv_env_merge(void)
{
	...
	b = strv_new("FOO=KKK", "FOO=", "PIEP=", "SCHLUMPF=SMURFF",
		"NANANANA=YES", NULL);
	assert_se(b);
...
static void
test_replace_env_arg(void)
{
	const char *env[] = { "FOO=BAR BAR", "BAR=waldo", NULL };
	const char *line[] = { "FOO$FOO", "FOO$FOOFOO", "FOO${FOO}$FOO",
...
static void
test_one_normalize(const char *input, const char *output)
{
...
static void
test_normalize_env_assignment(void)
{
...
test_env_clean(void)
{
	...
	e = strv_new("FOOBAR=WALDO", "FOOBAR=WALDO", "FOOBAR", "F", "X=", "F=F",
		"=", "=F", "", "0000=000", "äöüß=abcd", "abcd=äöüß",
		"xyz\n=xyz", "xyz=xyz\n", "another=one", "another=final one",
		NULL);
	assert_se(e);
...
test_env_name_is_valid(void)
{
	...
	assert_se(!env_name_is_valid(
		"#¤%&?_only_numbers_letters_and_underscore_allowed"));
}
