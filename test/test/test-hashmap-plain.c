...
static void
test_hashmap_remove_value(void)
{
	_cleanup_hashmap_free_ Hashmap *m = NULL;
	char *r;
...
static void
test_hashmap_remove_and_put(void)
{
	_cleanup_hashmap_free_ Hashmap *m = NULL;
	int valid;
	char *r;
	...
	valid = hashmap_remove_and_put(NULL, "key 1", "key 2",
		(void *)(const char *)"val 2");
	assert_se(valid == -ENOENT);

	valid = hashmap_remove_and_put(m, "key 1", "key 2",
		(void *)(const char *)"val 2");
	assert_se(valid == 0);
	...
	assert_se(valid == 1);
	valid = hashmap_remove_and_put(m, "key 3", "key 2",
		(void *)(const char *)"val 2");
	assert_se(valid == -EEXIST);
}

static void
test_hashmap_remove_and_replace(void)
{
	_cleanup_hashmap_free_ Hashmap *m = NULL;
	int valid;
	...
	for (i = 0; i < 20; i++) {
		...
		for (j = 1; j < 7; j++)
			hashmap_put(m, UINT_TO_PTR(10 * i + j),
				UINT_TO_PTR(10 * i + j));
		valid = hashmap_remove_and_replace(m, UINT_TO_PTR(10 * i + 1),
			UINT_TO_PTR(10 * i + 2), UINT_TO_PTR(10 * i + 2));
		assert_se(valid == 0);
...
static void
test_hashmap_ensure_allocated(void)
{
	Hashmap *m;
	int valid_hashmap;
...
test_hashmap_foreach_key(void)
{
	...
	const char *s;
	const char *key;
	static const char key_table[] = "key 1\0"
					"key 2\0"
					"key 3\0"
					"key 4\0";
...
crippled_hashmap_func(const void *p, const uint8_t hash_key[HASH_KEY_SIZE])
{
...
test_hashmap_many(void)
{
	...
	for (j = 0; j < ELEMENTSOF(tests); j++) {
		...
		for (i = 1; i < tests[j].n_entries * 3; i += 3) {
			assert_se(hashmap_put(h, UINT_TO_PTR(i),
					  UINT_TO_PTR(i)) >= 0);
			assert_se(PTR_TO_UINT(hashmap_get(h, UINT_TO_PTR(i))) ==
				i);
		}

		for (i = 1; i < tests[j].n_entries * 3; i++)
			assert_se(hashmap_contains(h, UINT_TO_PTR(i)) ==
				(i % 3 == 1));
...
static void
test_hashmap_steal_first_key(void)
{
	_cleanup_hashmap_free_ Hashmap *m = NULL;
...
static void
test_hashmap_clear_free_free(void)
{
	_cleanup_hashmap_free_ Hashmap *m = NULL;
