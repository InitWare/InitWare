...
static void
test_specifier_printf(void)
{
	static const Specifier table[] = { { 'a', specifier_string,
						   (char *)"AAAA" },
	...
	free(w);
	r = specifier_printf("machine=%m, boot=%B, host=%H, version=%v", table,
		NULL, &w);
	assert_se(r >= 0);
	assert_se(w);
	puts(w);
...
static void
test_strv_find_startswith(void)
{
	char *r;
...
test_strv_join(void)
{
	_cleanup_free_ char *p = NULL, *q = NULL, *r = NULL, *s = NULL,
			    *t = NULL;
...
static void
test_strv_quote_unquote(const char *const *split, const char *quoted)
{
	...
	assert_se(p);
	printf("-%s- --- -%s-\n", p,
		quoted); /* fprintf deals with NULL, puts does not */
	assert_se(p);
...
static void
test_strv_unquote(const char *quoted, char **list)
{
	_cleanup_strv_free_ char **s;
...
static void
test_invalid_unquote(const char *quoted)
{
	char **s = NULL;
	int r;
...
test_strv_sort(void)
{
	const char *input_table[] = { "durian", "apple", "citrus",
		"CAPITAL LETTERS FIRST", "banana", NULL };
...
static void
test_strv_extend_strv_concat(void)
{
...
static void
test_strv_foreach_backwards(void)
{
	_cleanup_strv_free_ char **a;
	unsigned i = 2;
	char **check;
...
static void
test_strv_foreach_pair(void)
{
	_cleanup_strv_free_ char **a = NULL;
	char **x, **y;

	a = strv_new("pair_one", "pair_one", "pair_two", "pair_two",
		"pair_three", "pair_three", NULL);
...
static void
test_strv_from_stdarg_alloca_one(char **l, const char *first, ...)
{
	char **j;
	unsigned i;
...
static void
test_strv_from_stdarg_alloca(void)
{
	test_strv_from_stdarg_alloca_one(STRV_MAKE("foo", "bar"), "foo", "bar",
		NULL);
...
main(int argc, char *argv[])
{
	...
	test_strv_quote_unquote(input_table_multiple,
		"\"one\" \"two\" \"three\"");
	...
	test_strv_unquote("    foo=bar     \"waldo\"    zzz    ",
		STRV_MAKE("foo=bar", "waldo", "zzz"));
