...
static void
setup_test_dir(char *tmp_dir, const char *files, ...)
{
	va_list ap;
	...
	while (files != NULL) {
		...
		assert_se(touch_file(path, true, USEC_INFINITY, UID_INVALID,
				  GID_INVALID, 0) == 0);
...
static void
test_conf_files_list(bool use_root)
{
	...
	setup_test_dir(tmp_dir, "/dir1/a.conf", "/dir2/a.conf", "/dir2/b.conf",
		NULL);
