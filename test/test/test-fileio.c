...
test_parse_env_file(void)
{
	...
	int fd, r;
	FILE *f;
	_cleanup_free_ char *one = NULL, *two = NULL, *three = NULL,
	...
	r = parse_env_file(t, NULL, "one", &one, "two", &two, "three", &three,
		"four", &four, "five", &five, "six", &six, "seven", &seven,
		"eight", &eight, "export nine", &nine, "ten", &ten, NULL);
...
static void
test_parse_multiline_env_file(void)
{
	char t[] = "/tmp/test-fileio-in-XXXXXX",
	     p[] = "/tmp/test-fileio-out-XXXXXX";
	int fd, r;
...
static void
test_executable_is_script(void)
{
...
static void
test_write_string_file_no_create(void)
{
	...
	assert_se(
		write_string_file_no_create(
			"/a/file/which/does/not/exists/i/guess", "boohoo") < 0);
...
test_load_env_file_pairs(void)
{
	...
	STRV_FOREACH_PAIR (k, v, l) {
		assert_se(STR_IN_SET(*k, "NAME", "ID", "PRETTY_NAME",
			"ANSI_COLOR", "HOME_URL", "SUPPORT_URL",
			"BUG_REPORT_URL"));
		printf("%s=%s\n", *k, *v);
		if (streq(*k, "NAME"))
...
static void
test_read_line_one_file(FILE *f)
{
	_cleanup_free_ char *line = NULL;

	assert_se(read_line(f, (size_t)-1, &line) == 15 &&
		streq(line, "Some test data"));
	line = mfree(line);

	assert_se(read_line(f, 1024, &line) == 30 &&
		streq(line, "With newlines, and a NUL byte"));
	line = mfree(line);

	assert_se(read_line(f, 1024, &line) == 1 && streq(line, ""));
	line = mfree(line);

	assert_se(read_line(f, 1024, &line) == 14 &&
		streq(line, "an empty line"));
	line = mfree(line);
	...
	assert_se(read_line(f, 1024, &line) == 61 &&
		streq(line,
