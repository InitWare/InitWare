...
static int
setup_test(Manager **m)
{
	char **tests_path = STRV_MAKE("exists", "existsglobFOOBAR", "changed",
		"modified", "unit", "directorynotempty", "makedirectory");
	char **test_path;
	Manager *tmp = NULL;
	int r;
	...
	STRV_FOREACH (test_path, tests_path) {
		rm_rf_dangerous(strjoina("/tmp/test-path_", *test_path), false,
			true, false);
	}
...
static void
check_stop_unlink(Manager *m, Unit *unit, const char *test_path,
	const char *service_name)
{
	...
	while (service->result != SERVICE_SUCCESS ||
...
test_path_exists(Manager *m)
{
	...
	assert_se(manager_load_unit(m, "path-exists.path", NULL, NULL, &unit) >=
		0);
...
static void
test_path_existsglob(Manager *m)
{
	const char *test_path = "/tmp/test-path_existsglobFOOBAR";
	Unit *unit = NULL;

	assert_se(m);
	assert_se(manager_load_unit(m, "path-existsglob.path", NULL, NULL,
			  &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
...
test_path_changed(Manager *m)
{
	...
	assert_se(manager_load_unit(m, "path-changed.path", NULL, NULL,
			  &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
...
test_path_modified(Manager *m)
{
	...
	assert_se(manager_load_unit(m, "path-modified.path", NULL, NULL,
			  &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
...
test_path_unit(Manager *m)
{
	...
	assert_se(
		manager_load_unit(m, "path-unit.path", NULL, NULL, &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
...
static void
test_path_directorynotempty(Manager *m)
{
	...
	assert_se(manager_load_unit(m, "path-directorynotempty.path", NULL,
			  NULL, &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
...
static void
test_path_makedirectory_directorymode(Manager *m)
{
	...
	assert_se(manager_load_unit(m, "path-makedirectory.path", NULL, NULL,
			  &unit) >= 0);
	assert_se(UNIT_VTABLE(unit)->start(unit) >= 0);
